...
{	
	...
	u8 cfg_max_players = cfg.read_u8("maximum_players_in_team", 1);
...
void onCommand(CRules@ this, u8 cmd, CBitStream@ params)
{
	...
	if (cmd == this.getCommandID(queue_lock_id)) {SpawnPlayers(queues);}
...
void SpawnPlayers(QueueObject@ queues)
{
	...
	{	
		if (i < queues.max_players && i < queues.current_red)
		{			
	...
	{				
		if (i < queues.max_players && i < queues.current_blue)
		{
	...
	if (queues.current_spawned == lowestCount*2)
	{
		for (uint i = 0; i < lowestCount; i++)
		{	
			u16 id = getPlayerByUsername(queues.BlueQueuePlayers[0]).getNetworkID();
		  	CBitStream params;
			params.write_u16(id);
			getRules().SendCommand(getRules().getCommandID(exit_queues_id), params);

			u16 id2 = getPlayerByUsername(queues.RedQueuePlayers[0]).getNetworkID();
		  	CBitStream params2;
			...
			//queues.current_blue--;	
			//queues.current_red--;		
			//queues.BlueQueuePlayers.removeAt(0);						
			//queues.RedQueuePlayers.removeAt(0);
		}
...
void JoinQueue(QueueObject@ queues, CPlayer@ p, int team)
{
	...
	else
	{
		...
		if (team == 0)
		{
			queues.BlueQueuePlayers.push_back(p.getUsername());
			queues.current_blue++;
		}
		else if (team == 1)
		{
			queues.RedQueuePlayers.push_back(p.getUsername());
			queues.current_red++;
		}
	}
}
