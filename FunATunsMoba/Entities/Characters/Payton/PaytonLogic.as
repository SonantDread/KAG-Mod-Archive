...
void onInit(CBlob@ this)
{
	...
	this.set_u32("DrillCD", 1000);
	this.set_u32("MineCD", 750);
...
void onTick(CBlob@ this)
{
	...
	if(this.get_u32("DrillCD") < 1 && this.isKeyPressed(key_taunts) && this.get_u32("select") == 0)
	{
		CBlob@ blob = server_CreateBlobNoInit("drill");
		if (blob !is null)
		{
	...
	else if(this.get_u32("MineCD") < 1 && this.isKeyPressed(key_taunts) && this.get_u32("select") == 1)
	{
		CBlob@ blob = server_CreateBlobNoInit("mine");
		if (blob !is null)
		{
			blob.server_setTeamNum(this.getTeamNum());
			blob.setPosition(this.getPosition() + Vec2f(4.0f, 0.0f));
		...
		this.set_u32("MineCD",1300);
	}
	
	if(this.get_u32("DrillCD") >= 1)
	{
		this.set_u32("DrillCD", this.get_u32("DrillCD") - 1);
	}
	if(this.get_u32("MineCD") >= 1)
	{
		this.set_u32("MineCD", this.get_u32("MineCD") - 1);
	}
}
