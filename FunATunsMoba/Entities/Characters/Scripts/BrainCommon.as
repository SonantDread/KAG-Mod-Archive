...
CBlob@ getBuildTarget(CBrain@ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false)
{
	...
	for (uint i = 0; i < players.length; i++)
	{
		...
		if (potential !is blob && blob.getTeamNum() != potential.getTeamNum()
				...
				&&  (potential.hasTag("getthis") || potential.hasTag("player")) && !potential.hasTag("invisa")
		   )
		{
...
CBlob@ getNewTarget(CBrain@ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false)
{
	...
	for (uint i = 0; i < players.length; i++)
	{
		...
		if (potential !is blob && blob.getTeamNum() != potential.getTeamNum()
		        ...
		        && !potential.hasTag("dead") && !potential.hasTag("migrant") && !potential.hasTag("invisa")
