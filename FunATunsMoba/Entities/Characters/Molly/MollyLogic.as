...
void onInit(CBlob@ this)
{
	...
	this.set_u32("SawCD", 1000);
	this.set_u32("ChickenCDCD", 750);
...
void onTick(CBlob@ this)
{
	...
	if(this.get_u32("SawCD") < 1 && this.isKeyPressed(key_taunts) && this.get_u32("select") == 0)
	{
		CBlob@ blob = server_CreateBlobNoInit("saw");
		if (blob !is null)
		{
			blob.server_setTeamNum(this.getTeamNum());
			blob.setPosition(this.getPosition() + Vec2f(4.0f, 0.0f));
			blob.Init();
			blob.server_SetTimeToDie(20);	 // delete after 6 minutes
		}
	
		this.set_u32("SawCD",1000);
	}
	else if(this.get_u32("ChickenCD") < 1 && this.isKeyPressed(key_taunts) && this.get_u32("select") == 1)
	{
		CBlob@ blob = server_CreateBlobNoInit("chicken");
		if (blob !is null)
		{
			blob.server_setTeamNum(this.getTeamNum());
			blob.setPosition(this.getPosition() + Vec2f(4.0f, 0.0f));
		...
		this.set_u32("ChickenCD",1300);
	}
	
	if(this.get_u32("SawCD") >= 1)
	{
		this.set_u32("SawCD", this.get_u32("SawCD") - 1);
	}
	if(this.get_u32("ChickenCD") >= 1)
	{
		this.set_u32("ChickenCD", this.get_u32("ChickenCD") - 1);
	}
}
