...
void onInit(CBlob@ this)
{
	...
	this.set_u32("AxeCD", 500);
	this.set_u32("DefendCD", 1000);
...
void onTick(CBlob@ this)
{
	...
	else if(this.get_u32("AxeCD") < 1 && this.isKeyPressed(key_taunts) && this.get_u32("select") == 0)
	{
		ShootArrow(this, this.getPosition() + Vec2f(0.0f, -2.0f), this.getAimPos() + Vec2f(0.0f, -2.0f), 17.59f, 0, false); 
		this.set_u32("AxeCD",500);
		
	}
	else if(this.get_u32("DefendCD") < 1 && this.isKeyPressed(key_taunts) && this.get_u32("select") == 1)
	{
		CBlob@[] blobsInRadius;	   
		if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				{
					b.set_u32("defend",1300);
				}
			}
		}
		this.set_u32("DefendCD",600);
	}
	...
	if(this.get_u32("AxeCD") >= 1)
	{
		this.set_u32("AxeCD", this.get_u32("AxeCD") - 1);
	}
	if(this.get_u32("DefendCD") >= 1)
	{
		this.set_u32("DefendCD", this.get_u32("DefendCD") - 1);
	}
...
{
	CBlob@ arrow = server_CreateBlobNoInit("axe");
	if (arrow !is null)
	{
		// fire arrow?
