...
void onInit(CBlob@ this)
{
	...
	{
		ShopItem@ s = addShopItem(this, "Sacrifice Gold", "$GOLD$", "sacrifice10", "Sacrifice gold to empower the golden being.", true);
		s.spawnNothing = true;
		s.customButton = true;
		s.buttonwidth = 1;
		s.buttonheight = 1;
	...
	{
		ShopItem@ s = addShopItem(this, "Sacrifice Gold", "$GOLD$", "sacrifice50", "Sacrifice gold to empower the golden being.", true);
		s.spawnNothing = true;
		s.customButton = true;
		s.buttonwidth = 1;
		s.buttonheight = 1;
	...
	{
		ShopItem@ s = addShopItem(this, "Sacrifice Gold", "$GOLD$", "sacrifice100", "Sacrifice gold to empower the golden being.", true);
		s.spawnNothing = true;
		s.customButton = true;
		s.buttonwidth = 1;
		s.buttonheight = 1;
	...
	{
		ShopItem@ s = addShopItem(this, "Sacrifice Gold", "$GOLD$", "sacrifice250", "Sacrifice gold to empower the golden being.", true);
		s.spawnNothing = true;
		s.customButton = true;
		s.buttonwidth = 1;
		s.buttonheight = 1;
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if(cmd == this.getCommandID("shop made item"))
	{
		...
		if(name == "sacrifice50"){
			...
			{
				if(fg[i].getPlayer().getUsername() == this.get_string("owner")){
					fg[i].set_s16("power",fg[i].get_s16("power")+50);
				}
			}
		}
		if(name == "sacrifice100"){
			CBlob@[] fg;
			getBlobsByName("goldenbeing", @fg);
			for(uint i = 0; i < fg.length; i++)
			{
				if(fg[i].getPlayer().getUsername() == this.get_string("owner")){
					fg[i].set_s16("power",fg[i].get_s16("power")+100);
				}
			}
		}
		if(name == "sacrifice250"){
			CBlob@[] fg;
			getBlobsByName("goldenbeing", @fg);
			for(uint i = 0; i < fg.length; i++)
			{
				if(fg[i].getPlayer().getUsername() == this.get_string("owner")){
					fg[i].set_s16("power",fg[i].get_s16("power")+250);
				}
			}
		}
	}
