...
void ManageBow(CBlob@ this,CrossbowmanInfo@ crossbowman,RunnerMoveVars@ moveVars)
{
	...
	if(state==CrossbowmanParams::ready) {
		...
		if(getNet().isServer() && crossbowman.stateTime>=5 && !crossbowman.dontHitMore){
			...
			f32 angle=	crossbowman.attackRot;
			...
			if(blobHit) {
				for(u32 i=0;i<hitInfos.length;i++) {
					if(hitInfos[i].blob !is null) {	
						...
						if(blob.getConfig()=="knight" || blob.getConfig()=="archer" || blob.getConfig()=="crossbowman" || blob.getConfig()=="builder" || blob.getConfig()=="juggernaut" || blob.getConfig()=="trader") { //poop
							...
							this.server_Hit(blob,this.getPosition(),dir,2.5f,HittersNew::sword,false); //2.5 heart damage!
							crossbowman.dontHitMore=true;
							break;
						}
					}
