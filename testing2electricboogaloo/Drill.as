...
void TrySetDelay(CBlob@ this, u8 delay)
{
	this.set_u8(delay_prop, Maths::Max(this.get_u8(delay_prop), delay)); // change delay to a new value, but only if its higher than current delay
	this.Sync(delay_prop, true);
}

void ForceSetDelay(CBlob@ this, u8 delay)
{
	this.set_u8(delay_prop, delay);
	this.Sync(delay_prop, true);
}
...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (holder.getName() == required_class || sv_gamemode == "TDM")
		{
			...
			u8 delay_amount = Maths::Max(default_delay, this.get_u8(delay_prop));
			...
			else
			{
				...
				ForceSetDelay(this, default_delay); // reset delay to default
			}
			...
			// delay drill
			{
				...
				f32 attack_range = Maths::Min(distance + Maths::Max(0.0f, 1.75f * holder.getShape().vellen * (holder.getVelocity() * direction)), distance + 2.0f);
				...
				{
					HitInfo@[] hitInfos;
					if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, attack_range, this, true, @hitInfos))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (b !is null) // blob
							{
								...
								if (isServer())
								{
									...
									if (b.getPlayer() !is null || player_blobs.find(b.getName()) >= 0)
									{
										TrySetDelay(this, 10);
									}
