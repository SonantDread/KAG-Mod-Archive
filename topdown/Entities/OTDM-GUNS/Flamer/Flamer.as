...
void onTick(CBlob@ this)
{
	...
	bool pressed_a1 = point.isKeyPressed(key_action1) || holder.get_bool("shooting");
	if (ammunition < 1)
	{
		this.server_Die();
	}
	...
	{	
		...
		if (this !is null)
		{
			...
			if (bullet !is null && point !is null && holder !is null)
			{
				...
				Vec2f norm = aim - pos;
				norm.Normalize();
				bullet.setVelocity(norm * (7.0f+XORRandom(3)));
				if(bullet.getSprite() !is null)
				{
					bullet.getSprite().RotateBy(XORRandom(360), Vec2f(0,0));
				}
			...
			if (bullet2 !is null && point !is null && holder !is null)
			{
				...
				bullet2.setVelocity(norm * (6.0f+XORRandom(3)));
				bullet2.setAngleDegrees(XORRandom(360));
				if(bullet2.getSprite() !is null)
				{
					bullet2.getSprite().RotateBy(XORRandom(360), Vec2f(0,0));
				}
			...
			if (bullet3 !is null && point !is null && holder !is null)
			{
				...
				bullet3.setVelocity(norm * (5.0f+XORRandom(3)));
				if(bullet3.getSprite() !is null)
				{
					bullet3.getSprite().RotateBy(XORRandom(360), Vec2f(0,0));
				}
			...
			if (bullet4 !is null && point !is null && holder !is null)
			{
				...
				if(bullet4.getSprite() !is null)
				{
					bullet4.getSprite().RotateBy(XORRandom(360), Vec2f(0,0));
				}
			...
			if (bullet5 !is null && point !is null && holder !is null)
			{
				...
				if(bullet5.getSprite() !is null)
				{
					bullet5.getSprite().RotateBy(XORRandom(360), Vec2f(0,0));
				}
