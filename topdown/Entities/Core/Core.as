...
{
	this.set_s32("amount", XORRandom(5));
	this.set_s32("interval", 10);
	...
	if (map !is null)
	{
		...
		//spawn a few chests around the map
...
void spawnEnemies(CBlob@ this)
{
	...
	for(uint i = 0; i < amount; i++)
	{
		...
		string enemyname = "person";// XORRandom(50) > 25 ? "skeleton" : "zombie";
		...
		CBlob@[] enemies3;
		CBlob@[] enemies4;
		if(getBlobsByTag("NPC", @enemies))
		{
			if (timer3 > 180 && enemies.length < 10 || enemies.length < 5)
			{
				leftpos.x = leftpos.x+XORRandom(48);
				rightpos.x = rightpos.x-XORRandom(48);
				CBlob@ enemy = server_CreateBlob(enemyname, 0, leftpos);
				CBlob@ enemy2 = server_CreateBlob(enemyname, 0, rightpos);
				CBlob@ enemy3 = server_CreateBlob(enemyname, 0, midpos);
				enemy.AddScript( XORRandom(12) > 6 ? "Barter.as" : "Raider.as");
				enemy2.AddScript( XORRandom(12) > 6 ? "Barter.as" : "Raider.as");
				enemy3.AddScript( XORRandom(12) > 6 ? "Barter.as" : "Raider.as");
			}
...
void spawnChests(CBlob@ this)
{
	...
	for(uint i = 0; i < amount; i++)
	{
		...
		this.set_s32("interval2", 10+XORRandom(5));
	}
