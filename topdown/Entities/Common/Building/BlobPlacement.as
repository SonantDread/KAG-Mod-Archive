...
{	
	f32 carryangle = this.get_f32("carryangle");
	...
	{
		carryBlob.SetFacingLeft(false);
		bool hand = this.get_bool("hand");
		carryBlob.setAngleDegrees((hittimer*2.5f)-45.0f);
	}
	AttachmentPoint@ hands = this.getAttachments().getAttachmentPointByName("PICKUP");
	if (hands !is null)
	{
		Vec2f carryoffset = this.get_Vec2f("carryoffset");
		//Vec2f off = Vec2f(-1, -5);
		Vec2f off = Vec2f(-0, -0);
		hands.offset = carryoffset+off;
		if (carryBlob !is null)
		{
			carryBlob.getSprite().SetRelativeZ(-1.0f);
			f32 carryanglecurrent = this.get_f32("carryanglecurrent");
			f32 diff = carryangle - carryanglecurrent;
			carryanglecurrent = carryangle + diff/2;
			carryBlob.setAngleDegrees(this.getAngleDegrees() + carryangle);
			//if(!carryBlob.hasTag("custom rotate")) carryBlob.setAngleDegrees(this.getAngleDegrees());
			this.set_f32("carryanglecurrent", carryanglecurrent);
		}
	...
	if (carryBlob.hasTag("place45"))
	{
		...
		if (hands !is null)
		{
			hands.offset.x = 50 + (aim_vec.x * 2 * (this.isFacingLeft() ? 1.0f : -1.0f)); // if blob config has offset other than 0,0 there is a desync on client, dont know why
			hands.offset.y = -50-(aim_vec.y * (distance < 0 ? 1.0f : 1.0f));
		}
	}
	else
	{
