	...
	if(canBuild(this, blocks, index))
	{
		...
		if(getNet().isServer())
		{
			...
			if (blockBlob !is null)
			{
				...
				//For disappearing chair fix (see Chair.as)
				blockBlob.Tag("wasPlaced");
				return blockBlob;
			}
		}
	}
...
{/*
	CBlob@[] spawnPoints;
  	if (getBlobsByTag("spawn point", @spawnPoints))
 	{
 		for (uint i = 0; i < spawnPoints.length; i++)
 		{		
			CBlob @b = spawnPoints[i];
			if(b !is null)
			{
				Vec2f Vector = b.getPosition() - this.getPosition();
				if(Vector.Length() > 10)
				{
					return false;
				}
