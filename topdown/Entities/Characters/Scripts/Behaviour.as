...
void InitBrain(CBrain@ this)
{
	...
	blob.set_u8("strategy", Strategy::invading);
...
void Invade(CBlob@ blob)
{
	//print("invading");
	CBrain@ brain = blob.getBrain();
	Vec2f targetpos = blob.get_Vec2f("targetpos");
	Vec2f myPos = blob.getPosition();

	if(myPos.x > targetpos.x + 48)
	{
		blob.setKeyPressed(key_left, true);
		blob.setKeyPressed(key_right, false);
	}

	else if(myPos.x < targetpos.x - 48)
	{
		blob.setKeyPressed(key_right, true);
		blob.setKeyPressed(key_left, false);
	}
...
{
	if (XORRandom(3) == 0 && target.hasTag("dead"))
	{
		@target = null;
		this.SetTarget(target);
		return true;
...
void Chase(CBlob@ blob, CBlob@ target)
{
	...
	{
		if(XORRandom(32) > 24)blob.setKeyPressed(key_up, true);
	}
}*/
/*
