...
//TODO: make it backup upon leaving
...
void saveLevelSpent(string username, string userClass, int level, string item)
{
	print("saveLevelSpent");
	CRules@ rules = getRules();

	string levelup_configstr = "../Cache/Roleplay/"+userClass+"/PlayerLevelStorage.cfg";

	ConfigFile levelup_cfg   = ConfigFile(levelup_configstr);

	levelup_cfg.add_string(username+"_Level"+level,item);

	levelup_cfg.saveFile("Roleplay/"+userClass+"/PlayerLevelStorage.cfg");

}

string getLevelSpent(string username, string userClass, int level)
{
	CRules@ rules = getRules();
	...
	return levelup_cfg.read_string(username+"_Level"+level);
}
