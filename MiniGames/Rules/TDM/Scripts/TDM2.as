...
void Config(TDMCore@ this)
{
	...
	if(getMap().getMapName() != "Maps/Cars.png")
		this.scramble_teams = true;
	else
		this.scramble_teams = false;
...
shared class TDMCore : RulesCore
{
	...
	void Update()
	{
		...
		if (((rules.isIntermission() || rules.isWarmup()) && (!allTeamsHavePlayers())) && !(getMap().getMapName() == "Maps/Cars.png"))  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
		...
		if(getMap().getMapName() == "Maps/Plates.png" && !rules.isWarmup())
		{
			if(getGameTime() % 40 == 0)
			{
				CMap@ map = getMap();
				CBlob@ star = server_CreateBlob("star", -1, Vec2f(XORRandom(map.tilemapwidth) * map.tilesize, 0.0f));
				if (star !is null)
				{
					float randXvel = XORRandom(20.0f)-10.0f;
					star.getShape().SetVelocity(Vec2f(randXvel, 6.0f));
				}
			}
		}
	}
	...
	{
		//if(getMap().getMapName() != "Maps/Car.png") 
	...
	{
		string clas = player.isBot() ? "knight" : (XORRandom(512) >= 256 ? "knight" : "archer");
		int teamts = player.getTeamNum();
		if(getMap().getMapName() == "Maps/Plates.png")
		{
			clas = "builder";
		}
		if(getMap().getMapName() == "Maps/Cars.png")
		...
		TDMPlayerInfo p(player.getUsername(), teamts, clas);
		players.push_back(p);
	...
	void CheckTeamWon()
	{
		...
		if(getMap().getMapName() == "Maps/Plates.png")
		{
			bool win = true;
			CBlob@[] plates;
			getBlobsByName("plate", @plates);
			int blue = 0;
			int red = 0;
			for (uint i = 0; i < plates.length; i++)
			{
				if (plates[i].getTeamNum() == 0)
				{
					if (plates[i].getPosition().y <= 240)
					blue = 1;
				}
				else if (plates[i].getTeamNum() == 1)
				{
					if (plates[i].getPosition().y <= 240)
					red = 1;
				}
			...
			{
				CBlob@[] points;
				getBlobsByName("point", @points);
				for (uint i = 0; i < plates.length; i++)
				{
					if (points[i].getTeamNum() == 0)
					{
						error("team 0");
						points[i].server_Die();
					}
