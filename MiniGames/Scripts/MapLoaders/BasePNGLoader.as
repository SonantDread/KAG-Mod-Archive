...
class PNGLoader
{
	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		// STUFF
		else if (pixel == color_pointb)
		{
			spawnBlob( map, "point", offset, 0, false, Vec2f(0.0f, 8.0f));
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_pointr)
		{
			spawnBlob( map, "point", offset, 1, false, Vec2f(0.0f, 8.0f));
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_plateb)
		{
			spawnBlob( map, "plate", offset, 0);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_plater)
		{
			spawnBlob( map, "plate", offset, 1);
			offsets[autotile_offset].push_back( offset );
		}
		else
		{
	...
	{
		if(map.getMapName() == "Maps/Plates.png")
			map.CreateTileMap(width, height, 8.0f, "Sprites/platesworld.png");
		else
	...
	void SetupBackgrounds()
	{
		...
		map.CreateSkyGradient("Sprites/platessky.png"); // override sky color with gradient
		}
		else
		{
		// sky
