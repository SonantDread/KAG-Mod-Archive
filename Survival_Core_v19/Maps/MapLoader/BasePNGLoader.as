...
class PNGLoader
{
	...
	void handlePixel(const SColor &in pixel, int offset)
	{
		...
		else
		{
			...
			case 0xff808000:   		 autotile(offset); spawnBlob(map, "ruins",  offset); break;
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		case tree_offset:
		{
			...
			string tree_name = map_random.NextRanged(35) < 21 ? "tree_pine" : "tree_bushy";
			if(XORRandom(3) == 0)tree_name = "tree_large";
		...
		break;
		case bush_offset:
			if(XORRandom(5) > 0)server_CreateBlob("bush", -1, map.getTileWorldPosition(offset) + Vec2f(4, 4));
