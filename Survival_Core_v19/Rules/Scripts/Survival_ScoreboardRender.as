...
float drawScoreboard(CPlayer@ localplayer, CPlayer@[] players, Vec2f topleft, Vec2f emblem, bool Neutral, int teams = 1)
{
	...
	//draw panel info
	if(Neutral)GUI::DrawText("Neutrals: "+players.length, Vec2f(topleft.x, topleft.y), SColor(0xffffffff));
	...
	for (u32 i = 0; i < players.length; i++)
	{
		...
		SColor underlinecolor = SColor(0xff404040);
		...
		if(p.getTeamNum() < 8){
			...
			playercolour = getRules().getTeam(p.getTeamNum()).color;
		}
