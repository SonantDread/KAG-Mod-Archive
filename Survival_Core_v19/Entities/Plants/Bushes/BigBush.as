...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if (customData == Hitters::burn || customData == Hitters::fire){
		if(!this.hasTag("added_burn_sprites")){
			...
			for (uint i = 0; i < this.getSprite().getSpriteLayerCount(); i++)
			{
				...
				if (layer !is null)
				if(layer.name.find("ire") <= 0){
					CSpriteLayer@ fire = this.getSprite().addSpriteLayer("fire"+i, "Entities/Effects/Sprites/LargeFire.png", 16, 16, -1, -1);
					if (fire !is null)
					{
	...
	if (isWaterHitter(customData)){
		if(this.hasTag("added_burn_sprites")){
			...
			int sprites = this.getSprite().getSpriteLayerCount();
			for (uint i = 0; i < sprites; i++)
			{
				this.getSprite().RemoveSpriteLayer("fire"+i);
			}
			this.Untag("added_burn_sprites");
		}
