...
#include "CommonMutant1Blocks.as"
...
void MakeBlocksMenu( CInventory@ this, CGridMenu @invmenu )
{
    ...
    if (blocks !is null)
    {
        ...
        Vec2f pos( invmenu.getUpperLeftPosition().x + 0.5f*(invmenu.getLowerRightPosition().x - invmenu.getUpperLeftPosition().x),
                   invmenu.getUpperLeftPosition().y - 24 * MENU_SIZE.y - 30 );
        ...
        if (menu !is null)
        {
            ...
            for (uint i = 0; i < blocks.length; i++)
            {
                ...
                CGridButton @button = menu.AddButton( b.icon, b.description, Builder::make_block + i );
                ...
                if (button !is null)
                {
                    ...
                    if (hasRequirements( this, b.reqs, missing )) {
                        button.hoverText = b.description + "  " + getButtonRequirementsText( b.reqs, false );
                    }
                    else
                    {
                        button.hoverText = b.description + "  " + getButtonRequirementsText( missing, true );
        ...
        if (blocks !is null && i >= 0 && i < blocks.length)
        {
            ...
            if (isServer)
            {
                ...
                if (carryBlob !is null)
                {
					...
					{
						if (!b.buildOnGround && !blob.server_PutInInventory( carryBlob ))	// creates infinite mats duplicating if used on nuild block
