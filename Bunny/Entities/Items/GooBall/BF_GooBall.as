//BF GooBall
const f32 DAMAGE = 0.3f;
const f32 AOE = 14.0f;//radius
...
{
	return this.getTeamNum() != blob.getTeamNum() && ( blob.hasTag( "flesh" ) || blob.hasTag( "block" ) || blob.hasTag( "turret" ) );//use blob.isCollidable()?
}
...
void onDie( CBlob@ this )
{
	...
	if ( !this.isOnGround() && !this.isInWater() && map.isTileBackground( tile ) && !map.isTileGrass( tile.type) )
		server_CreateBlob( "bf_gooladder", 1, pos );		
}