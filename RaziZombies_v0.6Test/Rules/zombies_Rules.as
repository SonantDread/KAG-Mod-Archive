...
shared class ZombiesCore : RulesCore
{
			...
			if (map !is null)
			{
				...
				if ((map.getDayTime()>0.8 || map.getDayTime()<0.1) && num_zombies<max_zombies)
                {
					...
					if (transition == 1 && (dayNumber % 25) == 0) //Every 25 days!
					{
						...
						{
							server_CreateBlob( "minotaur", -1, sp);
							server_CreateBlob( "minotaur", -1, sp);
							
							getNet().server_SendMsg("2x Minotaurs"); 
							server_CreateBlob("megabossmessage");
						}
					}	
					...
					else if (transition == 1 && (dayNumber % 10) == 0) //Every 10 days!
					{
						...
						else if (boss <= 12)
						{
							...
							getNet().server_SendMsg("1x Minotaur"); 
							server_CreateBlob("bossmessage");
						}
					}	
    ...
    void CheckTeamWon( )
    {
		...
		else if(num_survivors <= 0) //Seems to be more eficient
		{
			rules.SetTeamWon(1);
			rules.SetCurrentState(GAME_OVER);
