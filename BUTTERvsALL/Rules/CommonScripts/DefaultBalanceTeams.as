...
void onNewPlayerJoin( CRules@ this, CPlayer@ player )
{
	...
	{
		if(player.isRCON()){
			player.server_setTeamNum( ADMINTEAM );
		}else{
			player.server_setTeamNum( PUBTEAM );
		}
//WANT TO CHECK IS AN ADMIN AND PUT ON RED TEAM <-done
	}
}
...
//WANT TO CHECK IS ADMIN, AND ALLOW CHANGE TEAM <-done
...
//WANT TO CHECK IF IS ADMIN AND THEN ALLOW THE SWAP
...
void onPlayerRequestTeamChange( CRules@ this, CPlayer@ player, u8 newteam )
{
	...
	if(!player.isMod()){
		if(int(newteam)!=this.getSpectatorTeamNum()) newteam=PUBTEAM;
	}
...
void onPlayerRequestSpawn( CRules@ this, CPlayer@ player )
{
	...
	if (player.getTeamNum() == 255)
		if(player.isMod()){
			core.ChangePlayerTeam(player, ADMINTEAM);
		}else{
			core.ChangePlayerTeam(player, PUBTEAM);
		}
}
