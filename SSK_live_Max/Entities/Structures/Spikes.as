					...
					for (uint i = 0; i < blobsInRadius.length; i++)
					{
						...
						if (canStab(b)) //even hurts team when stabbing
						{
							...
							if (this.isOverlapping(b))
							{
								...
								if (b.get("fighterVars", @fighterVars))
								{
								...
								if (applyDamage)
								{
									...
									FighterHitData fighterHitData(4, 0, 0.1f);
									server_fighterHit(this, b, pos, hitVec, 2.0f, Hitters::spikes, true, fighterHitData);
								}	
							}
						}
					}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point)
{
	...
	if (state == falling)
	{
		...
		if (vel.y < 4.0f) //slow, minimal dmg
			dmg = 1.0f;
		else if (vel.y < 5.5f) //faster, kill superArcher
			dmg = 2.0f;
		...
		server_fighterHit(this, blob, point, Vec2f(0, 1), dmg, Hitters::spikes, true, fighterHitData);
		return;
	}
	...
	if (damage > 0)
	{
			...
			if (applyDamage)
			{
				...
				FighterHitData fighterHitData(4, 0, 0.1f);
				server_fighterHit(this, blob, point, hitVec, 2.0f, Hitters::spikes, true, fighterHitData);
			}
	}
}
