...
void AttackBlob(CBlob@ blob, CBlob @target)
{
	...
	if (targetDistance > 25.0f)
	{
		...
		if (!fireTime && (fTime == 0 || XORRandom(130 - 5.0f * difficulty) == 0))		// difficulty
		{
			...
			const u32 shootTime = Maths::Max(SuperArcherParams::ready_time, Maths::Min(uint(targetDistance * (0.3f * Maths::Max(130.0f, vert_dist) / 100.0f) + XORRandom(20)), SuperArcherParams::shoot_period));
