...
{
	RunnerTextures@ runner_tex = addRunnerTextures(this, "super_archer", "SuperArcher");
...
{
	ensureCorrectRunnerTexture(this, "super_archer", "SuperArcher");
...
void onTick(CSprite@ this)
{
	...
	if (inMoveAnimation)
	{
		setArmsInvisible(this, Vec2f(-1.0f, 4.0f + config_offset));
		return;
	}

	// freeze the animation frame if hitstunned
	if (hitstunTime > 0)
	{
		Animation@ currAnim = this.animation;
		currAnim.timer = 0;
	}
	...
	doRopeUpdate(this, blob, superArcher);

	// animations
	const bool firing = IsFiring(blob, superArcher);
	...
	bool legolas = superArcher.charge_state == SuperArcherParams::legolas_ready || superArcher.charge_state == SuperArcherParams::legolas_charging;
	needs_shiny = false;
	bool crouch = false;
	...
	if (firing || legolas)
	{
		...
		DrawBow(this, blob, superArcher, armangle, arrowType, armOffset);
	}
	else
	{
		setArmsInvisible(this, armOffset);
	}
	...
	DrawBowEffects(this, blob, superArcher, arrowType);

	//set the head anim
	if (tumbleTime > 0 || dazeTime > 0 || crouch)
	{
		blob.Tag("dead head");
	}
...
void setArmsInvisible(CSprite@ this, Vec2f armOffset)
{
...
void DrawBow(CSprite@ this, CBlob@ blob, SuperArcherInfo@ superArcher, f32 armangle, const u8 arrowType, Vec2f armOffset)
{
	...
	if (!superArcher.has_arrow || superArcher.charge_state == SuperArcherParams::no_arrows || superArcher.charge_state == SuperArcherParams::legolas_charging)
	{
		string animname = "no_arrow";

		if (superArcher.charge_time == SuperArcherParams::ready_time)
		{
			animname = "fired";
		}

		u16 frontframe = 0;
		f32 temp = Maths::Min(superArcher.charge_time, SuperArcherParams::ready_time);
		f32 ready_tween = temp / SuperArcherParams::ready_time;
	...
	else if (superArcher.charge_state == SuperArcherParams::readying)
	{
		u16 frontframe = 0;
		f32 temp = superArcher.charge_time;
		f32 ready_tween = temp / SuperArcherParams::ready_time;
		armangle = armangle * ready_tween;
		armOffset = Vec2f(-1.0f, 4.0f + config_offset + 2.0f * (1.0f - ready_tween));
	...
	else if (superArcher.charge_state != SuperArcherParams::fired || superArcher.charge_state == SuperArcherParams::legolas_ready)
	{
		u16 frontframe = Maths::Min((superArcher.charge_time / (SuperArcherParams::shoot_period_1 + 1)), 2);
		setArmValues(frontarm, true, armangle, 0.1f, "charge", Vec2f(-4.0f * sign, 0.0f), armOffset);
		frontarm.animation.frame = frontframe;

		const f32 arrowangle = (superArcher.charge_time > SuperArcherParams::shoot_period_2) ? armangle + -2.5f + float(XORRandom(500)) / 100.0f : armangle; // shiver arrow when fully charged
		...
		if (superArcher.charge_state == SuperArcherParams::legolas_ready)
		{
			needs_shiny = true;
...
void DrawBowEffects(CSprite@ this, CBlob@ blob, SuperArcherInfo@ superArcher, const u8 arrowType)
{
	// set fire light

	if (arrowType == ArrowType::fire)
	{
		if (IsFiring(blob, superArcher))
		{
			blob.SetLight(true);
...
bool IsFiring(CBlob@ blob, SuperArcherInfo@ superArcher)
{
	return (superArcher.charge_state != SuperArcherParams::not_aiming);
}

void doRopeUpdate(CSprite@ this, CBlob@ blob, SuperArcherInfo@ superArcher)
{
	CSpriteLayer@ rope = this.getSpriteLayer("rope");
	CSpriteLayer@ hook = this.getSpriteLayer("hook");

	bool visible = superArcher !is null && superArcher.grappling;
	...
	Vec2f off = superArcher.grapple_pos - blob.getPosition();
	...
	hook.ResetTransform();
	if (superArcher.grapple_id == 0xffff) //still in air
	{
		superArcher.cache_angle = -superArcher.grapple_vel.Angle();
	}
	hook.RotateBy(superArcher.cache_angle , Vec2f());

	hook.TranslateBy(off);
	hook.SetIgnoreParentFacing(true);
	hook.SetFacingLeft(false);

	//GUI::DrawLine(blob.getPosition(), superArcher.grapple_pos, SColor(255,255,255,255));
}
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/SuperArcher/SuperArcherGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/SuperArcher/SuperArcherGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/SuperArcher/SuperArcherGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/SuperArcher/SuperArcherGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
