...
//common superKnight header

namespace SuperKnightStates
{
	enum States
	{
		normal = 0,
...
namespace SuperKnightParams
{
	const ::s32 resheath_time = 2;

	const ::s32 slash_charge = 8; // 15
	const ::s32 slash_charge_level2 = 42; // 38
	const ::s32 slash_charge_limit = slash_charge_level2 + slash_charge + 10;
	const ::s32 slash_move_time = 4;
	const ::s32 slash_time = 24; // 13
	const ::s32 double_slash_time = 8; // 8

	const ::f32 slash_move_max_speed = 6.0f; // 3.5f

	const u32 glide_down_time = 50;
}

shared class SuperKnightInfo
{
	u8 swordTimer;
	u8 shieldTimer;
	bool doubleslash;
...
{
	return (state >= SuperKnightStates::shielding && state <= SuperKnightStates::shieldgliding);
}
...
{
	return (state >= SuperKnightStates::sword_drawn && state <= SuperKnightStates::sword_power_super);
}
...
f32 getCutAngle(CBlob@ this, u8 state)
{
	...
	if (state == SuperKnightStates::sword_cut_mid)
	{
		attackAngle += (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == SuperKnightStates::sword_cut_mid_down)
	{
		attackAngle -= (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == SuperKnightStates::sword_cut_up)
	{
		attackAngle += (this.isFacingLeft() ? 80.0f : -80.0f);
	}
	else if (state == SuperKnightStates::sword_cut_down)
	{
		attackAngle -= (this.isFacingLeft() ? 80.0f : -80.0f);
	}
...
f32 getCutAngle(CBlob@ this)
{
	...
	if (direction == -1)
	{
		tempState = SuperKnightStates::sword_cut_up;
	}
	else if (direction == 0)
	{
		if (aimpos.y < this.getPosition().y)
		{
			tempState = SuperKnightStates::sword_cut_mid;
		}
		else
		{
			tempState = SuperKnightStates::sword_cut_mid_down;
		}
	}
	else
	{
		tempState = SuperKnightStates::sword_cut_down;
	}

	return getCutAngle(this, tempState);
}

//shared attacking/bashing constants (should be in SuperKnightParams but used all over)
