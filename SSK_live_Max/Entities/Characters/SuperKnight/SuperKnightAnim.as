...
{
	addRunnerTextures(this, "super_knight", "SuperKnight");
...
{
	ensureCorrectRunnerTexture(this, "super_knight", "SuperKnight");
}
...
void onTick(CSprite@ this)
{
	...
	else if (superKnight.state == SuperKnightStates::shieldgliding)
	{
	...
	else if (superKnight.state == SuperKnightStates::shielding)
	{
		if (walking)
		{
			if (direction == 0)
	...
	else if (superKnight.state == SuperKnightStates::sword_drawn)
	{
		if (superKnight.swordTimer < SuperKnightParams::slash_charge)
		{
			this.SetAnimation("draw_sword");
		}
		else if (superKnight.swordTimer < SuperKnightParams::slash_charge_level2)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 0;
		}
		else if (superKnight.swordTimer < SuperKnightParams::slash_charge_limit)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 1;
			shinydot = true;
	...
	else if (superKnight.state == SuperKnightStates::sword_cut_mid_down)
	{
		this.SetAnimation("strike_mid_down");
	}
	else if (superKnight.state == SuperKnightStates::sword_cut_up)
	{
		this.SetAnimation("strike_up");
	}
	else if (superKnight.state == SuperKnightStates::sword_cut_down)
	{
		this.SetAnimation("strike_down");
	}
	else if (superKnight.state == SuperKnightStates::sword_power || superKnight.state == SuperKnightStates::sword_power_super)
	{
		this.SetAnimation("strike_power");

		if (superKnight.swordTimer <= 1)
			this.animation.SetFrameIndex(0);

		u8 mintime = 6;
		u8 maxtime = 8;
		if (superKnight.swordTimer >= mintime && superKnight.swordTimer <= maxtime)
		{
			wantsChopLayer = true;
			chopframe = superKnight.swordTimer - mintime;
			chopAngle = -vec.Angle();
		}
	}
	else if (inair)
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (SuperKnightParams::slash_charge_limit - SuperKnightParams::slash_charge_level2);
			f32 count = (superKnight.swordTimer - SuperKnightParams::slash_charge_level2);
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/SuperKnight/SuperKnightGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/SuperKnight/SuperKnightGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/SuperKnight/SuperKnightGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/SuperKnight/SuperKnightGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
