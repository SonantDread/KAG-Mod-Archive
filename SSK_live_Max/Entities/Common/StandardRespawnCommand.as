		...
		case SpawnCmd::changeClass:
		{
			if (getNet().isServer())
			{
				...
				if (caller !is null && canChangeClass(this, caller))
				{
					...
					if (newBlob !is null)
					{
						// set player blob name on change class
						if (newBlob.hasTag("fighter"))
						{
							if (player !is null)
							{
								...
								if (rules !is null)
								{
									rules.set_string("playerClassConfig"+player.getUsername(), classconfig);
								}
							}
						}
						...
						SSKFighterVars@ callerFighterVars;
						SSKFighterVars@ newBlobFighterVars;
						if (caller.get("fighterVars", @callerFighterVars) && newBlob.get("fighterVars", @newBlobFighterVars))
						{
							newBlobFighterVars.damageStatus = callerFighterVars.damageStatus;
							newBlobFighterVars.tumbleTime = callerFighterVars.tumbleTime;
						}
