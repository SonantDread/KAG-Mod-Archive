...
void Heal(CBlob@ this, CBlob@ blob)
{
	...
	if (getNet().isServer() && blob.hasTag("player") && fighterVars.damageStatus > 0.0f && !this.hasTag("healed") && exists)
	{
		CBitStream params;
	...
	if (cmd == this.getCommandID(heal_id))
	{
		...
		if (getNet().isServer())
		{
			...
			if (theBlob !is null)
			{
				...
				SSKFighterVars@ fighterVars;
				if (theBlob.get("fighterVars", @fighterVars))
				{
					fighterVars.damageStatus = Maths::Max(fighterVars.damageStatus - heal_amount, 0.0f);
