...
void Explode(CBlob@ this, f32 radius, f32 damage)
{
	...
	if (this.isInInventory())
	{
		...
		{
			FighterHitData fighterHitData(10, 1.0f, 0.1f);
			server_fighterHit(this, doomed, pos, Vec2f(), 100.0f, Hitters::explosion, true, fighterHitData);
		}
	}
             ...
             const bool bother_raycasting = true, const bool should_teamkill = false)
{
	...
	server_fighterHit(this, hit_blob, hit_blob_pos, bombforce, dam, hitter, 
	...
	if (map !is null)
	{
		...
		if (is_server)
		{
			...
			for (uint i = 0; i < blobs.length; i++)
			{
				...
				FighterHitData fighterHitData(10, 2.0f, 0, 80);

				if (shouldStun && (ownerBlob is blob || (this.isOverlapping(blob) && hitHard)))
				{
					server_fighterHit(this, blob, pos, bombforce, 0.0f, Hitters::water_stun_force, true, fighterHitData);
				}
				else if (hitHard)
				{
					server_fighterHit(this, blob, pos, bombforce, 0.0f, hitter, true, fighterHitData);
				}
				else //still have to hit teamies so we can put them out!
				{
					server_fighterHit(this, blob, pos, bombforce, 0.0f, Hitters::water, true, fighterHitData);
				}
			}
		}
	}
