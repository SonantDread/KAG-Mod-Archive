...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	if (blob !is null)
	{
		bool hitPlatform = blob.isCollidable() && blob.getShape().isStatic();

		if (!hitPlatform && !this.isAttached() && !this.hasTag("exploding") && this.hasTag("armed"))
		{
			if (!this.hasTag("_hit_water")) //smack that mofo
			{
				this.Tag("_hit_water");
				Vec2f pos = this.getPosition();
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	else if (cmd == this.getCommandID("die"))
	{
		...
		CParticle@ smartP = ParticleAnimated("blast2.png", pos, Vec2f(0, 0), 0.0f, MAX_EXPLOSION_SCALE*0.55f, 5, 0.0f, true);
		if (smartP !is null)
		{
			smartP.Z = 100.0f;
			smartP.deadeffect = 10;
			smartP.fadeout = true;
		}
