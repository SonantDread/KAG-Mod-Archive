...
const u8 heat_add = 8;
const u8 heat_add_constructed = 8;
const u8 heat_add_blob = heat_add * 3;
const u8 heat_cool_amount = 5;

const u8 heat_cooldown_time = 3;
...
void onTick(CBlob@ this)
{
	...
	if (heat > 0)
	{
		if ( gametime % heat_cooldown_time == 0 && heat >= heat_add )
		{
	...
	if (this.isAttached())
	{
		...
		if (int(heat) >= heat_max - (heat_add * 1.5))
		{
			...
			FighterHitData fighterHitData(8, 0, 0, 20);
			server_fighterHit(this, holder, holder.getPosition(), Vec2f(), 0.25f, Hitters::burn, true, fighterHitData);
			this.server_DetachFrom(holder);
			sprite.PlaySound("DrillOverheat.ogg");
		}

		if (holder.getName() == required_class || sv_gamemode == "TDM" || sv_gamemode == "SSK")
		{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (hi.blob !is null) // blob
							{
								...
								if (getNet().isServer())
								{
									...
									FighterHitData fighterHitData(8, 1.0f, 0.02f);
									server_fighterHit(this, hi.blob, hi.hitpos, attackVel, attack_dam, Hitters::drill, true, fighterHitData);
