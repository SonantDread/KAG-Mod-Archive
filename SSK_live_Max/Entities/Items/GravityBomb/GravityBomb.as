...
void onTick(CBlob@ this)
{
	...
	else if (this.hasTag("armed"))
	{
		...
		else
		{
			...
			Animation@ armedAnim = thisSprite.getAnimation("shes_gonna_blow");	
			armedAnim.time = Maths::Max(1, (1.0f - (1.0f*counter/bomb_fuse))*8);

			thisSprite.SetEmitSoundSpeed(Maths::Max(0.5f, (1.0f*counter/bomb_fuse)*4.0f));
		}
	}
}
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	if (!this.hasTag("exploding") && getNet().isServer() && damage >= 1.0f)
	{
		if (getNet().isServer())
		{
			this.SendCommand(this.getCommandID("arm bomb"));
		}
	}

	//this.AddForce(velocity*8.0f);
             ...
             const bool bother_raycasting = true, const bool should_teamkill = false)
{
	...
	//hit the object
	FighterHitData fighterHitData(10, 4.0f, 0.04f);
	server_fighterHit(this, hit_blob, hit_blob_pos, bombforce, dam, hitter, true, fighterHitData);
	return true;
}
...
{
	if (cmd == this.getCommandID("arm bomb"))
	{
		this.Tag("armed");
		...
		thisSprite.SetAnimation("shes_gonna_blow");

		thisSprite.SetEmitSound( "hum_loop1.ogg" );
		thisSprite.SetEmitSoundVolume( 0.5f );
		thisSprite.SetEmitSoundPaused( false );	
	}
	else if (cmd == this.getCommandID("trigger explosion"))
	{
		if (getNet().isServer())
		{
			CBlob@ black_hole = server_CreateBlob("black_hole", this.getTeamNum(), this.getPosition());
			if (black_hole !is null)
			{
				black_hole.setVelocity(Vec2f(0, 0));
			}
		}

		CParticle@ energyBall = ParticleAnimated("energyball1.png", this.getPosition(), Vec2f(0, 0), XORRandom(360), 1.0f, 2, 0.0f, true);
		if (energyBall !is null)
		{
			energyBall.Z = -100.0f;
		}
