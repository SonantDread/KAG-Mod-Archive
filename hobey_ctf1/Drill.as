...
void onTick(CSprite@ this)
{
    ...
    this.ResetTransform();
    if (getRules().isWarmup()) this.ScaleBy(2,2);
    ...
    {
        heatlayer.ResetTransform();
        if (getRules().isWarmup()) heatlayer.ScaleBy(2,2);
...
void onTick(CBlob@ this)
{
    ...
    if (heat > 0)
    {
        ...
        {
            heat--;
            if (getRules().isWarmup() && (heat > 0)) heat--;
        }
        ...
        if (inwater && heat >= heat_add && gametime % (Maths::Max(heat_cooldown_time_water, 1)) == 0)
        {
            ...
            if (getRules().isWarmup()) heat -= heat_cool_amount;
        }
        this.set_u8(heat_prop, heat);
    }
    ...
    if (this.isAttached())
    {
        ...
        if (holder.getName() == required_class || sv_gamemode == "TDM")
        {
            ...
            u8 delay_amount = inwater ? 20 : 8;
            if (getRules().isWarmup()) delay_amount /= 2;
            ...
            // delay drill
            {
                ...
                f32 distance = 20.0f;
                if (getRules().isWarmup()) distance *= 2.f;
                ...
                if (map !is null)
                {
                    ...
                    if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, true, @hitInfos))
                    {
                        ...
                        for (uint i = 0; i < hitInfos.length; i++)
                        {
                            ...
                            else // map
                            {
                                ...
                                if (isServer())
                                {
                                    for (uint i = 0; i < 2; i++)
                                    {
                                        ...
                                        if (map.isTileCastle(tile) || map.isTileWood(tile) || map.isTileGold(tile) || getRules().isWarmup())
                                        {
                            ...
                            if (hitsomething)
                            {
                                u8 building_time_divisor = 1;
                                if (getRules().isWarmup()) building_time_divisor = 2;
                                if (heat < heat_max)
                                {
                                    if (hit_constructed)
                                    {
                                        heat += heat_add_constructed / building_time_divisor;
                                    }
                                    else if (hitblob)
                                    {
                                        heat += heat_add_blob / building_time_divisor;
                                    }
                                    else
                                    {
                                        heat += heat_add / building_time_divisor;
                                    }
                                }
                                hitsomething = false;
                                hitblob = false;
