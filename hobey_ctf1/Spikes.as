                ...
                if (map.getBlobsInRadius(pos, this.getRadius() * 2.0f, @blobsInRadius))
                {
                    for (uint i = 0; i < blobsInRadius.length; i++)
                    {
                        ...
                        if (canStab(b)) //even hurts team when stabbing
                        {
                            ...
                            {
                                float damage = 0.5f;
                                if (getRules().isWarmup()) damage = 0.f;
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point)
{
    ...
    if (state == falling)
    {
        ...
        if      (vel.y < 4.0f) damage = 1.f; //slow, minimal dmg
        else if (vel.y < 5.5f) damage = 2.f; //faster, kill archer
        else if (vel.y < 7.0f) damage = 3.f; //faster, kill builder
        else		           damage = 4.f; 	//fast, instakill
