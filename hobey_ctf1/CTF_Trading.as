...
void onPlayerDie(CRules@ this, CPlayer@ victim, CPlayer@ killer, u8 customData)
{
    ...
    if (victim !is null)
    {
        ...
        if (!this.isWarmup())	//only reduce coins if the round is on.
        {
            ...
            if (blob !is null) {
                // server_DropCoins(blob.getPosition(), (lost/2) + XORRandom(lost/4)); // NOTE(hobey): 50-75%
                server_DropCoins(blob.getPosition(), lost); // NOTE(hobey): 100%
            }
        }
    }
}
        ...
        if (p !is null)
        {
            ...
            switch (g.getType())
            {
                ...
                
                {
                    ...
                    if        (tile == CMap::tile_castle)      { coins = 3;
                    } else if (tile == CMap::tile_castle_back) { coins = 0.4;
                    } else if (tile == CMap::tile_wood)        { coins = 1;
                    }
                }
                ...
                
                {
                    ...
                    if        (name == "stone_door"     ) { coins = 5;
                    } else if (name == "trap_block"     ) { coins = 0;
                    } else if (name == "spikes"         ) { coins = 4;
                    } else if (name == "wooden_platform") { coins = 1;
                        
                    } else if (name == "ladder"         ) { coins = 0.75;
                    } else if (name == "wooden_door"    ) { coins = 1.25;
                    } else if (name == "building"       ) { coins = coinsOnBuildWorkshop;
                    }
                }
            ...
            add_coins_to_player(p, coins);
        }
    }
}
// print(''+getRules().get_f32("fractional_coins_"+getPlayer(0).getUsername()))