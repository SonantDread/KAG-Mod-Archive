...
CBlob@ getClosestBlob(CBlob@ this)
{
    ...
    if (this.get("pickup blobs", @pickupBlobs))
    {
        ...
        for (uint i = 0; i < available.length; ++i)
        {
            ...
            bool is_corpse = b.hasTag("dead") && (b.getName() != "fishy");
            
            bool is_the_new_closest_blob = false;
            if (score < closestScore)
            {
                if ((is_corpse && closest_is_corpse) || // NOTE(hobey): found another corpse that's closer
                    !(have_target && !closest_is_corpse && is_corpse)) { // NOTE(hobey): ignore the corpse, we have a non-corpse item in sight
                    is_the_new_closest_blob = true;
                }
            } else {
                if (have_target && closest_is_corpse && !is_corpse) { // NOTE(hobey): forget about the corpse that's closer, we found a non-corpse item
                    is_the_new_closest_blob = true;
                }
            }
            
            if (is_the_new_closest_blob) {
                closestScore = score;
                @closest = @b;
                have_target = true;
                closest_is_corpse = is_corpse;
            }
        }
    }
