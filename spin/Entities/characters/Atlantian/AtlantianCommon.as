//common atlantian header
namespace AtlantianStates
{
	enum States
	{
		normal = 0,
...
{
	return (state >= AtlantianStates::shielding && state <= AtlantianStates::shieldgliding);
}
...
{
	return (state >= AtlantianStates::sword_drawn && state <= AtlantianStates::sword_power_super);
}
...
f32 getCutAngle(CBlob@ this, u8 state)
{
	...
	if (state == AtlantianStates::sword_cut_mid)
	{
		attackAngle += (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == AtlantianStates::sword_cut_mid_down)
	{
		attackAngle -= (this.isFacingLeft() ? 30.0f : -30.0f);
	}
	else if (state == AtlantianStates::sword_cut_up)
	{
		attackAngle += (this.isFacingLeft() ? 80.0f : -80.0f);
	}
	else if (state == AtlantianStates::sword_cut_down)
	{
		attackAngle -= (this.isFacingLeft() ? 80.0f : -80.0f);
	}
...
f32 getCutAngle(CBlob@ this)
{
	...
	if (direction == -1)
	{
		tempState = AtlantianStates::sword_cut_up;
	}
	else if (direction == 0)
	{
		if (aimpos.y < this.getPosition().y)
		{
			tempState = AtlantianStates::sword_cut_mid;
		}
		else
		{
			tempState = AtlantianStates::sword_cut_mid_down;
		}
	}
	else
	{
		tempState = AtlantianStates::sword_cut_down;
	}

	return getCutAngle(this, tempState);
}

//shared attacking/bashing constants (should be in AtlantianVars but used all over)
