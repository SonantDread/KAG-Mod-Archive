...
{
	addRunnerTextures(this, "atlantian", "Atlantian");
...
{
	ensureCorrectRunnerTexture(this, "atlantian", "Atlantian");
}
...
void onTick(CSprite@ this)
{
	...
	if (!blob.get("atlantianInfo", @atlantian))
	{
		return;
	}
	...
	else if (atlantian.state == AtlantianStates::shieldgliding)
	{
	...
	else if (atlantian.state == AtlantianStates::shielding)
	{
		if (walking)
		{
			if (direction == 0)
	...
	else if (atlantian.state == AtlantianStates::sword_drawn)
	{
		if (atlantian.swordTimer < AtlantianVars::slash_charge)
		{
			this.SetAnimation("draw_sword");
		}
		else if (atlantian.swordTimer < AtlantianVars::slash_charge_level2)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 0;
		}
		else if (atlantian.swordTimer < AtlantianVars::slash_charge_limit)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 1;
			shinydot = true;
	...
	else if (atlantian.state == AtlantianStates::sword_cut_mid_down)
	{
		this.SetAnimation("strike_mid_down");
	}
	else if (atlantian.state == AtlantianStates::sword_cut_up)
	{
		this.SetAnimation("strike_up");
	}
	else if (atlantian.state == AtlantianStates::sword_cut_down)
	{
		this.SetAnimation("strike_down");
	}
	else if (atlantian.state == AtlantianStates::sword_power || atlantian.state == AtlantianStates::sword_power_super)
	{
		this.SetAnimation("strike_power");

		if (atlantian.swordTimer <= 1)
			this.animation.SetFrameIndex(0);

		u8 mintime = 6;
		u8 maxtime = 8;
		if (atlantian.swordTimer >= mintime && atlantian.swordTimer <= maxtime)
		{
			wantsChopLayer = true;
			chopframe = atlantian.swordTimer - mintime;
			chopAngle = -vec.Angle();
		}
	}
	else if (inair)
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (AtlantianVars::slash_charge_limit - AtlantianVars::slash_charge_level2);
			f32 count = (atlantian.swordTimer - AtlantianVars::slash_charge_level2);
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/Atlantian/AtlantianGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/Atlantian/AtlantianGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/Atlantian/AtlantianGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/Atlantian/AtlantianGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
