...
void atlantian_actorlimit_setup(CBlob@ this)
{
	u16[] networkIDs;
	this.set("LimitedActors", networkIDs);
}

bool atlantian_has_hit_actor(CBlob@ this, CBlob@ actor)
{
	u16[]@ networkIDs;
...
u32 atlantian_hit_actor_count(CBlob@ this)
{
	u16[]@ networkIDs;
	this.get("LimitedActors", @networkIDs);
...
void atlantian_add_actor_limit(CBlob@ this, CBlob@ actor)
{
	this.push("LimitedActors", actor.getNetworkID());
}

void atlantian_clear_actor_limits(CBlob@ this)
{
...
{
	AtlantianInfo atlantian;

	atlantian.state = AtlantianStates::normal;
	atlantian.swordTimer = 0;
	atlantian.shieldTimer = 0;
	atlantian.slideTime = 0;
	atlantian.doubleslash = false;
	atlantian.shield_down = getGameTime();
	atlantian.tileDestructionLimiter = 0;

	this.set("atlantianInfo", @atlantian);

	this.set_f32("gib health", -1.5f);
	addShieldVars(this, SHIELD_BLOCK_ANGLE, 2.0f, 5.0f);
	atlantian_actorlimit_setup(this);
	...
	SetHelp(this, "help self action", "atlantian", "$Jab$Jab        $LMB$", "", 4);
	SetHelp(this, "help self action2", "atlantian", "$Shield$Shield    $KEY_HOLD$$RMB$", "", 4);
...
void onTick(CBlob@ this)
{
	...
	AtlantianInfo@ atlantian;
	if (!this.get("atlantianInfo", @atlantian))
	{
		return;
	}
	...
	bool shieldState = isShieldState(atlantian.state);
	bool specialShieldState = isSpecialShieldState(atlantian.state);
	bool swordState = isSwordState(atlantian.state);
	...
	//we'll see if atlantians dmging stuff while in menus is a real issue and go from there
	if (knocked > 0)// || myplayer && getHUD().hasMenus())
	{
		atlantian.state = AtlantianStates::normal; //cancel any attacks or shielding
		atlantian.swordTimer = 0;
		atlantian.shieldTimer = 0;
		atlantian.slideTime = 0;
		atlantian.doubleslash = false;
	...
	{
		moveVars.jumpFactor *= 1.7f;
		moveVars.walkFactor *= 1.5f;
		atlantian.swordTimer = 0;

		if (!canRaiseShield(this))
		{
			if (atlantian.state != AtlantianStates::normal)
			{
				atlantian.shield_down = getGameTime() + 40;
			}

			atlantian.state = AtlantianStates::normal;

			if (pressed_a2 && ((atlantian.shield_down - getGameTime()) <= 0))
			{
		...
		else
		{
			bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f) &&
			                  moveVars.fallCount > AtlantianVars::glide_down_time);

			if (pressed_a2 && inair && !this.isInWater())
			{
				if (direction == -1 && !forcedrop && !getMap().isInWater(pos + Vec2f(0, 16)) && !moveVars.wallsliding)
				{
					atlantian.state = AtlantianStates::shieldgliding;
					atlantian.shieldTimer = 1;
				}
				else if (forcedrop || direction == 1)
				{
					atlantian.state = AtlantianStates::shielddropping;
					atlantian.shieldTimer = 5;
					atlantian.slideTime = 0;
				}
				else //remove this for partial locking in mid air
				{
					atlantian.state = AtlantianStates::shielding;
				}
			}

			if (atlantian.state == AtlantianStates::shieldgliding && !this.isInWater() && !forcedrop)
			{
				moveVars.stoppingFactor *= 0.0f;

				f32 glide_amount = 1.0f - (moveVars.fallCount / f32(AtlantianVars::glide_down_time * 2));
			...
			else if (atlantian.state == AtlantianStates::shielddropping)
			{
				if (this.isInWater())
				{
				...
				else
				{
					...
					{
						atlantian.slideTime++;
						if (atlantian.slideTime > 0)
						{
							if (atlantian.slideTime == 5)
							{
								this.getSprite().PlayRandomSound("/Scrape");
							}

							f32 factor = Maths::Max(1.0f, 2.2f / Maths::Sqrt(atlantian.slideTime));
							moveVars.walkFactor *= factor;

							//  printf("atlantian.slideTime = " + atlantian.slideTime  );
							if (atlantian.slideTime > 30)
							{
								moveVars.walkFactor *= 1.4f;
								if (atlantian.slideTime > 45)
								{
									atlantian.state = AtlantianStates::shielding;
								}
							}
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		if (getNet().isClient())
		{
			if (atlantian.swordTimer == AtlantianVars::slash_charge_level2)
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
			else if (atlantian.swordTimer == AtlantianVars::slash_charge)
			{
				Sound::Play("SwordSheath.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
		}

		if (atlantian.swordTimer >= AtlantianVars::slash_charge_limit)
		{
			Sound::Play("/Stun", pos, 1.0f, this.getSexNum() == 0 ? 1.0f : 2.0f);
			SetKnocked(this, 15);
		}

		bool strong = (atlantian.swordTimer > AtlantianVars::slash_charge_level2);
		...
		if (atlantian.state == AtlantianStates::normal ||
		        ...
		        (!inMiddleOfAttack(atlantian.state) || shieldState))
		{
			atlantian.state = AtlantianStates::sword_drawn;
			atlantian.swordTimer = 0;
		}

		if (atlantian.state == AtlantianStates::sword_drawn && getNet().isServer())
		{
			atlantian_clear_actor_limits(this);
		}

		//responding to releases/noaction
		s32 delta = atlantian.swordTimer;
		if (atlantian.swordTimer < 128)
			atlantian.swordTimer++;

		if (atlantian.state == AtlantianStates::sword_drawn && !pressed_a1 &&
		        !this.isKeyJustReleased(key_action1) && delta > AtlantianVars::resheath_time)
		{
			atlantian.state = AtlantianStates::normal;
		}
		else if (this.isKeyJustReleased(key_action1) && atlantian.state == AtlantianStates::sword_drawn)
		{
			atlantian.swordTimer = 0;

			if (delta < AtlantianVars::slash_charge)
			{
				if (direction == -1)
				{
					atlantian.state = AtlantianStates::sword_cut_up;
				}
				else if (direction == 0)
				{
					if (aimpos.y < pos.y)
					{
						atlantian.state = AtlantianStates::sword_cut_mid;
					}
					else
					{
						atlantian.state = AtlantianStates::sword_cut_mid_down;
					}
				}
				else
				{
					atlantian.state = AtlantianStates::sword_cut_down;
				}
			}
			else if (delta < AtlantianVars::slash_charge_level2)
			{
				atlantian.state = AtlantianStates::sword_power;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				atlantian.slash_direction = aiming_direction;
			}
			else if (delta < AtlantianVars::slash_charge_limit)
			{
				atlantian.state = AtlantianStates::sword_power_super;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				atlantian.slash_direction = aiming_direction;
		...
		else if (atlantian.state >= AtlantianStates::sword_cut_mid &&
		         atlantian.state <= AtlantianStates::sword_cut_down) // cut state
		{
			...
			{
				f32 attackarc = 90.0f;
				f32 attackAngle = getCutAngle(this, atlantian.state);

				if (atlantian.state == AtlantianStates::sword_cut_down)
				{
					attackarc *= 0.9f;
				}

				DoAttack(this, 1.0f, attackAngle, attackarc, Hitters::sword, delta, atlantian);
			}
			else if (delta >= 9)
			{
				atlantian.swordTimer = 0;
		...
		else if (atlantian.state == AtlantianStates::sword_power ||
		         atlantian.state == AtlantianStates::sword_power_super)
		{
			//setting double
			if (atlantian.state == AtlantianStates::sword_power_super &&
			        this.isKeyJustPressed(key_action1))
			{
				atlantian.doubleslash = true;
			}
			...
			{
				DoAttack(this, 2.0f, -(vec.Angle()), 120.0f, Hitters::sword, delta, atlantian);
			}
			else if (delta >= AtlantianVars::slash_time ||
			         (atlantian.doubleslash && delta >= AtlantianVars::double_slash_time))
			{
				atlantian.swordTimer = 0;

				if (atlantian.doubleslash)
				{
					atlantian_clear_actor_limits(this);
					atlantian.doubleslash = false;
					atlantian.state = AtlantianStates::sword_power;
		...
		if ((atlantian.state == AtlantianStates::sword_power ||
		        atlantian.state == AtlantianStates::sword_power_super) &&
		        delta < AtlantianVars::slash_move_time)
		{

			if (Maths::Abs(vel.x) < AtlantianVars::slash_move_max_speed &&
			        vel.y > -AtlantianVars::slash_move_max_speed)
			{
				Vec2f slash_vel =  atlantian.slash_direction * this.getMass() * 0.5f;
	...
	else if (this.isKeyJustReleased(key_action2) || this.isKeyJustReleased(key_action1) || this.get_u32("atlantian_timer") <= getGameTime())
	{
		atlantian.state = AtlantianStates::normal;
	}
	...
	if (myplayer)
	{
		...
		{
			SetHelp(this, "help self action", "atlantian", "$Slash$ Slash!    $KEY_HOLD$$LMB$", "", 13);
		}
	}
	...
	if (shieldState)
	{
		...
		{
			if (atlantian.state == AtlantianStates::shieldgliding)
			{
		...
		if ((atlantian.state == AtlantianStates::shielding && direction == -1) ||
		        atlantian.state == AtlantianStates::shieldgliding)
		{
...
{
	// play cling sound if other atlantian attacked us
	// dmg could be taken out here if we ever want to

	if (hitterBlob.getPosition().x < this.getPosition().x && hitterBlob.getName() == "atlantian") // atlantian and the left one (to play only once)
	{
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, AtlantianInfo@ info)
{
	if (!getNet().isServer())
	{
		return;
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (atlantian_has_hit_actor(this, b))
				{
					if (large)
						dontHitMore = true;
				...
				atlantian_add_actor_limit(this, b);
				if (!dontHitMore)
				{
...
bool isSliding(AtlantianInfo@ atlantian)
{
	return (atlantian.slideTime > 0 && atlantian.slideTime < 45);
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (this.getShape().vellen > SHIELD_KNOCK_VELOCITY || onground)
	{
		...
		//printf("atlantian.stat " + atlantian.state );
		if (atlantian.state == AtlantianStates::shielddropping &&
		        (!onground || isSliding(atlantian)) &&
		        ...
		        getKnocked(blob) == 0)
		{
			...
			if (vel * normal < 0.0f && atlantian_hit_actor_count(this) == 0) //only bash one thing per tick
			{
				ShieldVars@ shieldVars = getShieldVars(this);
				//printf("shi " + shieldVars.direction * normal );
				if (shieldVars.direction * normal < 0.0f)
				{
					atlantian_add_actor_limit(this, blob);
...
bool hasItem(CBlob@ this, const string &in name)
{
	...
	{
		warn("our inventory was null! AtlantianLogic.as");
	}
...
void TakeItem(CBlob@ this, const string &in name)
{
	...
	if (inv !is null)
	{
		...
		{
			warn("took a bomb even though we dont have one! AtlantianLogic.as");
		}
	}
	else
	{
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (this.isMyPlayer() && this.getInventory().getItemsCount() > 1)
	{
		for (uint j = 1; j < bombTypeNames.length; j++)
		{
			...
			{
				SetHelp(this, "help inventory", "atlantian", "$Help_Bomb1$$Swap$$Help_Bomb2$         $KEY_TAP$$KEY_F$", "", 2);
				break;
			}
		}
	}
