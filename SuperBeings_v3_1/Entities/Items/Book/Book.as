	...
	if (cmd == this.getCommandID("usebook"))
	{
		...
		if (caller !is null)
		{
			...
			if (player !is null)
			{
				...
				if(player.get_s16("book_level") < this.get_s16("level")){
					if(XORRandom(10) == 0){
						player.set_s16("book_level",this.get_s16("level"));
					}
				}
			}
		}
...
void onTick(CBlob@ this){
	...
	this.set_s16("mold",this.get_s16("mold")+1);
	
	this.SetInventoryIcon("Books.png", this.get_s16("type"), Vec2f(16,16));
	
	if(this.isInInventory())this.set_s16("mold",0);
	
	if(this.get_s16("mold") > 60*30)this.server_Die();
}
