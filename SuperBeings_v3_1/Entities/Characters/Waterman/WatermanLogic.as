...
{
	this.set_s16("empowerwaterlvl2",600);
	...
	if (this.isKeyPressed(key_action2) && !this.isKeyPressed(key_action1)){
		...
		CMap@ map = getMap();
		int range = 0;
		if(this.get_s16("empowerfire") > 0)range += 2;
		if(this.get_s16("empowerfirelvl2") > 0)range += 4;
		if(this.get_s16("empowerfirelvl3") > 0)range += 8;
		
		if (map != null)
		for (int doFirey = -80; doFirey <= 80; doFirey += 1 * 8) //8 - tile size in pixels
		{
			for (int doFirex = 0; doFirex <= (4+range) * 8; doFirex += 1 * 8) //8 - tile size in pixels
			{
