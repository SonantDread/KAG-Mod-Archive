		...
		if ( map.getHitInfosFromRay( pos, angle, FLAK_REACH, this, @hitInfos ) )
		{
			for ( uint i = 0; i < hitInfos.length; i++ )//sharpnel trail
			{
				...
				if ( Block::isSolid( blockType ) || ( !sameTeam
					&& ( blockType == Block::SEAT || b.hasTag( "weapon" ) || b.hasTag( "rocket" ) || blockType == Block::MOTHERSHIP5 || blockType == Block::SECONDARYCORE || blockType == Block::DOOR || Block::isBomb( blockType ) || ( b.hasTag( "player" ) && !b.isAttached() ) ) ) )
				{
...
f32 getDamage( CBlob@ hitBlob, int blockType )
{
	...
	if ( blockType == Block::SEAT || (hitBlob.hasTag( "weapon" ) && hitBlob.getName() != "hyperflek" ))
		return 0.25f;
