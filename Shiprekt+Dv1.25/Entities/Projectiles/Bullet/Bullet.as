	...
	if (getMap().getHitInfosFromRay( pos, -vel.Angle(), vel.Length(), this, @hitInfos ))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if ( !b.hasTag( "booty" ) && (color > 0 || !isBlock) )
			{
				...
				{
					if ( Block::isSolid(blockType) || ( b.getTeamNum() != this.getTeamNum() && ( blockType == Block::MOTHERSHIP5 || blockType == Block::SECONDARYCORE || b.hasTag("weapon") || blockType == Block::BOMB || blockType == Block::DOOR ) ) )//hit these and die
						killed = true;
...
f32 getDamage( CBlob@ hitBlob, int blockType )
{
	...
	if ( hitBlob.hasTag( "weapon" ) && hitBlob.getName() != "hyperflek" )
		return 0.07f;
	
	if ( hitBlob.getName() == "hyperflek" )
		return 0.05f;
...
void onHitBlob( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData )
{
	...
	else	if (Block::isSolid(blockType) || blockType == Block::MOTHERSHIP5 || blockType == Block::SECONDARYCORE || hitBlob.hasTag("weapon") || blockType == Block::PLATFORM || blockType == Block::SEAT || Block::isBomb( blockType ) || blockType == Block::DOOR)
	{
		//effects
		sparks(worldPoint, 8);
