...
const Vec2f BUILD_MENU_SIZE = Vec2f( 5, 4 );
		...
		if ( core !is null && !core.hasTag( "critical" ) )
		{
			...
			bool canShop = pIsle !is null && pIsle.centerBlock !is null 
							&& ( (pIsle.centerBlock.getShape().getVars().customData == core.getShape().getVars().customData) 
									|| ((pIsle.isStation || pIsle.isSecondaryCore) && pIsle.centerBlock.getTeamNum() == this.getTeamNum()));
				...
				if ( !hud.hasButtons() )
				{
					if ( canShop )
					{
						...
						if ( gameTime - this.get_u32( "menu time" ) > BUILD_MENU_COOLDOWN )
						{
							...
							BuildShopMenu( this, core, "mCore Block Transmitter", Vec2f(0,0), pIsle.isStation);
						}
						else
...
void BuildShopMenu( CBlob@ this, CBlob@ core, string description, Vec2f offset, bool isStation = false)
{
	...
	if ( menu !is null ) 
	{
		...
		if (isStation)
		{
			...
			CGridButton@ button = menu.AddButton( "$SECONDARYCORE$", "Secondary Core $" + c.secondaryCore, core.getCommandID("buyBlock"), params );
			...
			if ( gameTime > WARMUP_TIME )
				button.SetHoverText( "A secondary ship core.\nWeight: " + w.secondaryCore * 100 + "rkt" );
			else
			{
		...
		}
		{
			CBitStream params;
			params.write_u16( netID );
			params.write_string( "hyperflek" );
				
			CGridButton@ button = menu.AddButton( "$HYPERFLEK$", "Hyper-Flak $" + c.hyperflek, core.getCommandID("buyBlock"), params );
	
			bool select = lastBuy == "hyperflek";
			if ( select )
				button.SetSelected(2);
				
			if ( gameTime > WARMUP_TIME )
				button.SetHoverText( "A long-ranged manual defensive turret that fires high-explosive fragmentation shells with a proximity fuse. Best used as an unarmored ship deterrent. Effective against everything.\nWeight: " + w.hyperflek * 100 + "rkt \nAmmoCap: medium\n" + ( select ? repBuyTip : "" ) );
			else
			{
				button.SetHoverText( warmupText );
				button.SetEnabled( false );
