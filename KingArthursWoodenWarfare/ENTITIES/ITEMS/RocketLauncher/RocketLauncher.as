...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _unused)
{
	if (bullet !is null)
	{
		...
		Vec2f vel = Vec2f(140.0f / 16.0f * (this.isFacingLeft() ? -1 : 1), 0.0f).RotateBy(angle);
		bullet.setVelocity(vel);
		Vec2f offset = arm_offset;
		offset.RotateBy(angle);
		bullet.setPosition(this.getPosition() + offset * .2f);
		
		bullet.getShape().setDrag(bullet.getShape().getDrag() * 0.45f);

		bullet.server_SetTimeToDie(-1);   // override lock
		bullet.server_SetTimeToDie(10.5f);
		...
		if (isClient())
		{
			...
			ParticleAnimated("Explosion", (this.getPosition() + offset * .2f) + vel*0.6, getRandomVelocity(0.0f, XORRandom(40) * 0.01f, this.isFacingLeft() ? 90 : 270) + Vec2f(0.0f, -0.05f), float(XORRandom(360)), 0.6f + XORRandom(50) * 0.01f, 2 + XORRandom(3), XORRandom(70) * -0.00005f, true);
		}
	}
}
