...
void ManageBow(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	bool isLoading = archer.isLoading;
	...
	else if (this.isKeyPressed(key_action1))
	{
		...
		if (charge_time == 102 && !isStabbing && !isLoading)
		{
	...
	// Reload SFX
	if (charge_time == 9 && !archer.isStabbing && !archer.isLoading && hasarrow && this.isMyPlayer())
	{
		this.getSprite().PlaySound("ReloadSound.ogg");
	}
	...
	if (responsible)
	{
		...
		if (ismyplayer && !getHUD().hasButtons())
		{
			...
			if (archer.isLoading)
			{
				...
				else if (charge_time <= 19)
				{
					frame = 6;
				}
				else if (charge_time <= 25)
				{
					frame = 7;
				}
				else if (charge_time <= 31)
				{
					frame = 8;
				}
				else if (charge_time <= 37)
				{
					frame = 9;
				}
				else if (charge_time <= 44)
				{
					frame = 10;
				}
...
void ClientFire(CBlob@ this, const s8 charge_time, const bool hasarrow, const u8 arrow_type, const bool legolas)
{
	...
	if (hasarrow && canSend(this))  // client-logic
	{
		...
		{
			ShootArrow(this, this.getPosition() - Vec2f(0.0f, 7.0f), this.getAimPos() + Vec2f(-2.0f + XORRandom(4), -2.0f + XORRandom(4)), ArcherParams::shoot_max_vel*1.6f, ArrowType::normal, legolas);
		}
		else
		{
			ShootArrow(this, this.getPosition() - Vec2f(0.0f, 7.0f), this.getAimPos() + Vec2f(-4.0f + XORRandom(8), -6.0f + XORRandom(8)), ArcherParams::shoot_max_vel*1.6f, ArrowType::normal, legolas);
		}
		
	
		CMap@ map = getMap();
		ParticleAnimated("SmallExplosion2", this.getPosition() + Vec2f(this.isFacingLeft() ? -12.0f : 12.0f, -0.0f), getRandomVelocity(0.0f, XORRandom(40) * 0.01f, this.isFacingLeft() ? 90 : 270) + Vec2f(0.0f, -0.05f), float(XORRandom(360)), 0.6f + XORRandom(50) * 0.01f, 2 + XORRandom(3), XORRandom(70) * -0.00005f, true);
		...
		if (p !is null)
		{
			...
			if (local !is null)
			{
				...
				{
					Recoil(this, local, 13.0f);

					ShakeScreen(48, 16, this.getPosition());
				}
			}
		}
	}
...
{
	CBlob@ arrow = server_CreateBlobNoInit("ballista_bolt");
	if (arrow !is null)
	{
		// fire arrow?
		...
		arrow.getShape().setDrag(arrow.getShape().getDrag() * 0.6f);
		arrow.server_SetTimeToDie(-1);   // override lock
		arrow.server_SetTimeToDie(1.2f);
	}
	return arrow;
}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		this.getSprite().PlaySound("RPGFire.ogg");
