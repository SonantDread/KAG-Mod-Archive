	...
	else if (this.isKeyPressed(key_action1))
	{
		...
		if (charge_time == 102 && !isStabbing && !isLoading)
		{
			...
			if (!hasarrow)
			{
				...
				{
					this.getSprite().PlaySound("Entities/Characters/Sounds/EmptyFire.ogg", 0.5);
				}
			}
			else
			{
...
void ClientFire(CBlob@ this, const s8 charge_time, const bool hasarrow, const u8 arrow_type, const bool legolas)
{
	...
	if (hasarrow && canSend(this))  // client-logic
	{
		...
		{
			ShootArrow(this, this.getPosition(), this.getAimPos(), ArcherParams::shoot_max_vel*2.3f, ArrowType::normal, legolas);
		}
		else
		{
			ShootArrow(this, this.getPosition(), this.getAimPos() + Vec2f(-4.0f + XORRandom(8), -6.0f + XORRandom(8)), ArcherParams::shoot_max_vel*2.3f, ArrowType::normal, legolas);
		}
		...
		if (p !is null)
		{
			...
			if (local !is null)
			{
				...
				{
					Recoil(this, local, 8.5f);

					ShakeScreen(38, 14, this.getPosition());
...
{
	CBlob@ arrow = server_CreateBlobNoInit("sniperbullet");
	if (arrow !is null)
	{
		// fire arrow?
		...
		arrow.getShape().setDrag(arrow.getShape().getDrag() * 0.1f);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		this.getSprite().PlaySound("SniperFire.ogg");
