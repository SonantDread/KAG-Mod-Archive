...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if (hitterBlob.getDamageOwnerPlayer() !is null)
	{
		if (hitterBlob.getDamageOwnerPlayer().getBlob() !is null)
		{
			...
			{
				hitterBlob.getDamageOwnerPlayer().getBlob().getSprite().PlaySound("Hitmarker.ogg", 2.0f, 1.0f);
			}
		}
	}
		...
		if (hitterBlob.getDamageOwnerPlayer() !is null)
		{
			if (hitterBlob.getDamageOwnerPlayer().getBlob() !is null)
			{
				if (hitterBlob.getDamageOwnerPlayer().getBlob().getSprite() !is null && this !is hitterBlob)
				{
					...
					{
						hitterBlob.getDamageOwnerPlayer().getBlob().server_Heal(10.0f);
					}
				}
			}
		}
		...
		if (this.getSprite() !is null) //moved here to prevent other logic potentially not getting run
		{
			...
			if (this !is hitterBlob || customData == Hitters::fall)
			{
				...
				if (hitterBlob.getDamageOwnerPlayer() !is null)
				{
					if (hitterBlob.getDamageOwnerPlayer().getBlob() !is null)
					{
						if (hitterBlob.getDamageOwnerPlayer().getBlob().getSprite() !is null && hitterBlob.getDamageOwnerPlayer().getBlob() !is null)
						{
							hitterBlob.getDamageOwnerPlayer().getBlob().getSprite().PlaySound("KillSound.ogg", 2.0f, 1.0f);
						}
					}
				}
			}
	...
	if (player !is null && player.hasTag("Kevlar"))
	{
		if (customData == Hitters::arrow) damage = -0.125f;
		else if (customData == Hitters::explosion) damage = -1.5f;
		else if (customData == Hitters::bomb) damage = -1.5f;
		else if (customData == Hitters::sword) damage = -1.0f;
	}
