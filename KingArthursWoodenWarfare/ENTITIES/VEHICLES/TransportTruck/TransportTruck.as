...
void onInit(CBlob@ this)
{
	...
	if (getNet().isServer())
	{
	    ...
	    for(i = 0; i < 3; i++)
	    {
			CBlob@ loot = server_CreateBlob("mat_heatwarhead",-1,this.getPosition());
		    if (loot !is null)
			{
				if (!this.server_PutInInventory(loot))
	    ...
	    for(i2 = 0; i2 < 2; i2++)
	    {
			CBlob@ loot = server_CreateBlob(XORRandom(100) < 50 ? "food" : "steak",-1,this.getPosition());
		    if (loot !is null)
			{
				if (!this.server_PutInInventory(loot))
					loot.setPosition(this.getPosition());
	    ...
	    for(i3 = 0; i3 < 2; i3++)
	    {
			CBlob@ loot = server_CreateBlob("mine",this.getTeamNum(),this.getPosition());
		    if (loot !is null)
			{
				if (!this.server_PutInInventory(loot))
					loot.setPosition(this.getPosition());
	    ...
	    for(i4 = 0; i4 < 2; i4++)
	    {
			CBlob@ loot = server_CreateBlob("helmet",this.getTeamNum(),this.getPosition());
		    if (loot !is null)
			{
				if (!this.server_PutInInventory(loot))
					loot.setPosition(this.getPosition());
	    ...
	    for(i5 = 0; i5 < 2; i5++)
	    {
			CBlob@ loot = server_CreateBlob("grenade",this.getTeamNum(),this.getPosition());
		    if (loot !is null)
			{
				if (!this.server_PutInInventory(loot))
					loot.setPosition(this.getPosition());
	    ...
	    for(i6 = 0; i6 < 4; i6++)
	    {
			CBlob@ loot = server_CreateBlob("medkit",this.getTeamNum(),this.getPosition());
		    if (loot !is null)
			{
				if (!this.server_PutInInventory(loot))
					loot.setPosition(this.getPosition());
