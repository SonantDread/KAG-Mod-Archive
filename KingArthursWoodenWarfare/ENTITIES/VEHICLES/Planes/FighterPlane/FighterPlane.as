...
void onInit(CBlob@ this )
{
	...
	if (getNet().isServer())
	{
		CBlob@ bow = server_CreateBlob(XORRandom(100) <= 10 ? "rocketlauncher" : "heavygun");	
...
void onTick( CBlob@ this )
{
	...
	if (Maths::Abs(this.getVelocity().x) > 0.25f)
	{
		...
		this.getSprite().SetEmitSoundVolume(vellen/13.0f);
		this.getSprite().SetEmitSoundSpeed(1.0f + vellen/30.0f);
	}
	else
	{
		this.getSprite().SetEmitSoundPaused(true);
	...
	if (this.getHealth() <= this.getInitialHealth()*0.25f)
	{
		...
		this.setVelocity(this.getVelocity() * (this.isOnMap() ? 0.998 : 0.992));
	}
...
void shootGun(CBlob@ this)
{
	...
	if (isClient())
	{
		this.getSprite().PlaySound("MachineGunShoot", 1.5f, 0.9f);
	...
	if (isServer())
	{
		...
		if (arrow !is null)
		{
			...
			arrow.setPosition(this.getPosition() + Vec2f(0.0f, -3.0f));
			arrow.setVelocity(Vec2f(this.isFacingLeft() ? -28 : 28, (this.getVelocity().y*2.2) + (4 + XORRandom(3))));
			...
			arrow.server_SetTimeToDie(1.8f);
		}
	}
	
	this.set_u32("shootDelay", getGameTime() + 3);
}
