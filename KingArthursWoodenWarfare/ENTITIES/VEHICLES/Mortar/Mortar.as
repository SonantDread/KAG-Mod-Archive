...
const u8 cooldown_time = 80; //1440
...
{
	this.AddForce(Vec2f((this.isFacingLeft() ? 1 : -1)*450.0f, -360.0f));
	...
	{
		Vec2f aim_vec = gunner.getPosition() - gunner.getAimPos();
		ShakeScreen(150.0f, 25.0f, gunner.getAimPos());
	}
	...
	if (bullet !is null)
	{
		...
		angle = (angle - angle%6);
		angle += ((XORRandom(512) - 256) / 90.0f);
		Vec2f vel = Vec2f(0.0f, -charge).RotateBy(angle);
		bullet.setVelocity(vel);
		bullet.setPosition(bullet.getPosition() + Vec2f((this.isFacingLeft() ? -1 : 1)*7.0f, -5.0f)); //here1   2.5
