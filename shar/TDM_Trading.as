...
bool kill_traders_and_shops = true;

const string[] rep = { "mat_bombs",
                     "mat_waterbombs"
                   };
void onBlobCreated(CRules@ this, CBlob@ blob)
{
	...
	{
		if (blob.getName() == rep[i])
		{
			int rndNum = XORRandom(4);
			string rnd = rndNum == 0 ? "mat_arrows" : 
						 rndNum == 1 ? "mat_waterarrows" :
						 rndNum == 2 ? "mat_bombarrows" : "mat_firearrows";

			Vec2f pos = blob.getPosition();
			int team = blob.getTeamNum();

			server_CreateBlob(rnd, team, pos);
			blob.server_Die();
		}
	}
}
