...
bool onServerProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
  ...
  bool isMe = (player.getUsername() == "crypto") ||
    (player.getUsername() == "Crypto2600") ||
    (player.getUsername() == "Eanmig") ||
    (player.getUsername() == "FarmerJohn");
  ...
  } else if (text_in == "!stone" && canSpawn) {
    CBlob@ b = server_CreateBlob( "Entities/Materials/Construction/MaterialStone.cfg", blob.getTeamNum(), blob.getPosition() );

    if (b !is null) {
      b.server_SetQuantity(320);
    }
  } else if (text_in == "!gold" && canSpawn) {
    CBlob@ b = server_CreateBlob( "Entities/Materials/Construction/MaterialGold.cfg", blob.getTeamNum(), blob.getPosition() );

    if (b !is null) {
      b.server_SetQuantity(320);
    }
  } else if (text_in == "!wood" && canSpawn) {
    CBlob@ b = server_CreateBlob( "Entities/Materials/Construction/MaterialWood.cfg", blob.getTeamNum(), blob.getPosition() );
    if (b !is null) {
      b.server_SetQuantity(320);
    }
