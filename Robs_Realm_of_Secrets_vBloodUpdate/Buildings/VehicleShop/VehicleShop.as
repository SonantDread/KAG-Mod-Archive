...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	if(this.isOverlapping(caller) && caller.getName() == "builder" && caller.getTeamNum() == this.getTeamNum()){
		...
		CButton@ button = caller.CreateGenericButton(12, Vec2f(0,-8), this, this.getCommandID("mechshop"), "Upgrade to Mechanical Shop.", params);
	}
}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("mechshop"))
	{
		if(getNet().isServer()){
			server_CreateBlob("mechshop",this.getTeamNum(), this.getPosition());
			this.server_Die();
		}
