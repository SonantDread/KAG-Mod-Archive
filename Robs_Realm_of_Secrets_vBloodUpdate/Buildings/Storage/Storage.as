...
void GetButtonsFor(CBlob@ this, CBlob@ caller)
{
	...
	if(this.isOverlapping(caller) && caller.getName() == "builder" && caller.getTeamNum() == this.getTeamNum()){
		CBitStream params;
		params.write_u16(caller.getNetworkID());
		CButton@ button = caller.CreateGenericButton(12, Vec2f(0,-8), this, this.getCommandID("library"), "Convert into library.", params);
	}
}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("library"))
	{
		if(getNet().isServer()){
			server_CreateBlob("library",this.getTeamNum(), this.getPosition());
			this.server_Die();
		}
