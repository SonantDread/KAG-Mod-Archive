...
void onTick(CBlob@ this)
{
	...
	if(this.isInWater()){
		this.set_s16("size",this.get_s16("size")+1);
		getMap().server_setFloodWaterWorldspace(this.getPosition(), false);
	}
...
{
	return (blob.getName() != "wooden_platform") && (blob.getShape().isStatic() || blob.hasTag("player") || blob.hasTag("element"));
}
...
void DestroySelf(CBlob@ this){
	if(getNet().isServer()){
		...
		if(this.getName() == "fire_blob"){
			...
			for (int doFirey = -this.get_s16("size"); doFirey <= this.get_s16("size"); doFirey += 8)
			{
				for (int doFirex = -this.get_s16("size"); doFirex <= this.get_s16("size"); doFirex += 8)
				{
					Vec2f pos = Vec2f(this.getPosition().x + doFirex, this.getPosition().y + doFirey);
					if(Maths::Sqrt((Maths::Pow(pos.x-this.getPosition().x,2))+(Maths::Pow(pos.y-this.getPosition().y,2))) <= this.get_s16("size")/2){
						getMap().server_setFireWorldspace(pos, true);
					}
				}
			}
			SetupBomb(this, 1, this.get_s16("size"), Maths::Min(this.get_s16("size")/50,250), this.get_s16("size"), this.get_s16("size")/1000, false);
		}
		this.server_Die();
	}
}