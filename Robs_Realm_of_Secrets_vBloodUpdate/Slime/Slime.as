	...
	if (this.isAttached())
	{
		if(!this.hasTag("baby")){
			...
			if (att !is null)
			{
				...
				{
					this.server_Hit(b, b.getPosition(), Vec2f(0,0.1), 0.25f, Hitters::suddengib, false);
					if(XORRandom(10) == 0){
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal)
{
	...
	if(blob !is null && blob.hasTag("flesh") && this.getTeamNum() != blob.getTeamNum() && !blob.hasTag("onewithnature"))
	{
		if(!this.hasTag("baby"))
		if(getNet().isServer()){
			blob.server_Hit(blob, this.getPosition(), this.getVelocity()*-0.5f, 0.5f, Hitters::suddengib, false);
		}
	}
