...
void Growth(Vec2f target)
{
	for(int loop = 0; loop < 2; loop += 1)
	for(int span = -5; span <= 5; span += 1){
		CMap@ map = getMap();
		
		int newPos = target.x+span*8;
		...
		if (map.isTileGround(tile) && !map.isInWater(tilePos + Vec2f(0, -8)))
		{
			if (map.getTile(tilePos + Vec2f(0, -8)).type == CMap::tile_empty)
			{
				...
				server_MakeSeed(tilePos + Vec2f(0, -8), seeds[XORRandom(seeds.length)]);
			}
			else if (!map.isTileSolid(tilePos + Vec2f(0, -8)))
			{
				server_MakeSeed(tilePos + Vec2f(0, -8), seeds[XORRandom(3)]);
			}
