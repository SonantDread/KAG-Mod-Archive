...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	if (blob is null || !blob.hasTag("ignore blocking actors"))
	{
		...
		if (!isSeed && !isLadder && !isSpikes && (buildSolid || isDoor)
			&& (map.getSectorAtPosition(middle, "no build") !is null
				|| map.getSectorAtPosition(middle, "no_build_new") !is null))
		{
			return false;
		}

		// ...this seems to do the exact same thing as the previous if statement??
...
bool inNoBuildZone(CBlob@ blob, CMap@ map, Vec2f here, TileType buildTile)
{
	...
	return (!isLadder && (buildSolid || isSpikes)
			&& (map.getSectorAtPosition(here, "no build") !is null));
				// || map.getSectorAtPosition(here, "no_build_new") !is null));
}
