...
void onRender(CSprite@ this)
{
	    ...
	    (!getHUD().hasButtons()))
	{
		...
		int left = blob.hasTag("flip_sign") ? -200.0f : 200.0f;
		int margin = 4;
		Vec2f dim;
		...
		dim.y *= 1.0f;
		// top += dim.y;
		Vec2f upperleft(pos2d.x - dim.x / 2 - left, pos2d.y - dim.y / 2); // Maths::Min(int(2 * dim.y), 250));
		Vec2f lowerright(pos2d.x + dim.x / 2 - left, pos2d.y + dim.y / 2);
