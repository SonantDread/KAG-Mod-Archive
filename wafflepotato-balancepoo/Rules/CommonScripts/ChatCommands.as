...
bool onServerProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
	...
	else if (tokens[0] == "!shrink" && player.isMod())
	{
		u8 shrink_times = 1;
		if (tokens.length > 1)
		{
		...
		if (getBlobsByName("flag_base", @flags))
		{
			...
			{
				CBitStream params;
				params.write_u8(shrink_times);
				flags[i].SendCommand(flags[i].getCommandID("shrink_zone"), params);
			}
		}
		return true;
	}
	else if (tokens[0] == "!suddendeath" && player.isMod())
	{
		...
		this.set_u16("suddendeath_zone_interval", 60);
		this.set_u8("suddendeath_zone_max_times", 15);
		if (tokens.length > 1)
		{
		...
		if (getBlobsByName("flag_base", @flags))
		{
			for (int i = 0; i < flags.length; ++i)
			{
				...
				else if (flag.hasTag("sudden_death_zone"))
				{
					sd_deactivated = true;
					flag.SendCommand(flag.getCommandID("stop_sudden_death"));
				}
