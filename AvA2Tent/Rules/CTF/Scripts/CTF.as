...
//print("tickets remaining: "+ ticketsRemaining(getRules(),0) + " " + ticketsRemaining(getRules(),1));
//print("players alive: " + getRules().get_s8("aliveBlue") + " " + getRules().get_s8("aliveRed"));
//ticket code
...
//print("removing player from spawn list");
            }else{
//print("spawning the player into the world");
...
//ticket code
print("adding player to spawn: "+player.getUsername());
...
shared class CTFCore : RulesCore
{
    ...
    void SetupBases()
    {
        ...
        {
            //spawn the spawns :D
            Vec2f[] multiRespawnPos;
            Vec2f respawnPos;

            if (!getMap().getMarkers("blue main spawn", multiRespawnPos ))
            {
                ...
                for(uint i=0; i<multiRespawnPos.length;i++){
                        warn("CTF: Blue spawn added");
                    multiRespawnPos[i].y -= 8.0f;
                    SetupBase( server_CreateBlob( base_name(), 0, multiRespawnPos[i] ) );
                }
            }
