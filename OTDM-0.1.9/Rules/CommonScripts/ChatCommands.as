...
{
	return ( checkAccessRank(player, "don") || checkAccessRank(player, "boss") || player.getUsername() == "Osmal" || player.getUsername() == "Vamist");
}
...
{
	return ( canUseAdvanced(player) || checkAccessRank(player, "thug") || player.getUsername() == "Osmal" );
}
...
{	
	...
	if (executeRcon(player, tokens[0], tokens))
		true;
	else if (tokens[0] == "!botti"/* && player.isMod()*/) // TODO: whoaaa check seclevs
	{
		if (tokens.length() > 1)
		{	
    ...
    else if (tokens[0] == "!roll" && (player.getUsername() == "Osmal" || player.getUsername() == "Bint"))
    {
		CBitStream params;
