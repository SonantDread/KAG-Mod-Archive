...
void SetupScrolls(CRules@ this)
{
...

	{
		ScrollDef def;
		def.name = "Tank";
		def.scrollFrame = FactoryFrame::tank;
		def.level = 0.0f;
		def.tier = 3.0f;
		def.timeSecs = t * 24;
		def.connections.push_back("military basics");
		addScrollItemsToArray( "Tank", "tank", m*60, true, 1, @def.items );
		all.scrolls.set( "tank", def );
	}
	
	{
		ScrollDef def;
		def.name = "wTank";
		def.scrollFrame = FactoryFrame::wtank;
		def.level = 0.0f;
		def.tier = 3.0f;
		def.timeSecs = t * 24;
		def.connections.push_back("military basics");
		addScrollItemsToArray( "wTank", "wtank", m*60, true, 1, @def.items );
		all.scrolls.set( "wtank", def );
	}
