...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1) {
    if (blob !is null && doesCollideWithBlob( this, blob ) && !this.hasTag("collided")) {
		...
		CMap@ map = getMap();
		if(map.rayCastSolid(this.getInterpolatedPosition(), blob.getPosition()))	
			return;
		f32 dmg = blob.getTeamNum() == this.getTeamNum() ? 0.0f : this.get_f32("dmg");		
		this.server_Hit(blob, point1, normal, 0.2f, Hitters::arrow);
	}
}
...
void onTick(CBlob@ this) {
	...
	print(" Onground : "+ this.isOnGround() + " onmap : " + this.isOnMap());
}
