	...
	Vec2f getSpawnLocation(PlayerInfo@ p_info)
	{
		...
		if (c_info !is null)
		{
			...
			if (map !is null)
			{
				...
				return Vec2f(x, map.getLandYAtX(s32(x / map.tilesize)) * map.tilesize /2);
			}
		}
...
void ReloadPreloadBlocks()
{
	...
	for (uint x = 0; x < map.tilemapwidth; x++)
	{
		for (uint y = 0; y < map.tilemapheight; y++)
		{
			...
			u8 type = map.getTile(position).type;
			...
			{
				map.server_SetTile(position, CMap::tile_ground_back);
			}		
			if ((type >= 106 && type <= 111) || type == 186) // bedrock
			{
				map.server_SetTile(position, CMap::tile_bedrock);
			}	
			if ((type >= 80 && type <= 85) || (type >= 90 && type <= 94) || type == 160) // gold
			{
				map.server_SetTile(position, CMap::tile_gold);
			}
			if ((type >= 96 && type <= 97) || (type >= 100 && type <= 104) || type == 176) // stone
			{
				map.server_SetTile(position, CMap::tile_stone);
			}

			if ((type >= 208 && type <= 209) || (type >= 214 && type <= 218) || type == 192) // thckstone
			{
				map.server_SetTile(position, CMap::tile_thickstone);
			}
		}
	}
}
