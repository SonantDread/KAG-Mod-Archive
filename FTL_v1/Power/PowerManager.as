...
void AssignPower(CBlob@ this){
...

	{
		...
		for (u32 k = 0; k < blobs.length; k++)
		{
			CBlob@ blob = blobs[k];
			if(blob.getTeamNum() == this.getTeamNum()){
				blob.set_f32("Power",this.get_u8("Power_Engines")/Amount*blob.get_u8("Level"));
			}
		}
	}
}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (getNet().isClient() && cmd == this.getCommandID("ftl_jump")){
		if(this.get_u16("FTLDrive") >= this.get_u16("FTLDriveMax")){
	...
	if (isServer && cmd == this.getCommandID("ftl_jump"))
	{
		if(this.get_u16("FTLDrive") >= this.get_u16("FTLDriveMax")){
		
			loadEvent(getMap(),"test_event.png",-1);
			this.set_u16("FTLDrive",0);
