...
void onTick( CMovement@ this )
{
    ...
    if ( blob.getPlayer() !is null ) {
    	int team_i = blob.getTeamNum();
    	if ( team_i != 0 && team_i != 1 )
			{ fly = true; }

    }*/

    bool fly = false; //true; //blob.get_bool("fly");

	/* Modifications */

	// if action key pressed, dispense with money
	//if ( fly == true ) {
	/*
	if ( blob.isKeyPressed(key_action3) && fly &&  blob.getPlayer() !is null) {
			if( blob.getPlayer().getUsername() == "r0ber7" ) {
				...
				if (mat !is null)
				{
					...
					mat.server_SetQuantity(25);
					mat.setPosition( blob.getPosition() );
					server_DropCoins(blob.getPosition(), 25 );
				...
				if (mat !is null)
				{
					...
					//server_DropCoins(blob.getPosition(), 25 );
					CBlob@ orb =  server_CreateBlob( "orb", getPlayerByUsername("r0ber7").getBlob().getTeamNum(), getPlayerByUsername("r0ber7").getBlob().getPosition() );

					// shoot the orb
							if (orb !is null)
							{
								// take aim
								Vec2f pos = getPlayerByUsername("r0ber7").getBlob().getPosition();
								Vec2f aim = getPlayerByUsername("r0ber7").getBlob().getAimPos();

								Vec2f norm = aim - pos;
								norm.Normalize();
								orb.setVelocity( norm * 5.0f );
...
//							server_CreateBlob( "bison", 1, getPlayerByUsername("Leemo").getBlob().getPosition() );

	}
    */

	// if we are a flying dude, custom code for movement. gravity of zero
	if ( fly == true ) {
...
//			MakeDustParticle(pos, "/dust2.png");
		...
		if (up || down || left || right) {
			Vec2f rainbowpos = (Vec2f( right ? 4.0f : -4.0f, 0.0f ) + pos);

			string texture = "Entities/Effects/Sprites/Star.png";
/*
			print ("Making rainbow particle");

			switch (XORRandom(XORRandom(1) == 0 ? 4 : 2))
			{
				case 0: texture = "Entities/Effects/Sprites/Rainbow.png"; break;

				case 1: texture = "Entities/Effects/Sprites/Rainbow2.png"; break;

				case 2: texture = "Entities/Effects/Sprites/Rainbow3.png"; break;

				case 3: texture = "Entities/Effects/Sprites/Rainbow4.png"; break;
			}*/

			ParticleAnimated(texture, pos, Vec2f(0, 0), 0.0f, 0.7f, 5, -0.1, true);

		}
		// zero gravity
		shape.SetGravityScale( 0.0f );

//        blob.AddForce(flyforce * moveVars.overallScale * 10.0f);
        blob.AddForce(flyforce * 20.0f);
        moveVars.jumpCount = -1;
        moveVars.fallCount = -1;
