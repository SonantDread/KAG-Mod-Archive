...
bool GiveSpawnResources(CRules@ this, CBlob@ blob, CPlayer@ player, SandboxPlayerInfo@ info)
{
	bool ret = false;
...
void onPlayerDie(CRules@ this, CPlayer@ victim, CPlayer@ attacker, u8 customData)
{
	if (victim !is null)
	{
		...
		if (core !is null)
		{
			SandboxPlayerInfo@ info = cast < SandboxPlayerInfo@ > (core.getInfoFromPlayer(victim));
			if (info !is null)
			{
				info.items_collected &= ~ItemFlag::Archer;
			}
...
bool canGetSpawnmats(CRules@ this, CPlayer@ p, RulesCore@ core)
{
	...
	SandboxPlayerInfo@ info = cast < SandboxPlayerInfo@ > (core.getInfoFromPlayer(p));
