...
CSpriteLayer@ LoadHead( CSprite@ this, u8 headIndex )
{
    ...
    CSpriteLayer@ breath = this.addSpriteLayer( "breath", CFileMatcher("breath.png").getFirst(), 8, 8,
                         this.getBlob().getTeamNum(),
                         this.getBlob().getSkinNum() );
    ...
    if (breath !is null)
    {
        Animation@ anim = breath.addAnimation( "default", 5, false );
        anim.AddFrame(0);
        anim.AddFrame(1);
        anim.AddFrame(2);
		anim.AddFrame(3);
		anim.AddFrame(4);
		anim.AddFrame(5);
        breath.SetAnimation( anim );
		breath.SetFacingLeft(!blob.isFacingLeft());
		breath.SetOffset(Vec2f(8,-4));
...
void onTick( CSprite@ this )
{
	...
	CSpriteLayer@ breath = this.getSpriteLayer( "breath" );	 
	if (breath !is null)
	{
		if (breath.isAnimationEnded() && XORRandom(10) == 0) 
		{
			breath.SetVisible( true );
			breath.animation.frame=0;
		}
		if (breath.isAnimationEnded())
		{
