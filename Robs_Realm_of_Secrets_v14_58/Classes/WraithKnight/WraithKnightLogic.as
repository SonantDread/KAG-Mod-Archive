...
void onTick(CBlob@ this)
{
	...
	if(this.hasTag("water"))this.getShape().getConsts().buoyancy = 0;
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		else
		{
			bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f) &&
			                  moveVars.fallCount > KnightVars::glide_down_time*2);
...
{
	//if(!hitterBlob.hasTag("ghost") && hitterBlob.get_s16("death") < 50)damage = damage*0.25;
	
	if(this.get_s16("death") > 0){
		this.set_s16("death",this.get_s16("death")-damage*2);
		MakeMat(hitterBlob, worldPoint, "ectoplasm", damage);
	}
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					if(!b.hasTag("ghost") && b.get_s16("death") < 50)damage = damage*0.25;
