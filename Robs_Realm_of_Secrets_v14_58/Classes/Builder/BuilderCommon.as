	...
	if(canBuild(this, blocks, index))
	{
		...
		if(getNet().isServer())
		{
			if(b.name != "flag_base"){ //////////'GOTOFLAG'//////////
				...
				if(this.getTeamNum() > 8){ //If we aren't part of the 8 teams
					newteam = 0;
					...
					if(newteam == 8){ //All 8 teams have been taken.
						return null;
					}
				
				}
				
				CBlob@ blockBlob = server_CreateBlob(b.name, newteam, pos+Vec2f(0,8));
				if (blockBlob !is null)
				{
					if(this.get_u8("race") == 0 || this.get_u8("race") == 1 || this.get_u8("race") == 2 || this.get_u8("race") == 5 || this.get_u8("race") == 7)
					blockBlob.set_u8("race",this.get_u8("race"));
					this.set_u8("buildblob", index);
					return blockBlob;
				}
			}
