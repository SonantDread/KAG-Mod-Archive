...
void onInit(CBlob @this){
	this.set_f32("food",50); //There used to be 3 food types, people were too simple to understand though, 'bug reports' everywhere. Reminder to develop puking at some point
...
void onTick(CBlob @this){
	...
	if(getGameTime() % 15 == 0){
		...
		if(getNet().isServer())if(XORRandom(2) == 0)this.Sync("heat",true);
	...
	if(getGameTime() % 30 == 0){
		...
		if(this.get_f32("food") > 0){
			this.set_f32("food",this.get_f32("food")-(HungerRate));
			if(getNet().isServer())this.Sync("food",true);
		} 
	...
	if(this.get_f32("food") > 100)this.set_f32("food",100);
}
...
void onRender(CSprite@ this)
{
	...
	f32 AmountPerc = blob.get_f32("food")/100.0;
