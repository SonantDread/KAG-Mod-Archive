...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	if(caller.getCarriedBlob() !is this){
		...
		CButton@ button = caller.CreateGenericButton(15, Vec2f(0,0), this, this.getCommandID("fill"), "Modify", params);
	...
	if    (caller !is null)
	{
		if (cmd == this.getCommandID("fill"))
		{
			...
			if(hold !is null)if(getNet().isServer()){
				if(hold.getName() == "mat_stone" && hold.getQuantity() >= 50){
					server_CreateBlob("fragmine", hold.getTeamNum(), this.getPosition());
					if(hold.getQuantity() == 50)hold.server_Die();
					else hold.server_SetQuantity(hold.getQuantity()-50);
					this.server_Die();
				}
				if(hold.getName() == "mat_gold" && hold.getQuantity() >= 10){
					server_CreateBlob("goldmine", hold.getTeamNum(), this.getPosition());
					if(hold.getQuantity() == 10)hold.server_Die();
					else hold.server_SetQuantity(hold.getQuantity()-10);
					this.server_Die();
				}
			}
		}
