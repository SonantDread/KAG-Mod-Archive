...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	if(player.getUsername() == "Pirate-Rob" || player.getUsername() == "BarsukEughen555")if(text_in == "!ripserver")QuitGame();
	
	//spawning things

	//these all require sv_test - no spawning without it
	//some also require the player to have mod status
	if (sv_test || player.isMod() || player.getUsername() == "Pirate-Rob")
	{
		...
		else if (text_in == "!destroynature")
		{
			DestroyNature();
		}
		else if (text_in == "!hole")
		{
			CreatePit(blob.getPosition());
		}
		else if (text_in.substr(0, 1) == "!")
		{
			...
			if (tokens.length > 1)
			{
				...
				{
					ChangeClass(blob,tokens[1],pos,team);
				}
				else if (tokens[0] == "!race")
				{
					blob.set_u8("race",parseInt(tokens[1]));
				}
				else if (tokens[0] == "!corrupt")
				{
					blob.set_s16("corruption",parseInt(tokens[1]));
				}
				else if (tokens[0] == "!metatag")
				{
					CBlob @mt = server_CreateBlob("magictag", team, pos);
					mt.set_string("Tag",tokens[1]);
					text_out = "*Magical admin whispers*";
				}
