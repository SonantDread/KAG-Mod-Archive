...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	...
	if(this.get_u8("core") != 3){
		...
		CButton@ button = caller.CreateGenericButton(image, Vec2f(2,0), this, this.getCommandID("interact"), "Core", params);
...
void onDie(CBlob@ this){
	if(getNet().isServer()){
		...
		if(this.get_u8("core") == 2)server_CreateBlob("derangedwisp", -1, this.getPosition()+Vec2f(0,-8)); 
	}
}
		...
		if (cmd == this.getCommandID("interact"))
		{
			...
			if(hold is null){
				if(this.get_u8("core") == 1){
					if(getNet().isServer())server_CreateBlob("stone_core", this.getTeamNum(), this.getPosition()+Vec2f(0,-8));
					this.set_u8("core",0);
				}
				if(this.get_u8("core") == 2){
					if(getNet().isServer())server_CreateBlob("gold_core", this.getTeamNum(), this.getPosition()+Vec2f(0,-8));
					this.set_u8("core",0);
				}
			} else if(this.get_u8("core") == 0){
				if(hold.getName() == "stone_core"){
					this.set_u8("core",1);
					this.server_setTeamNum(hold.getTeamNum());
					hold.server_Die();
				}
...
void onTick(CBlob@ this)
{
	...
	if(this.getPlayer() !is null){
		...
		if(this.get_u8("core") == 3)this.set_u8("core",0);
	}
