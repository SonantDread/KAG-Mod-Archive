			...
			else
			{
				if (ismyplayer)
				{
					if (just_action1)
					{
						...
						else if (type == ArrowType::fireworks)
						{
							sprite.PlaySound("SparkleShort.ogg");
						}
					}
	...
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		if (legolas)
		{
			...
			if (arrowType == ArrowType::fireworks)
				{
				this.getSprite().PlaySound("../Mods/Parkour+/Entities/Items/Projectiles/FW_Launch.ogg");
				}
			else
				{
