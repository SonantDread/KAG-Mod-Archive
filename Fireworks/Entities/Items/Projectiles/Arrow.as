...
void onInit(CBlob@ this)
{
	...
	consts.mapCollisions = (arrowType == ArrowType::fireworks ? true : false);	 // weh ave our own map collision
	consts.bullet = false;
	...
	{
		Animation@ anim = sprite.addAnimation("fireworks", 4, true);
		anim.AddFrame(16);
		anim.AddFrame(17); 
		anim.AddFrame(18);
		anim.AddFrame(19);
		anim.AddFrame(20);
		anim.AddFrame(21);
		if (arrowType == ArrowType::fireworks)
			sprite.SetAnimation(anim);
	}
}
...
void onTick(CBlob@ this)
{
	...
	bool processSticking = (arrowType == ArrowType::fireworks ? false : true); // don't stick if fireworks arrow
	...
	if (arrowType == ArrowType::fireworks)
	{
   	...
   	if (this.getTickSinceCreated() == 1)
		{
			if (this.hasTag("popper"))
			{
				Sound::Play("FW_Whistle.ogg", this.getPosition(), 20.0f);
			}
			else
			{ 
				Sound::Play("FW_Launch.ogg", this.getPosition(), 20.0f);
			}
		}
		...
		if (this.getTickSinceCreated() > 15 + XORRandom(40))
		{
			this.server_Die();
		}
	}
...
{
	CSpriteLayer@ arrow = sprite.addSpriteLayer("arrow", "../Mods/Parkour+/Entities/Items/Projectiles/Arrow.png", 16, 8, this.getTeamNum(), this.getSkinNum());
...
void ArrowHitMap(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, u8 customData)
{
	...
	if (arrowType == ArrowType::fireworks)
	{		
		Vec2f velocity_old = this.getOldVelocity();
		//if (velocity_old.Length() < 0.1f) return;
...
shared CBlob@ CreateExplosion( Vec2f pos, Vec2f vel, int team)
{  
	
	CBlob @blob = server_CreateBlobNoInit("fw_explosion");
	if (blob !is null)
	{
		//SetScreenFlash( 30, 0, 0, 150 );
...
void onDie(CBlob@ this)
{
	...
	if (arrowType == ArrowType::fireworks)
	{
		CBlob@ FW_Explosion = CreateExplosion(this.getPosition(), this.getVelocity(), this.getTeamNum());
		if (this.hasTag("popper"))
	    {
	        Sound::Play("FW_Pop.ogg", this.getPosition(),10.0f);
	    }
	    else
	    {
	        Sound::Play("FW_Deep.ogg", this.getPosition(), 20.0f);
	    }
	}
}
