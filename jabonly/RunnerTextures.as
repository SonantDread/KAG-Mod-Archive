...
shared class RunnerTextures
{
	...
	void LoadTeamNum(Vec2f framesize, int teamNum)
	{
		if(loaded) return;
		...
		print(teamNum+male_filename);
		createAndLoadPixelOffsets(male_shortname, teamNum+male_filename, framesize, col, @male_offsets);
		createAndLoadPixelOffsets(female_shortname, teamNum+female_filename, framesize, col, @female_offsets);
	...
	void Load(CSprite@ sprite)
	{
		if(loaded) return;
		print("loading");
		LoadTeamNum(_sprite_to_framesize(sprite),sprite.getBlob().getTeamNum());
	}
...
RunnerTextures@ addRunnerTextures(CSprite@ sprite, string shortname, string texture_prefix)
{
	...
	//load it out
	print("loadingg");
	tex.male_filename = sprite.getBlob().getTeamNum()+ tex.male_filename;
	tex.female_filename = sprite.getBlob().getTeamNum() + tex.female_filename;
	tex.LoadTeamNum(_sprite_to_framesize(sprite),sprite.getBlob().getTeamNum());
