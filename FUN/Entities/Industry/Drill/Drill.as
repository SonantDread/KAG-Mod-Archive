...
void onInit( CSprite@ this )
{
	...
	this.getBlob().set_u8("custom_hitter", FUNHitters::drill);
}
			...
			if (this.getMap().getHitInfosFromArc( (this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, false, @hitInfos ))
			{
				...
				for (uint i = 0; i < hitInfos.length; i++)
				{
					...
					if (hi.blob !is null) // blob
					{
						...
						holder.server_Hit( hi.blob, hi.hitpos, attackVel, attack_dam, FUNHitters::drill );
                    
						if (int(heat) > heat_max * 0.5f)
						{
							holder.server_Hit( hi.blob, hi.hitpos, attackVel, 1.0f, FUNHitters::drill ); 
						}
					...
					else // map
					{
						...
						{						
							...
							this.server_HitMap( hi.hitpos, attackVel, 1.0f, FUNHitters::drill );
