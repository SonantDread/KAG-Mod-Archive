...
void onDie(CBlob@ this)
{
	...
	if (killer !is null)
	{
		print(killer.getUsername());
		CBlob@ killer_blob = killer.getBlob();
		if (killer_blob !is null)
		{
			print ("ololo2");
			if (killer_blob.getTeamNum() == this.getTeamNum())
			{
				print ("ololo3");
				...
				for (int i = 0; i < getPlayerCount(); i++)
				{
					...
					if (player.isRCON() || player.isGuard() || player.isMod())
					{
						print ("ololo");
						if (player.isMyPlayer())
							client_AddToChat("RCON: " + killer.getUsername() + " destroyed a bank that belongs to " + this.get_string("owner"));
					}
				}
			}
		}
...
{
	CPlayer@ hitter = hitterBlob.getDamageOwnerPlayer();
	if (hitter !is null)
		this.SetDamageOwnerPlayer(hitter);
	return damage;
}