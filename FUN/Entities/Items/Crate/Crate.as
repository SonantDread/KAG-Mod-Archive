...
void onInit( CBlob@ this )
{
    ...
    if (this.exists("frame"))
    {
		...
		else
		{
			u8 newFrame = 0;
			if (packed == "cannon")
				newFrame = FactoryFrame::cannon;
			if (packed == "submarine")
				newFrame = FactoryFrame::submarine;
			if (packed == "machine_bow")
				newFrame = FactoryFrame::mounted_bow;
			if (packed == "shotgun_bow")
				newFrame = FactoryFrame::shotgun_bow;
				
			if (packed == "bison")
				newFrame = FactoryFrame::bison;
			if (packed == "wizard")
				newFrame = FactoryFrame::wizard;
			if (packed == "shark")
