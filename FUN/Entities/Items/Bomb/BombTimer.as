...
void Explode(CBlob@ this)
{
	...
	{
		explodeBedrocks(this.getPosition());
		this.getSprite().Gib();
		this.server_Die();
	}
	else
...
void explodeBedrocks(Vec2f at)
{
	CMap@ map = getMap();
	explodeBedrock(map, at + Vec2f(0, -8));
	explodeBedrock(map, at + Vec2f(0, 8));
	explodeBedrock(map, at + Vec2f(8, 0));
	explodeBedrock(map, at + Vec2f(-8, 0));
}
void explodeBedrock(CMap@ map, Vec2f at)
{
	bool noBuildZone = map.getSectorAtPosition(at, "no build") !is null;
	if (map.getTile(at).type == CMap::tile_bedrock && !noBuildZone)
		map.server_DestroyTile(at, 1000.0f);
}