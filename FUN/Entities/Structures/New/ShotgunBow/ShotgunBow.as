...
{
	this.set_u32("fire delay", 10);
	this.set_u32("fire bullets amount", 1);
	this.set_f32("distance", 2.0f);

	this.addCommandID( "add arrow" );
	this.addCommandID( "remove arrow" );
...
{
	int fireBullets = this.get_u32("fire bullets amount");
	...
	if (caller.isAttached())
	{
		if (fireBullets != 7) 
		{ 
			caller.CreateGenericButton( 5, Vec2f(0.0,-15.0), this, this.getCommandID("add arrow"), "+1 arrows. Now " + fireBullets + " arrows setted", params ); 
		}
		else if (fireBullets == 7)
		{
			CButton@ button = caller.CreateGenericButton( 5, Vec2f(0,-15.0), this, 0, "Max arrows count is 7. You can't add more." );
			if (button !is null) button.SetEnabled( false );
		}
	
		if(fireBullets != 1) { caller.CreateGenericButton( 4, Vec2f(0.0,15.0), this, this.getCommandID("remove arrow"), "-1 arrow. Now " + fireBullets + " arrows setted", params ); }
		else if (fireBullets == 7)
		{
			CButton@ button = caller.CreateGenericButton( 5, Vec2f(0,-15.0), this, 0, "Min arrows count is 1. You can't subtract more." );
			if (button !is null) button.SetEnabled( false );
		}
	}
}
...
{
	float distance = this.get_f32("distance");
    ...
    if (bullet !is null)
    {
		...
		bullet.getShape().setDrag( bullet.getShape().getDrag() * distance ); ////////////////////////////////////////////////////////////////////////////
...
{
	int fireDelay = this.get_u32("fire delay");
	int fireBullets = this.get_u32("fire bullets amount");
	float distance = this.get_f32("distance");
	...
	else if (cmd == this.getCommandID("add arrow"))
	{
		fireBullets++;
		distance += 0.67f;
		fireDelay += 5;
		...
		v.fire_amount = fireBullets;
		v.fire_delay = fireDelay;
		
		this.set_u32("fire delay", fireDelay);
		this.set_u32("fire bullets amount", fireBullets);
		this.set_f32("distance", distance);
	}
	else if (cmd == this.getCommandID("remove arrow"))
	{
		fireBullets--;
		distance -= 0.7f;
		fireDelay -= 5;
