						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (other !is null)
							{
								if (other.hasTag("flesh") && other.getTeamNum() != this.getTeamNum())
								{
									...
									this.server_Hit(other,hi.hitpos,vel,power,FUNHitters::zombie, false);
...
{
	if ((blob.hasTag("zombie") && blob.getHealth()<0.0)
	|| this.getTeamNum() == blob.getTeamNum() 
	|| !blob.hasTag("blocks water")
	|| blob.hasTag("dead")) return false;
	else return true;
}
