...
void onTick(CBlob@ this)
{
	...
	/*if (this.isAttached())
	{
		AttachmentPoint@ att = this.getAttachments().getAttachmentPointByName("DRIVER");   //only have one
		if (att !is null)
		{
			CBlob@ b = att.getOccupied();
			CBlob@ friend = getBlobByNetworkID(this.get_netid(friend_property));
			if (b !is null && b == friend)
			{
				AnimalVars@ vars;
				if (!this.get("vars", @vars))
				return;
				vars.walkForce.Set(10.0f, -0.1f);
				vars.runForce.Set(10.0f, -1.0f);
				vars.slowForce.Set(1.0f, 0.0f);
				vars.jumpForce.Set(0.0f, -40.0f);
				vars.maxVelocity = 5.1f;
			}
			else
			{
				AnimalVars@ vars;
