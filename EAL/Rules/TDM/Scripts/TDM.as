...
void Config(TDMCore@ this)
{
	...
	this.warmUpTime = rules.get_u32("warmuptime");
...
shared class TDMCore : RulesCore
{
	...
	{
		//set each players coins
		if (!getRules().get_bool("keepcoins"))
			player.server_setCoins(getRules().get_u32("spawncoins"));
...
{
	if (this.get_bool("randomcoins"))
		this.set_u32("spawncoins", (XORRandom(8)+3)*10);
...
{
	this.set_u32("spawncoins"	, 50);
	this.set_bool("randomcoins"	, false);
	this.set_bool("keepcoins"	, true);
	this.set_u32("warmuptime"	, 15);
	Reset(this);
}
