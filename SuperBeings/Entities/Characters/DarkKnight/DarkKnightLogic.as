...
void onTick(CBlob@ this)
{
	...
	if(this.isKeyPressed(key_action2))
	{
		...
		CBlob@[] blobInRadius;	   
		if (this.getMap().getBlobsInRadius(this.getPosition(), 16.0f, @blobInRadius)) 
		{
			for (uint i = 0; i < blobInRadius.length; i++)
			{
				CBlob@ b = blobInRadius[i];
				if    (b.hasTag("dead"))
				{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					if(b.getHealth()-damage <= 0)if(b.hasTag("player")){
