...
void onRender(CRules@ this)
{
	...
	if (vote.mapname != "")
	{
		GUI::GetTextDimensions(vote.mapname, text_dim);		
		tl.x -= (text_dim.x- (text_dim.x/3));
	}
	else
	{
	...
	if (vote.mapname != "")
	{
		GUI::DrawText(getTranslatedString("Map: {MAPNAME}").replace("{MAPNAME}", getTranslatedString(vote.mapname)), tl + Vec2f(3, 3 + text_dim.y * 2), color_white);
	}
	else
	{
