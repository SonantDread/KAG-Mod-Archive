...
{
	VoteFunctor() {}
	void Pass(VoteObject@ vote, bool outcome) { /* do your vote action in here - remember to check server/client */ }
};
...
shared SColor vote_message_colour() { return SColor(0xff0044ff); }
...
void Rules_SetVote(CRules@ this, VoteObject@ vote)
{
	if (!Rules_AlreadyHasVote(this))
	{
		...
		else
		{
			...
			if (vote.mapname != "")
			{
				//special case; voteloadmap
				client_AddToChat(
					getTranslatedString("--- Vote \"{TITLE}\" was started by {USER}.\n Map Name: {MAPNAME} ---")
						.replace("{TITLE}", getTranslatedString(vote.title)).replace("{USER}", vote.byuser)
						.replace("{MAPNAME}", getTranslatedString(vote.mapname)),
					vote_message_colour()
				);
			}			
			else if (vote.user_to_kick != "")
			{
...
void PassVote(VoteObject@ vote)
{
	...
	vote.onvotepassed.Pass(vote,outcome);
}
...
void ForcePassVote(VoteObject@ vote, CPlayer@ player)
{
	...
	vote.onvotepassed.Pass(vote,true);
}
