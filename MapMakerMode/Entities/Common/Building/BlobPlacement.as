...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (getNet().isServer() && cmd == this.getCommandID("placeBlob"))
	{
		...
		if (carryBlob !is null)
		{
			...
			TileType maptile = getMap().getTile(pos).type;
									
			HistoryInfo@ history;
			if (this.get("historyInfo", @history))
			{
				history.PushHistory(pos, maptile, carryBlob.getName(), carryBlob.getTeamNum());
	...
	else if (getNet().isServer() && cmd == this.getCommandID("settleLadder"))
	{
		CBlob @carryBlob = getBlobByNetworkID(params.read_u16());
		Vec2f pos = params.read_Vec2f();
		if (carryBlob !is null)
