		...
		case SpawnCmd::changeClass:
		{
			...
			if (caller !is null && canChangeClass(this, caller))
			{
				...
				if(classconfig == "builder"){
					if(caller.getTeamNum() > 8)equipType(caller, "marm", Equipment::Hammer, 0);
					else equipType(caller, "marm", Equipment::Hammer, 2);
					equipType(caller, "sarm", Equipment::Pick, 0);
					equipType(caller, "tors", Equipment::Shirt, 0);
					caller.set_u16("marm_default",Equipment::Hammer);
					caller.set_u16("sarm_default",Equipment::Pick);
				}
				
				if(classconfig == "knight"){
					equipType(caller, "marm", Equipment::Sword, 0);
					equipType(caller, "sarm", Equipment::Shield, 0);
					equipType(caller, "tors", Equipment::KnightArmour, 0);
					caller.set_u16("marm_default",Equipment::Sword);
					caller.set_u16("sarm_default",Equipment::Shield);
				}
				
				if(classconfig == "archer"){
					equipType(caller, "marm", Equipment::Bow, 0);
					equipType(caller, "sarm", Equipment::Grapple, 0);
					equipType(caller, "tors", Equipment::Shirt, 0);
					caller.set_u16("marm_default",Equipment::Bow);
					caller.set_u16("sarm_default",Equipment::Grapple);
