...
void onInit(CBlob@ this)
{
	...
	AddIconToken("$craftbench_icon$", "CraftBench.png", Vec2f(24, 24), 0);
	{
		ShopItem@ s = addShopItem(this, "Crafting Bench", "$craftbench_icon$", "craft_bench", "For crafting simple items.");
		AddRequirement(s.requirements, "blob", "mat_wood", "Wood", 50);
	}
	AddIconToken("$cot_icon$", "Cot.png", Vec2f(24, 24), 2);
	{
		ShopItem@ s = addShopItem(this, "Cot", "$cot_icon$", "cot", "A rickety spot for sleeping and healing.");
		AddRequirement(s.requirements, "blob", "mat_wood", "Wood", 50);
	}
	AddIconToken("$barracks_icon$", "TribalBarracks.png", Vec2f(24, 24), 0);
	{
		ShopItem@ s = addShopItem(this, "Barracks", "$barracks_icon$", "tribal_barracks", "A place to switch out equipment and gear up.");
		AddRequirement(s.requirements, "blob", "mat_wood", "Wood", 50);
	}
	...
	AddIconToken("$writing_desk_icon$", "WritingDesk.png", Vec2f(24, 24), 0);
	{
		ShopItem@ s = addShopItem(this, "Writing Desk", "$writing_desk_icon$", "writing_desk", "A place to do crude research.");
		AddRequirement(s.requirements, "blob", "mat_wood", "Wood", 50);
	}
		...
		if (item !is null && caller !is null)
		{
			...
			if(item.getName().find("bulwark") >= 0){
				...
				if(caller.getPlayer() !is null){
					caller.getPlayer().server_setTeamNum(newteam);
				}
