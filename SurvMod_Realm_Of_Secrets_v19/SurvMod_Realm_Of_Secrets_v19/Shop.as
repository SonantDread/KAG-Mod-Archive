...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("shop buy"))
	{
		...
		if (inv !is null && isInRadius(this, caller))
		{
			...
			CBitStream missing;
			if (hasRequirements(inv, this.getInventory(), s.requirements, missing))
			{
			...
			if (!tookReqs)
			{
				...
				if (getBlobsByTag("storage", @storages))
					for (uint step = 0; step < storages.length; ++step)
					{
						...
						{
							CBitStream missing;
							if (hasRequirements(inv, storage.getInventory(), s.requirements, missing))
							{
	...
	if (this.get(SHOP_ARRAY, @shop_items))
	{
		for (uint i = 0 ; i < shop_items.length; i++)
		{
			...
			if (button !is null)
			{
				...
				// try taking from the caller + this shop first
				CBitStream missing;
				if (hasRequirements(this.getInventory(), caller.getInventory(), s_item.requirements, missing))
				{
					tookReqs = true;
				}
				// try taking from caller + storages second
				...
				SetItemDescription(button, caller, s_item.requirements, getTranslatedString(desc), this.getInventory());
