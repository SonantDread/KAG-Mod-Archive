		...
		if(this.getPlayer() is null){
			if(XORRandom(10) == 0){
				...
				if(this.isAttached()){
					...
					if(p !is null){
						this.server_Hit(p, this.getPosition(), Vec2f(0,0), 0.25f, Hitters::life_flame, true);
					}
				}
			}
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal)
{
	if(blob !is null){
		if(getNet().isServer()){
			...
			if(XORRandom(3) == 0){
				this.server_Hit(blob, this.getPosition(), Vec2f(0,0), 0.25f, Hitters::life_flame, true);
			}
		}
	}
}
