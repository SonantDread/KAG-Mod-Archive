...
{
	return (blob.get_u8("sex") == 0 ? 0 : 1) * NUM_HEADFRAMES;
}

CSpriteLayer@ LoadHead(CSprite@ this, u8 headIndex)
{
	this.RemoveSpriteLayer("head");
	this.RemoveSpriteLayer("fore_eye");
	this.RemoveSpriteLayer("back_eye");
	...
	blob.set_u8("head sex", blob.get_u8("sex"));
	
	CSpriteLayer@ head = this.addSpriteLayer("head", getBodyTypeName(blob.get_u8("head_type"))+"_Head.png", 16, 16, 0, 0);
	InitLayer(this,head, headFrame);
	
	CSpriteLayer@ left_eye = this.addSpriteLayer("fore_eye", "Eyes_Fore.png", 16, 16, 0, 0);
	InitLayer(this,left_eye,headFrame+blob.get_u8("fore_eye")*NUM_HEADFRAMES*2);
	blob.set_u8("sprite_fore_eye",blob.get_u8("fore_eye"));
	
	CSpriteLayer@ right_eye = this.addSpriteLayer("back_eye", "Eyes_Back.png", 16, 16, 0, 0);
	InitLayer(this,right_eye,headFrame+blob.get_u8("back_eye")*NUM_HEADFRAMES*2);
	blob.set_u8("sprite_back_eye",blob.get_u8("back_eye"));
	
	CSpriteLayer@ helmet = this.addSpriteLayer("helmet", "character_no_helmet.png", 32, 32, 0, 0);
	
	if(blob.get_u8("head_type") != BodyType::None){
		int hair_num = blob.get_u8("hair_index");
		int hair_colour = blob.get_u8("hair_colour");
		
		if(blob.get_u8("head_type") == BodyType::Wraith)hair_colour = 1;
		if(blob.get_u8("head_type") == BodyType::Gold)hair_colour = 4;
		if(blob.get_u8("head_type") == BodyType::Shadow)hair_colour = 16;
	
		CSpriteLayer@ hair = null;
		if(!blob.hasTag("bald"))
		if(hair_num != 0 || blob.get_u8("sex") != 0){
			...
			InitHair(this, hair, hair_num*NUM_HAIR_FRAMES*2 + (blob.get_u8("sex") == 0 ? 0 : 1)*2);
		}
	}
...
void onTick(CSprite@ this)
{
	...
	if (po !is null)
	{
		...
		CSpriteLayer@ left_eye = this.getSpriteLayer("fore_eye");
		if (left_eye !is null){
		...
		CSpriteLayer@ right_eye = this.getSpriteLayer("back_eye");
		...
		if (helmet !is null){
			if(blob.get_u8("sex") == 1)helmet.SetOffset(headoffset);
