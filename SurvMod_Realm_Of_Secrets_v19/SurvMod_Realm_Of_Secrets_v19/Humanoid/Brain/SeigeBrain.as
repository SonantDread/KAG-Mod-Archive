...
void onTick(CBrain@ this)
{
	...
	{	
		...
		if (can_search)
		{
			...
			for (int i = 0; i < blobs.length; i++)
			{
				...
				if (b.getTeamNum() != myTeam && (b.hasTag("alive") || b.hasTag("animated"))  && b.hasTag("flesh") && !b.hasTag("invincible"))
				{
	...
	{			
		...
		if (!target.hasTag("alive") && !target.hasTag("animated")) 
		{
			ResetTarget(this);
			return;
		}
...
void AttackBlob(CBlob@ blob, CBlob @target)
{
	...
	int equipment = blob.get_u16("marm_equip");
	keys key = key_action1;
	for(int i = 0;i < 2;i++){
		if(i == 1){
			equipment = blob.get_u16("sarm_equip");
			key = key_action2;
		}
