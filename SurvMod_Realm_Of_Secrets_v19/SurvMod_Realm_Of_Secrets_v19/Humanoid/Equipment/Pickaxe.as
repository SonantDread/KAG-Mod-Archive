...
void onTick(CBlob@ this)
{
	...
	if(this.get_u16("marm_equip") == Equipment::Pick){
		// activate/throw
		if(ismyplayer || (isServer() && this.isBot()))
		{
			Pickaxe(this);
		}
		if(this.isKeyPressed(key_action1) && isLimbUsable(this,this.get_u8("marm_type")))
		{
			...
			if(this.get_u8("pickaxe_counter") < hit_time)this.add_u8("pickaxe_counter",Maths::Max(5.0f*(getLimbSpeed(this.get_u8("marm_type"))+getLimbSpeed(this.get_u8("sarm_type")))*pickMod,1.0f));
		} else {
			this.set_u8("pickaxe_counter",0);
		}
	}
	
	if(this.get_u16("sarm_equip") == Equipment::Pick){
		// activate/throw
		if(ismyplayer || (isServer() && this.isBot()))
		{
			Pickaxe(this);
		}
		if(this.isKeyPressed(key_action2) && isLimbUsable(this,this.get_u8("sarm_type")))
		{
			RunnerMoveVars@ moveVars;
			if(this.get("moveVars", @moveVars))
			{
...
{
	if(this.get_u8("pickaxe_counter") >= hit_time)this.set_u8("pickaxe_counter",0);
...
void Pickaxe(CBlob@ this)
{
	...
	if(this.get_u8("pickaxe_counter") <= 0)
	{
	...
	bool justCheck = Maths::Min(this.get_u8("pickaxe_counter"),hit_time) != hit_time;
	bool adjusttime = this.get_u8("pickaxe_counter") < hit_time-1;
	...
	if (!adjusttime)
	{
		if (!justCheck)
		{
			...
			f32 hit_damage = 0.0f;
			if(this.get_u16("marm_equip") == Equipment::Pick)hit_damage += getEquipmentDamage(this.get_u16("marm_equip"),this.get_u16("marm_equip_type"))*getLimbStrength(this.get_u8("marm_type"));
			if(this.get_u16("sarm_equip") == Equipment::Pick)hit_damage += getEquipmentDamage(this.get_u16("sarm_equip"),this.get_u16("sarm_equip_type"))*getLimbStrength(this.get_u8("sarm_type"));
...
void onRender(CSprite@ this)
{
	...
	if (blob.get_u8("pickaxe_counter") > 0)
	{
