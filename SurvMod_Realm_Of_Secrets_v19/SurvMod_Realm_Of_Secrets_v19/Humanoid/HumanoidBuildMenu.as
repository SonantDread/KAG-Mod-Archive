...
void onTick(CInventory@ this)
{
	...
	if(getGameTime() % 15 == 0)blob.set_u8("build page",getTechLevel(blob));
}
...
void MakeBlocksMenu(CInventory@ this, Vec2f menu_size, const Vec2f &in INVENTORY_CE)
{
	...
	const Vec2f MENU_CE = Vec2f(0, menu_size.y * -24 -24*3.5f) + INVENTORY_CE;
	...
	if(menu !is null)
	{
		...
		const Vec2f TOOL_POS = Vec2f(+108,-24*8.5f) + INVENTORY_CE;
...
void onCreateInventoryMenu(CInventory@ this, CBlob@ forBlob, CGridMenu@ menu)
{
	...
	Vec2f menu_size = Vec2f(3,5);
	
	const u8 PAGE = blob.get_u8("build page");
	
	BuildBlock[][]@ blocks;
	if(blob.get(blocks_property, @blocks)){
		menu_size = Vec2f(3,Maths::Max(Maths::Ceil(blocks[PAGE].length/3.0f),1));
	}
...
int getTechLevel(CBlob @this){
	int techLevel = 0;
	if(this.get_u16("marm_equip") == Equipment::Hammer)techLevel = this.get_u16("marm_equip_type");
	if(this.get_u16("sarm_equip") == Equipment::Hammer)techLevel = Maths::Max(this.get_u16("sarm_equip_type"),techLevel);
	//0 - feudal
	//1 - medieval
	if(techLevel > 0)techLevel = techLevel/2;
	
	if(techLevel < 1 && this.getTeamNum() < 8)techLevel = 1;
