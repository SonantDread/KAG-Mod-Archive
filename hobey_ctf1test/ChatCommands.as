...
bool onServerProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
    ...
    else if (tokens[0] == "!shuffleallbut")
    {
        tokens.removeAt(0);
    ...
    if (text_in == "!endgame" && player.isMod())
    {
    ...
    if(sv_test)
    {
        ...
        else if (text_in == "!invall") {
            for (int i = 0; i < getPlayerCount(); i++) {
                CPlayer@ p = getPlayer(i);
                CBlob@ b = p.getBlob();
                if (b !is null) b.server_SetHealth(999999);
            }
        }
        else if (text_in == "!inv") {
            if (tlen >= 2) {
...
bool onClientProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
    ...
    /*
     ...
     print ("time between messages:" + (getGameTime() - lastmessagetime));
    */
    
    /*
    if (text_in == "!spam") this.set_bool(player.getUsername()+"nospam", false);
    else if (text_in == "!nospam") this.set_bool(player.getUsername()+"nospam", true);
    
    string prevmessage = this.get_string(player.getUsername()+"lastmessage");
    u32 lastmessagetime = this.get_u32(player.getUsername()+"lastmessagetime");
    
    if (text_in == prevmessage && getGameTime() - lastmessagetime < 30)
    {
        print("there was spam");
        return !this.get_bool(getLocalPlayer().getUsername()+"nospam");
    }
    
    this.set_string(player.getUsername()+"lastmessage", text_in);
    this.set_u32(player.getUsername()+"lastmessagetime", getGameTime());
    */
