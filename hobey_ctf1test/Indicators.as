...
{
    {
        ...
        bool hideall          = rules.get_bool("hideall");
...
void onTick(CRules@ this)
{
    ...
    if ((controls.ActionKeyPressed(AK_PARTY) && (controls.ActionKeyPressed(AK_MENU)))) {
    ...
    } else if (controls.ActionKeyPressed(AK_PARTY)) {
...
bool onClientProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
    ...
    if (tokens[0] == "!hide" && tlen >=2) {
        CPlayer@ target = GetPlayerByIdent(tokens[1]);
        if (target != null) { this.set_bool(target.getUsername() + "is_hidden", true); }
    } else if (tokens[0] == "!unhide" && tlen >=2) {
        CPlayer@ target = GetPlayerByIdent(tokens[1]);
        if (target != null) { this.set_bool(target.getUsername() + "is_hidden", false); }
    }
    
    if      (text_in == "!hideall")   { this.set_bool("hideall", true); }
    else if (text_in == "!unhideall") { this.set_bool("hideall", false); }
...
void onCommand(CRules@ rules, u8 cmd, CBitStream @params)
{
    if (cmd == rules.getCommandID(add_indicator_cmd_id)) {
        ...
        if (should_show_indicator(rules, player)) {
            ...
            bool muteall = rules.get_bool("muteall");
            ...
            if (player_is_muted == false) {
                if ((time_since_last_sound_use >= soundcooldown)/* || player.isMod()*/) {
                    if (muteall == false) {
                        ...
                        } else if (kind == 20) { Sound::Play("Entities/Characters/Migrant/MigrantSayHello.ogg", pos, .3f); new_cooldown = 20;
                        } else if (kind == 21) { Sound::Play("Sounds/coinpick.ogg",     pos, .8f);  new_cooldown = 15;
                        } else if (kind == 22) { Sound::Play("Sounds/build_wall.ogg",   pos, .58f); new_cooldown = 10;
                        } else if (kind == 23) { Sound::Play("Sounds/build_door.ogg",   pos, .55f); new_cooldown = 10;
                        } else if (kind == 24) { Sound::Play("Sounds/build_wall2.ogg",  pos, .95f); new_cooldown = 10;
                        } else if (kind == 25) { Sound::Play("Sounds/build_ladder.ogg", pos, .85f); new_cooldown = 10;
...
void onRender(CRules@ rules)
{
    for (int player_index = 0;
        ...
        if (indicator_kind == 22) { phrase = "Build backwall here!";  color_1 = SColor( 255, 200, 160,  30); }
        if (indicator_kind == 23) { phrase = "Build door here!";      color_1 = SColor( 255, 220,  160,  80); }
        if (indicator_kind == 24) { phrase = "Build block here!";     color_1 = SColor( 255, 230, 120,  90); }
        if (indicator_kind == 25) { phrase = "Build ladder here!";    color_1 = SColor( 255,  190, 160, 30); }
