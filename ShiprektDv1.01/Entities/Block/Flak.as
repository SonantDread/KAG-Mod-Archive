...
const f32 PROJECTILE_SPREAD = 1.35;
const f32 PROJECTILE_RANGE = 350.0f;

const int MIN_TICKS_PER_SHOT = 50;
const int RELOAD_TICKS = 100;
const u8 MAX_AMMO = 6;
...
void Manual( CBlob@ this, CBlob@ controller )
{
	...
	// fire
	if ( controller.isMyPlayer() && controller.isKeyPressed( key_action1 ) && canShootManual( this, true ) && isClearShot( this, aimVec, false ) )
	{
	...
	if ( getNet().isServer() && this.getMap().getBlobsInRadius( this.getPosition(), AUTO_RADIUS, @blobsInRadius ) )
	{
		for ( uint i = 0; i < blobsInRadius.length; i++ )
		{
					...
					&& ( b.getName() == "human"|| b.hasTag( "mothership" ) ||  b.hasTag( "rocket" ) || ( b.hasTag( "weapon" ) && b.getShape().getVars().customData > 0 ) ) )
			{
				...
				if ( distance < minDistance && isClearShot( this, aimVec, true, merged ) )
				{
					shoot = true;					
					shootVec = aimVec;
...
void Clone( CBlob@ this, CBlob@ parent, CBlob@ controller )
{
	...
	CPlayer@ player = controller.getPlayer();
	// fire
	if ( isClearShot( this, aimVec, false ) )
	{
		Rotate( this, aimVec );
		if ( controller.isMyPlayer() && controller.isKeyPressed( key_action1 ) && canShootManual( this, true ) && ( getGameTime() - parent.get_u32("fire time") == MIN_TICKS_PER_SHOT/2 ) )
		{
			Island@ isle = getIsland( this.getShape().getVars().customData );
			u16 netID = 0;
			if ( isle !is null && player !is null && ( !isle.isMothership || isle.owner != player.getUsername() ) )
...
bool isClearShot( CBlob@ this, Vec2f aimVec, bool autoMode, bool targetMerged = false )
{
	Vec2f pos = this.getPosition();
	...
	if ( hitInfos.length > 0 )
	{
		...
		for ( uint i = 0; i < hitInfos.length; i++ )
		{
			...
			bool isOwnTeam = this.getTeamNum() == b.getTeamNum();
			bool isOwnCore = Block::isCore( blockType ) && isOwnTeam;
			...
			{	
				if ( ( Block::isSolid( blockType ) || isOwnCore || b.hasTag("weapon") ) && sameIsland && !canShootSelf )
				{
					//print ( "not clear " + ( b.getName() == "block" ? " (block) " : "" ) + ( !canShootSelf ? "!canShootSelf; " : "" )  );
					return false;
				}
				
				if ( autoMode && isOwnTeam )
				{
					Island@ island = getIsland( bColor );
					if ( island !is null )
					{
						CPlayer@ islandOwner = getPlayerByUsername( island.owner );
						if ( islandOwner !is null )
