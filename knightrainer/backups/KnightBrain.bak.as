...
#include "KnightBrainCommon.as"


//TODO: pickup bombs and throw them back, distance is 3 blocks
//TODO: make one new file that the knight brain can use
...
void onTick(CBrain@ this, CBlob@ blob)
{
	...
	if (target !is null)
	{
        ...
        /*getBlobsByName("bomb", @bomb);*/
		CBlob@[] bomb;
		if (this.getMap().getBlobsInRadius(this.getPosition(), 3*8, @bomb))
		{
			CBlob @carried = blob.getCarriedBlob();
			if (carried is null) //bot has 0 ping so we should be able to pick it up and throw it back instantly
			{
				//blob.server_Pickup(@bomb);
				blob.setKeyPressed(KEY_KEY_C, true);
			}
			else
			{
				//blob.server_Pickup(@bomb);
		...
		if (visibleTarget && distance < 35.0f) ///50.0f
		{
			if (visibleTarget && distance < 60.0f)
			{
				strategy = Strategy::speed_slide;
			}
			else {
		...
		else if (strategy == Strategy::attacking)
		{
			...
			{
				strategy = Strategy::speed_slide; //chasing
			}
		}
