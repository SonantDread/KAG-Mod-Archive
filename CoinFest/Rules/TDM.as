...
shared class TDMCore : RulesCore
{
	...
	void CheckTeamWon()
	{
		...
		//set up an array of teams' coin count
		array<int> team_coins;
		for (int i = 0; i < teams.length; i++)
		{
			team_coins.push_back(0);
		}
		...
		for (int i = 0; i < getPlayersCount(); i++)
		{
			...
			if (team > team_coins.length())
			{
				continue;
			}
			team_coins[team] = team_coins[team] + p.getCoins();
		}
		...
		for (int i = 0; i < teams.length; i++)
		{
			if (team_coins[i] > highest_coins)
			{
				highest_coins = team_coins[i];
			}
		}
		...
		for (int i = 0; i < teams.length; i++)
		{
			if (team_coins[i] == highest_coins || team_coins[i] > highest_coins)
			{
		...
		rules.set_s32("max_coins", highest_coins);
	}
