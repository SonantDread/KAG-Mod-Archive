...
void onInit(CBlob@ this)
{
	...
	AddIconToken( "$out_migrant$", "MigrantIcon.png", Vec2f(16,16), 0 );
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
   	...
   	if ((carried is null || !carried.hasTag("migrant")) && this.get_u8("migrants count") > 0)
	{
	   ...
	   caller.CreateGenericButton( "$out_migrant$", Vec2f(0.0f, 0.0f), this, this.getCommandID("out migrant"), "Wake up and take migrant out", params );
	}*/
}
...
void onTick( CBlob@ this )
{
	...
	if (getNet().isServer())
	{
		...
		CBlob@[] blobsInRadius;
		if (this.getTeamNum() != 255 && !isRoomFullOfMigrants(this) && this.getMap().getBlobsInRadius( this.getPosition(), this.getRadius()/2, @blobsInRadius ))
		{
			// first check if enemies nearby
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				CBlob @b = blobsInRadius[i];
				if (b !is this && b.getTeamNum() != this.getTeamNum() && (b.hasTag("player") || b.hasTag("zombie")))
				{
					return;
				}
			}
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				if (b !is this && !b.isAttached()
					&& b.getTickSinceCreated() > 200 // so they dont get in immediately
					&& b.getShape().vellen < 0.1f
					&& b.getTeamNum() == this.getTeamNum()
					&& b.hasTag("migrant") && !b.hasTag("dead")
