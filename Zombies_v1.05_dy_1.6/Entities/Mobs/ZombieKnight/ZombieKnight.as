...
void onInit(CBlob@ this)
{
	...
	f32 zombie_knight_player_dmg = getRules().get_f32("zombie_knight_player_dmg");
	this.set_f32("bite damage", zombie_knight_player_dmg);
...
void onTick(CBlob@ this)
{
	...
	if (getGameTime() % 65 == 0)
	{
		...
		if (mad > 0)
		{
			...
			if (mad < 0 ) {
				...
				//if (zGrowl==1)
				//this.getSprite().PlaySound( "/ZombieKnightBreath" );
			}
			else
			{
				int zGrowl = XORRandom(2);
	...
	if ((this.isKeyPressed(key_left) || this.isKeyPressed(key_right) || this.isKeyPressed(key_down) || this.isKeyPressed(key_up)))
	{
		if (XORRandom(break_chance)==0)
		{
			...
			if ( !this.getMap().isTileBedrock( tile ) ) {	//Kill everything except bedrock		
	...
	if(getNet().isServer() && getGameTime() % 10 == 0)
	{
		...
		{
			CBlob@ b = getBlobByNetworkID(this.get_netid(target_property));
			if((this.get_s16("mad timer") >= MAD_TIME/8) || (b !is null && this.getDistanceTo(b) < 106.0f) || (this.getDistanceTo(b) < XORRandom(150.0f)) || (getGameTime() % 600 == 0))
			{
				this.Tag(chomp_tag);
			}
			else
