...
void onGib(CSprite@ this)
{
    ...
    CParticle@ Head     = makeGibParticle( "BossRabidDogGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 0, Vec2f (32,32), 2.0f, 20, "/BodyGibFall", team );																									//collumn, //row
    CParticle@ Body     = makeGibParticle( "BossRabidDogGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       1, 0, Vec2f (32,32), 2.0f, 20, "/BodyGibFall", team );																									//collumn, //row
    CParticle@ Leg1     = makeGibParticle( "BossRabidDogGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 1, Vec2f (32,32), 2.0f, 20, "/BodyGibFall", team );
   	CParticle@ Leg2   = makeGibParticle( "BossRabidDogGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       1, 1, Vec2f (32,32), 2.0f, 20, "/BodyGibFall", team );
   	CParticle@ Leg3     = makeGibParticle( "BossRabidDogGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 2, Vec2f (32,32), 2.0f, 20, "/BodyGibFall", team );
    CParticle@ Leg4     = makeGibParticle( "BossRabidDogGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       1, 2, Vec2f (32,32), 2.0f, 20, "/BodyGibFall", team );
    CParticle@ Tail     = makeGibParticle( "BossRabidDogGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 3, Vec2f (32,32), 2.0f, 20, "/BodyGibFall", team );
...
void onInit(CBlob@ this)
{
	...
	f32 rabid_dog_player_dmg = getRules().get_f32("rabid_dog_player_dmg");
	this.set_f32("bite damage", rabid_dog_player_dmg);
...
void onTick(CBlob@ this)
{
	...
	int break_chance = break_chance_modifier - 2*(difficulty-1.0);	
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (other !is null)
							{
								if (other.hasTag("flesh") && other.getTeamNum() != this.getTeamNum() || (other.getTeamNum() != this.getTeamNum() && (other.hasTag("boat") || other.hasTag("vehicle"))))
								{
									...
									if (XORRandom(5)==0)
									this.server_Hit( other, other.getPosition(), vel, power/10, Hitters::saw, false);
								}
								else
								{
	...
	if (getGameTime() % 65 == 0)
	{
		...
		if (mad > 0)
		{
			...
			else
			{
				if (XORRandom(5)==0)
				{
					int zGrowl = XORRandom(3);
					if (zGrowl==0)
					this.getSprite().PlaySound( "/dog_growl1" );
					if (zGrowl==1)
					this.getSprite().PlaySound( "/dog_growl2" );
					if (zGrowl==2)
					this.getSprite().PlaySound( "/dog_growl3" );
				}
			}
	...
	if (this.isKeyPressed(key_left) || this.isKeyPressed(key_right) || this.isKeyPressed(key_down) || this.isKeyPressed(key_up))
	{
		if (XORRandom(break_chance)==0 || (this.hasTag(chomp_tag) && XORRandom(10)==0))
		{
				...
				s32 rabid_dog_tile_dmg_range = getRules().get_s32("rabid_dog_tile_dmg_range");
				int xMod = 0;
				int yMod = 0;
				...
				dir1.Normalize();
				Vec2f tp1 = this.getPosition() + (dir1)*(this.getRadius() + XORRandom(rabid_dog_tile_dmg_range));
				s32 rabid_dog_tile_dmg = getRules().get_s32("rabid_dog_tile_dmg");
				TileType tile = this.getMap().getTile( tp1 ).type;
				if (this.getMap().isTileSolid(tp1) && !this.getMap().isTileGroundStuff(tile))
				this.Tag(chomp_tag); this.getMap().server_DestroyTile(tp1, ((difficulty/14) + rabid_dog_tile_dmg));
				
				if (this.getMap().isTileSolid(tp1) && this.getMap().isTileGroundStuff(tile) && XORRandom(5)==0)
				{
					//printf("DOG DIRT ATTACK");
	...
	if(getNet().isServer() && getGameTime() % 10 == 0)
	{
		...
		{
			f32 rabid_dog_aggro_range = getRules().get_f32("rabid_dog_aggro_range");
			CBlob@ b = getBlobByNetworkID(this.get_netid(target_property));
			if((this.get_s16("mad timer") >= MAD_TIME/8) || (b !is null && this.getDistanceTo(b) < 206.0f) || (b !is null && this.getDistanceTo(b) < XORRandom(rabid_dog_aggro_range)) || (getGameTime() % 600 == 0))
			{
				this.Tag(chomp_tag);
			}
			else
...
{		
    ...
    {
    	int rabid_dog_reward_coins = getRules().get_s32("rabid_dog_reward_coins");
		...
		if (hitterBlob.hasTag("player"))
		{
			...
			if (player !is null)
			{
				...
				player.server_setCoins( player.getCoins() + (rabid_dog_reward_coins/10) );
			}
		} else
		...
		if (getNet().isServer())
		{
			...
			{
				server_DropCoins(hitterBlob.getPosition() + Vec2f(0,-3.0f), rabid_dog_reward_coins);
