...
void onInit(CBlob@ this)
{
	...
	f32 greg_player_dmg = getRules().get_f32("greg_player_dmg");
	this.set_f32("bite damage", greg_player_dmg);
...
void onTick(CBlob@ this)
{
	...
	if (getGameTime() % 65 == 0)
	{
		...
		if (mad > 0)
		{
			...
			if (mad < 0 ) {
				this.set_u8(personality_property, DEFAULT_PERSONALITY);
				this.getSprite().PlaySound("/WraithSpawn");
			}
...
//			this.getSprite().PlaySound("/BisonBoo");
	...
	if ((this.isKeyPressed(key_left) || this.isKeyPressed(key_right) || this.isKeyPressed(key_up) || this.isKeyPressed(key_down)) )
	{
		if (XORRandom(20)==0)
		{
			...
			if ( this.getMap().isTileWood( tile ) || this.getMap().isTileCastle( tile ) ) {		
			this.getMap().server_DestroyTile(tp, 0.05);
			}
		}	
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
	...
	if ((friend is null || blob.getTeamNum() != friend.getTeamNum()) && blob.getName() != this.getName() && blob.hasTag("flesh") && !blob.hasTag("dead") && blob.getName() != "Wraith")
	{
		...
		if (vellen > 0.1f)
		{
			...
			if (getNet().isServer() && XORRandom(4)==0 && !blob.hasTag("boss") && blob.hasTag("flesh"))
			{
				f32 power = this.get_f32("bite damage");
