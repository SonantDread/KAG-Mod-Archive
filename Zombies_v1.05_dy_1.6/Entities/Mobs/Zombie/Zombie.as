    ...
    if (blob.getHealth() > 0.0)
    {
		...
		if( blob.hasTag(chomp_tag) && !this.isAnimation("bite"))
		{
			if (!this.isAnimation("bite")) {
				int r = XORRandom(2);
				if(r==0)
				this.PlaySound( "/ZombieBite1" );
				else
				this.PlaySound( "/ZombieBite2" );
...
void onInit(CBlob@ this)
{
	...
	f32 zombie_player_dmg = getRules().get_f32("zombie_player_dmg");
	this.set_f32("bite damage", zombie_player_dmg);
	int bitefreq = 30;
	...
	if ((this.isKeyPressed(key_left) || this.isKeyPressed(key_right) || this.isKeyPressed(key_down) || this.isKeyPressed(key_up)) )
	{
		...
		if (XORRandom(break_chance)==0)
		{
			...
			//if ( !this.getMap().isTileBedrock( tile ) ) { //kill everything except bedrock
				this.getMap().server_DestroyTile(tp, 0.075);
				this.Tag(chomp_tag);
			//}
			}
		}
...
{		
    ...
    if (this.getHealth() <= gibHealth)
    {
		...
		if (hitterBlob.hasTag("player"))
		{
			...
			if (player !is null || hitterBlob.getName() == "archer")
			{
		...
		server_DropCoins(hitterBlob.getPosition() + Vec2f(0,-3.0f), 15);
