...
const SColor color_Caravel(255, 255, 140, 255);
...
class PNGLoader
{
	...
	bool loadMap( CMap@ _map, const string& in filename)
	{
		...
		string currentMapName = filename;
		getRules().set_string("current_map",currentMapName);
		//printf("currentMapName = "+currentMapName);
		 

		//zCfg.add_string("current_map", currentMapName);
	...
	if (pixel == color_tile_ground) {
		...
		else if (pixel == color_Caravel)
		{
			spawnVehicle( map, "Caravel", offset);
			offsets[autotile_offset].push_back( offset );
		}
