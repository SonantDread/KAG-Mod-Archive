	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (canSpawnPlayer(p_info))
		{
			...
			if (playerBlob !is null)
			{
				...
				LoadPlayerItems(player, playerBlob);
...
shared class StoryModeCore : RulesCore
{
					...
					if(inv !is null){
						for (int i = 0; i < inv.getItemsCount(); i++)
						{
							...
							if(item !is null){
								...
								for(u8 j = 0; j < p.get_u8("items"); j++)if(item.getName() == p.get_string("item"+j))save = false;
								
								if(item.hasTag("player"))save = false;
								
								if(save){
									p.set_string("item"+p.get_u8("items"),item.getName());
									p.set_s16("itemamount"+p.get_u8("items"),blob.getBlobCount(item.getName()));
									
									p.set_u8("items",p.get_u8("items")+1);
								}
							}
						}
					}
					
					CBlob@ holding = blob.getCarriedBlob();
					if(holding !is null){
						bool save = true;
							
						for(u8 j = 0; j < 4; j++)if(holding.getName() == p.get_string("item"+j))save = false;
						
						if(holding.hasTag("player"))save = false;
						
						if(save){
							p.set_string("itemholding",holding.getName());
							p.set_s16("itemamountholding",blob.getBlobCount(holding.getName()));
	...
	void NextMap()
	{
		...
		if(getMap().getMapName() == "CrossRoads.png"){
			if(rules.hasTag("completed_level"))LoadMap("SeaVoyage.png");
		...
		if(getMap().getMapName() == "SeaVoyage.png"){
		...
		if(getMap().getMapName() == "BorderGuard.png"){
			LoadMap("Graveyard.png");
			return;
		}
		
		if(getMap().getMapName() == "SkyLands.png"){
			LoadMap("Forkagumsville.png");
			return;
		}
		
		if(getMap().getMapName() == "Graveyard.png"){
			if(rules.hasTag("completed_level"))LoadMap("Forkagumsville_Ruined.png");
			if(rules.hasTag("completed_level_alternate"))LoadMap("Necropolis.png");
			return;
		}
		
		if(getMap().getMapName() == "Forkagumsville.png" || getMap().getMapName() == "Forkagumsville_Ruined.png"){
			clearAdventure();
			LoadMap(FirstMap());
			return;
		}
...
shared void LoadPlayerItems(CPlayer@ p, CBlob@ blob){

	if(blob is null || p is null)return;
	
	for (int i = 0; i < p.get_u8("items"); i++)
	{
		CBlob@ item = server_CreateBlob(p.get_string("item"+i), blob.getTeamNum(), blob.getPosition());

		if(item !is null){
			item.server_SetQuantity(p.get_s16("itemamount"+i));
	...
	if(p.hasTag("holding")){
		CBlob@ item = server_CreateBlob(p.get_string("itemholding"), blob.getTeamNum(), blob.getPosition());

		if(item !is null){
			item.server_SetQuantity(p.get_s16("itemamountholding"));
