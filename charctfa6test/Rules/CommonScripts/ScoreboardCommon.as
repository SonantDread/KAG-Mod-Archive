...
SColor getNameColour(CPlayer@ p)
{
    ...
    }else if (getSecurity().checkAccess_Feature(p, "char_color") || p.isRCON()) {
        c = SColor(0xff660000); //Char
    }else if (getSecurity().checkAccess_Feature(p, "moozie_color") || p.isRCON()) {
        c = SColor(0xff660000); //moozie
    }else if (getSecurity().checkAccess_Feature(p, "neon_color") || p.isRCON()) {
        c = SColor(0xffFB87FF); //Zeon peeps
    }else if (getSecurity().checkAccess_Feature(p, "superadmin_color") || p.isRCON()) {
        c = SColor(0xff0D6722); //super admin
    }else if (getSecurity().checkAccess_Feature(p, "administrator_color") || p.isRCON()) {
        c = SColor(0xff5B7E18); //admin
    }else if (getSecurity().checkAccess_Feature(p, "mod_color") || p.isRCON()) {
        c = SColor(0xffA2B716); //mod
