//made by blackjoker77777
//for Lover Butt sandbox server.
...
void onTick( CBlob@ this )
{
	if (getNet().isServer() && this.isKeyPressed( key_action1 ))
	{
		...
		if (diff > 0)
		{
			...
			CBlob@ sceptileattack = server_CreateBlob( "attack", this.getTeamNum(), pos + Vec2f(0.0f,-0.5f*this.getRadius()) );   
			if (sceptileattack !is null)
			{
				Vec2f norm = aim - pos;
				norm.Normalize();
				sceptileattack.setVelocity( norm * (diff <= FIRE_FREQUENCY ? ORB_SPEED : 2.0f*ORB_SPEED) );
				
				if(targetID != 0xffff)
				{
					sceptileattack.set_u16("target", targetID);
				}
			}
		}
	}   
