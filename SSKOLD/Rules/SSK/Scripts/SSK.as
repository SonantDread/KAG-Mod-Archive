...
shared class SSKCore : RulesCore
{
				...
				if (player.team == team_num)
				{
					...
					if (e_player !is null)
					{
						...
						if (blob_alive)
						{
							...
							if (player_blob.getName() == "ssk_archer")
							{
								player_char = "a";
							}
	...
	{
		SSKPlayerInfo p(player.getUsername(), player.getTeamNum(), player.isBot() ? "ssk_knight" : "ssk_knight");
		players.push_back(p);
	...
	{
		// give ssk_archer arrows

		if (blob.getName() == "ssk_archer")
		{
