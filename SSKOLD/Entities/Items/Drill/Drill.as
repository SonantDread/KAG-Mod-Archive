...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (int(heat) >= heat_max - (heat_add * 1.5))
		{
			...
			CustomHitData customHitData(8, 0, 0, 20);
			server_customHit(this, holder, holder.getPosition(), Vec2f(), 0.25f, Hitters::burn, true, customHitData);
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (hi.blob !is null) // blob
							{
								...
								if (getNet().isServer())
								{
									...
									CustomHitData customHitData(8, 1.0f, 0.02f);
									server_customHit(this, hi.blob, hi.hitpos, attackVel, attack_dam, Hitters::drill, true, customHitData);
