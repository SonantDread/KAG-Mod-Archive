	...
	if (map.getHitInfosFromArc(pos, -angle, 30, vellen, this, false, @hitInfos))
	{
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			else if (team != u8(hi.blob.getTeamNum()))
			{
				...
				server_customHit(this, hi.blob, pos, vel, dmg, Hitters::cata_boulder, true, customHitData);
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (solid && blob !is null)
	{
		...
		hitvel.Normalize();
		f32 dmg = vellen > 8.0f ? 20.0f : (vellen > 4.0f ? 15.0f : 5.0f);
		...
		//hurt
		CustomHitData customHitData(10, 4.0f, 0.04f);
		server_customHit(this, blob, point1, hitvel, dmg, Hitters::cata_boulder, true, customHitData);
