					...
					for (uint i = 0; i < blobsInRadius.length; i++)
					{
						...
						if (canStab(b)) //even hurts team when stabbing
						{
							...
							if (this.isOverlapping(b))
							{
								...
								if (b.get("statusVars", @statusVars))
								{
								...
								if (applyDamage)
								{
									...
									server_customHit(this, b, pos, hitVec, 2.0f, Hitters::spikes, true, customHitData);
								}	
							}
						}
					}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point)
{
	...
	if (state == falling)
	{
		...
		if (vel.y < 4.0f) //slow, minimal dmg
			dmg = 1.0f;
		else if (vel.y < 5.5f) //faster, kill ssk_archer
			dmg = 2.0f;
		...
		server_customHit(this, blob, point, Vec2f(0, 1), dmg, Hitters::spikes, true, customHitData);
		return;
	}
	...
	if (damage > 0)
	{
			...
			if (applyDamage)
			{
				...
				CustomHitData customHitData(4, 0, 0.1f);
				server_customHit(this, blob, point, hitVec, 2.0f, Hitters::spikes, true, customHitData);
			}
	}
}
