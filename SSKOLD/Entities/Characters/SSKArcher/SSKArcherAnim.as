...
{
	RunnerTextures@ runner_tex = addRunnerTextures(this, "ssk_archer", "SSKArcher");
...
{
	ensureCorrectRunnerTexture(this, "ssk_archer", "SSKArcher");
...
void onTick(CSprite@ this)
{
	...
	doRopeUpdate(this, blob, ssk_archer);
	...
	bool legolas = ssk_archer.charge_state == SSKArcherParams::legolas_ready || ssk_archer.charge_state == SSKArcherParams::legolas_charging;
	needs_shiny = false;
	bool crouch = false;

	const u16 tumbleTime = statusVars.tumbleTime;
	...
	if (firing || legolas)
	{
		...
		DrawBow(this, blob, ssk_archer, armangle, arrowType, armOffset);
	}
	else
	{
	...
	DrawBowEffects(this, blob, ssk_archer, arrowType);

	//set the head anim
	if (tumbleTime > 0 || crouch)
	{
		blob.Tag("dead head");
	}
...
void DrawBow(CSprite@ this, CBlob@ blob, SSKArcherInfo@ ssk_archer, f32 armangle, const u8 arrowType, Vec2f armOffset)
{
	...
	if (!ssk_archer.has_arrow || ssk_archer.charge_state == SSKArcherParams::no_arrows || ssk_archer.charge_state == SSKArcherParams::legolas_charging)
	{
		string animname = "no_arrow";

		if (ssk_archer.charge_time == SSKArcherParams::ready_time)
		{
			animname = "fired";
		}

		u16 frontframe = 0;
		f32 temp = Maths::Min(ssk_archer.charge_time, SSKArcherParams::ready_time);
		f32 ready_tween = temp / SSKArcherParams::ready_time;
	...
	else if (ssk_archer.charge_state == SSKArcherParams::readying)
	{
		u16 frontframe = 0;
		f32 temp = ssk_archer.charge_time;
		f32 ready_tween = temp / SSKArcherParams::ready_time;
		armangle = armangle * ready_tween;
		armOffset = Vec2f(-1.0f, 4.0f + config_offset + 2.0f * (1.0f - ready_tween));
	...
	else if (ssk_archer.charge_state != SSKArcherParams::fired || ssk_archer.charge_state == SSKArcherParams::legolas_ready)
	{
		u16 frontframe = Maths::Min((ssk_archer.charge_time / (SSKArcherParams::shoot_period_1 + 1)), 2);
		setArmValues(frontarm, true, armangle, 0.1f, "charge", Vec2f(-4.0f * sign, 0.0f), armOffset);
		frontarm.animation.frame = frontframe;

		const f32 arrowangle = (ssk_archer.charge_time > SSKArcherParams::shoot_period_2) ? armangle + -2.5f + float(XORRandom(500)) / 100.0f : armangle; // shiver arrow when fully charged
		...
		if (ssk_archer.charge_state == SSKArcherParams::legolas_ready)
		{
			needs_shiny = true;
...
void DrawBowEffects(CSprite@ this, CBlob@ blob, SSKArcherInfo@ ssk_archer, const u8 arrowType)
{
	// set fire light
...
void doRopeUpdate(CSprite@ this, CBlob@ blob, SSKArcherInfo@ ssk_archer)
{
	CSpriteLayer@ rope = this.getSpriteLayer("rope");
	CSpriteLayer@ hook = this.getSpriteLayer("hook");

	bool visible = ssk_archer !is null && ssk_archer.grappling;
	...
	Vec2f off = ssk_archer.grapple_pos - blob.getPosition();
	...
	hook.ResetTransform();
	if (ssk_archer.grapple_id == 0xffff) //still in air
	{
		ssk_archer.cache_angle = -ssk_archer.grapple_vel.Angle();
	}
	hook.RotateBy(ssk_archer.cache_angle , Vec2f());

	hook.TranslateBy(off);
	hook.SetIgnoreParentFacing(true);
	hook.SetFacingLeft(false);

	//GUI::DrawLine(blob.getPosition(), ssk_archer.grapple_pos, SColor(255,255,255,255));
}
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/SSKArcher/SSKArcherGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/SSKArcher/SSKArcherGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/SSKArcher/SSKArcherGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/SSKArcher/SSKArcherGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
