...
{
	addRunnerTextures(this, "ssk_knight", "SSKKnight");
...
{
	ensureCorrectRunnerTexture(this, "ssk_knight", "SSKKnight");
}
...
void onTick(CSprite@ this)
{
	...
	else if (ssk_knight.state == SSKKnightStates::shieldgliding)
	{
	...
	else if (ssk_knight.state == SSKKnightStates::shielding)
	{
		if (walking)
		{
			if (direction == 0)
	...
	else if (ssk_knight.state == SSKKnightStates::sword_drawn)
	{
		if (ssk_knight.swordTimer < SSKKnightParams::slash_charge)
		{
			this.SetAnimation("draw_sword");
		}
		else if (ssk_knight.swordTimer < SSKKnightParams::slash_charge_level2)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 0;
		}
		else if (ssk_knight.swordTimer < SSKKnightParams::slash_charge_limit)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 1;
			shinydot = true;
	...
	else if (ssk_knight.state == SSKKnightStates::sword_cut_mid_down)
	{
		this.SetAnimation("strike_mid_down");
	}
	else if (ssk_knight.state == SSKKnightStates::sword_cut_up)
	{
		this.SetAnimation("strike_up");
	}
	else if (ssk_knight.state == SSKKnightStates::sword_cut_down)
	{
		this.SetAnimation("strike_down");
	}
	else if (ssk_knight.state == SSKKnightStates::sword_power || ssk_knight.state == SSKKnightStates::sword_power_super)
	{
		this.SetAnimation("strike_power");

		if (ssk_knight.swordTimer <= 1)
			this.animation.SetFrameIndex(0);

		u8 mintime = 6;
		u8 maxtime = 8;
		if (ssk_knight.swordTimer >= mintime && ssk_knight.swordTimer <= maxtime)
		{
			wantsChopLayer = true;
			chopframe = ssk_knight.swordTimer - mintime;
			chopAngle = -vec.Angle();
		}
	}
	else if (inair)
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (SSKKnightParams::slash_charge_limit - SSKKnightParams::slash_charge_level2);
			f32 count = (ssk_knight.swordTimer - SSKKnightParams::slash_charge_level2);
	...
	//set the head anim
	if (tumbleTime > 0 || statusVars.dazeTime > 0)
	{
		blob.Tag("dead head");
	}
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/SSKKnight/SSKKnightGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/SSKKnight/SSKKnightGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/SSKKnight/SSKKnightGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/SSKKnight/SSKKnightGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
