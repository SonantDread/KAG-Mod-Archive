...
void ssk_knight_actorlimit_setup(CBlob@ this)
{
	u16[] networkIDs;
	this.set("LimitedActors", networkIDs);
}

bool ssk_knight_has_hit_actor(CBlob@ this, CBlob@ actor)
{
	u16[]@ networkIDs;
...
u32 ssk_knight_hit_actor_count(CBlob@ this)
{
	u16[]@ networkIDs;
	this.get("LimitedActors", @networkIDs);
...
void ssk_knight_add_actor_limit(CBlob@ this, CBlob@ actor)
{
	this.push("LimitedActors", actor.getNetworkID());
}

void ssk_knight_clear_actor_limits(CBlob@ this)
{
...
void onInit(CBlob@ this)
{
	...
	ssk_knight.state = SSKKnightStates::normal;
	ssk_knight.swordTimer = 0;
	ssk_knight.shieldTimer = 0;
	ssk_knight.slideTime = 0;
	ssk_knight.doubleslash = false;
	ssk_knight.shield_down = getGameTime();
	ssk_knight.tileDestructionLimiter = 0;

	this.set("ssk_knightInfo", @ssk_knight);

	this.set_f32("gib health", -1.5f);
	addShieldVars(this, SHIELD_BLOCK_ANGLE, 2.0f, 5.0f);
	ssk_knight_actorlimit_setup(this);
	...
	SetHelp(this, "help self action", "ssk_knight", "$Jab$Jab        $LMB$", "", 4);
	SetHelp(this, "help self action2", "ssk_knight", "$Shield$Shield    $KEY_HOLD$$RMB$", "", 4);
...
void onTick(CBlob@ this)
{
	...
	SSKKnightInfo@ ssk_knight;
	if (!this.get("ssk_knightInfo", @ssk_knight))
	{
		return;
	}
	...
	bool shieldState = isShieldState(ssk_knight.state);
	bool specialShieldState = isSpecialShieldState(ssk_knight.state);
	bool swordState = isSwordState(ssk_knight.state);
	...
	//we'll see if ssk_knights dmging stuff while in menus is a real issue and go from there
	if (isHitstunned || tumbleTime > 0 || dazeTime > 0 || inMoveAnimation || holdingUsableItem)// || myplayer && getHUD().hasMenus())
	{
		ssk_knight.state = SSKKnightStates::normal; //cancel any attacks or shielding
		ssk_knight.swordTimer = 0;
		ssk_knight.shieldTimer = 0;
		ssk_knight.slideTime = 0;
		ssk_knight.doubleslash = false;
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		if (!canRaiseShield(this))
		{
			...
			{
				ssk_knight.shield_down = getGameTime() + 40;
			}

			ssk_knight.state = SSKKnightStates::normal;

			if (pressed_a2 && ((ssk_knight.shield_down - getGameTime()) <= 0))
			{
		...
		else
		{
			bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f) &&
			                  moveVars.fallCount > SSKKnightParams::glide_down_time);

			if (pressed_a2 && inair && !this.isInWater())
			{
				if (direction == -1 && !forcedrop && !getMap().isInWater(pos + Vec2f(0, 16)) && !moveVars.wallsliding)
				{
					ssk_knight.state = SSKKnightStates::shieldgliding;
					ssk_knight.shieldTimer = 1;
				}
				else if (forcedrop || direction == 1)
				{
					ssk_knight.state = SSKKnightStates::shielddropping;
					ssk_knight.shieldTimer = 5;
					ssk_knight.slideTime = 0;
				}
				else //remove this for partial locking in mid air
				{
					ssk_knight.state = SSKKnightStates::shielding;
				}
			}

			if (ssk_knight.state == SSKKnightStates::shieldgliding && !this.isInWater() && !forcedrop)
			{
				moveVars.stoppingFactor *= 0.5f;

				f32 glide_amount = 1.0f - (moveVars.fallCount / f32(SSKKnightParams::glide_down_time * 2));
			...
			else if (ssk_knight.state == SSKKnightStates::shielddropping)
			{
				if (this.isInWater())
				{
				...
				else
				{
					...
					if (!inair)
					{
						ssk_knight.slideTime++;
						if (ssk_knight.slideTime > 0)
						{
							if (ssk_knight.slideTime == 5)
							{
								this.getSprite().PlayRandomSound("/Scrape");
							}

							f32 factor = Maths::Max(1.0f, 2.2f / Maths::Sqrt(ssk_knight.slideTime));
							moveVars.walkFactor *= factor;

							//  printf("ssk_knight.slideTime = " + ssk_knight.slideTime  );
							if (ssk_knight.slideTime > 30)
							{
								moveVars.walkFactor *= 0.75f;
								if (ssk_knight.slideTime > 45)
								{
									ssk_knight.state = SSKKnightStates::shielding;
								}
							}
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		if (getNet().isClient())
		{
			if (ssk_knight.swordTimer == SSKKnightParams::slash_charge_level2)
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
			else if (ssk_knight.swordTimer == SSKKnightParams::slash_charge)
			{
				Sound::Play("SwordSheath.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
		}

		if (ssk_knight.swordTimer >= SSKKnightParams::slash_charge_limit)
		{
			Sound::Play("/Stun", pos, 1.0f, this.getSexNum() == 0 ? 1.0f : 2.0f);

			statusVars.dazeTime = 15;

			if (getNet().isServer())
				SyncDazeTime(this);
		}

		bool strong = (ssk_knight.swordTimer > SSKKnightParams::slash_charge_level2);
		...
		if (ssk_knight.state == SSKKnightStates::normal ||
		        ...
		        (!inMiddleOfAttack(ssk_knight.state) || shieldState))
		{
			ssk_knight.state = SSKKnightStates::sword_drawn;
			ssk_knight.swordTimer = 0;
		}

		if (ssk_knight.state == SSKKnightStates::sword_drawn && getNet().isServer())
		{
			ssk_knight_clear_actor_limits(this);
		}

		//responding to releases/noaction
		s32 delta = ssk_knight.swordTimer;
		if (ssk_knight.swordTimer < 128)
			ssk_knight.swordTimer++;

		if (ssk_knight.state == SSKKnightStates::sword_drawn && !pressed_a1 &&
		        !this.isKeyJustReleased(key_action1) && delta > SSKKnightParams::resheath_time)
		{
			ssk_knight.state = SSKKnightStates::normal;
		}
		else if (this.isKeyJustReleased(key_action1) && ssk_knight.state == SSKKnightStates::sword_drawn)
		{
			ssk_knight.swordTimer = 0;

			if (delta < SSKKnightParams::slash_charge)
			{
				if (direction == -1)
				{
					ssk_knight.state = SSKKnightStates::sword_cut_up;
				}
				else if (direction == 0)
				{
					if (aimpos.y < pos.y)
					{
						ssk_knight.state = SSKKnightStates::sword_cut_mid;
					}
					else
					{
						ssk_knight.state = SSKKnightStates::sword_cut_mid_down;
					}
				}
				else
				{
					ssk_knight.state = SSKKnightStates::sword_cut_down;
				}
			}
			else if (delta < SSKKnightParams::slash_charge_level2)
			{
				ssk_knight.state = SSKKnightStates::sword_power;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				ssk_knight.slash_direction = aiming_direction;
			}
			else if (delta < SSKKnightParams::slash_charge_limit)
			{
				ssk_knight.state = SSKKnightStates::sword_power_super;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				ssk_knight.slash_direction = aiming_direction;
		...
		else if (ssk_knight.state >= SSKKnightStates::sword_cut_mid &&
		         ssk_knight.state <= SSKKnightStates::sword_cut_down) // cut state
		{
			...
			{
				f32 attackarc = 90.0f;
				f32 attackAngle = getCutAngle(this, ssk_knight.state);

				if (ssk_knight.state == SSKKnightStates::sword_cut_down)
				{
					attackarc *= 0.9f;
				}

				DoAttack(this, JAB_DAMAGE, attackAngle, attackarc, Hitters::sword, delta, ssk_knight);
			}
			else if (delta >= 9)
			{
				ssk_knight.swordTimer = 0;
		...
		else if (ssk_knight.state == SSKKnightStates::sword_power ||
		         ssk_knight.state == SSKKnightStates::sword_power_super)
		{
			//setting double
			if (ssk_knight.state == SSKKnightStates::sword_power_super &&
			        this.isKeyJustPressed(key_action1))
			{
				ssk_knight.doubleslash = true;
			}
			...
			{
				DoAttack(this, 10.0f, -(vec.Angle()), 120.0f, Hitters::sword, delta, ssk_knight);
			}
			else if (delta >= SSKKnightParams::slash_time ||
			         (ssk_knight.doubleslash && delta >= SSKKnightParams::double_slash_time))
			{
				ssk_knight.swordTimer = 0;

				if (ssk_knight.doubleslash)
				{
					ssk_knight_clear_actor_limits(this);
					ssk_knight.doubleslash = false;
					ssk_knight.state = SSKKnightStates::sword_power;
		...
		if ((ssk_knight.state == SSKKnightStates::sword_power ||
		        ssk_knight.state == SSKKnightStates::sword_power_super) &&
		        delta < SSKKnightParams::slash_move_time)
		{

			if (Maths::Abs(vel.x) < SSKKnightParams::slash_move_max_speed &&
			        vel.y > -SSKKnightParams::slash_move_max_speed)
			{
				Vec2f slash_vel =  ssk_knight.slash_direction * this.getMass() * 0.5f;
	...
	else if (this.isKeyJustReleased(key_action2) || this.isKeyJustReleased(key_action1) || this.get_u32("ssk_knight_timer") <= getGameTime())
	{
		ssk_knight.state = SSKKnightStates::normal;
	}
	...
	*/
	...
	if (myplayer)
	{
		...
		{
			SetHelp(this, "help self action", "ssk_knight", "$Slash$ Slash!    $KEY_HOLD$$LMB$", "", 13);
		}
	}
	...
	if (shieldState)
	{
		...
		{
			if (ssk_knight.state == SSKKnightStates::shieldgliding)
			{
		...
		if ((ssk_knight.state == SSKKnightStates::shielding && direction == -1) ||
		        ssk_knight.state == SSKKnightStates::shieldgliding)
		{
...
{
	// play cling sound if other ssk_knight attacked us
	// dmg could be taken out here if we ever want to

	if (hitterBlob.getPosition().x < this.getPosition().x && hitterBlob.getName() == "ssk_knight") // ssk_knight and the left one (to play only once)
	{
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, SSKKnightInfo@ info)
{
	if (!getNet().isServer())
	{
		return;
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (ssk_knight_has_hit_actor(this, b))
				{
					if (large)
						dontHitMore = true;
				...
				ssk_knight_add_actor_limit(this, b);
				if (!dontHitMore)
				{
					...
					if (jab)
					{
						CustomHitData customHitData(4, 1.0f, 0.02f);
						server_customHit(this, b, hi.hitpos, velocity, damage, type, true, customHitData);
					}
					else
					{
	...
	if (carriedBlob !is null && carriedBlob.hasTag("player"))
	{
		...
		CustomHitData customHitData(8, 2.0f, 0.06f);
		server_customHit(this, carriedBlob, carriedBlobPos, carriedBlobPos - pos, damage, type, true, customHitData);
	}
...
bool isSliding(SSKKnightInfo@ ssk_knight)
{
	return (ssk_knight.slideTime > 0 && ssk_knight.slideTime < 45);
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (this.getShape().vellen > SHIELD_KNOCK_VELOCITY || onground)
	{
		...
		//printf("ssk_knight.stat " + ssk_knight.state );
		if (ssk_knight.state == SSKKnightStates::shielddropping &&
		        (!onground || isSliding(ssk_knight)) &&
		        ...
		        statusVars.tumbleTime == 0)
		{
			...
			if (vel * normal < 0.0f && ssk_knight_hit_actor_count(this) == 0) //only bash one thing per tick
			{
				ShieldVars@ shieldVars = getShieldVars(this);
				//printf("shi " + shieldVars.direction * normal );
				if (shieldVars.direction * normal < 0.0f)
				{
					ssk_knight_add_actor_limit(this, blob);
					CustomHitData customHitData(4, 4.0f, 0.0f, 30);
					server_customHit(this, blob, pos, vel, 0.0f, Hitters::shield, true, customHitData);
...
bool hasItem(CBlob@ this, const string &in name)
{
	...
	{
		warn("our inventory was null! SSKKnightLogic.as");
	}
...
void TakeItem(CBlob@ this, const string &in name)
{
	...
	if (inv !is null)
	{
		...
		{
			// warn("took a bomb even though we dont have one! SSKKnightLogic.as");
		}
	}
	else
	{
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	{
		SSKStatusVars@ hitBlobStatusVars;
		if (hitBlob.get("statusVars", @hitBlobStatusVars))
		{
			hitBlobStatusVars.dazeTime = 20;
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (this.isMyPlayer() && this.getInventory().getItemsCount() > 1)
	{
		for (uint j = 1; j < bombTypeNames.length; j++)
		{
			...
			{
				SetHelp(this, "help inventory", "ssk_knight", "$Help_Bomb1$$Swap$$Help_Bomb2$         $KEY_TAP$$KEY_F$", "", 2);
				break;
			}
		}
	}
