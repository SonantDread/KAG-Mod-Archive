...
{
	if (this.get_u32("rektime") <= getGameTime() && this.get_bool("keep rekking")) 
	{
		CPlayer@ evilplayer = this.getPlayerOfRecentDamage();
		if (evilplayer !is null)
		{
			CBlob@ evilblob = evilplayer.getBlob();
			if (evilblob !is null)
			{
				this.set_bool("keep rekking", false);
				evilblob.setVelocity(Vec2f(0, 100));
				this.getSprite().PlaySound("/EvilNotice");
			}
		}
	}
		...
		if (att !is null)
		{
			...
			if (b !is null)
			{
				...
				if (vel.y > 0.5f)
				{
					...
					if (XORRandom(100) == 1)
					{
					b.setVelocity(Vec2f(0, 100));
					this.getSprite().PlaySound("/EvilLaughShort2");
					}
				}
			}
		}
...
{
	CPlayer@ bishe = hitterBlob.getDamageOwnerPlayer();
	if (bishe is null) return(damage);
	CBlob@ bisheBlob = bishe.getBlob();
	if (bisheBlob is null) return(damage);
	bisheBlob.setVelocity(Vec2f(0, -15));
	this.set_u32("rektime", getGameTime()+ 40);
	this.set_bool("keep rekking", true);
	return(damage);
}
