...
void onInit(CSprite@ this)
{
	const string texname = this.getBlob().getSexNum() == 0 ?
	                       "ArmsMasterMale.png" :
	                       "ArmsMasterFemale.png"; //These three lines are like coder magic. Don't touch em unless you want it to stop working. Only change the names of the .pngs.
...
void onTick(CSprite@ this)
{
	...
	if (!blob.hasTag(burning_tag)) //Are we burning? If so, lets not screw with the burning animations.
	{
		...
		else if (action2 && !blob.get_bool("loading") || (this.isAnimation("actionanim") && !this.isAnimationEnded()))
		{
			this.SetAnimation("actionanim");
		}
		else if (blob.hasTag("loading") || (this.isAnimation("loadanim") && !this.isAnimationEnded()))
		{
			this.SetAnimation("loadanim");
		}
		else if (action1 && !blob.hasTag("loaded") || (this.isAnimation("shootanim") && !this.isAnimationEnded()))
		{
			this.SetAnimation("shootanim");
		}
		else if (inair)
		{
			RunnerMoveVars@ moveVars;
