...
void onInit(CBlob@ this)
{
	if (this.exists("sprite index"))
	{
		...
		if (spriteIndex < seed_sprites.length && spriteIndex != 7)
		{
			LoadSprite(this, seed_sprites[spriteIndex], spriteIndex);
		}
		if (spriteIndex == 7)
		{
			this.setInventoryName(this.get_string("seed_grow_blobname") + " seed");
		}
...
void onTick(CBlob@ this)
{
	...
	if (seed_grow_time <= OPT_TICK)
	{
		...
		if (getNet().isServer())
		{
			...
			CBlob@ b = server_CreateBlobNoInit(this.get_string("seed_grow_blobname"));
			if (b !is null)
			{
				b.setPosition(this.getPosition() + Vec2f(0, rad));
				b.server_setTeamNum(-1);
				b.set_u16("quality", this.get_u16("quality"));
				b.Init();
			}
