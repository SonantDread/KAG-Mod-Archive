...
{
	spawnFish(40, "fishy", 4);
	spawnFish(15, "pirhanna", 8);
	spawnFish(2, "orca", 200);
}
void spawnFish(int min_fish, string fish_name, int spawn_chance)
{
	if (getGameTime() % 29 != 0) return;
	if (XORRandom(spawn_chance) != 1) return; //50% chance of actually doing anything
	...
	{
		if (fish.length > 2 && XORRandom(3) < 1) //breed fish (33% chance)
		{
