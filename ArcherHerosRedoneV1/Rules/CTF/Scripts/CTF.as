...
shared class GCTFSpawns : RespawnSystem
{
	GCTFCore@ CTF_core;
...
shared class GCTFCore : RulesCore
{
	s32 warmUpTime;
	s32 gameDuration;
	s32 spawnTime;
  ...
  bool eloDone = false;
	...
	GCTFSpawns@ ctf_spawns;

	GCTFCore() {}

	GCTFCore(CRules@ _rules, RespawnSystem@ _respawns)
	{
	...
	void Setup(CRules@ _rules = null, RespawnSystem@ _respawns = null)
	{
		...
		@ctf_spawns = cast < GCTFSpawns@ > (_respawns);
      ...
      print("" +eloDone);
        ...
        CBlob@[] players;
            ...
            if (profile !is null )
            {
              ...
              {
                average0 += profile.elo;
              }
              else
              {
                average1 += profile.elo;
              }
            }
					}
				}
        average0 = average0 / teams[0].players_count;
        average1 = average1 / teams[1].players_count;
        
        print("averages: " + average0 + " " + average1);
            ...
            if (profile !is null )
            {
              if(players[i].getTeamNum() == 0)
              {
                int winner = winteamIndex == 0 ? 1:0;
                float r = Maths::Pow(10,profile.elo/400.0f);
                float r1 = Maths::Pow(10,average1/400.0f);
                ...
                print("second e" + e1);
                print("averages: " + average0 + " " + average1);
                profile.elo = Maths::Round(profile.elo + ( 100* (winner-e)));
                player.setScore(profile.elo);
              }
              else
              {
                int winner = winteamIndex == 1 ? 1:0;
                float r = Maths::Pow(10,profile.elo/400.0f);
                float r1 = Maths::Pow(10,average0/400.0f);
