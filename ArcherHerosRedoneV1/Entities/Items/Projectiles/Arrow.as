...
void onInit(CBlob@ this)
{
  ...
  if (!this.exists("dmgmult"))
	...
	{
		SetupBomb(this, bomb_fuse, 48.0f, 1.5f, 0.0f, 0.0f, false);
...
void onTick(CBlob@ this)
{
  ...
  if (!this.exists("start"))
	{
		this.set_Vec2f("start", this.getPosition());
	}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
      ...
      if (dmg > 0.0f)
			{
      if (this.exists("reloadid") && blob.hasTag("player"))
      {
        CBlob@ turret1 = getBlobByNetworkID(this.get_u16("reloadid"));
        if(turret1 !is null && turret1.getName() == "") 
        {
          turret1.set_u32("Reload",0);
        }
...
f32 ArrowHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType)
{
  ...
  if(hitBlob.get_u32("trapped") > 1 && this.get_bool("hunter"))
  {
    damage = 8.0f;
  }
  
  if(hitBlob.getName() == "tank" && hitBlob.isKeyPressed(key_action2) && hitBlob.get_f32("Shield") > 0.0f) 
  {
    hitBlob.set_f32("Shield",hitBlob.get_f32("Shield")-damage);
    return 0.0f;
    print("health left: " + hitBlob.get_f32("Shield"));
  }
  ...
  if(hitBlob.getName() == "knight" && hitBlob.isKeyPressed(key_action2) && hitBlob.get_f32("Deflect") > 1.0f)
  {
    ShootArrow(this, this.getPosition() + Vec2f(0.0f, -2.0f), this.get_Vec2f("start") + Vec2f(0.0f, -2.0f),  18.0f, 0,hitBlob.getTeamNum(), false); 
    this.server_Die();
    return 0.0f;
  }
...
void ShootArrow(CBlob @this, Vec2f arrowPos, Vec2f aimpos, f32 arrowspeed, const u8 arrow_type,u8 team, const bool legolas = true)
{
		Vec2f arrowVel = (aimpos - arrowPos);
		arrowVel.Normalize();
		arrowVel *= arrowspeed;
		CreateArrow(this, arrowPos, arrowVel, arrow_type,team);
}

CBlob@ CreateArrow(CBlob@ this, Vec2f arrowPos, Vec2f arrowVel, u8 arrowType,u8 team)
{
