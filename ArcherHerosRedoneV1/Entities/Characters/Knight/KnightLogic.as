...
void onTick(CBlob@ this)
{
  if (this !is null)
				{
					CBlob @newBlob = server_CreateBlob("soldier", this.getTeamNum(), this.getPosition());
  ...
  if(pressed_a2 ) 
  {
    if(this.get_f32("Deflect") > 1.0f)
    {
      this.set_f32("Deflect",this.get_f32("Deflect") - 1.25f);
    }
    
  }
  else if (this.get_f32("Deflect") < 45.0f)
  {
    this.set_f32("Deflect",this.get_f32("Deflect") + 0.2f);
...
f32 angle_difference(f32 a, f32 b)
{
	f32 dif = Maths::Abs(a -b);
	if (dif > 180)
		dif -= 360;
	if (dif < -180)
		dif += 360;
	return Maths::Abs(dif);
}
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info)
{
	...
	Vec2f vel = this.getVelocity();
	Vec2f forward_vec(1, 0);
	forward_vec.RotateBy(aimangle);
	Vec2f pos = blobPos - forward_vec * 6.0f + vel + Vec2f(0, -2);
	vel.Normalize();

	f32 attack_distance_mod = Maths::Max(0.0f, 1.75f * this.getShape().vellen * (vel * forward_vec));
	f32 attack_distance = Maths::Min(DEFAULT_ATTACK_DISTANCE + attack_distance_mod, MAX_ATTACK_DISTANCE);

	f32 radius = this.getRadius();
	CMap@ map = this.getMap();

	const f32 blocked_angle_range = 35.0f;
	const f32 blocked_angle_thresh = blocked_angle_range / 2.0f;
	array<f32> blockedAngles;
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null) // blob
			{
				...
				//get and compare all angles
				Vec2f velocity = ((hi.hitpos + b.getPosition()) * 0.5f) - pos;
				f32 bangle = velocity.Angle();

				//(if there's anything blocking at all)
				if (blockedAngles.length > 0)
				{
					//check angles
					bool blocked = false;
					for (int j = 0; j < blockedAngles.length; j++)
					{
						f32 dif = angle_difference(bangle, blockedAngles[j]);
						if (dif < blocked_angle_thresh)
						{
							blocked = true;
							break;
						}
				...
				if (!canHit(this, b) || knight_has_hit_actor(this, b))
				{
					// no TK
					if (large)
					{
						blockedAngles.push_back(bangle);
					}
				...
				{
					// server_Hit() is server-side only
			...
			else if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
			{
				...
				if (ground || wood || dirt_stone || gold)
				{
					...
					f32 dif = angle_difference(exact_aimangle, tileangle);
