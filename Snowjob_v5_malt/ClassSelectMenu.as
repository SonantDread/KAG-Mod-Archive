	...
	if (this.get("playerclasses", @classes))
	{
		for (uint i = 0 ; i < classes.length; i++)
		{
			...
			if(button !is null){
				...
				if(CanPlayClass(pclass.configFilename, getLocalPlayer().getUsername(),  getLocalPlayer().getClantag()))canPlay = true;
				
				button.SetEnabled(canPlay);
				
				if(getClassOwners(pclass.configFilename) != "")
				button.SetHoverText(getClassOwners(pclass.configFilename));
			}
		}
	}
}

void addClassesToMenuNormal(CBlob@ this, CGridMenu@ menu, u16 callerID)
{
	PlayerClass[]@ classes;
...
void addClassesToMenuUnique(CBlob@ this, CGridMenu@ menu, u16 callerID)
{
	PlayerClass[]@ classes;

	if (this.get("playerclasses", @classes))
	{
		for (uint i = 3 ; i < classes.length; i++)
		{
			PlayerClass @pclass = classes[i];
			CBitStream params;
			write_classchange(params, callerID, pclass.configFilename);
