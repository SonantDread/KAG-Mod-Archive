...
{
	const string texname = "Twister.png"; //These three lines are like coder magic. Don't touch em unless you want it to stop working. Only change the names of the .pngs.
...
void onTick(CSprite@ this)
{
	...
	if (!blob.hasTag(burning_tag)) //Are we burning? If so, lets not screw with the burning animations.
	{
		...
		{
			if(blob.get_u8("charge") < 30){
				this.SetAnimation("spincharge");
			} else {
				if(!this.isAnimation("spin"))
	...
	else if (action2 || blob.isInFlames()) //Are we using our left click ability or are we on fire?
	{
