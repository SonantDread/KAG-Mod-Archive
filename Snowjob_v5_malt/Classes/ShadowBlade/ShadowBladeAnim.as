...
{
	string texname = "ShadowBlade.png";
	...
	this.RemoveSpriteLayer("pull");
	CSpriteLayer@ aura = this.addSpriteLayer("pull", "Pull.png" , 64, 64, this.getBlob().getTeamNum(), this.getBlob().getSkinNum());
	...
	if (aura !is null)
	{
		...
		aura.SetRelativeZ(-0.1f);
		aura.SetIgnoreParentFacing(true);
	}
}
...
void onTick(CSprite@ this)
{
	...
	this.getSpriteLayer("pull").SetVisible(false);
	if(!blob.hasTag("dead"))if(blob.isKeyPressed(key_action3))if(blob.get_s16("shadow_amount") > 0)this.getSpriteLayer("pull").SetVisible(true);
	this.getSpriteLayer("pull").RotateBy(-20, Vec2f(0,0));
