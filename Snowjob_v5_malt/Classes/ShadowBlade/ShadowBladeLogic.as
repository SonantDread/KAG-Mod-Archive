...
void onInit(CBlob@ this)
{
	...
	this.server_setTeamNum(100+XORRandom(100));
	this.Tag("teleport");
}
...
void onTick(CBlob@ this)
{
	...
	if(this.hasTag("teleport")){
		this.setPosition(Vec2f(getMap().tilemapwidth*getMap().tilesize/2,getMap().tilemapheight*getMap().tilesize/4));
		this.Untag("teleport");
	}
	...
	if(this.get_s16("shadow_amount") > 0)
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				if(b.hasTag("flesh"))
				{
					...
					dir.Normalize();
					b.setVelocity(dir+b.getVelocity());
				}
			}
		}
		if(getGameTime() % 5 == 0){
			this.set_s16("shadow_amount",this.get_s16("shadow_amount")-1);
	...
	if(this.isKeyPressed(key_taunts))
	{
		if(getGameTime() % 5 == 0){
			this.set_s16("shadow_amount",this.get_s16("shadow_amount")-5);
			if(getNet().isServer()){
				this.Sync("shadow_amount",true);
				this.server_Heal(1.0);
			}
		}
	}
