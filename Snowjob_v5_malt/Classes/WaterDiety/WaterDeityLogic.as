...
{
	if(this.hasTag("dead"))this.server_Die();
	...
	{
		// cancel charging
		if (archer.charge_state != ArcherParams::not_aiming)
		{
	...
	if(this.isKeyPressed(key_up)){
		this.AddForce(Vec2f(0,-25));
				Splash(this.getPosition()+Vec2f(0,3),Vec2f(0,2),XORRandom(3));
	}
...
void Splash(Vec2f pos, Vec2f vel, int randomnum)
{
	CParticle@ p = ParticleAnimated("Splash.png", pos,
	                                vel, 0.0f, Maths::Max(1.0f, 0.5f * (1.0f + Maths::Abs(vel.x))),
	                                2 + randomnum,
	                                0.1f, false);
	if (p !is null)
	{
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		if (legolas)
		{
			if (getNet().isServer())
			{
				CreateArrow(this, arrowPos, arrowVel, ArrowType::water);
			}
