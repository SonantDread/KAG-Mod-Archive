    ...
    if (blob.getHealth() > 0.0)
    {
		...
		else
		{
			if (XORRandom(100)==0)
			{
				...
				if (zGrowl==0)
				this.PlaySound( "/troll_groan1" );
				if (zGrowl==1)
				this.PlaySound( "/troll_groan2" );
			}
...
void onGib(CSprite@ this)
{
	...
	const u8 team = blob.getTeamNum();
																											//Collumn  //Row
	CParticle@ Head     = makeGibParticle( "GreenTrollGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 0, Vec2f (48,48), 2.0f, 20, "/BodyGibFall", team );																									//collumn, //row
    CParticle@ Body     = makeGibParticle( "GreenTrollGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       1, 0, Vec2f (48,48), 2.0f, 20, "/BodyGibFall", team );																									//collumn, //row
    CParticle@ Leg1     = makeGibParticle( "GreenTrollGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 1, Vec2f (48,48), 2.0f, 20, "/BodyGibFall", team );
   	CParticle@ Leg2   = makeGibParticle( "GreenTrollGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       1, 1, Vec2f (48,48), 2.0f, 20, "/BodyGibFall", team );
   	CParticle@ Arm1     = makeGibParticle( "GreenTrollGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 2, Vec2f (48,48), 2.0f, 20, "/BodyGibFall", team );
    CParticle@ Arm2     = makeGibParticle( "GreenTrollGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       1, 2, Vec2f (48,48), 2.0f, 20, "/BodyGibFall", team );
    CParticle@ Ball     = makeGibParticle( "GreenTrollGibs.png", pos, vel + getRandomVelocity( 90, hp , 80 ),       0, 3, Vec2f (48,48), 2.0f, 20, "/BodyGibFall", team );
...
void onInit(CBlob@ this)
{
	...
	f32 green_troll_player_dmg = getRules().get_f32("green_troll_player_dmg");
	this.set_f32("bite damage", green_troll_player_dmg);
	int bitefreq = 85;//-difficulty;
...
void onTick(CBlob@ this)
{
	...
	if (getGameTime() % 65 == 0)
	{
		...
		if (mad > 0)
		{
			...
			else
			{
				int zGrowl = XORRandom(2);
				if (zGrowl==0)
				this.getSprite().PlaySound( "/troll_groan1" );
				if (zGrowl==1)
				this.getSprite().PlaySound( "/troll_groan2" );
			}
	...
	if (this.isKeyPressed(key_left) || this.isKeyPressed(key_right) || this.isKeyPressed(key_down) || this.isKeyPressed(key_up))
	{
		if (XORRandom(break_chance)==0 || (this.hasTag(chomp_tag) && XORRandom(10)==0))
		{
				...
				s32 green_troll_tile_dmg_range = getRules().get_s32("green_troll_tile_dmg_range");
				int xMod = 0;
				int yMod = 0;
				...
				dir1.Normalize();
				Vec2f tp1 = this.getPosition() + (dir1)*(this.getRadius() + XORRandom(green_troll_tile_dmg_range));
				s32 green_troll_tile_dmg = getRules().get_s32("green_troll_tile_dmg");
				TileType tile = this.getMap().getTile( tp1 ).type;
				if (this.getMap().isTileSolid(tp1) && !this.getMap().isTileGroundStuff(tile))
				this.Tag(chomp_tag); this.getMap().server_DestroyTile(tp1, ((difficulty/14) + green_troll_tile_dmg));
				...
				this.getMap().server_DestroyTile(tp1, ((difficulty/16) + green_troll_tile_dmg));
	...
	if(getNet().isServer() && getGameTime() % 10 == 0)
	{
		...
		{	
			f32 green_troll_aggro_range = getRules().get_f32("green_troll_aggro_range");
			CBlob@ b = getBlobByNetworkID(this.get_netid(target_property));
			if((this.get_s16("mad timer") >= MAD_TIME/8) || (b !is null && this.getDistanceTo(b) < 106.0f) || (b !is null && this.getDistanceTo(b) < XORRandom(green_troll_aggro_range)) || (getGameTime() % 600 == 0))
			{
				this.Tag(chomp_tag);
			}
			else
...
{		
    ...
    {
    	int green_troll_reward_coins = getRules().get_s32("green_troll_reward_coins");
		...
		if (hitterBlob.hasTag("player"))
		{
			...
			if (player !is null)
			{
				...
				player.server_setCoins( player.getCoins() + (green_troll_reward_coins/10) );
			}
		} else
		...
		if (getNet().isServer())
		{
			...
			{
				server_DropCoins(hitterBlob.getPosition() + Vec2f(0,-3.0f), green_troll_reward_coins);
			}
		}
        this.server_Die();
    }
...
void onHitBlob( CBlob@ this, f32 damage, CBlob@ hitBlob)
{
	if (hitBlob !is null)
	{
		...
		Vec2f force = forcePow * this.getMass() * damage * green_troll_knockback_power;
