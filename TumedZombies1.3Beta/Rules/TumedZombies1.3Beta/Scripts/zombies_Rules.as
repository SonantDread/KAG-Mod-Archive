			...
			if (map !is null)
			{
				...
				if (map.getDayTime()>0.8 || map.getDayTime()<0.1)
				{
					...
					if (transition == 1 && (dayNumber % 5) == 0)
					{
						...
						Vec2f sp = zombiePlaces[XORRandom(zombiePlaces.length)];
						int boss = XORRandom(26);
						if (boss <= 10) server_CreateBlob( "BossZombieKnight", -1, sp);
						else
						if (boss <= 15) server_CreateBlob( "BossGreenTroll", -1, sp);
						else
						if (boss <= 20) server_CreateBlob( "BossMinotaurKing", -1, sp);
						else
						if (boss <= 25) server_CreateBlob( "BossRabidDog", -1, sp);
						else
						if (boss == 26) server_CreateBlob( "BossRedDragon", -1, sp); 
					}
