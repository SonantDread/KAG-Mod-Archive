             ...
             const bool bother_raycasting = true, const bool should_teamkill = false)
{
  ...
  if(hit_blob.getName() == "wooden_platform"||hit_blob.getName() == "wooden_door"||hit_blob.getName() == "stone_door") 
  {
  return false;
  }

	f32 scale;
	Vec2f bombforce = hit_blob.getName() == "tank"? Vec2f(0.0f,0.0f) : getBombForce(this, radius, hit_blob_pos, pos, hit_blob.getMass(), scale);
	f32 dam = hit_blob.getName() == "tank"? 0.0f:damage * scale;
  if (isOwnerBlob(this, hit_blob))
  {
  hit_blob.AddForce(bombforce);
  }
	...
	this.server_Hit(hit_blob, hit_blob_pos,
	                bombforce, dam,
	                hitter, hitter == Hitters::water  ||//allow selfkill with bombs
