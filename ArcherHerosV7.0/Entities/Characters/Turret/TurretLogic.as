...
void onTick(CBlob@ this) //This script is called 30 times a second. It's a general update script. Most of your modding will be done here.
{
  ...
  else 
  {
   ...
   moveVars.swimspeed = 0;
    moveVars.swimforce = 0;
  }
  if(this.isKeyPressed(key_action1) && !this.isOnLadder()&& getKnocked(this) < 1) 
  {
    if(this.get_u32("Reload") < 1 && !this.isKeyPressed(key_action2) && this.get_bool("Mode"))
    {
      ShootArrow(this, this.getPosition() + Vec2f(0.0f, -2.0f), this.getAimPos() + Vec2f(0.0f, -2.0f),  18.0f, 0, false); 
      this.set_u32("Reload",30);
    }
    else if(this.get_u32("Reload") < 1 && !this.isKeyPressed(key_action2) && !this.get_bool("Mode"))
    {
      ShootArrow(this, this.getPosition() + Vec2f(0.0f, -2.0f), this.getAimPos() + Vec2f(0.0f, -2.0f),  18.0f, 0, false); 
      this.set_u32("Reload",3);
    }
