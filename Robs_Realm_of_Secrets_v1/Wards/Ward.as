...
void Aura(CBlob@ this, int type, int subtype){
	...
	if(water && fire){
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 40.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				{
					if(b.get_u32("wardweakentime") >= getGameTime()-5)
					if(b.get_f32("wardweaken") > b.getHealth()){
						float dif = b.get_f32("wardweaken")-b.getHealth();
						b.server_SetHealth(b.getHealth()+dif/(0.5+power));
					}
					b.set_f32("wardweaken",b.getHealth());
					b.set_u32("wardweakentime",getGameTime());
				}
			}
		}
	} else
	...
	if(air && !fire && !rock && !water){
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 24.0f, @blobsInRadius)) 
		{
			...
			{
				CBlob@ b = blobsInRadius[i];
				if((b.hasTag("flesh") || b.hasTag("plant")) && !b.hasTag("dead"))if(b.getTeamNum() != this.getTeamNum())
				{
					Vec2f dir = b.getPosition()-this.getPosition();
					dir.Normalize();
					b.setVelocity(dir*(2.0/power)+b.getVelocity());
				}
			}
		}
	}
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 24.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				if((b.hasTag("flesh") || b.hasTag("plant")) && !b.hasTag("dead"))if(b.getTeamNum() != this.getTeamNum())
				{
					...
					dir.Normalize();
					b.setVelocity(dir*(3.0/power)+b.getVelocity());
				}
			}
		}
	}
