	...
	if (this.hasAttached())
	{
		...
		if (this.getAttachmentPoints(@aps))
		{
			for (uint i = 0; i < aps.length; i++)
			{
				...
				{
					// GET OUT
					bool getOut = ap.isKeyJustPressed(key_action3) || ap.isKeyJustPressed(key_use) || ap.isKeyJustPressed(key_pickup);
					if (blob.isMyPlayer() && getOut)
...
{		
	...
	this.server_DetachAll(); // drop blobs
	return damage;
}														
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
	...
	if ((friend is null || blob.getTeamNum() != friend.getTeamNum()) && not(blob.getName() == "necromancer" && blob.getTeamNum() == this.getTeamNum()) && blob.getName() != this.getName() && blob.hasTag("flesh") && !blob.hasTag("dead") && blob.getName() != "Wraith")
	{
