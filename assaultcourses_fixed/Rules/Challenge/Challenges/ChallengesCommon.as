...
{
	this.set_string(stats_tag, this.get_string(stats_tag) + text + "\n\n");
	this.Sync(stats_tag, true);
}
...
{
	if (!stats.exists(playerName))//if you have never completed the course
		stats.add_u32(playerName, currentTime);//then set your first record
	else//if you have completed the course before
	{
		u32 your_fastest_time = stats.read_u32(playerName);
		if(your_fastest_time > currentTime)//if you beat your personal record
		{
			stats.add_u32(playerName, currentTime);//then change your old record
		}
	}
}
...
class Stats_Player
{
	string name;
	s32 score;
	Stats_Player(string name, u32 score) 
	{
		this.name = name;
		this.score = score;
	}
...
string Stats_Output_TeamTimeMeasures(ConfigFile@ stats, bool showIndividualNames = true)
{
	...
	if (true)
	{
		for (uint i = 0; i < getPlayerCount(); i++)
		{
			...
			if (player !is null) 
			{
				...
				const s32 fastestPlayerTime = stats.exists(name) ? stats.read_u32(name) : -1;
				Stats_Player p(name, fastestPlayerTime);

				bool inserted = false;
				for(u32 j = 0; j < players.length; j++)
				{
					if(players[j].score > fastestPlayerTime)
					{
						players.insert(j, p);
						inserted = true;
						break;
	...
	for(uint i = 0; i < players.length; i++)
	{
		...
		if(p.score != -1)
			stat += "   " + p.name + ":     $RED$" + formatFloat((float(p.score) / 30.0f) / 60.0f, "", 4, 2) + "mins$RED$\n";
	}
