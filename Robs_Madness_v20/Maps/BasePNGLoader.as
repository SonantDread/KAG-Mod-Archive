...
class PNGLoader
{
	...
	void HandleCustomTile(CMap@ map, int offset, SColor pixel)
	{
		...
		if(color_blueprincess == pixel){
			server_CreateBlob("princess", 0, getMap().getTileWorldPosition(offset));
			autotile(offset);
		}
		const SColor color_redprincess(0xffff80C8);
		if(color_redprincess == pixel){
			server_CreateBlob("princess", 1, getMap().getTileWorldPosition(offset));
			autotile(offset);
		}
		const SColor color_hall(255, 211, 249, 193);
		if(color_hall == pixel){
			server_CreateBlob("hall", -1, getMap().getTileWorldPosition(offset)+Vec2f(8,0));
			autotile(offset);
		}
	}
