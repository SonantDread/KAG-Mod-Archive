...
CSpriteLayer@ LoadHead(CSprite@ this, int headIndex)
{
	...
	string filename = dlc.filename;
	
	CBlob@ blob = this.getBlob();
	
	headIndex = headIndex % 256; // DLC heads
	
	if(blob.getName() == "necro"){
		filename = "NecroHead.png";
		headIndex = 0;
	}
	
	if(blob.getName() == "ninja"){
		filename = "NinjaHead.png";
		headIndex = 0;
	}
	
	s32 headFrame = getHeadFrame(blob, headIndex);
	
	CSpriteLayer@ head = this.addSpriteLayer("head", filename, 16, 16,
