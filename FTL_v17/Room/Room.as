...
void onTick(CBlob@ this)
{
	...
	if(checkForLeaks(this)){
		if(!this.hasTag("leaking"))Sound::Play("leak.ogg");
		this.Tag("leaking");
	} else {
		this.Untag("leaking");
	}
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	if(this.getTeamNum() != 0 && this.getName() != "reactorroom"){
		CBlob @scrap = MakeMat(this, worldPoint, "mat_scrap", 3 * damage);
		if(scrap !is null)scrap.setVelocity(Vec2f(-5,XORRandom(4)-2));
	}
	return damage;
}