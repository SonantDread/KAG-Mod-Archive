...
const SColor color_ship(0xffc81414);

const SColor color_sky(0xffa5bdc8);

void wipeWorld(CMap@ map){
	...
	{
		CBlob@ blob = blobs[k];
		if(blob.getPosition().x > getMap().tilemapwidth/2*8){
			blob.Tag("no loot");
			blob.server_Die();
		}
	}
	...
	for(int j = 0;j < getMap().tilemapheight;j++){
...
void loadShip(CMap@ map, int team, Vec2f position)
{

	string filename = "ship"+XORRandom(4)+".png";
...
void handlePixel(SColor pixel, Vec2f offset, int team)
{
	...
	{
		spawnBlob(map, "mat_scrap", offset+Vec2f(4,4), -1);
	}
	else if (pixel == color_ship)
	{
		loadShip(map, team, offset);
	}
}
