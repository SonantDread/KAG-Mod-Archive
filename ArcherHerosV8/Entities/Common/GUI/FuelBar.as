...
void onRender(CSprite@ this)
{
	...
	if (blob.isKeyPressed(key_action2))
	{
		...
		const f32 initialHealth = 90.0f;
		if (initialHealth > 0)
		{
			const f32 perc = blob.get_f32("fuel") / initialHealth;
			if (perc >= 0)
			{
				GUI::DrawRectangle(Vec2f(pos2d.x - dim.x - 2, pos2d.y + y - 2), Vec2f(pos2d.x + dim.x + 2, pos2d.y + y + dim.y +2));
				GUI::DrawRectangle(Vec2f(pos2d.x - dim.x + 2, pos2d.y + y +2), Vec2f(pos2d.x - dim.x + perc * 2.0f * dim.x - 2, pos2d.y + y + dim.y - 2), SColor(0xff000012));
