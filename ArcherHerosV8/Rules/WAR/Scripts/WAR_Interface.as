...
CRules@ lol = getRules();
	...
	if (stream.getBitsUsed() > 0 && stream.saferead_u16(checkbits) && checkbits == 0x54f3)
	{
		...
		GUI::DrawPane(upperleft, upperleft + size);
            s32 score = lol.get_s32("best rank");
            string username = lol.get_string("best name");
            Vec2f bop(10, 84);
            Vec2f bip(256 + hud.halls.length * 32 , 84 + 24);
    
    GUI::DrawPane(bop, bip);
    GUI::DrawText("Best Player: " +username + " Rank: " + score ,
					              Vec2f (14, 89),
