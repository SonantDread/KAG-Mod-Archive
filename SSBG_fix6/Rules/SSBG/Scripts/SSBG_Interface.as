...
void onInit( CRules@ this )
{
    ...
    this.set_CBitStream("SSBG_serialised_team_hud", stream);
}
...
void onRender( CRules@ this )
{
    ...
    this.get_CBitStream("SSBG_serialised_team_hud", serialised_team_hud);
    ...
    if (serialised_team_hud.getBytesUsed() > 10)
    {
        ...
        if (serialised_team_hud.saferead_u16(check) && check == 0x5afe)
        {
            ...
            {
                SSBG_HUD hud(serialised_team_hud);
                ...
                if (hud.deaths_limit > 0)
                {
					s16 livesCount = hud.deaths_limit - hud.deaths;
                    lives = "LIVES: "+livesCount;
                }

                GUI::DrawText(lives, topLeft+Vec2f(64,42), SColor(255,255,255,255));
            }
        }
    ...
    string propname = "SSBG spawn time "+p.getUsername();	
