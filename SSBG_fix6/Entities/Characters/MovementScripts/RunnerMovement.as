		...
		!blob.isOnLadder() && (up || left || right) ) //key pressed
    {
			...
			!onground )											
		{
			...
			if(moveVars.jumpCount > 5) //wait some time to be properly in the air
			{
						...
						pos.y + 1.0f > moveVars.wallrun_start - map.tilesize * moveVars.wallrun_length))
					{
						...
						if(set_contact || getGameTime() % 5 == 0)
						{
							...
							if(vel.y > -wallrun_speed || set_contact)
							{
								if (surface_left)
								{
									vel.Set(0, -wallrun_speed); //This way you can push off the wall instead of going straight up
								}
								if (surface_right)
								{
							...
							if(!set_contact)
							{
								blob.getSprite().PlaySound("/StoneJump");
							}
						}
					}
					else
				...
				{
					f32 walljumpforce = 5.0f;
					vel.Set( surface_right ? -walljumpforce : walljumpforce,
							 blob.isKeyPressed(key_action2) ? -2.0f : -4.0f );
					blob.setVelocity(vel);
