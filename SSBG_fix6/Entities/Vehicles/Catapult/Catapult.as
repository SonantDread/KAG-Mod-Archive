...
{		
	...
	this.addCommandID("pack into crate");
...
{	
	...
	else
	{
		if (time % 30 == 0)
		{
			...
			u32 packTime = v.pack_time;
			if (packTime != 0 && getGameTime() >= packTime)
			{
				// put out stone
				for (int i = 0; i < this.getInventory().getItemsCount(); i++)
				{
					CBlob@ item = this.getInventory().getItem(i);
					if (item.getName() == "mat_stone")
					{								 
						if (item.getQuantity() > startStone)
							item.server_SetQuantity( item.getQuantity()-startStone );
						else
							item.server_Die();
					}
				}
				PackIntoCrate( this, FactoryFrame::catapult );
			}
		}
	}
}
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	...
	if (caller.getTeamNum() == this.getTeamNum() && !this.hasAttached() ) {
		caller.CreateGenericButton( 24, Vec2f(0.0f, 30.0f), this, this.getCommandID("pack into crate"), "Pack into crate");
	}
}
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	else if (!this.hasAttached() && cmd == this.getCommandID("pack into crate"))
	{
		VehicleInfo@ v;
		if (!this.get( "VehicleInfo", @v )) {
			return;
		}
		v.pack_time = getGameTime() + v.pack_secs * getTicksASecond();
	}
}
...
void onRender( CSprite@ this )
{
	...
	if (unpackTime > gameTime)
	{
		...
		f32 progress = 1.0f - (float(secs) / float(v.pack_secs));
