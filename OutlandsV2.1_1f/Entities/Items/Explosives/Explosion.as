					...
					for (int i = 0; i < 4; i++)
					{
						...
						if (dist < map_damage_radius)
						{
							...
							if (canHit)
							{
								...
								bool goldenblocks = (tile > 255 && tile < 261) || (tile > 266 && tile < 272);
								bool mixedgold = (tile > 271 && tile < 277);
								bool magic = (tile > 287 && tile < 292);
								if (canExplosionDamage(map, tpos, tile))
								{
									if (!map.isTileBedrock(tile) && !goldenblocks && !mixedgold && !magic)
									{
										if (dist >= rad_thresh ||
									...
									{
										map.server_DestroyTile(tpos, 0.2f, this);
									}
									else if (mixedgold || magic)
									{
										if (getNet().isClient())
										{
											Sound::Play("metal_stone.ogg", tpos, 1.0f, 1.0f);
											sparks(tpos, 1, 1);
										}
									}
								}
	...
	for (int step = 0; step <= steps; ++step)
	{
		...
		for (int width_step = 0; width_step < width_steps + 2; width_step++)
		{
			...
			if (isserver)
			{
				...
				bool goldenblocks = (t > 255 && t < 261) || (t > 266 && t < 272);
				bool mixedgold = (t > 271 && t < 277);
				...
				/*if (mixedgold)
				{
					if (!justhurt && width_step == width_steps / 2 + 1) //central bedrock only
					{
						//steps = step;
						//damagedsteps = max_depth; blocked!
						map.server_DestroyTile(tpos, 2.0f, this);
						break;
					}
				}*/
