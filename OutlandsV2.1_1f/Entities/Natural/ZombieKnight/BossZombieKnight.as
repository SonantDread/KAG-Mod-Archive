		...
		if (lastbite > bitefreq)
		{
			...
			if(this.get_u8(state_property) == MODE_TARGET )
			{
				...
				else vel = Vec2f(1,0);
				{
					...
					HitInfo@[] hitInfos;
					CMap @map = getMap();
					if (map.getHitInfosFromArc( this.getPosition()- Vec2f(2,0).RotateBy(-vel.Angle()), -vel.Angle(), 90, this.getRadius(), this, @hitInfos ))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							else
							{
								...
								if ( hi.tile != CMap::tile_empty ) {		
									bool goldenblocks = (hi.tile > 255 && hi.tile < 261) || (hi.tile > 266 && hi.tile < 272);
									bool mixedgold = (hi.tile > 271 && hi.tile < 277);
									if (!this.getMap().isTileGroundStuff( hi.tile ) && !goldenblocks && !mixedgold)
									{
										//if (XORRandom(2) == 0) 
										{
									...
									else if (!goldenblocks && !mixedgold)
									{
										//if (XORRandom(2) == 0) 
										{
									...
									else if (mixedgold)
									{
										if (getNet().isClient())
										{
											Sound::Play("metal_stone.ogg", hi.hitpos, 1.0f, 1.0f);
...
void dropHead(CBlob@ this)
{
	if (!this.hasTag("dropped head")) //double check
	{
		this.Tag("dropped head");
		MakeMat(this, this.getPosition(), "mat_rflesh", XORRandom(35)+250);
		CBlob@ head = server_CreateBlob("bosshead", -1, this.getPosition());

			if (head !is null)
			{
				Vec2f vel(0.0f, -10.0f);
