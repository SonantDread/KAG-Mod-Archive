						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (other !is null)
							{
								...
								else if (!hit_block)
								{
									...
									if (other.getTeamNum() != this.getTeamNum() && (other.getName() != "goldenblock" || other.getName() != "cgdoor"))
									{
										this.server_Hit(other,other.getPosition(),vel,0.2,Hitters::saw, false);
										this.set_u16("lastbite",0);
										hit_block=true;
...
//			this.getSprite().PlaySound("/BisonBoo");
	...
	if (this.isOnGround() && (this.isKeyPressed(key_left) || this.isKeyPressed(key_right)) )
	{
		...
		if (XORRandom(break_chance)==0)
		{
			...
			bool goldenblocks = (tile > 255 && tile < 261) || (tile > 266 && tile < 277);
			if ( !this.getMap().isTileGroundStuff( tile ) && !goldenblocks ) {		
			this.getMap().server_DestroyTile(tp, 0.1);
			}
			else if (goldenblocks) {		
			if (getNet().isClient())
						{
							Sound::Play("metal_stone.ogg", tp, 1.0f, 1.0f);
							sparks(tp, 1, 1);
						}
			}
		}
...
{		
    ...
    if (this.getHealth() <= gibHealth)
    {
		...
		if (XORRandom(2)+1 == 2) MakeMat(this, worldPoint, "mat_wood", XORRandom(10)+5);
		else if (XORRandom(2)+1 == 3) MakeMat(this, worldPoint, "mat_stone", XORRandom(10)+5);
		if (XORRandom(5)+1 == 6) MakeMat(this, worldPoint, "mat_gold", XORRandom(10)+5);
		MakeMat(this, worldPoint, "mat_rflesh", XORRandom(10)+5);
