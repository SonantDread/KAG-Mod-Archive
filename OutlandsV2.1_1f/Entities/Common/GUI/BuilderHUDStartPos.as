...
void DrawInventoryOnHUD(CBlob@ this, Vec2f tl)
{
	...
	for (int i = 0; i < inv.getItemsCount(); i++)
	{
		...
		if (drawn.find(name) == -1)
		{
			...
			GUI::DrawIcon(item.inventoryIconName, item.inventoryIconFrame, item.inventoryFrameDimension, tl + Vec2f(0 + (drawn.length - 1) * 20, -6), 1.0f);
			...
			GUI::DrawText(disp, tl + Vec2f(14 + (drawn.length - 1) * 20 - (dimensions.x-10) , 24), col);
		}
	}
}
...
void DrawCoinsOnHUD(CBlob@ this, const int coins, Vec2f tl, const int slot)
{
	if (coins > 0)
	{
		...
		GUI::SetFont("menu");
		GUI::DrawText("" + coins, tl + Vec2f(4 + slot * 40 -2, 24), color_white);
	}
}
