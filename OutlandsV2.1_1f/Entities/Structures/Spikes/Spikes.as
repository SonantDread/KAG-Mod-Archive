...
void onTick(CBlob@ this)
{
	...
	if (getNet().isClient() && !this.hasTag("_frontlayer"))
	{
		...
		if (sprite !is null)
		{
			CSpriteLayer@ panel = sprite.addSpriteLayer("panel", sprite.getFilename() , 8, 16, this.getTeamNum() - int(this.getTeamNum() / 8 )*8, this.getSkinNum());
		...
		if (state == hidden)
		{
			...
			if (map.getBlobsInRadius(pos, this.getRadius() * 1.0f, @blobsInRadius))
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					...
					if (team != b.getTeamNum() && !isFriendly(b.getTeamNum(), this.getTeamNum()) && canStab(b))
					{
						state = stabbing;
						timer = delay_stab;
...
{
	return (!b.hasTag("dead") && b.hasTag("flesh")) || b.hasTag("zombie");
}
