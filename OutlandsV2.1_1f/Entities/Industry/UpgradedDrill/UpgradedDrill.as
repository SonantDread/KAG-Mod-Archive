...
const f32 speed_thresh = 5.4f;
const f32 speed_hard_thresh = 5.6f;
...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (int(heat) >= heat_max - (heat_add * 1.5))
		{
			...
			this.server_Hit(holder, holder.getPosition(), Vec2f(), 0.5f, Hitters::burn, true);
		...
		if (holder.getName() == required_class || sv_gamemode == "TDM")
		{
			...
			// delay drill
			{
				...
				const f32 attack_distance = 8.0f;
				Vec2f attackVel = direction * attack_distance;

				const f32 distance = 24.0f;
				const f32 attack_dam = 8.0f;
				...
				{
					HitInfo@[] hitInfos;
					if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 50, distance, this, false, @hitInfos))
					{
