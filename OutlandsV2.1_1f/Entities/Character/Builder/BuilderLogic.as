...
bool RecdHitCommand(CBlob@ this, CBitStream@ params)
{
	...
	if(blobID == 0)
	{
		...
		f32 t = tile.type;
		bool magic = (t > 287 && t < 292);
		if(map !is null)
		{
			if(map.getSectorAtPosition(tilepos, "no build") is null && !magic)
			{
...
void Pickaxe(CBlob@ this)
{
	...
	if (map.isTileSolid(map.getTile(aimPos).type))
	{
		print("tile is solid: "+map.getTile(aimPos).type);
	}
	...
	if ((tilepos - aimPos).Length() < bestDistance - 4.0f && map.getBlobAtPosition(tilepos) is null)
	{
		...
		if (!noBuildZone && !map.isTileGroundBack(tile.type))
		{
			...
			if ((map.isTileBackgroundNonEmpty(tile) || map.isTileSolid(tile)) && !magic)
			{
				hasHit = true;
