		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					bool newores = (hi.tile > 260 && hi.tile < 267) || (hi.tile > 276 && hi.tile < 288);
					bool wood = map.isTileWood(hi.tile);
					bool goldenblocks = (hi.tile > 255 && hi.tile < 261) || (hi.tile > 266 && hi.tile < 277);
					bool magic = (hi.tile > 287 && hi.tile < 292);
					if ((ground || wood || dirt_stone || gold || newores) && !goldenblocks && !magic)
					{
					...
					else if (goldenblocks || magic)
					{
						dontHitMoreMap = true;
						if (getNet().isClient())
						{
							Sound::Play("metal_stone.ogg", pos, 1.0f, 1.0f);
							sparks(pos, 1, 1);
						}
					}
				}
