	...
	if (serialised_team_hud.getBytesUsed() > 10)
	{
		...
		if (serialised_team_hud.saferead_u16(check) && check == 0x5afe)
		{
			...
			while (!serialised_team_hud.isBufferEnd())
			{
				...
				if (this.get_bool("winCounterEnabled"))
				{
				    CTeam@ team = getRules().getTeam(hud.team_num);
				    GUI::DrawText(this.get_u32("team_"+hud.team_num+"_win_score")+'', topLeft + (Vec2f(190, 42)), SColor(team.color));
				}
			}
		}
