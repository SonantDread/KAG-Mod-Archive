	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			if (playerBlob !is null)
			{
				...
				u8 item = getRules().get_u8("Item_CurrentNum");
				if (item > 4)
					return;

				CBlob@ itemBlob = server_CreateBlob(ItemList[item], player.getTeamNum(), playerBlob.getPosition());
				switch(item)
				{
					case 0:
					case 1:
					case 2:
						playerBlob.server_PutInInventory(itemBlob);
						break;

					case 3:
					case 4:
						playerBlob.server_Pickup(itemBlob);
						break;
				}
			}
		}
...
shared class TDMCore : RulesCore
{
	...
	void CheckTeamWon()
	{
		...
		if (winteamIndex >= 0)
		{
			...
			if (rules.get_bool("winCounterEnabled"))
			{
				rules.add_u32("team_"+winteamIndex+"_win_score",1);
				rules.Sync("team_"+winteamIndex+"_win_score", true);
			}
