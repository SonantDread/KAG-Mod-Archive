...
const f32 bulletMediumSpeed = 27.0f;
const f32 bulletFastSpeed = 29.0f;
//maximum is 15 as of 22/11/12 (see ArcherCommon.as)

const f32 BULLET_PUSH_FORCE = 8.0f;
...
void onInit(CBlob@ this)
{
	...
	if (!this.exists("bullet type"))
	{
		this.set_u8("bullet type", BulletType::normal);
	}
	...
	const u8 bulletType = this.get_u8("bullet type");
	...
	//set a random frame
	{
		Animation@ anim = sprite.addAnimation("bullet", 0, false);
...
void turnOffFire(CBlob@ this)
{
	...
	this.getSprite().SetAnimation("bullet");
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		else*/
		{
			...
			dmg = BulletHitBlob(this, point1, initVelocity, dmg, blob, Hitters::arrow, bulletType);
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		BulletHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::arrow);
	}
}

void AddBulletLayer(CBlob@ this, CBlob@ hitBlob, CSprite@ sprite, Vec2f worldPoint, Vec2f velocity)
{
	CSpriteLayer@ bullet = sprite.addSpriteLayer("bullet", "Entities/Items/Projectiles/Bullet.png", 16, 8, this.getTeamNum(), this.getSkinNum());

	if (bullet !is null)
	{
		Animation@ anim = bullet.addAnimation("default", 13, true);
		...
		{
			warn("exception: bullet has no anim");
			anim.AddFrame(0);
		}

		bullet.SetAnimation(anim);
		...
		{
			// soffset.x *= -1;
			bullet.RotateBy(180.0f, Vec2f(0, 0));
			bullet.SetFacingLeft(true);
		}
		else
		{
			soffset.x *= -1;
			bullet.SetFacingLeft(false);
		}

		bullet.SetIgnoreParentFacing(true); //dont flip when parent flips


		bullet.SetOffset(soffset);
		bullet.SetRelativeZ(-0.01f);

		f32 angle = velocity.Angle();
		bullet.RotateBy(-angle - hitBlob.getAngleDegrees(), Vec2f(0, 0));
	}
}


f32 BulletHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 bulletType)
{
	if (hitBlob !is null)
	{
		Pierce(this, hitBlob);
		...
		// check if shielded
		const bool hitShield = true;
		/*(hitBlob.hasTag("shielded") && blockAttack(hitBlob, velocity, 0.0f));*/
		...
		if (!hitShield)
		{
			...
			{
				{
					if (velocity.Length() > bulletFastSpeed)
					{
		...
		else*/
		{
			...
			{
				BulletHitMap(this, worldPoint, velocity, damage, Hitters::arrow);
			}
		}
	}
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		{
			f32 force = (BULLET_PUSH_FORCE * 0.125f) * Maths::Sqrt(hitBlob.getMass() + 1);

			if (this.hasTag("musket bullet"))
			{
				force *= 1.3f;
			}
			...
			if (this.getTickSinceCreated() <= 4 &&
			        speed > MusketmanParams::shoot_max_vel * 0.845f &&
...
f32 getBulletDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	if (vellen >= bulletFastSpeed)
	{
		return 2.0f;
	}
	else if (vellen >= bulletMediumSpeed)
	{
		return 1.5f;
	}
