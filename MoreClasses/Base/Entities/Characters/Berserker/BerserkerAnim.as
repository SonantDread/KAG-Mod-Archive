...
void onInit(CSprite@ this)
{
	string texname = this.getBlob().getSexNum() == 0 ?
	                 "Entities/Characters/Berserker/BerserkerMale.png" :
	                 "Entities/Characters/Berserker/BerserkerFemale.png";
...
void onTick(CSprite@ this)
{
	...
	if (!blob.get("berserkerInfo", @berserker))
	{
		return;
	}
	...
	else if (berserker.state == BerserkerStates::sword_drawn)
	{
		if (berserker.swordTimer < BerserkerVars::slash_charge)
		{
			this.SetAnimation("draw_sword");
		}
		else if (berserker.swordTimer < BerserkerVars::slash_charge_level2)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 0;
		}
		else if (berserker.swordTimer < BerserkerVars::slash_charge_limit)
		{
			this.SetAnimation("strike_power_ready");
	...
	else if (berserker.state == BerserkerStates::sword_cut_mid_down)
	{
		this.SetAnimation("strike_mid_down");
	}
	else if (berserker.state == BerserkerStates::sword_cut_up)
	{
		this.SetAnimation("strike_up");
	}
	else if (berserker.state == BerserkerStates::sword_cut_down)
	{
		this.SetAnimation("strike_down");
	}
	else if (berserker.state == BerserkerStates::sword_power || berserker.state == BerserkerStates::sword_power_super)
	{
		this.SetAnimation("strike_power");

		if (berserker.swordTimer <= 1)
			this.animation.SetFrameIndex(0);

		u8 mintime = 6;
		u8 maxtime = 8;
		if (berserker.swordTimer >= mintime && berserker.swordTimer <= maxtime)
		{
			wantsChopLayer = true;
			chopframe = berserker.swordTimer - mintime;
			chopAngle = -vec.Angle();
		}
	}
	else if (inair)
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (BerserkerVars::slash_charge_limit - BerserkerVars::slash_charge_level2);
			f32 count = (berserker.swordTimer - BerserkerVars::slash_charge_level2);
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/Berserker/BerserkerGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/Berserker/BerserkerGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/Berserker/BerserkerGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/Berserker/BerserkerGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
