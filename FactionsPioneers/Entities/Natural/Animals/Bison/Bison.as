...
{
	this.ReloadSprites(this.getBlob().getTeamNum(), 0); //always blue
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	else if (blob.getName() != this.getName() && blob.hasTag("flesh"))
	{
	...
	// eat cake	and make friends
	{
		if (blob.getName() == "food")
		{
			...
			if (owner !is null && (this.getTeamNum() == 255 || owner.getBlob().getTeamNum() == this.getTeamNum()))
			{
				CBlob@ ownerblob = owner.getBlob();
				if (ownerblob !is null)
				{
					this.server_setTeamNum(owner.getBlob().getTeamNum());
