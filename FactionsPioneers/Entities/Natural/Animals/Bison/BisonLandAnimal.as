...
void onTick(CMovement@ this)
{
	...
	f32 waterFac = blob.isInWater() ? 0.2f : 1.0f;
	if (left)
	{
		blob.AddForce(Vec2f(-1.0f * vars.walkForce.x * waterFac, vars.walkForce.y * waterFac));
	}
	if (right)
	{
		blob.AddForce(Vec2f(1.0f * vars.walkForce.x * waterFac, vars.walkForce.y * waterFac));
	...
	if (left || right || up)
	{
		...
		if ((blob.isOnGround()) && (up || (right && map.isTileSolid(Vec2f(pos.x + radius, pos.y + 0.45f * radius))) || (left && map.isTileSolid(Vec2f(pos.x - radius, pos.y + 0.45f * radius)))
		                                               )
		   )
		{
			f32 mod = blob.isInWater() ? 0.5f : 1.0f;
