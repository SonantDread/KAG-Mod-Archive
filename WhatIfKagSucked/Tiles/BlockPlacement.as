...
void PlaceBlock(CBlob@ this, u8 index, Vec2f cursorPos)
{
	...
	if (bc.tile > 0 && hasRequirements(inv, bc.reqs, missing))
	{
		...
		int currentTile = getMap().getTile(cursorPos).type;
		
		if(bc.tile == 48){
			if(currentTile < 64 && currentTile > 58){
				getMap().server_SetTile(cursorPos, currentTile-1);
			}
			else if(currentTile == 58)getMap().server_SetTile(cursorPos, 48);
			else getMap().server_SetTile(cursorPos, 63);
			
		} else
		if(bc.tile == 196){
			if(currentTile == 205)getMap().server_SetTile(cursorPos, 196);
			else getMap().server_SetTile(cursorPos, 205);
