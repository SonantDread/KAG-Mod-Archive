...
bool loadMap(CMap@ _map, const string& in filename)
{
	...
	for (int x = 0; x < width; ++x)
	{
		...
		bool spawn_base = (x == 4 || x == width - 4);

		if(spawn_base)
		{
			server_CreateBlob("ruins", -1, Vec2f(x*8, (60*8) - 24));
		}
		...
		if (force_tree || grass_frac > 0.5f)
		{
			        ...
			        (x > map_margin && width - x > map_margin) && (x % 7 == 0 || x % 23 == 3))
			{
				...
				if (tree_skip < tree_limit || (_g > 0.5f) || (bush_skip > bush_limit))  //bush
				{
					bush_skip = 0;
					server_CreateBlob("bush", -1, pos);
					tree_skip++;
				}
				if (tree_skip >= tree_limit || force_tree || _g > 0.1f)  //tree
				{
					tree_skip = 0;
