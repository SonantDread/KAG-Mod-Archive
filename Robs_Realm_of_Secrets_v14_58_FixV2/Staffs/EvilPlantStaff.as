	...
	if (this.isAttached())
	{
		...
		if (holder.get_u8("knocked") <= 0)
		{
			...
			if(point.isKeyPressed(key_action2)){
				ControlElements(this.get_f32("power"),holder.getAimPos(),false,false,false,true,false,true,false,false,false,false,false);
			}
			...
			if(point.isKeyPressed(key_action3)){
				...
				if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
				{
					...
					{
						CBlob@ b = blobsInRadius[i];
						if((b.hasTag("flesh") || b.hasTag("plant")) && b.getTeamNum() != holder.getTeamNum() && b.get_s16("poison") <= 0)
						{
							b.set_s16("poison",300);
							break;
						}
					}
				}
