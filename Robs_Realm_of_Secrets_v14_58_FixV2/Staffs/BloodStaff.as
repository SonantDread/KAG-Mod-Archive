	...
	if (this.isAttached())
	{
		...
		if (holder.get_u8("knocked") <= 0)
		{
			...
			if(point.isKeyPressed(key_action1))if(getNet().isServer()){
				CBlob @blob = server_CreateBlob("bloodbolt", holder.getTeamNum(), this.getPosition());
				if (blob !is null)
				{
			...
			if(point.isKeyPressed(key_action2)){
				ControlElements(this.get_f32("power"),holder.getAimPos(),false,false,false,false,true,false,false,false,false,false,false);
			}
