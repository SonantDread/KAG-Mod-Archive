...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	else if (text_in == "!tickstart" && player.isMod()) // TODO: whoaaa check seclevs
	{
		reload(this);
		this.set_bool("TicketsOn?",true);
		this.set_u8("TicketsJustOn", 1);
		this.Sync("TicketsOn?",true);
		this.Sync("TicketsJustOn",true);
		return true;
	}
	else if (text_in == "!tickstop" && player.isMod()) // TODO: whoaaa check seclevs
	{
		this.set_bool("TicketsOn?",false);
		this.set_u8("TicketsJustOn", 2);
		this.Sync("TicketsOn?",true);
		this.Sync("TicketsJustOn",true);
		return true;
	}
	...
	else if (text_in.substr(0, 1) == "!")
	{
		...
		if (tokens.length > 1)
		{
			if (tokens[0] == "!bluetick")
			{
				this.add_s16("blueTickets", parseInt(tokens[1]));
				this.Sync("blueTickets", true);
			}
			else if (tokens[0] == "!redtick")
			{
				this.add_s16("redTickets", parseInt(tokens[1]));
				this.Sync("redTickets", true);
			}
			else if(tokens[0] == "!teamtick")
			{
				this.add_s16("redTickets", parseInt(tokens[1]));
				this.add_s16("blueTickets", parseInt(tokens[1]));
				this.Sync("redTickets", true);
