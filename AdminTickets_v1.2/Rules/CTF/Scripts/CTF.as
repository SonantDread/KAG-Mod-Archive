...
shared class CTFSpawns : RespawnSystem
{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (canSpawnPlayer(p_info))
		{
			...
			//ticket stuff
			if(getRules().get_bool("TicketsOn?"))
			{
				print("yes this is on");
				if(player.hasTag("Suicide"))
				{
					player.Untag("Suicide");
				...
				else if(getRules().isMatchRunning() && decrementTickets(getRules(), p_info.team)==1)//if match not running, spawn and dont take ticket
				{             
					p_info.spawnsCount++;
...
shared class CTFCore : RulesCore
{
	...
	void Setup(CRules@ _rules = null, RespawnSystem@ _respawns = null)
	{
		...
		_rules.set_bool("TicketsOn?", false);
		_rules.Sync("TicketsOn?",true);
	}
