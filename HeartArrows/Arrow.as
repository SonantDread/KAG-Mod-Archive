...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		if (!solid && !blob.hasTag("flesh") && !specialArrowHit(blob) &&
		        (blob.getName() != "mounted_bow" || (this.getTeamNum() != blob.getTeamNum() && !this.hasTag("hit team")) || (this.getTeamNum() == blob.getTeamNum() && this.hasTag("hit team")) ))
		{
			return;
		}
		...
		else
		{
				...
				else if (arrowType == ArrowType::heart)
			{
					if (blob.getHealth() < blob.getInitialHealth() - 0.5f)
						{ this.server_Hit(blob, point1, initVelocity, dmg, Hitters::arrow, true); }
					else							blob.server_SetHealth(blob.getInitialHealth());
			}
				else
					this.server_Hit(blob, point1, initVelocity, dmg, Hitters::arrow, true);
			}
...
bool doesCollideWithBlob(CBlob@ this, CBlob@ blob)
{
	...
	bool check = this.getTeamNum() != blob.getTeamNum() && !this.hasTag("hit team");
	
	if (!check)
	{
		check = this.hasTag("hit team") && this.getTeamNum() == blob.getTeamNum();
	}
	...
	if (this.hasTag("hit team") && (!blob.hasTag("flesh") || blob.getInitialHealth() == blob.getHealth()   )) //heart arrow checks
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		if (speed > ArcherParams::shoot_max_vel * 0.5f && !this.hasTag("hit team"))
		{
			f32 force = (ARROW_PUSH_FORCE * 0.125f) * Maths::Sqrt(hitBlob.getMass() + 1) * scale;
