...
void LoadSprites(CSprite@ this)
{
	...
	string tex = this.getBlob().getSexNum() == 0 ?
					"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherMale.png" :
					"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherFemale.png";
	if (team >= 8 && team <= 15)
		tex = this.getBlob().getSexNum() == 0 ?
				"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherMaleB.png" :
				"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherFemaleB.png";
	else if (team >= 16 && team <= 23)
		tex = this.getBlob().getSexNum() == 0 ?
				"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherMaleS.png" :
				"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherFemaleS.png";
	else if (team >= 24 && team <= 31)
		tex = this.getBlob().getSexNum() == 0 ?
				"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherMaleW.png" :
				"../Mods/"+mod_name+"/Entities/Character/Archer/ArcherFemaleW.png";
	const string texname = tex;
	this.ReloadSprite(texname, this.getConsts().frameWidth, this.getConsts().frameHeight,
	                  team-int(team/8)*8, this.getBlob().getSkinNum());
	this.RemoveSpriteLayer("frontarm");
	CSpriteLayer@ frontarm = this.addSpriteLayer("frontarm", texname , 32, 16, team-int(team/8)*8, this.getBlob().getSkinNum());
	...
	CSpriteLayer@ backarm = this.addSpriteLayer("backarm", texname , 32, 16, team-int(team/8)*8, this.getBlob().getSkinNum());
	...
	CSpriteLayer@ arrow = this.addSpriteLayer("held arrow", "Arrow.png" , 16, 8, team-int(team/8)*8, this.getBlob().getSkinNum());
	...
	CSpriteLayer@ quiver = this.addSpriteLayer("quiver", texname , 16, 16, team-int(team/8)*8, this.getBlob().getSkinNum());
	...
	CSpriteLayer@ hook = this.addSpriteLayer("hook", texname , 16, 8, team-int(team/8)*8, this.getBlob().getSkinNum());
	...
	CSpriteLayer@ rope = this.addSpriteLayer("rope", texname , 32, 8, team-int(team/8)*8, this.getBlob().getSkinNum());
...
void onTick(CSprite@ this)
{
	...
	if (blob.getSexNum() == 1 && sex == 0)
	{
		LoadSprites(this);
		sex = 1;
		//ReloadFriends(this.getBlob());
		//print("loop? (body): "+sex);
	}
	...
	if (blob.hasTag("dead"))
	{
		...
		ResetFriends( rules, this.getBlob().getTeamNum());
		return;
	}
