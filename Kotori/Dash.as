...
const u16 DASH_COOLDOWN = 25;//seconds * 30
...
void onTick( CBlob@ this )
{
	...
	if ( !dashing )
	{
		if ( /*onground &&*/ down && ( left || right ) && vel.Length() < 3.0f )// we want dashing to be performed while on air too
		{
			this.set_bool( "dashing", true );
			this.set_u8( "dashCoolDown", 0 );
			MakeDustParticle( this.getPosition() + Vec2f( 0.0f, 9.0f ), "/DustSmall.png");
			this.getSprite().PlaySound("/Ha_A");//custom leap sound
			f32 xCompensate;
			if ( left )
			{
	...
	else
	{
		...
		if ( ( /*onground &&*/ ( !down || ( !left && !right ) ) && dashCoolDown > DASH_COOLDOWN ) || dashCoolDown > DASH_COOLDOWN * 3 )
			this.set_bool( "dashing", false ); //for consistency
	}
}