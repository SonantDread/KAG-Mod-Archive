...
shared class CTFSpawns : RespawnSystem
{
	...
	void AddPlayerToSpawn(CPlayer@ player)
	{
		...
		if (oldSpawn.exists("additional respawn time"))
		{
			tickspawndelay += oldSpawn.get_u8("additional respawn time");
		}
		...
		if (info.team < CTF_core.teams.length)
		{
			...
			CBlob@ newSpawn = getSpawnBlob(info);
			...
			if (newSpawn.exists("additional respawn time") && oldSpawn.getName() != newSpawn.getName())
			{
				old_spawn_time += newSpawn.get_u8("additional respawn time");
			}
			else if(!newSpawn.exists("additional respawn time") && oldSpawn.exists("additional respawn time"))
			{
