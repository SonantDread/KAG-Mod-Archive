...
{
	if (!this.hasTag("dead") && caller.getTeamNum() == this.getTeamNum())
	{
		CBitStream params;
	...
	if (item.instantShipping)
	{
		...
		if (blob !is null)
		{
			if(blob.getName() == "crate")
			{
				...
				CBlob@ mat3 = server_CreateBlob("mat_gold");
				if (mat !is null)
				{
					mat.server_SetQuantity(50);						//num of wood
					blob.server_PutInInventory(mat);
				}
				if (mat2 !is null)
				{
					mat2.server_SetQuantity(50);					//num of stone
					blob.server_PutInInventory(mat2);
				}
				if (mat3 !is null)
				{
					mat3.server_SetQuantity(50);					//num of gold
					blob.server_PutInInventory(mat3);
				}
			}
