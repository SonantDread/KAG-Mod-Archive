...
void onTick(CBlob@ this)
{
	...
	CBlob@ carrieddrill = this.getCarriedBlob();
	bool pressed_a1 = this.isKeyPressed(key_action1) && !(carrieddrill !is null && carrieddrill.getName() == "drill");
	//CBlob@ carrieddrill = this.getCarriedBlob();
	//if(carrieddrill !is null && carrieddrill.getName() != "drill")
	//print(""+);
	//	pressed_a1 = false;
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					bool goldBlock = hi.tile >= 272 && hi.tile <= 276;
					if ((ground || wood || dirt_stone || gold || goldBlock) && !castle)
					{
