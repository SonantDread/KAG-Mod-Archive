...
void PlaceBlock(CBlob@ this, u8 index, Vec2f cursorPos)
{
	...
	if (bc.tile > 0 && hasRequirements(inv, bc.reqs, missing))
	{
		...
		{
			if(index == 2)
				p.server_setCoins(p.getCoins()+2);
			else if(index == 0)
				p.server_setCoins(p.getCoins()+4);
			else if(index == 4)
				p.server_setCoins(p.getCoins()+3);
			else if(index == 5)
				p.server_setCoins(p.getCoins()+5);
		}
	...
	if (buildtile > 0)
	{
		...
		if (!getHUD().hasButtons() && this.isKeyPressed(key_action1))
		{
			if (bc.cursorClose && bc.buildable && bc.supported)
			{
				...
				SetBuildDelay(this, this.get_u32("srune_timer") == 0 ? delay : delay / 2);
