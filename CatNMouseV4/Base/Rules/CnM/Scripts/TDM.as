...
const f32 spawnTimeCat = 15.0f; //in seconds
...
void Config(TDMCore@ this)
{
	...
	f32 gameDurationMinutes = cfg.read_f32("gameDurationMinutes", 4.0f);
...
shared class TDMSpawns : RespawnSystem
{
	...
	void Update()
	{
		for (uint team_num = 0; team_num < TDM_core.teams.length; ++team_num)
		{
			...
			int aliveCats = 0;
			for (int cats = 0; cats < getPlayersCount(); cats++)
			{
				CPlayer@ p = getPlayer(cats);
				CBlob@ b = p.getBlob();
			...
			for (uint i = 0; i < team.spawns.length; i++)
			{
				...
				UpdateSpawnTime(info, i+(aliveCats >= 7 ? 1 : 0));
				DoSpawnPlayer(info);
			}
		}
	}
	...
	void UpdateSpawnTime(TDMPlayerInfo@ info, int i)
	{
		...
		if (no_respawn && info.team != 0)
		{
			spawn_property = 253;
		}

		if (i == 0 && info !is null && info.can_spawn_time > 0 && (!no_respawn || info.team == 0))
		{
			if (huge_respawn)
			{
				info.can_spawn_time = (spawnTimeCat+1) * 30;
			}
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			if(p_info.team == 1) // mise
			{
				p_info.blob_name = "builder";
			}
			else if (p_info.team == 0) // cats
			{
				if(p_info.blob_name!="knight" && p_info.blob_name!="archer")
				{
					p_info.blob_name="knight";
				}
			}
			...
			if (playerBlob !is null)
			{
				...
				//GiveSpawnResources(playerBlob, player);
				if (playerBlob.getName() == "builder")
				{
					...
					bool foundw = false;
					bool foundg = false;
					if (map.getBlobsInRadius(playerBlob.getPosition(), 60.0f, @blobsInRadius))
					{
						for (uint i = 0; i < blobsInRadius.length; i++)
						{
							...
							if (b.getName() == "mat_stone")
							{
								...
								if (!founds)
								{
									playerBlob.server_PutInInventory(b);
								}
								else
								{
									b.server_Die();
				...
				else if (playerBlob.getName() == "knight")
				{
					if(player.getCoins() <= 50)
						player.server_setCoins(50);
					// first check if its in surroundings
...
shared class TDMCore : RulesCore
{
	...
	void onPlayerDie(CPlayer@ victim, CPlayer@ killer, u8 customData)
	{
		...
		if (victim !is null)
		{
			...
			if(victim.getTeamNum() == 1 && rules.isMatchRunning())
			{
				RulesCore@ core;
				CRules@ rules = getRules();
				rules.get("core", @core);
				core.ChangePlayerTeam(victim, 0);
				victim.server_setTeamNum(0);
			}
		...
		if (winteamIndex >= 0)
		{
			...
			if (rules.isMatchRunning())
			{
				...
				for (uint i = 0; i < players.length; i++)
				{
					...
					{
						player.server_setCoins(player.getCoins() + 40);
					}
				}
			}
