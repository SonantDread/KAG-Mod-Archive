...
const int MUSIC_INTERVAL = 60 * getTicksASecond(); // Music update interval
...
void Config(CTFCore@ this)
{
	...
	this.warmUpTime = 10 * getTicksASecond();
...
shared class CTFSpawns : RespawnSystem
{
    ...
    CBlob@[] skele;
		getBlobsByName("birdskeleton", @skele);
    
    for (uint i = 0; i < skele.length; i++)
		{
			if(skele[i].getTeamNum() == p_info.team)
      {
        return true;
      }
		}
...
shared class CTFCore : RulesCore
{
	...
	void AddPlayer(CPlayer@ player, u8 team = 0, string default_config = "")
	{
		...
		CTFPlayerInfo p(player.getUsername(), team, (XORRandom(512) >= 256 ? "link" : "ralph"));
		players.push_back(p);
    print("Player States created " + player.getUsername());
	...
	void CheckTeamWon()
	{
		...
		// get all the flags
		CBlob@[] flags;
		getBlobsByName("point", @flags);

		int winteamIndex = -1;
		CTFTeamInfo@ winteam = null;
		s8 team_wins_on_end = -1;
    
    if(flags[0].getTeamNum() == 0 && flags[0].get_u32("Win0") >= 180 && !flags[0].hasTag("under attack"))
    {
      CTFTeamInfo@ team = cast < CTFTeamInfo@ > (teams[0]);
      winteamIndex = 0;
			@winteam = team;
    }
    else if(flags[0].getTeamNum() == 1 && flags[0].get_u32("Win1") >= 180 && !flags[0].hasTag("under attack"))
    {
      CTFTeamInfo@ team = cast < CTFTeamInfo@ > (teams[1]);
      winteamIndex = 1;
			@winteam = team;
    }
