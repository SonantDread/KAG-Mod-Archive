...
{
addRunnerTextures(this, "tarin", "Tarin");
...
{
ensureCorrectRunnerTexture(this, "tarin", "Tarin");
}
...
void onTick(CSprite@ this)
{
	...
	if (!blob.hasTag(burning_tag)) //Are we burning? If so, lets not screw with the burning animations.
	{
		...
		if(action2 && !blob.get_bool("isCoon") )
		{
			this.SetAnimation("transform");
		}
    else if(action2 && blob.get_bool("isCoon") )
		{
			this.SetAnimation("coonshoot");
		}
    else if(action1 && blob.get_bool("isCoon") )
		{
			this.SetAnimation("coonshoot");
		}
    else if(blob.get_bool("isCoon") )
		{
			this.SetAnimation("coon");
		}
