...
{
addRunnerTextures(this, "ram", "Ram");
...
{
ensureCorrectRunnerTexture(this, "ram", "Ram");
}
...
void onTick(CSprite@ this)
{
	...
	if (!blob.hasTag(burning_tag)) //Are we burning? If so, lets not screw with the burning animations.
	{
		...
		if(blob.get_bool("IsOut"))
			prefix = "out";
    if(blob.get_u32("HitTime") > 0)
    {
        this.SetAnimation("charge");
    }
		else if (left || right || up || down)
		{
			this.SetAnimation(prefix +"run");
      
		}
		else
		{
			this.SetAnimation(prefix+ "default");
		}
	}
}
