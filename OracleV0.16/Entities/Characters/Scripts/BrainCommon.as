...
CBlob@ getNewTarget(CBrain@ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false)
{
  ...
  u32 indexs = 1000;
	...
	map.getBlobsInRadius(pos, 150.0f, @players);
	for (uint i = 0; i < players.length; i++)
	{
		...
		if (potential !is blob && blob.getTeamNum() != potential.getTeamNum()
		        && (pos2 - pos).getLength() < 150.0f
		        && (seeThroughWalls || isVisible(blob, potential))
		        && !potential.hasTag("dead") && !potential.hasTag("migrant")
				&&  (potential.hasTag("hitthis") || potential.hasTag("player")) 
		   )
		{
      if(indexs == 1000)
      {
        distance = (pos2 - pos).getLength();
        indexs = i;
      }
      else
      {
        if(distance > (pos2 - pos).getLength())
        {
          distance = (pos2 - pos).getLength();
          indexs = i;
        }
  ...
  if(indexs != 1000)
  {
    blob.set_Vec2f("last pathing pos", players[indexs].getPosition());
    return players[indexs];
  }
    
	return null;
}

Vec2f getCheckPos(Vec2f pos) {
  CBlob@[] traders;
  traders.clear();
  getBlobsByName("check", @traders);
  Vec2f thepos = Vec2f_zero;
  f32 bestdistance = 0.0f;
  for (int i = 0; i < traders.length;i++ ) {
     f32 distance = (pos - traders[i].getPosition()).getLength();
     if( i == 0) {
      bestdistance = distance;
      thepos = traders[i].getPosition();
     }
