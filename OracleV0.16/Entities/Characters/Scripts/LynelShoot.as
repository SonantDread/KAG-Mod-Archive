...
{ 
  if(this.isKeyPressed(key_action1) && !this.isKeyPressed(key_action2) && this.get_u32("Reload") >= this.get_u32("ReloadMax"))
  {
    if(getNet().isServer())
    {
      ShootArrow(this, this.getPosition() , this.getAimPos() , this.get_f32("ArrowForce"), 1.0f); 
    }
  ...
  else if (!this.isKeyPressed(key_action2) && this.get_u32("Reload2") > 30)
  {
    Vec2f Force = Vec2f(800.0f * (float(this.get_u32("Reload2"))/ float(this.get_u32("Reload2Max"))), 0);
    ...
    this.getSprite().PlaySound("LynelDash.ogg");
  }
