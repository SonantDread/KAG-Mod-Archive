...
void onTick(CBlob@ this)
{
	...
	this.Untag("sub_pickaxing");
	this.Untag("main_pickaxing");
	if(MainHandType == 2)if(Action1)Pickaxe(this,"main");
	if(SubHandType == 2)if(Action2)Pickaxe(this,"sub");
...
void onRemoveFromInventory( CBlob@ this, CBlob@ blob ){
	if(blob !is null)
	if(getNet().isServer()){
		if(getEquipedType(blob.getName()) == 0){
			blob.Untag("head");
			blob.Sync("head",true);
		}
		if(getEquipedType(blob.getName()) == 1){
			blob.Untag("torso");
			blob.Sync("torso",true);
		}
		if(getEquipedType(blob.getName()) == 2){
			blob.Untag("legs");
			blob.Sync("legs",true);
		}
		if(getEquipedType(blob.getName()) == 3){
			blob.Untag("main_arm");
			blob.Sync("main_arm",true);
			blob.Untag("sub_arm");
			blob.Sync("sub_arm",true);
		}
		if(getEquipedType(blob.getName()) == 4){
			blob.Untag("back");
			blob.Sync("back",true);
		}
