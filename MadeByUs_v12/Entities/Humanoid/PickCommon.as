...
void Pickaxe(CBlob@ this, string type)
{
	HitData@ hitdata;
	CSprite @sprite = this.getSprite();
	bool strikeAnim = true;//sprite.isAnimation("strike");

	this.Tag(type+"_pickaxing");
	...
	this.set_s16(type+"_pick_counter",this.get_s16(type+"_pick_counter")+1);
	if(this.get_s16(type+"_pick_counter") > 10)this.set_s16(type+"_pick_counter",0);
	
	// no damage cause we just check hit for cursor display
	bool justCheck = this.get_s16(type+"_pick_counter")/5 != hit_frame;
	bool adjusttime = this.get_s16(type+"_pick_counter")/5 < hit_frame - 1;
