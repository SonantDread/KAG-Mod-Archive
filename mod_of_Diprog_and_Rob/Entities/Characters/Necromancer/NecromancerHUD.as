...
void onRender( CSprite@ this )
{
	...
	if (diff > 0)
	{
		GUI::DrawIcon( "LightningZombie.png", 0, Vec2f(32,32), tl+Vec2f(6 + (slotsSize-1)*32, 0), 0.8f);
	}
	else
	{
		GUI::DrawIcon( "MenuItems.png", 13, Vec2f(32,32), tl+Vec2f(6 + (slotsSize-1)*32, -2), 0.5f);
		GUI::DrawText("" + cooldownTeleportSecsHUD + " secs", tl+Vec2f((slotsSize-1)*32, 22), SColor(255, 255, 216, 0));
	}
	//orb icons
	...
	if(count == ORB_LIMIT)
	{
		...
		GUI::DrawText("" + cooldownOrbSecsHUD + " secs", tl+delta+Vec2f(5 + (slotsSize-3)*32, 25), SColor(255, 255, 216, 0));
	}
	else
	{
		if (type == 0) GUI::DrawIcon( "Orbs.png", 0, Vec2f(9,9), tl+delta+Vec2f(21 + (slotsSize-3)*32, 7), 1.0f, 1);
		else if (type == 1) GUI::DrawIcon( "Orbs.png", 4, Vec2f(9,9), tl+delta+Vec2f(21 + (slotsSize-3)*32, 7), 1.0f, 1);
		else if(type == 2) GUI::DrawIcon( "Orbs.png", 8, Vec2f(9,9), tl+delta+Vec2f(21 + (slotsSize-3)*32, 7), 1.0f, 1);
		else if (type == 3) GUI::DrawIcon( "Orbs.png", 12, Vec2f(9,9), tl+delta+Vec2f(21 + (slotsSize-3)*32, 7), 1.0f, 1);
