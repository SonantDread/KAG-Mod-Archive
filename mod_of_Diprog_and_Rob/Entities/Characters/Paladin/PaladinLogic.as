		...
		if(this.isKeyPressed(key_action2) && !this.isKeyPressed(key_action1))
		{
			...
			if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					...
					if(b.getTeamNum() == this.getTeamNum() && !b.hasTag("evil"))
					{
						b.set_s16("blessed",90);
...
bool RecdHitCommand(CBlob@ this, CBitStream@ params)
{
	...
	else
	{
		...
		if(blob !is null)
		{
			...
			if(blob.hasTag("flesh"))this.server_Hit(blob, tilepos, attackVel, attack_power, Hitters::builder, teamHurt);
			else this.server_Hit(blob, tilepos, attackVel, 0.25, Hitters::builder, teamHurt);
		}
	}
