...
void onInit(CBlob@ this)
{
	...
	this.set_s16("mad timer", -999999);

	this.getShape().SetOffset(Vec2f(0,6));
	
	this.set_u8("custom_hitter", FUNHitters::bison);
	...
	if ((friend is null || blob.getTeamNum() != friend.getTeamNum()) && blob.getName() != this.getName() && blob.hasTag("flesh"))
	{
		...
		if (vellen > 0.1f)
		{
			...
			if (vel * direction > 0.33f)
			{
				...
				this.server_Hit( blob, point1, vel, power, FUNHitters::bison, false);
			}
		}	
...
{
	if (hitBlob !is null && customData == FUNHitters::bison)
	{
