...
#include "/Entities/Common/Attacks/FUNHitters.as";	   
    ...
    if (map.getHitInfosFromArc( pos, -angle, 30, vellen, this, false, @hitInfos ))
    {
        for (uint i = 0; i < hitInfos.length; i++)
        {
            ...
            if (hi.blob is null) // map
            {
				...
				if (BoulderHitMap( this, hi.hitpos, hi.tileOffset, vel, dmg, FUNHitters::boulder ))
					return;
            }
			else
			if (team != u8(hi.blob.getTeamNum()))
			{
				this.server_Hit( hi.blob, pos, vel, dmg, FUNHitters::boulder, true);
				this.setVelocity(vel*0.9f); //damp

				// die when hit something large
				if (hi.blob.getRadius() > 32.0f) {
					this.server_Hit( this, pos, vel, 10, FUNHitters::boulder, true);
				}
			}
        }
    }
...
bool BoulderHitMap( CBlob@ this, Vec2f worldPoint, int tileOffset, Vec2f velocity, f32 damage, u8 customData )
{
    ...
    if (stuck)
    {
       this.server_Hit( this, worldPoint, velocity, 10, FUNHitters::boulder, true);
    }
