...
void readRunes(CBlob@ this, CBlob@ invoker, string runes){
	...
	if(this.get_s16("power") < 0){
		f32 damage = ((this.get_s16("power")*-1)-25)/25;
...
int processRune(CBlob@ this, CBlob@ invoker, int MainRune, int argument0, int argument1, int argument2){
	...
	if(MainRune == 5){ //Water
		return WaterEffect(this, invoker, argument0, argument1);
	}
	...
	if(MainRune == 10){ //Devour
		return DevourEffect(this, invoker);
	}
	...
	if(MainRune == 12){ //Poly
		return PolyEffect(this, invoker, argument0, argument1, argument2);
	}
	...
	if(MainRune == 18){ //Haste
		return HasteEffect(this, invoker, argument0, argument1);
	}
	
	if(MainRune == 22){ //Slow
		return SlowEffect(this, invoker, argument0, argument1);
	}
	
	if(MainRune == 19){ //Cure
		return CureEffect(this, invoker, argument0, argument1);
	}
	
	if(MainRune == 23){ //Infect
		return InfectEffect(this, invoker, argument0, argument1);
	}
...
int WaterEffect(CBlob@ this, CBlob@ invoker, int target, int filter){

	if(getTargetType(target) == 0){
		invoker.set_u16( "invisDuration", getGameTime() + 30*30 );
		invoker.SetVisible(false);
		invoker.AddScript("/InvisPotionDuration.as");
		this.set_s16("power",this.get_s16("power")-100);
		return 1;
	}
	...
	if(getTargetType(target) == 1){
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
		{
			...
			{
				CBlob@ b = blobsInRadius[i];
				if(blobMatchesFilter(b,getFilter(filter)))if(!b.hasTag("negrunetatoo")){
					b.set_u16( "invisDuration", getGameTime() + 30*20 );
					b.SetVisible(false);
					b.AddScript("/InvisPotionDuration.as");
					this.set_s16("power",this.get_s16("power")-100);
				}
			}
		}
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				if(blobMatchesFilter(b,getFilter(filter)))if(!b.hasTag("negrunetatoo"))if(b.getTeamNum() == invoker.getTeamNum()){
					b.set_u16( "invisDuration", getGameTime() + 30*10 );
					b.SetVisible(false);
					b.AddScript("/InvisPotionDuration.as");
					this.set_s16("power",this.get_s16("power")-100);
				}
...
int DevourEffect(CBlob@ this, CBlob@ invoker){

	invoker.set_s16("drainlvl2",this.get_s16("power")*18);
	invoker.set_s16("lifesteallvl2",this.get_s16("power")*18);
	this.set_s16("power",0);
	return 0;
}
...
int PolyEffect(CBlob@ this, CBlob@ invoker, int target, int type1, int type2){

	string polytype = "none";
	
	if(type1 == 8){
		if(type2 == 7)polytype = "polychicken";
		if(type2 == 5)polytype = "polyfish";
		if(type2 == 21)polytype = "polyzombie";
		if(type2 == 8)polytype = "polybison";
	}
	if(type1 == 10)if(type2 == 5)polytype = "polyshark";
	if(type1 == 9)polytype = "polyent";
	if(type1 == 6)polytype = "polygolem";
	
	if(polytype != "none"){
		if(getTargetType(target) == 0){
			Polymorph(invoker, polytype, 600);
			this.set_s16("power",this.get_s16("power")-100);
		}

		if(getTargetType(target) == 1)if(this.get_s16("power") > 150){
			CBlob@[] blobsInRadius;	   
			if (this.getMap().getBlobsInRadius(this.getPosition(), 32.0f, @blobsInRadius)) 
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					CBlob@ b = blobsInRadius[i];
					if(!b.hasTag("negrunetatoo"))if(b.hasTag("flesh") || b.hasTag("plant")){
						Polymorph(b, polytype, 600);
						this.set_s16("power",this.get_s16("power")-100);
					}
				}
			}
		...
		if(getTargetType(target) == 2)if(this.get_s16("power") >= 300){
			CBlob@[] blobsInRadius;	   
			if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					CBlob@ b = blobsInRadius[i];
					if(!b.hasTag("negrunetatoo"))if((b.hasTag("flesh") || b.hasTag("plant")) && b.getTeamNum() != invoker.getTeamNum()){
						Polymorph(b, polytype, 600);
						this.set_s16("power",this.get_s16("power")-100);
					}
				}
		...
		if(getTargetType(target) == 3)if(this.get_s16("power") >= 500){
			...
			if (this.getMap().getBlobsInRadius(this.getPosition(), 1600.0f, @blobsInRadius)) 
			{
				...
				{
					CBlob@ b = blobsInRadius[i];
					if(b is invoker)invoker.server_Die();
					else
					if(b.hasTag("flesh") || b.hasTag("plant"))if(!b.hasTag("negrunetatoo")){
						Polymorph(b, polytype, 600);
						this.set_s16("power",this.get_s16("power")-100);
					}
				}
...
int LifeEffect(CBlob@ this, CBlob@ invoker, int target, int type1, int type2){
	...
	if(type1 == 8){
		...
		if(type2 == 21){summon = "zombie";mod = 10;}
		if(type2 == 8){summon = "bison";mod = 40;}
	}
	if(type1 == 10)if(type2 == 5){summon = "shark";mod = 40;}
	if(type1 == 9){summon = "ent";mod = 4;}
	if(type1 == 6){summon = "golem";mod = 20;}
	...
	/* //////////////////Old life effect!////////////////////////////////////////////////////////
	if(getTargetType(target) == 0){
		if(invoker.getHealth() <= invoker.getInitialHealth()*2){
			this.set_s16("power",this.get_s16("power")-100*((invoker.getInitialHealth()*2)-invoker.getHealth()));
			invoker.server_SetHealth(invoker.getInitialHealth()*2);
		}
		return 1;
	}
	
	if(getTargetType(target) == 1)if(this.get_s16("power") > 200){
		CBlob@[] blobsInRadius;	   
		if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				if(blobMatchesFilter(b,getFilter(filter))){
					if(b.getHealth() <= b.getInitialHealth()*2){
						this.set_s16("power",this.get_s16("power")-100*((b.getInitialHealth()*2)-b.getHealth()));
						b.server_SetHealth(b.getInitialHealth()*2);
					}
				}
			}
		}
	...
	if(getTargetType(target) == 2)if(this.get_s16("power") > 250){
		CBlob@[] blobsInRadius;	   
		if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				CBlob@ b = blobsInRadius[i];
				if(blobMatchesFilter(b,getFilter(filter)))if(b.getTeamNum() == invoker.getTeamNum()){
					if(b.getHealth() <= b.getInitialHealth()*2){
						this.set_s16("power",this.get_s16("power")-120*((b.getInitialHealth()*2)-b.getHealth()));
						b.server_SetHealth(b.getInitialHealth()*2);
					}
				}
			}
	...
	if(getTargetType(target) == 3)if(this.get_s16("power") > 500){
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 1600.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				CBlob@ b = blobsInRadius[i];
				if(blobMatchesFilter(b,getFilter(filter))){
					if(b.getHealth() <= b.getInitialHealth()*4){
						this.set_s16("power",this.get_s16("power")-100*((b.getInitialHealth()*4)-b.getHealth()));
						b.server_SetHealth(b.getInitialHealth()*4);
					}
				}
			}
		}
...
int HasteEffect(CBlob@ this, CBlob@ invoker, int target, int filter){

	if(getTargetType(target) == 0){
		invoker.set_s16("hastelvl2",300);
		this.set_s16("power",this.get_s16("power")-100);
		return 1;
	}
	...
	if(getTargetType(target) == 2){
		...
		this.set_s16("power",this.get_s16("power")-125);
		return 2;
	}
...
int SlowEffect(CBlob@ this, CBlob@ invoker, int target, int filter){

	if(getTargetType(target) == 0){
		if(invoker.get_s16("slowlvl3") < 60)this.set_s16("power",this.get_s16("power")+50);
		invoker.set_s16("slowlvl3",300);
		return 1;
	}
	...
	if(getTargetType(target) == 2){
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
		{
			...
			{
				CBlob@ b = blobsInRadius[i];
				if(b.getTeamNum() == invoker.getTeamNum())if(!b.hasTag("negrunetatoo")){
					if(b.get_s16("slowlvl3") < 60)this.set_s16("power",this.get_s16("power")+50);
					b.set_s16("slowlvl3",300);
				}
			}
		}
...
int CureEffect(CBlob@ this, CBlob@ invoker, int target, int filter){
...
int InfectEffect(CBlob@ this, CBlob@ invoker, int target, int filter){

	if(getTargetType(target) == 0){
		invoker.Tag("infect");
		invoker.set_s16("poison",600);
		this.set_s16("power",this.get_s16("power")-25);
		return 1;
	}
