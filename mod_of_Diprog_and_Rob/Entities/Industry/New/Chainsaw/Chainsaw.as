				...
				for (uint i = 0; i < hitInfos.length; i++)
				{
					...
					if (hi.blob !is null) // blob
					{
						...
						if (  hi.blob.getConfig() == "log" || hi.blob.getConfig() == "tree_pine" || hi.blob.getConfig() == "tree_bushy" || hi.blob.getConfig() == "seed" || hi.blob.hasTag("wooden")) 
							hitwood = true;
						if (!hi.blob.hasTag("stone")) holder.server_Hit( hi.blob, hi.hitpos, attackVel, attack_dam, FUNHitters::chainsaw );
					...
					else // map
					{
						...
						{						
							...
							{
								this.server_HitMap( hi.hitpos, attackVel, 1.0f, FUNHitters::chainsaw );
							}
						}
