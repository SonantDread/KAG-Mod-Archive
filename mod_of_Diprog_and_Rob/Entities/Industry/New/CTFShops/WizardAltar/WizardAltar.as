...
{	 
	...
	{
		ShopItem@ s = addShopItem( this, "30 Regular Orbs", "$Mat_orbs$", "mat_orbs", "Normal damage", true);
		AddRequirement( s.requirements, "coin", "", "Coins", 25 );
	}
	{
		ShopItem@ s = addShopItem( this, "5 Fire Orbs", "$Mat_fireorbs$", "mat_fireorbs", "Normal damage, fires enemies", true);
		AddRequirement( s.requirements, "coin", "", "Coins", 25 );
	}
	{
		ShopItem@ s = addShopItem( this, "3 Bomb Orbs", "$Mat_bomborbs$", "mat_bomborbs", "Big damage, bigger explosive radius", true);
		AddRequirement( s.requirements, "coin", "", "Coins", 25 );
	}
	{
		ShopItem@ s = addShopItem( this, "10 Water Orbs", "$Mat_waterorbs$", "mat_waterorbs", "Splashes enemies", true);
		AddRequirement( s.requirements, "coin", "", "Coins", 25 );
	}
	{
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	...
	{
		caller.CreateGenericButton( "$Cristal$", Vec2f(-6,0), this, this.getCommandID("put_cristal"), "Build Cristal", params );
	}
	else if (!this.hasTag("cristal") && !hasCristal)
	{
		CButton@ btn = caller.CreateGenericButton( "$Cristal$", Vec2f(-6,0), this, 0, "Build Cristal Requires: Cristal" );
    ...
    if (canChangeClass( this, caller ) && caller.getTeamNum() == this.getTeamNum() && this.hasTag("cristal"))
    {
        CBitStream params;
        params.write_u16(caller.getNetworkID());
        caller.CreateGenericButton( "$change_class$", Vec2f(-6,0), this, SpawnCmd::buildMenu, "Swap Class", params );
    }
}
