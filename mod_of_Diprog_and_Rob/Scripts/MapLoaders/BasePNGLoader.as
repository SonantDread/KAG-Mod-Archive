...
class PNGLoader
{
	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		//
		else if (pixel == color_tile_sand) {
			spawnBlob( map, "sand_block", offset, 255, true);
			offsets[autotile_offset].push_back( offset );
			map.SetTile(offset, CMap::tile_ground_back );
		}
		else if (pixel == color_chaparral) {
			spawnBlob( map, "chaparral", offset, 255, true);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_water_plant) {
			spawnBlob( map, "water_plant", offset, 255, true);
			offsets[autotile_offset].push_back( offset );
			map.SetTile(offset, CMap::tile_ground_back );
		}
		else if (pixel == color_water_plant_no) {
			spawnBlob( map, "water_plant", offset, 255, true);
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_gold_nugget) {
			spawnBlob( map, "gold_block", offset, 255, true);
			offsets[autotile_offset].push_back( offset );
