...
bool loadMap(CMap@ _map, const string& in filename)
{
	...
	int NodeOffset = -3-XORRandom(3);
	int NodeSize = 10;
	
	int SewerLine = (Maths::Floor(height/NodeSize)-1)*NodeSize+5+NodeOffset;
	
	for(int i = 0; i < width; i += 1)
	if(World[i][SewerLine] != 0){ getNet().server_KeepConnectionsAlive();
	...
	for(int i = 0; i < width/NodeSize; i += 1){ //Init underground tunnel node grid
		bool[] temp;
		for(int j = 0; j < height/NodeSize; j += 1){ getNet().server_KeepConnectionsAlive();
	...
	for(int i = 1; i < width/NodeSize-1; i += 1) //Find random suitable nodes
		for(int j = 1; j < height/NodeSize; j += 1){ getNet().server_KeepConnectionsAlive();
			Nodes[i][j] = false;
			
			if(XORRandom(j) > 6 || XORRandom(15) == 0)
		...
		for(int j = 1; j < height/NodeSize; j += 1){ getNet().server_KeepConnectionsAlive();
			if(Nodes[i][j]){
				...
				for(int k = 0; k < NodeSize-2; k += 1){
					...
					if(XORRandom(4) == 0)World[i*NodeSize+k+2][j*NodeSize+2+NodeOffset] = CMap::tile_castle;
					if(XORRandom(4) == 0)World[i*NodeSize+k+2][j*NodeSize-1+NodeOffset] = CMap::tile_castle;
				}
				...
				if(j < height/NodeSize-1){
					...
					for(int k = 0; k < NodeSize-2; k += 1){
						...
						if(XORRandom(4) == 0)World[i*NodeSize-1][j*NodeSize+k+2+NodeOffset] = CMap::tile_castle;
						if(XORRandom(4) == 0)World[i*NodeSize+2][j*NodeSize+k+2+NodeOffset] = CMap::tile_castle;
					}
	...
	//////Piers
	for(int i = 10; i < width-10; i += 1)if(XORRandom(10) == 0){ getNet().server_KeepConnectionsAlive();
	...
	for(int i = 1; i < width/NodeSize-1; i += 1){ getNet().server_KeepConnectionsAlive();
		bool CanBuild = (XORRandom(20) == 0); //I know this is bad code, don't judge me;
		
		for(int j = -2; j < 4; j += 1){
			if(!SurfacePlanner[i*NodeSize+j] || biome[i*NodeSize+j] == 3)CanBuild = false;
		}
		...
		if(CanBuild){
			...
			if(Highest >= SeaLevel)continue; //Heck no.
