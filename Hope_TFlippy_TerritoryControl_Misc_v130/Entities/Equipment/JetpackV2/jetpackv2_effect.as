...
void onTick(CBlob@ this)
{
	...
	if (this.get_f32("fuel_count") > this.get_f32("max_fuel")) 
		this.set_f32("fuel_count", this.get_f32("max_fuel"));
	if (controls !is null) 
		if (shift)
			if (this.get_f32("fuel_count") > 0) this.set_f32("fuel_count", this.get_f32("fuel_count") - 2);
	...
	if (cmd == this.getCommandID("load_fuel"))
	{
		...
		for (int i = 0; i < oil.length; i++)
		{
			if (oil[i] !is null && oil[i].isAttached())
			{
				...
				if (isServer() && remain > 0 && remain <= 50)
				{
					CBlob@ giveoil = server_CreateBlob("mat_oil", -1, this.getPosition());
					giveoil.server_SetQuantity(remain);
					giveoil.setVelocity(Vec2f(0, 0));
				}
