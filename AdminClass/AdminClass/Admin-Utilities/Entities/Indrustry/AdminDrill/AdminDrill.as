	...
	if (this.isAttached())
	{
		...
		if (holder.getName() == required_class || sv_gamemode == "TDM")
		{
			...
			// delay drill
			{
				...
				Vec2f direction = Vec2f(1, 0).RotateBy(this.getAngleDegrees() + (facingleft ? 0.0f : 180.0f));
				const f32 sign = (facingleft ? -1.0f : 1.0f);

				const f32 attack_distance = -6.0f;
				Vec2f attackVel = direction * attack_distance;

				const f32 distance = 18.0f;
				const f32 attack_dam = 50.0f;
				...
				{
					HitInfo@[] hitInfos;
					if (map.getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 60, distance, this, false, @hitInfos))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							else // map
							{
								...
								this.server_HitMap(hi.hitpos, attackVel, 100.0f, Hitters::drill);
