...
void onInit(CBlob@ this)
{
	...
	vars.runForce.Set(4.0f, -1.0f);
	vars.slowForce.Set(1.0f, 0.0f);
	vars.jumpForce.Set(0.0f, -15.0f);
	vars.maxVelocity = 3.3f;
	
	this.SetLight(true);
	this.SetLightColor(SColor(11, 213, 255, 171));
	this.SetLightRadius(32.0f);
	
	this.server_SetTimeToDie(XORRandom(50)+10);
}
...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (att !is null)
		{
			...
			if (b !is null)if(b.get_s16("life") <= 50)
			{
				this.server_Hit(b, b.getPosition(), Vec2f(0,-1), 0.5f, Hitters::suddengib, false);
...
{
	if(blob !is null)
	if(getNet().isServer() && blob.get_s16("life") <= 50){
