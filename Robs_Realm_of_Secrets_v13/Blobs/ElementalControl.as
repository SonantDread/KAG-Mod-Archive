...
void ControlElements(Vec2f pos, bool fire, bool water, bool gold, bool evil, bool blood, bool sap, bool life, bool death, bool air, bool stone, bool legendary){
	CBlob@[] blobsInRadius;
	if (getMap().getBlobsInRadius(pos, 64.0f, @blobsInRadius)) 
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			|| ((b.getName() == "death_blob" || b.hasTag("death_element")) && (death || air))){
				...
				dir.Normalize();
				b.setVelocity(dir*2+b.getVelocity()/2);
				b.set_u8("moldable", 2);
			}
			...
			|| ((b.getName() == "life_blob" || b.hasTag("life_element")) && (life || air))){
				Vec2f dir = pos-b.getPosition();
				dir.Normalize();
				b.setVelocity(dir*1+b.getVelocity());
				b.set_u8("moldable", 2);
			}
			...
			if(((b.getName() == "fire_blob" || b.hasTag("fire_element")) && (fire || air))){
				Vec2f dir = pos-b.getPosition();
				dir.Normalize();
				b.setVelocity(dir*1.5+b.getVelocity());
				b.set_u8("moldable", 2);
			}
			...
			if(death){
				...
				if(XORRandom(100) == 0)
				if(b.getName() == "ghost")if(b.get_s16("death") > 0)if(getNet().isServer()){
					CBlob @blob = server_CreateBlob("death_blob", b.getTeamNum(), b.getPosition());
					blob.set_u8("moldable", 2);
					blob.set_s16("size", 1);
					b.set_s16("death",b.get_s16("death")-1);
					b.Sync("death",true);
					b.server_SetTimeToDie(b.getTimeToDie()*1.09);
				}
			}
