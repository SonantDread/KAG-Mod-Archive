...
void onInit(CBlob@ this)
{
	if(!this.hasTag("ghost")){
		this.set_s16("life",10);
		this.set_s16("death",0);
	} else {
		this.set_s16("life",0);
		this.set_s16("death",10);
	}
	
	this.set_u8("revive_knowledge",0);
...
void onTick(CBlob @ this)
{
	...
	if(this.get_s16("life") > 10){
		this.Tag("LifeMenu");
	} else {
		this.Untag("LifeMenu");
	}
	if(getNet().isServer())if(XORRandom(1000) == 0){
		this.Sync("death",true);
		this.Sync("life",true);
	}
}
...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu @gridmenu)
{
	if(this.hasTag("DeathMenu")){
		...
		if (menu !is null)
		{
			...
			{
				CGridButton@ b = menu.AddButton("$gbomb$", "Create a fake bomb.", this.getCommandID("gbomb"));
				if(this.get_s16("death") < 50)if(b !is null)b.SetEnabled(false);
			}
			...
			if(this.hasTag("ghost"))
			{
				...
				if(this.get_s16("death") < 150)if(b !is null)b.SetEnabled(false);
			}
			if(this.getName() != "ghost"){
				{
	...
	if(this.hasTag("LifeMenu")){
		...
		if (menu !is null)
		{
			menu.deleteAfterClick = false;
			{
				...
				if(this.get_s16("life") < 20)if(b !is null)b.SetEnabled(false);
			}
			{
			...
			}
			{
				...
				if(this.get_s16("life") < 30)if(b !is null)b.SetEnabled(false);
			}
			{
				CGridButton@ b = menu.AddButton("$lifeturreticon$", "Summon a rapid fire turret.", this.getCommandID("maketurret"));
				if(this.get_s16("life") < 60)if(b !is null)b.SetEnabled(false);
			}
		}
	}
}
			...
			if (getNet().isServer()){
				...
				if (this.getMap().getBlobsInRadius(this.getPosition(), 128.0f, @blobsInRadius)) 
				{
					for (uint i = 0; i < blobsInRadius.length; i++)
					{
						...
						if(b.get_s16("life") > 0 && b !is this)
						{
							this.set_s16("life",this.get_s16("life")+b.get_s16("life"));
