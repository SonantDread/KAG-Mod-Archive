...
void onTick(CBlob@ this)
{
	...
	if (XORRandom(200) < 1) this.getSprite().PlaySound("/man_scream", 0.3f, 0.60f); // Tragic
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	{
		if(blob.hasTag("ghost") && !blob.hasTag("goldenstatue")){
			if(blob.getPlayer() !is null)
			if(blob.getPlayer().getUsername() == this.get_string("name"))
			if(getNet().isServer()){
				CBlob @gold = ChangeClass(blob, "builder", this.getPosition(), blob.getTeamNum());
				gold.set_u8("race", 6);
				this.server_Die();
			}
		}
