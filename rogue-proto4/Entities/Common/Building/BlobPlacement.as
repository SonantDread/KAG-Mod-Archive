...
void PositionCarried(CBlob@ this, CBlob@ carryBlob)
{
	...
	if (carryBlob.hasTag("place45"))
	{
		...
		if (hands !is null)
		{
			...
			hands.offset.y = 0;//-(aim_vec.y * (distance < 0 ? 1.0f : 1.0f));
		}
	}
	else
	{
		...
		carryBlob.setAngleDegrees(this.getAngleDegrees());
		AttachmentPoint@ hands = this.getAttachments().getAttachmentPointByName("PICKUP");
		if (hands !is null)
		{
		...
		carryBlob.getSprite().SetRelativeZ(-10.0f);
		hands.offsetZ = -10.0f;
		u32 timer = this.get_u8("hit_timer");
		if(carryBlob.hasTag("melee")) carryBlob.setAngleDegrees(this.getAngleDegrees() -timer*5);
	}
}
