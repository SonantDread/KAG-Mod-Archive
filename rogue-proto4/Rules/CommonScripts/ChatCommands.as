...
{
	return (player.getUsername() == "Osmal");
}
...
{	
	...
	{ 
		if(tokens.length == 2) if(!canUseBasic(player) && tokens[1] == "!hall" || tokens[1] == "!roifel" || tokens[1] == "!shotgun" || tokens[1] == "!ak" || tokens[1] == "!rl" || tokens[1] == "!vortex"  || tokens[1] == "!strike" || tokens[1] == "!airstrike" || tokens[1] == "!revolver" || tokens[1] == "!mg" || tokens[1] == "!flamer" || tokens[1] == "!detonator" || tokens[1] == "!roifel" || tokens[1] == "!pg" || tokens[1] == "!portal") return false;
	...
	else if (tokens[0] == "!d") 
	{
		...
		u32 daytime = parseInt(tokens[1]);
		CMap@ map = getMap();
		if (map is null) return false;
		map.SetDayTime(float(daytime) / 100.0f);
