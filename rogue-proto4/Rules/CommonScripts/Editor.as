    ...
    if (cmd == this.getCommandID(cmd_place))
	{
		...
		if (MayUseEditor(player))
		{
			...
			if    (blob !is null)
			{
				...
				if(brushsize > 1)
				{			
				    Vec2f cursorPos2 = blob.getAimPos();
					
				    Vec2f cursorPos3 = blob.getAimPos();

				    Vec2f cursorPos4 = blob.getAimPos();

				    Vec2f cursorPos5 = blob.getAimPos();
					 ...
					 if (tile != 0)
					{
			            ...
			            getMap().server_SetTile( cursorPos3, tile );
			            getMap().server_SetTile( cursorPos4, tile );
			            getMap().server_SetTile( cursorPos5, tile );
						return;
					}

						if(brushsize > 2)
						{
						    Vec2f cursorPos6 = blob.getAimPos();
							
						    Vec2f cursorPos7 = blob.getAimPos();

						    Vec2f cursorPos8 = blob.getAimPos();

						    Vec2f cursorPos9 = blob.getAimPos();

						    Vec2f cursorPos10 = blob.getAimPos();
							
						    Vec2f cursorPos11 = blob.getAimPos();

						    Vec2f cursorPos12 = blob.getAimPos();

						    Vec2f cursorPos13 = blob.getAimPos();
				            ...
				            getMap().server_SetTile( cursorPos6, tile );
				            getMap().server_SetTile( cursorPos7, tile );
				            getMap().server_SetTile( cursorPos8, tile );
				            getMap().server_SetTile( cursorPos9, tile );
	...
	if ((args[0] == "!e" || args[0] == "!editor" ) && MayUseEditor(player))
	{
		...
		if    (blob !is null && args.length >= 2)
		{
		    ...
		    if(args[1] == "s")
			{
				...
				this.set_u32("bs_"+player.getUsername(), bs);
			}
			...
			{
				if(args[1] == "b" && MaySpawnBlobs(player))
				{
			        ResetEditorData(blob);
