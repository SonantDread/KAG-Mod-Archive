...
CBlob@ getNewTarget(CBrain@ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false)
{
	...
	for (uint i = 0; i < players.length; i++)
	{
		...
		{
      			f32 distance = (pos-pos2).getLength();
...
bool JustGoShield(CBlob@ blob, CBlob@ target)
{
  ...
  bool inair = (!blob.isOnGround() && !blob.isOnLadder());
	...
	if (horiz_distance > blob.getRadius() * 0.75f)
	{
      ...
      blob.setAimPos(mypos + Vec2f(-7, 80));
		}
		else
		{
      ...
      blob.setAimPos(mypos + Vec2f(7, 80));
		...
		blob.setKeyPressed(key_up, !inair);
...
bool isVisible(CBlob@ blob, CBlob@ target)
{
	...
	// BC let bots see through doors
	if (target.hasTag("door"))
	{
...
void JustGo(CBlob@ blob, Vec2f pos)
{
	Vec2f mypos = blob.getPosition();
	Vec2f targetPos = pos;
...
void MaintainDistance(CBlob@ blob, CBlob@ target, f32 distance) {
  Vec2f mypos = blob.getPosition();
	Vec2f point = target.getPosition(); 

  f32 dist = (mypos-point).getLength();
