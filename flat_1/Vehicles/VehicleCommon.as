...
bool Vehicle_doesCollideWithBlob_boat(CBlob@ this, CBlob@ blob)
{
	...
	// BC no colliding with doors
	if (blob.hasTag("door") && !this.hasTag("medium weight"))
		return false;

	// BC no colliding with teammates
	if (this.getTeamNum() == blob.getTeamNum() && blob.hasTag("player") && blob.isInWater())
		return false;
