...
void onAttach(CBlob@ this, CBlob@ attached, AttachmentPoint @attachedPoint)
{
	...
	if (!this.hasTag('exploding')) { this.server_setTeamNum(attached.getTeamNum()); }
...
void findEnemy(CBlob@ this){
	...
	CBlob@[] blobs;
	map.getBlobsInRadius(pos, 80, @blobs);
	for (uint i = 0; i < blobs.size(); i++)
	{
		CBlob@ player_blob = blobs[i];
		if (player_blob.getTeamNum() == this.getTeamNum() || player_blob is this || player_blob.getPlayer() is null) { continue; }

		Vec2f difference = player_blob.getPosition() - pos;
		float distance = difference.Length();

		Vec2f homingForce = (distance > distanceThreshold) ? (difference / distance * accelerationSpeed) : Vec2f(0, 0);
		if (this.isOnWall() && this.isOnGround()) { homingForce.y *= accelerationSpeed; }

		this.AddForce(homingForce);
		return;
