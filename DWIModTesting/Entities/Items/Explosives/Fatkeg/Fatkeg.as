// Keg logic
#include "Hitters.as";
#include "FatkegBoom.as";


void onInit(CBlob@ this)
{
	//this.Tag("bomberman_style");
	//this.set_f32("map_bomberman_width", 500.0f);
	this.set_f32("explosive_radius", 150.0f);
	this.set_f32("explosive_damage", 10.0f);
	this.set_u8("custom_hitter", Hitters::keg);
	this.set_string("custom_explosion_sound", "Entities/Items/Explosives/KegExplosion.ogg");
	this.set_f32("map_damage_radius", 150.0f);
	this.set_f32("map_damage_ratio", 0.8f);
	this.set_bool("map_damage_raycast", true);
	this.Tag("heavy weight");

	this.set_f32("fatkeg_time", 180.0f);  // 180.0f
	...
	CSpriteLayer@ fuse = this.getSprite().addSpriteLayer("fuse", "Fatkeg.png" , 16, 20, 0, 0);

	if (fuse !is null)
	{
		fuse.addAnimation("default", 0, false);
		int[] frames = {6, 7, 8, 9, 10, 11};
		fuse.animation.AddFrames(frames);
		fuse.SetOffset(Vec2f(3, -6));
		fuse.SetRelativeZ(1);
	}
}
...
void onTick(CSprite@ this)
{
	...
	if (fuse !is null)
	{
		fuse.animation.frame = 1 + (fuse.animation.getFramesCount() - 1) * (1.0f - ((timer + 5) / f32(blob.get_f32("fatkeg_time"))));
	}
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	switch (customData)
	{
		...
		case Hitters::keg:
			...
			this.set_s32("explosion_timer", Maths::Min(this.get_s32("explosion_timer"), getGameTime() + XORRandom(this.get_f32("fatkeg_time")) / 3));
