...
void onRender(CSprite@ this)
{
	...
	AttachmentPoint@ ap = localBlob.getAttachmentPoint(0);
	CBlob@ carriedBlob = ap.getOccupied();
	if (carriedBlob is null) return;
	
	if (localBlob.getTeamNum() != blob.getTeamNum() && carriedBlob.getName() == flag_name)
	{
		int deltaY = -2 + Maths::FastSin(getGameTime() / 4.5f) * 3.0f;
		Vec2f p = blob.getInterpolatedPosition() + Vec2f(2.0f, -blob.getHeight() * 3.0f);
		p.x -= 8;
		Vec2f pos = getDriver().getScreenPosFromWorldPos(p);
		pos.y += deltaY;
		GUI::DrawIcon("GUI/server_icons.png", 0, Vec2f(16, 16), pos, getCamera().targetDistance);
	}
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	{
		if (canPickupFlag(blob) && this.hasAttached() && !getRules().isWarmup())
		{
