...
namespace Material
{
	...
	{
		damage = Maths::Min(damage,blob.getHealth()*2.0f);
  ...
  void fromTile(CBlob@ this, uint16 &in type, float &in damage)
  {
	...
	if (type >= CMap::tile_gold_brick && type <= CMap::tile_gold_brick+7)
    {
      createFor(this, 'mat_gold', 1.f * damage);
    }
	if (type >= CMap::tile_gold_pile && type <= CMap::tile_gold_pile+4)
    {
      createFor(this, 'mat_gold', 10.f * damage);
    }
	
	if (type >= CMap::tile_gold_gem_weak && type <= CMap::tile_gold_gem_unstable+19)
    {
      createFor(this, 'mat_gold', 1.f * damage);
	  if(isServer() && type >= CMap::tile_gold_gem_unstable){
		MagicExplosion(this.getPosition(), "UnstableMagic"+XORRandom(4)+".png", 3.0f);
	  }
    }
	
	if(type == CMap::tile_gold_gem_weak+5){
		if(isServer())server_CreateBlob("weak_gem",-1,this.getPosition());
	}
	if(type == CMap::tile_gold_gem+7){
		if(isServer())server_CreateBlob("gem",-1,this.getPosition());
	}
	if(type == CMap::tile_gold_gem_strong+12){
		if(isServer())server_CreateBlob("strong_gem",-1,this.getPosition());
	}
	if(type == CMap::tile_gold_gem_unstable+19){
		if(isServer())server_CreateBlob("unstable_gem",-1,this.getPosition());
	}
