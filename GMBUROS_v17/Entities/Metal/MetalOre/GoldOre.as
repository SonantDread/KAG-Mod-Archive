...
void onTick(CBlob @this){
	...
	if(this.get_u8("heat") > 10){
		...
		if(getNet().isServer()){
			...
			{CBlob @blob = server_CreateBlob("molten_gold_small",-1,this.getPosition()+Vec2f(-4,0));
	...
	if(this.isInWater()){
		...
		if(getNet().isServer()){
			if(XORRandom(2) == 0){
				server_CreateBlob("gold_drop",-1,this.getPosition()+Vec2f(0,0));
			} else {
				server_CreateBlob("gold_drop_small",-1,this.getPosition()+Vec2f(0,0));
			}
		}
