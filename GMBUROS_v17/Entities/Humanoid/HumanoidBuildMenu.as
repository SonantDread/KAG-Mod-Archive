...
void onTick(CInventory@ this)
{
	...
	blob.set_u8("build page",getTechLevel(blob));
}

void MakeBlocksMenu(CInventory@ this, Vec2f menu_size, const Vec2f &in INVENTORY_CE)
{
	CBlob@ blob = this.getBlob();
	if(blob is null) return;
	...
	const Vec2f MENU_CE = Vec2f(-24*(3.5f + menu_size.x), menu_size.y * -24 -24*3.5f) + INVENTORY_CE;
	...
	if(PAGE > 0){
		...
		if(menu !is null)
		{
			...
			for(u8 i = 0; i < blocks[PAGE].length; i++)
			{
				...
				{
					button.hoverText = block_desc + "\n" + getButtonRequirementsText(blob, b.reqs);
				}
				else
				{
					button.hoverText = block_desc + "\n" + getButtonRequirementsText(blob, b.reqs);
			...
			const Vec2f TOOL_POS = Vec2f(-24*(10.0f + menu_size.x),-24*4.5f) + INVENTORY_CE;
			...
			const Vec2f INDEX_POS = Vec2f(menu.getLowerRightPosition().x + GRID_PADDING + GRID_SIZE, menu.getUpperLeftPosition().y + GRID_SIZE * Builder::PAGE_COUNT);
...
{
	if(this.getBlob().getPlayer() !is getLocalPlayer())return;
	
	CBlob@ blob = this.getBlob();
	if(blob is null) return;

	Vec2f menu_size = MENU_SIZE;
	
	const u8 PAGE = blob.get_u8("build page");
	
	BuildBlock[][]@ blocks;
	if(blob.get(blocks_property, @blocks)){
		menu_size = Vec2f(menu_size.x,Maths::Max(Maths::Ceil(blocks[PAGE].length/f32(menu_size.x)),1));
	}
	...
	MakeBlocksMenu(this, menu_size, INVENTORY_CE);
	
	
	LimbInfo@ limbs;
	if (!blob.get("limbInfo", @limbs))return;
	
	if(!isLimbUsable(blob,LimbSlot::MainArm) && !isLimbUsable(blob,LimbSlot::SubArm)){
		if(!isLimbMovable(blob,LimbSlot::MainArm) || !isLimbMovable(blob,LimbSlot::SubArm)){
			blob.ClearGridMenus();
		}
	}
}

int getTechLevel(CBlob @this){

	int techLevel = 0;
	
	EquipmentInfo@ equip;
	if (!this.get("equipInfo", @equip))return techLevel;
	
	if(equip.MainHand == Equipment::Hammer)techLevel = getHammerLevel(equip.MainHandType);
	if(equip.SubHand == Equipment::Hammer)techLevel = Maths::Max(getHammerLevel(equip.SubHandType),techLevel);
	
	return techLevel;
}

int getHammerLevel(int hammerType){

	if(hammerType == 0)return 2;
	
	if(hammerType == 1)return 3;
	if(hammerType == 2)return 3;
	
	if(hammerType == 3)return 1;
