...
void PlaceBlock(CBlob@ this, u8 index, Vec2f cursorPos)
{
	...
	if (bc.tile > 0 && hasRequirements(inv, bc.reqs, missing))
	{
		...
		if(tile == CMap::tile_gold_pile){
			if(getMap().getTile(cursorPos).type > CMap::tile_gold_pile && getMap().getTile(cursorPos).type <= CMap::tile_gold_pile+4){
				tile = getMap().getTile(cursorPos).type-1;
			} else {
				tile = CMap::tile_gold_pile+4;
			}
		}
		...
		SendGameplayEvent(createBuiltBlockEvent(this.getPlayer(), tile));
	}
}
	...
	if (buildtile > 0)
	{
		...
		if (!getHUD().hasButtons() && this.isKeyPressed(key_action1))
		{
			if (bc.cursorClose && bc.buildable && bc.supported)
			{
				...
				if(block !is null)SetBuildDelay(this, block.tile < 255 ? delay : delay / 3);
