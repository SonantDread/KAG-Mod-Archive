...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	// tilemap check
	const bool buildSolid = (map.isTileSolid(buildTile) || (blob !is null && blob.isCollidable()) || buildTile > 255);
	...
	else if (buildTile != CMap::tile_gold_pile && backtile.type >= CMap::tile_gold_pile && backtile.type <= CMap::tile_gold_pile+4)
	{
		//cant build wood
		return false;
	}
	...
	{
		//can only build dirt on dirt background
		if(backtile.type != CMap::tile_ground_back && (backtile.type < CMap::tile_ground || backtile.type > CMap::tile_ground+8) && (backtile.type < CMap::tile_ground_d1 || backtile.type > CMap::tile_ground_d1+2))return false;
	}
...
bool isBuildRayBlocked(Vec2f pos, Vec2f target, Vec2f &out point)
{
	...
	bool result = map.rayCastSolid(pos + Vec2f(0, halfsize), target, point) &&
		   map.rayCastSolid(pos + Vec2f(halfsize, 0), target, point) &&
		   map.rayCastSolid(pos + Vec2f(0, -halfsize), target, point) &&
		   map.rayCastSolid(pos + Vec2f(-halfsize, 0), target, point);
	
	if(!isBuildRayBlockedHead(pos, target))result = false;

	return result;
}

bool isBuildRayBlockedHead(Vec2f pos, Vec2f target)
{
	CMap@ map = getMap();
	...
	return map.rayCastSolid(pos + Vec2f(0, halfsize), target) &&
		   map.rayCastSolid(pos + Vec2f(halfsize, 0), target) &&
		   map.rayCastSolid(pos + Vec2f(0, -halfsize), target) &&
		   map.rayCastSolid(pos + Vec2f(-halfsize, 0), target);
}
