...
void onTick(CBlob@ this)
{
	...
	if(equip.MainHand == Equipment::Casting){
		int type = equip.MainHandType;
		Cast(this,type,key_action1);
	}
	
	if(equip.SubHand == Equipment::Casting){
		int type = equip.SubHandType;
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("set_cast_marm")){
		...
		equipType(this,EquipSlot::Main,Equipment::Casting,CastType);
	}
	if (cmd == this.getCommandID("set_cast_sarm")){
		const u16 CastType = params.read_u16();
		
		equipType(this,EquipSlot::Sub,Equipment::Casting,CastType);
	}
}
