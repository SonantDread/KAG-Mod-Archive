...
CSpriteLayer@ LoadHead(CSprite@ this, u8 headIndex)
{
	...
	LimbInfo@ limbs;
	if (!blob.get("limbInfo", @limbs))return null;
	
	CSpriteLayer@ head = this.addSpriteLayer("head", getBodyTypeName(limbs.Head)+"_Head.png", 16, 16, 0, 0);
	...
	if(limbs.Head != BodyType::None){
		int hair_num = blob.get_u8("hair_index");
		int hair_colour = blob.get_u8("hair_colour");
		
		if(limbs.Head == BodyType::Wraith)hair_colour = 1;
		if(limbs.Head == BodyType::Gold)hair_colour = 4;
		if(limbs.Head == BodyType::Shadow)hair_colour = 16;
