...
void onTick(CBlob@ this)
{
	...
	f32 pickMod = 1.0f;
	if(this.hasTag("short_hitbox"))pickMod *= 0.5f;
	...
	if(equip.MainHand == Equipment::Pick){
		if(ismyplayer || (isServer() && this.isBot()))
		{
			equip.mainSwingTimer = Pickaxe(this,getEquipmentDamage(equip.MainHand,equip.MainHandType)*getLimbStrength(this,LimbSlot::MainArm,limbs.MainArm),equip.mainSwingTimer);
		}
		if(this.isKeyPressed(key_action1) && isLimbUsable(this,LimbSlot::MainArm))
		{
			...
			if(equip.mainSwingTimer < hit_time)equip.mainSwingTimer += getLimbSpeed(this,LimbSlot::MainArm,limbs.MainArm)*getEquipmentSpeed(equip.MainHand,equip.MainHandType,getLimbStrength(this,LimbSlot::MainArm,limbs.MainArm))*pickMod*5.0f;
		} else {
			equip.mainSwingTimer = 0;
		}
	}
	
	if(equip.SubHand == Equipment::Pick){
		if(ismyplayer || (isServer() && this.isBot()))
		{
			equip.subSwingTimer = Pickaxe(this,getEquipmentDamage(equip.SubHand,equip.SubHandType)*getLimbStrength(this,LimbSlot::SubArm,limbs.SubArm),equip.subSwingTimer);
		}
		if(this.isKeyPressed(key_action2) && isLimbUsable(this,LimbSlot::SubArm))
		{
			RunnerMoveVars@ moveVars;
			if(this.get("moveVars", @moveVars))
			{
			...
			this.Tag("prevent crouch");
			if(equip.subSwingTimer < hit_time)equip.subSwingTimer += getLimbSpeed(this,LimbSlot::SubArm,limbs.SubArm)*getEquipmentSpeed(equip.SubHand,equip.SubHandType,getLimbStrength(this,LimbSlot::SubArm,limbs.SubArm))*pickMod*5.0f;
		} else {
	...
	if(ismyplayer || (isServer() && this.isBot()))
	if(equip.mainSwingTimer <= 0 && equip.subSwingTimer <= 0){
		HitData@ hitdata;
		if(this.get("hitdata", @hitdata)){
			hitdata.blobID = 0;
...
bool RecdHitCommand(CBlob@ this, CBitStream@ params)
{
	...
	if(blobID == 0)
	{
		...
		if(map !is null)
		{
			if(map.getSectorAtPosition(tilepos, "no build") is null)
			{
				...
				if (getNet().isServer())
				{
					this.server_HitMap(tilepos, Vec2f(0,0), 1.0f, Hitters::builder);
				}
	...
	else
	{
		...
		if(blob !is null)
		{
			...
			const bool teamHurt = !blob.hasTag("flesh") || (!blob.hasTag("alive") && !blob.hasTag("animated"));
	...
	if(equip.mainSwingTimer >= hit_time)equip.mainSwingTimer = 0;
	if(equip.subSwingTimer >= hit_time)equip.subSwingTimer = 0;
...
f32 Pickaxe(CBlob@ this, f32 hit_damage, f32 swing_timer)
{
	HitData@ hitdata;

	if(swing_timer <= 0)return swing_timer;

	// no damage cause we just check hit for cursor display
	bool justCheck = Maths::Min(swing_timer,hit_time) != hit_time;
	bool adjusttime = swing_timer < hit_time-1;

	// pickaxe!

	this.get("hitdata", @hitdata);
	if(hitdata is null) return swing_timer;

	Vec2f blobPos = this.getPosition();
	Vec2f aimPos = this.getAimPos();
	if(this.get_bool("short_hitbox"))blobPos.y += 4;
	Vec2f aimDir = aimPos - blobPos;
	
	LimbInfo@ limbs;
	if (!this.get("limbInfo", @limbs))return swing_timer;
	EquipmentInfo@ equip;
	if (!this.get("equipInfo", @equip))return swing_timer;
	...
	{
		SortHits(this, hitInfos, 0.5f, hit_p);
	}
...
void onRender(CSprite@ this)
{
	...
	if((equip.mainSwingTimer > 0 && equip.MainHand == Equipment::Pick)
	|| (equip.subSwingTimer > 0 && equip.SubHand == Equipment::Pick))
	{
		...
		{
			DrawCursorAt(hitdata.tilepos, "Entities/Characters/Sprites/TileCursor.png");
		}
	}
}
