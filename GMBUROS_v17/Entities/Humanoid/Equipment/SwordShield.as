...
void onTick(CBlob@ this)
{
	...
	OldKnightInfo@ knight;
	if (!this.get("OldKnightInfo", @knight))
	{
		return;
	}
	...
	int equipMain = equip.MainHand;
	int equipSub = equip.SubHand;
	bool canUseMain = canLimbAttack(this,LimbSlot::SubArm);
	bool canUseSub = canLimbAttack(this,LimbSlot::MainArm);
	...
	if(canUseMain)if(equipMain == Equipment::Sword || equipMain == Equipment::GreatSword){
		...
		Damage = getEquipmentDamage(equipMain,equip.MainHandType)*getLimbStrength(limbs.MainArm);
	}
	if(canUseSub)if(equipSub == Equipment::Sword || equipSub == Equipment::GreatSword){
		...
		Damage = getEquipmentDamage(equipSub,equip.SubHandType)*getLimbStrength(limbs.SubArm);
	}
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			if (delta == 2)
			{
				if(equipMain == Equipment::GreatSword && equip.MainHandType == 2 && this.isKeyPressed(key_action2)){
			...
			{
				if(equipMain == Equipment::GreatSword && equip.MainHandType == 2 && this.isKeyPressed(key_action2))DoAttack(this, Damage, -(vec.Angle()), 180.0f, 48.0f, Hitters::sword, delta, knight);
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, f32 distance, u8 type, int deltaInt, OldKnightInfo@ info)
{
	if (!getNet().isServer())
	{
		return;
...
bool isSliding(OldKnightInfo@ knight)
{
