...
{
	ParticleAnimated("DE0.png",pos, Vec2f(0, 0.5f), 0.0f, 1.0f,1,-0.1f, false);
}

void makeLargeExplosionParticle(Vec2f pos)
{
	ParticleAnimated("DE" + (XORRandom(3)+1) + ".png",pos, Vec2f(0, 0.5f), 0.0f, 1.0f,3,-0.1f, false);
}
...
void Explode(CBlob@ this, f32 radius, f32 damage)
{
	...
	Sound::Play("splat", this.getPosition());
	Sound::Play("thud", this.getPosition());
							...
							if (canHit)
							{
								...
								if (canExplosionDamage(map, tpos, tile))
								{
									if (!map.isTileBedrock(tile))
									{
										...
										{
											this.server_HitMap(tpos, Vec2f(0,0), 1.0f, Hitters::explosion);
										}
										else
										{
											this.server_HitMap(tpos, Vec2f(0,0), 10.0f, Hitters::explosion);
										}
									}
								}
							}
			...
			if (isserver)
			{
				...
				else if (t != CMap::tile_empty && t != CMap::tile_ground_back)
				{
					if (canExplosionDamage(map, tpos, t))
					{
						...
						this.server_HitMap(tpos, Vec2f(0,0), justhurt ? 5.0f : 10.0f, Hitters::explosion);
					}
					else
					{
						damaged = true;
