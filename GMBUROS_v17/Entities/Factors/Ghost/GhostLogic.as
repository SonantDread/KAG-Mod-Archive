...
void onTick(CBlob@ this)
{
	...
	if(!this.hasTag("free")){
		...
		removeAbility(this,Ability::ShardSelf);
	} else {
		bool found = false;
		
		if(this.getPlayer() !is null){
			CBlob@[] playerBlobs;
			getBlobsByTag("soul_"+this.getPlayer().getUsername(), @playerBlobs);
			
			for (uint i = 0; i < playerBlobs.length; i++){
				CBlob@ b = playerBlobs[i];
				
				if(b.get_string("player_name") == this.getPlayer().getUsername()){
					this.Untag("free");
					this.set_u16("soul_link",b.getNetworkID());
					found = true;
					break;
				}
			}
		...
		if(!found)addAbility(this,Ability::ShardSelf);
		
		
		/*
		//Testing code that forces henry to drop his shard
		if(this.getPlayer() !is null && this.getPlayer().isBot() && !this.hasTag("test_ghost")){
	...
	if(getGameTime() % 35 == 0){
		...
		if(isNight())this.server_SetTimeToDie(15);
