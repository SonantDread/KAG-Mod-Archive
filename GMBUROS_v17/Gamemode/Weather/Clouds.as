...
// Cloud vars
const Vec2f MOVE_VEL = Vec2f(0.02,0); // Base speed
const f32 PARRALEX_EFFECT = 2.2f; // Base effect, each cloud will have a custom one based on this
const u16 CLOUD_COOLDOWN = 600;
const u16 PADDING = 200;

SColor CLOUDS_COL = color_white; // colour that changes over time

Vec2f SPAWN_VARIATION_HEIGHT = Vec2f(-200,0); // x = highest, y = lowest
...
void onTick(CRules@ this) 
{
	if (CLEAR_WIDTH_POS == 0) // Required here because map is null in onInit and onRestart
	{
		...
		SPAWN_VARIATION_HEIGHT.y = (getMap().tilemapheight * 8) / 4;
		if (isClient())
		{
