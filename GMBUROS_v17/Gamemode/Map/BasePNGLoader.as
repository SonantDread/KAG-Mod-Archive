...
class PNGLoader
{
	...
	bool loadMap(CMap@ _map, const string& in filename)
	{
		...
		if(image.isLoaded())
		{
			...
			LoadSpecialBlobs("SavedBlobs.cfg");
...
void PlaceMostLikelyTile(CMap@ map, int offset)
{
	...
	if (up != CMap::tile_empty)
	{
		...
		if ((neighborhood.find(CMap::tile_castle) != -1) ||
		    ...
		    (neighborhood.find(CMap::tile_castle_moss) != -1) ||
		    (neighborhood.find(CMap::tile_castle_back_moss) != -1))
		{
...
void SaveMap(CMap@ map, const string &in fileName)
{
	...
	for(uint i = 0; i < space; i++)
	{
		...
		if(map.isInWater(map.getTileWorldPosition(i)))
		{
			...
			{
				if(color == map_colors::tile_ground_back){
...
void getInfoFromBlob(CBlob@ this, SColor &out color, Vec2f &out offset)
{
	...
	{
		color = map_colors::lantern;
	}
	else if(name == "knightshop")color = map_colors::knight_shop;
	else if(name == "buildershop")color = map_colors::builder_shop;
	else if(name == "archershop")color = map_colors::archer_shop;
	else if(name == "boatshop")color = map_colors::boat_shop;
	else if(name == "vehicleshop")color = map_colors::vehicle_shop;
	else if(name == "quarters")color = map_colors::quarters;
	else if(name == "storage")color = map_colors::storage_noteam;
	else if(name == "barracks")color = map_colors::barracks_noteam;
	else if(name == "tunnel")color = map_colors::tunnel_noteam;
	else if(name == "workbench")color = map_colors::workbench;
	else if(name == "saw")color = map_colors::saw;
...
{
	if(HandleCustomTileSave(tile) != map_colors::unused)return HandleCustomTileSave(tile);
