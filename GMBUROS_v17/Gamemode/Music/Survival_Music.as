...
void onInit(CRules@ this){PlayMusic(this);}
void onRestart(CRules@ this){PlayMusic(this);}
void PlayMusic(CRules@ this){if(isServer())server_CreateBlob("survival_music");}
...
void AddGameMusic(CBlob@ this, CMixer@ mixer)
{
	...
	mixer.AddTrack(getFilePath(getCurrentScriptName())+"/cauldros.ogg", the_sun);
	mixer.AddTrack(getFilePath(getCurrentScriptName())+"/piotrek.ogg", world_night);
...
void GameMusicLogic(CBlob@ this, CMixer@ mixer)
{
	...
	if (timer % 2 == 0)
	{
		...
		{
			//top one tenth is windy
			if (pos.y < map.tilemapheight * map.tilesize * 0.1f)
	...
	
	{
		...
		CBlob@[] suns;
		getBlobsByName("sn", @suns);
		for (uint i = 0; i < suns.length; i++)
		{
			CBlob @sun = suns[i];
			if(sun.get_u16("radius") > 150){
				found_sun = true;
			}
		}
		
		if(found_sun && !mixer.isPlaying(the_sun)){
			changeMusic(mixer, the_sun, 5.0f, 5.0f);
		}
		
		if(!found_sun && mixer.isPlaying(the_sun)){
			mixer.FadeOut(the_sun, 5.0f);
		}
	}

	if(isNight()){
		if(!mixer.isPlaying(world_night))changeMusic(mixer, world_night, 5,60);
	} else {
		if(mixer.isPlaying(world_night)){
			mixer.FadeOut(world_night, 30.0f);
			mixer.FadeInRandom(world_home, 5.0f);
		}
		...
		{	
			...
			if(chosen == world_calm){
				CBlob@[] bases;
				getBlobsByName("bed_frame", @bases);
