		...
		if (canSpawnPlayer(p_info))
		{
			...
			CBlob@ playerBlob;
			{
				...
				else if (!war_core.rules.isMatchRunning())
				{
					...
					{
						p_info.blob_name = sv_test ? "knight" : "knight"; // force knight
...
{
	if (blob.getName() == "kamp" && oldTeam < 2)
	{
		DoUpdateTeamsLost();
	}
}
