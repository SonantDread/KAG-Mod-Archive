...
void onTick( CBlob@ this )
{
			...
			if(this.hasTag("tryReload"))
	{
			...
			if(currentTime > lastReloadTime + 250)
			{
...
void onInit( CBlob@ this )
{
	...
	if (getNet().isServer())
	{
		CBlob@ ammo = server_CreateBlob( "heavybomb" );
		if (ammo !is null)	{
			if (!this.server_PutInInventory( ammo ))
				ammo.server_Die();
		}
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	   ...
	   if (!Vehicle_AddFlipButton( this, caller))
    {
	   ...
	   if (inv.getItemsCount() == 0 && !this.hasTag("tryReload"))
					{
	   CButton@ button = caller.CreateGenericButton("$mat_heavybomb$",  Vec2f(0, 0), this, this.getCommandID("Arm Siege Bomb"), "Reload", params);
					}
    }
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	this.Tag("tryReload");
	this.Tag("ResetCurrentTime");
	this.getSprite().PlaySound("/ReloadBomb.ogg");	
