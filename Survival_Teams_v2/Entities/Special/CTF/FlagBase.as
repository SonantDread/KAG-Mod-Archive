...
void onTick(CBlob@ this)
{
	if (getNet().isServer())
	{
		...
		if (!this.hasAttached())
		{
			...
			else
			{
				...
				if(StillFlags < 2){
					CBlob@[] fg;
					getBlobsByName("archer", @fg);
					getBlobsByName("builder", @fg);
					getBlobsByName("knight", @fg);
					for(uint i = 0; i < fg.length; i++)
					{
						if(fg[i].getTeamNum() == this.getTeamNum())
						fg[i].server_Die();
	...
	for(uint i = 0; i < fg.length; i++)
	{
		if(fg[i].getPlayer() !is null){
			if(this.hasTag(fg[i].getPlayer().getUsername()))
			if(fg[i].getTeamNum() != this.getTeamNum() && fg[i].getTeamNum() > 50){
				fg[i].setPosition(this.getPosition());
			}
			if(fg[i].getTeamNum() == this.getTeamNum()){
				if(!this.hasTag(fg[i].getPlayer().getUsername()))this.Tag(fg[i].getPlayer().getUsername());
			}
		}
	}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	if (blob.getTeamNum() > 50){
		if (getNet().isServer()){
			blob.server_setTeamNum(this.getTeamNum());
			//blob.getPlayer().server_setTeamNum(this.getTeamNum());
		}
		return;
	}
