...
float drawScoreboard(CPlayer@ localplayer, CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	if (rules.hasTag("Official") && rules.exists("teamElos")){
	...
	for (u32 i = 0; i < players.length; i++)
	{
		...
		if(rules.hasTag("Official")){
			...
			GUI::DrawText("" + getStats(rules, p, "Wins"), Vec2f(bottomright.x - 320, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + getStats(rules, p, "Losses"), Vec2f(bottomright.x - 260, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + getStats(rules, p, "Elo"), Vec2f(bottomright.x - 180, topleft.y), SColor(0xffffffff));
