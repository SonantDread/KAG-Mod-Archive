...
void onCommand(CBlob@ this, u8 cmd, CBitStream@ params)
{
	if (cmd == this.getCommandID(heal_id))
	{
		...
		if (getNet().isServer())
		{
			...
			if (theBlob !is null && !getRules().get_bool("healing"))
			{
				u8 heal_amount;
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	if (getNet().isServer() && !blob.hasTag("dead") && (this.getTeamNum() == blob.getTeamNum() || this.getName() == "flowers" || (!getRules().get_bool("hearts dont collide") && this.getName() == "heart")))
	{
		Heal(blob, this);
	}
}
