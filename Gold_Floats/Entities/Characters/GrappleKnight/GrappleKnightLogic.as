	...
	if (knight.grappling)
	{
		...
		else
		{
			...
			else //stuck -> pull towards pos
			{
				...
				if (b !is null)
				{
					float mod = 1 - Maths::Min(b.getMass()*10, this.getMass()) / Maths::Max(b.getMass()*10, this.getMass());
					Vec2f resForce = Vec2f(Maths::Max(-1000, Maths::Min(force.x * mod, 1000)), Maths::Max(-1000, Maths::Min(force.y * mod, 1000)));     
					
					// print('P:T ' + mod + ':' + (1 - mod));
					// print('Force: ' + resForce.x + '; ' + resForce.y);
					// print('Highest:' + Maths::Max(b.getMass(), this.getMass()) + '; Lowest:' + Maths::Min(b.getMass(), this.getMass()));
					
					this.AddForce(resForce);
				}
				else 
					this.AddForce(force);
				...
				if (b !is null){
					float mass = Maths::Max(100, b.getMass());
					
					float mod = 1 - Maths::Min(b.getMass(), this.getMass()) / Maths::Max(b.getMass(), this.getMass());
					Vec2f resForce = Vec2f(Maths::Max(-mass, Maths::Min(-force.x * mod, mass)), Maths::Max(-mass, Maths::Min(-force.y * mod, mass)));   
					
					b.AddForce(resForce * b.getMass() / mass);
				}
			}
		}
	}
