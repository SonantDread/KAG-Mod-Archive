...
void onInit(CBlob@ this)
{
	...
	{
		Animation@ anim = sprite.addAnimation("heart arrow", 0, false);
		anim.AddFrame(16);
		if (arrowType == ArrowType::heart)
			sprite.SetAnimation(anim);
	}
	{
		Animation@ anim = sprite.addAnimation("piercing arrow", 0, false);
		anim.AddFrame(17);
		if (arrowType == ArrowType::piercing)
			sprite.SetAnimation(anim);
	}
	{
		Animation@ anim = sprite.addAnimation("frost arrow", 0, false);
		anim.AddFrame(18);
		if (arrowType == ArrowType::frost)
			sprite.SetAnimation(anim);
	}
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		if (!solid && !blob.hasTag("flesh") && !specialArrowHit(blob) &&
		        (blob.getName() != "mounted_bow" || (this.getTeamNum() != blob.getTeamNum() && !this.hasTag("heal team"))
				|| (this.getTeamNum() == blob.getTeamNum() && this.hasTag("heal team"))))
		{
			return;
		}
		...
		f32 dmg = getArrowDamage(this, vellen);
	...
	//this.server_Hit(blob, pos, bombforce, 0.0f, Hitters::water_stun_force, true);
		...
		else if (arrowType == ArrowType::heart)
		{
			if (blob.getHealth() < blob.getInitialHealth() - 0.5f) 
				this.server_Hit(blob, point1, initVelocity, -1.0f, Hitters::arrow, true);
			else 							
				blob.server_SetHealth(blob.getInitialHealth());
				
			blob.getSprite().PlaySound("Heart.ogg");
		}
		else
		{
			
			// this isnt synced cause we want instant collision for arrow even if it was wrong
			//if (!this.hasTag("hit team"))
			...
			if (dmg > 0.0f)
			{
				if (arrowType == ArrowType::fire)
					...
					this.server_Hit(blob, point1, initVelocity, dmg, Hitters::suddengib);
				else
...
bool doesCollideWithBlob(CBlob@ this, CBlob@ blob)
{
	...
	bool check = ( this.getTeamNum() != blob.getTeamNum() && !this.hasTag("heal team") )
					|| (this.getTeamNum() == blob.getTeamNum() && this.hasTag("heal team") && blob.hasTag("flesh") && blob.getHealth() < blob.getInitialHealth());
	
	CShape@ shape = blob.getShape();
	
	if (shape.isStatic() && this.hasTag("pierce") )
	{
		return false;
	}
	...
	/*if (this.hasTag("heal team") && this.getTeamNum() != blob.getTeamNum() && blob.hasTag("flesh") )
	{
		return false;
	}*/
	
	if (!shape.isStatic() && this.hasTag("heal team") && !blob.hasTag("flesh"))
	{
		return false;
	}
