...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("cycle"))  //from standardcontrols
	{
		...
		while (count < arrowTypeNames.length)
		{
			...
			if (this.getBlobCount(arrowTypeNames[type]) > 0 && canUseArrows[type])
			{
...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu @gridmenu)
{
	...
	for (int i = 0; i < arrowTypeNames.length; i++){
		if ( canUseArrows[i])
			arrowCount++;
	}
	...
	CGridMenu@ menu = CreateGridMenu(pos, this, Vec2f(arrowCount, 2), "Current arrow");
