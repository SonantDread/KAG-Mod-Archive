// Archer logic
// REMOVE STRING 724 - ... FOR ALLOWING GRAPPLE PASS THROUGH BACKWALLS
...
const int STAB_DELAY = 999999999;
const int STAB_TIME = 1;
...
void onTick(CBlob@ this)
{
	...
	if (this.get_bool("has_arrow") == false && this != null && getGameTime() % 300 == 0)
	{
		server_CreateBlob("mat_arrows", 0, this.getPosition());
	}
...
{
	if (map.isTileBackground(map.getTile(archer.grapple_pos)))
	{
		if (!map.isTilePlatform(map.getTile(archer.grapple_pos)))
		{
			if (canSend(this))
			{
				archer.grappling = false;
