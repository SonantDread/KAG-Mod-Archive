...
//	this.addCommandID("detach"); in StandardControls
	...
	this.addCommandID("menu_pick_up");
}

void onTick(CBlob@ this)
{
	if (this.isInInventory() || this.get_u8("knocked") > 0 || this.hasTag("no hands"))
	{
	...
	else
	{
		...
		if (this.isKeyJustReleased(key_pickup))
		{
			if (this.get_bool("release click"))
			{
				...
				int amount = 0;
				for(uint i = 0; i < pickupBlobs.length; i += 1)if(canBlobBePickedUp(this,pickupBlobs[i]))amount += 1;
				...
				if(this.getPlayer() is getLocalPlayer()){
					...
					CGridMenu@ menu = CreateGridMenu(getDriver().getScreenCenterPos() + Vec2f(0.0f, 0.0f), this, Vec2f(width, height), "Pick Up");
					if (menu !is null)
					{
						for(uint i = 0; i < pickupBlobs.length; i += 1)
						if(canBlobBePickedUp(this,pickupBlobs[i])){
							CBitStream params;
							params.write_u16(pickupBlobs[i].getNetworkID());
							menu.AddButton("$"+pickupBlobs[i].getName()+"$", pickupBlobs[i].getInventoryName(), this.getCommandID("menu_pick_up"),params);
						}
					}
				}
			}
...
{
	if (cmd == this.getCommandID("menu_pick_up"))
	{	
		CBlob@ pickup = getBlobByNetworkID(params.read_u16());
		if    (pickup !is null)
		{
			if(getNet().isServer())
			if(canBlobBePickedUp(this,pickup))server_Pickup(this, this, pickup);
		}
	}
}
...
f32 getPriorityPickupScale(CBlob@ this, CBlob@ b, f32 scale)
{
	...
	const string name = b.getName();
	{
		if (name == "boulder" || name == "drill" || name == "keg" ||
				...
				|| name == "axe" || name == "pick" || name == "hachet"
				|| name == "axehead" || name == "pickhead" || name == "hachethead")
		{
			scale *= 0.41f;
		}
	}
	...
	//low priority
	if (b.hasTag("player") || name == "table" || name == "stone_anvil")
	{
		scale *= 5.0f;
	}
