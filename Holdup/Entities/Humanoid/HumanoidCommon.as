...
void equipItem(CBlob @ this, CBlob @ item, string slot){
	...
	if(this !is null && item !is null){
		...
		if(equiped !is null && getNet().isServer()){
			...
			equiped.Untag(slot);
			this.set_string("equipment_"+slot+"_name","");
			equiped.Sync(slot,true);
			this.Sync("equipment_"+slot+"_name",true);
		}
	
		this.set_string("equipment_"+slot+"_name",item.getName());
		item.Tag(slot);
		item.Tag("equiptag");
...
bool isSharp(u8 customData){
	if(customData == Hitters::sword || customData == Hitters::saw || customData == Hitters::spikes || customData == Hitters::stab || customData == Hitters::bite)return true;
	return false;
}
...
void HealBody(CBlob@ this, int heal){
	...
	for(int i = 0;i < heal;i++){
		...
		if(this.get_f32("main_arm_hp")/bodyPartMaxHealth(this,"main_arm") < HP && canBeHealed(this.get_s8("main_arm_type"))){
			HP = this.get_f32("main_arm_hp")/bodyPartMaxHealth(this,"main_arm");
			limb = "main_arm";
		}
		if(this.get_f32("sub_arm_hp")/bodyPartMaxHealth(this,"sub_arm") < HP && canBeHealed(this.get_s8("sub_arm_type"))){
			HP = this.get_f32("sub_arm_hp")/bodyPartMaxHealth(this,"sub_arm");
			limb = "sub_arm";
		}
		if(this.get_f32("front_leg_hp")/bodyPartMaxHealth(this,"front_leg") < HP && canBeHealed(this.get_s8("front_leg_type"))){
			HP = this.get_f32("front_leg_hp")/bodyPartMaxHealth(this,"front_leg");
			limb = "front_leg";
		}
		if(this.get_f32("back_leg_hp")/bodyPartMaxHealth(this,"back_leg") < HP && canBeHealed(this.get_s8("back_leg_type"))){
			HP = this.get_f32("back_leg_hp")/bodyPartMaxHealth(this,"back_leg");
			limb = "back_leg";
		}

		if(this.get_f32(limb+"_hp") < bodyPartMaxHealth(this,limb)){
...
f32 bodyPartMaxHealth(CBlob@ this, string limb){
	...
	if(limb == "main_arm"){
		...
		if(MainArmType == BodyType::Hook)return 25.0f;
	...
	if(limb == "sub_arm"){
		...
		if(SubArmType == BodyType::Hook)return 25.0f;
...
f32 bodyPartGibHealth(CBlob@ this, string limb){
	...
	if(limb == "main_arm"){
		...
		if(MainArmType == BodyType::Hook)return 0;
	...
	if(limb == "sub_arm"){
		...
		if(SubArmType == BodyType::Hook)return 0;
