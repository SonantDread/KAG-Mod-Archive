	...
	if(getNet().isClient()){
		if(this.hasTag("shot")){
			if(!this.isInInventory() && !this.isAttached()){
				this.setPosition(this.get_Vec2f("shot_pos"));
				this.setVelocity(this.get_Vec2f("shot_vel"));
				this.Untag("shot");
			}	
		}
	}
...
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided") && !this.hasTag("hit_once"))
	{
