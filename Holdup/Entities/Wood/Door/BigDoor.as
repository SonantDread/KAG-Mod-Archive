...
void onTick(CBlob@ this)
{
	...
	for (uint step = 0; step < count; ++step)
	{
		...
		if(getEquippedBlob(blob,"back") !is null){
			...
			CBlob @invBlob = getEquippedBlob(blob,"back");
		...
		if(blob.getCarriedBlob() !is null && blob.getCarriedBlob().getName() == "simplekey"){
			if(blob.getCarriedBlob().get_s16("password") == this.get_s16("password"))canUnlock = true;
		}
		
		if (canUnlock && !isOpen(this))
		{
