    ...
    if (lastOnSetPlayerTick < getGameTime() && blobs.length > 0) {
        ...
        else
		{
			...
			if (!(hasPower(blob, Powers::BOUNCE) || hasPower(blob, Powers::HEALER) || hasPower(blob, Powers::STRENGTH) || hasPower(blob, Powers::VAMPIRISM) || hasPower(blob, Powers::DRAIN) ||
					hasPower(blob, Powers::SPEED) || hasPower(blob, Powers::QUICK_ATTACK) || hasPower(blob, Powers::FEATHER) ||
					hasPower(blob, Powers::TRIPLE_JUMP) || hasPower(blob, Powers::MONKEY) || hasPower(blob, Powers::FIRE_LORD) ||
					hasPower(blob, Powers::FORCE) || hasPower(blob, Powers::GHOST) || hasPower(blob, Powers::TELEPORT)))
			{
