...
shared bool checkGameOver(CRules@ this, int teamNum){
		if(teamNum==1){	
		TeamCheck(this, 1);	//if one team is dead, other wins (no consideration for more teams)
		}else if(teamNum==0){
		TeamCheck(this, 0);
		}
	return false;			//team not red or blue (probably spectator so dont want to check game over)
}
shared bool TeamCheck(CRules@ this, int team){

	string teamName = this.getTeam(team).getName();
	string teamTickets = teamName.split(" ")[0].toLower() + "Tickets";	//if one team is dead, other wins (no consideration for more teams)
	
	if(this.get_s16(teamTickets)>0) return false;
	if(isPlayersLeft(this, team)) return false;
	if(this.getCurrentState()==GAME_OVER) return true;
	this.SetTeamWon(Maths::Abs(team - 1)); //game over!
	this.SetCurrentState(GAME_OVER);
	this.SetGlobalMessage("{WINNING_TEAM} wins the game!");
	this.AddGlobalMessageReplacement("WINNING_TEAM", this.getTeam(Maths::Abs(team - 1)).getName());
	if(this.hasTag("Official")){
		officialMatchHandling( this, team);
	}
