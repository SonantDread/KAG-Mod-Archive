...
void onInit(CBlob@ this)
{
	...
	if (isServer() && rules.isWarmup())
	{
		if (XORRandom(2) == 0)
		{
			CBlob@ food = server_CreateBlob("food",-1,this.getPosition());
	        if (food !is null)
			{
				if (!this.server_PutInInventory(food))
				{
		...
		if (XORRandom(2) == 0) //
		{
	        ...
	        if (stone !is null)
			{
				...
				stone.server_SetQuantity(120);

				if (!this.server_PutInInventory(stone))
				{
	        ...
	        if (wood !is null)
			{
				...
				wood.server_SetQuantity(100);

				if (!this.server_PutInInventory(wood))
				{
		...
		else
		{
			...
			CBlob@ drill = server_CreateBlob("drill",-1,this.getPosition());
	        if (drill !is null)
			{
