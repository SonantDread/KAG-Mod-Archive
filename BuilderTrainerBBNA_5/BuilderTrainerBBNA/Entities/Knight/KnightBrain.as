...
{	
	...
	if (target !is null)
	{
		...
		if (visibleTarget && distance < 94.0f) //44.0f
		{
		...
		if (targetDistance > 76.0f && !blob.isOnWall())
		{
...
void AttackBlob(CBlob@ blob, CBlob @target)
{
	...
	if (blob.get_s16("target_slashtime") > 10)
	{
		...
		if (targetDistance < 170.0f && !isKnocked(target))
		{
	...
	if (targetDistance < 18.0f && ((getKnockedRemaining(target) > 10 && blob.getInitialHealth() != blob.getHealth()) || (getKnockedRemaining(target) > 6 && blob.getInitialHealth() == blob.getHealth())))
	{
		if (emote)
		{
	...
	else if (targetDistance < 8.5f && !target.isKeyPressed(key_action1))
	{
		...
		if (target.isKeyPressed(key_action2))
		{
	...
	if (targetDistance < 29.0f && blob.get_s16("target_slashtime") > 15 && blob.get_s16("slashtime") > 15)
	{
		blob.setKeyPressed(key_action1, false);
		Chase(blob, target);
	}
	...
	// Don't stun myself. ////Another Big Improvement!
