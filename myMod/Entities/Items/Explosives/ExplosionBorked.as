...
bool aisOwnerBlob(CBlob@ this, CBlob@ that)
{
	//easy check
...
void amakeSmallExplosionParticle(Vec2f pos)
{
...
void amakeLargeExplosionParticle(Vec2f pos)
{
...
void Explodeee(CBlob@ this, f32 radius, f32 damage)
{
	...
	amakeLargeExplosionParticle(pos);
	...
	for (int i = 0; i < radius * 0.16; i++)
	{
		...
		if (map !is null)
		{
			if (!map.rayCastSolid(pos, partpos, endpos))
				amakeSmallExplosionParticle(endpos);
		}
	}
                     ...
                     bool should_teamkill = false)
{
	...
	for (int step = 0; step <= steps; ++step)
	{
		...
		for (int width_step = 0; width_step < width_steps + 2; width_step++)
		{
			...
			if (!justhurt && (((step + width_step) % 3 == 0) || XORRandom(3) == 0)) amakeSmallExplosionParticle(tpos);
             ...
             const bool bother_raycasting = true, const bool should_teamkill = false)
{
	...
	//explosion particle
	amakeSmallExplosionParticle(hit_blob_pos);

	//hit the object
	this.server_Hit(hit_blob, hit_blob_pos,
	                bombforce, dam,
	                hitter, hitter == Hitters::water || //hit with water
	                aisOwnerBlob(this, hit_blob) ||	//allow selfkill with bombs
