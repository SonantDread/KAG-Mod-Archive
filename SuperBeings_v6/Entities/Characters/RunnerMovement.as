...
void onTick(CMovement@ this)
{
	...
	if ((blob.isOnLadder() || blob.hasTag("flying")) && !blob.isAttached() && !blob.isOnGround() && !isknocked)
	{
		...
		{
			ladderforce.y += 1.2f;
			if(blob.hasTag("flying"))ladderforce.y -= 0.2;
		}

		if (left)
		{
			ladderforce.x -= 1.0f;
			if(blob.hasTag("flying"))ladderforce.x -= 1.0f;
		}
	...
	if (blob.isInWater() && !isknocked)
	{
		...
		f32 ss = moveVars.swimspeed;
		f32 sf = moveVars.swimforce;
		if(blob.get_s16("empowerwater") > 0){
			ss *= 1.5f;
			sf *= 1.5f;
		}
		if(blob.get_s16("empowerwaterlvl2") > 0){
			ss *= 2.0f;
			sf *= 2.0f;
		}
		if(blob.get_s16("empowerwaterlvl3") > 0){
			ss *= 2.5f;
			sf *= 2.5f;
		}
		
		const f32 swimspeed = ss;
		const f32 swimforce = sf;
		const f32 edgespeed = ss * moveVars.swimEdgeScale;
