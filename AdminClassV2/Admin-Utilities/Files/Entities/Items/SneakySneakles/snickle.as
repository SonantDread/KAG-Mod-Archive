...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		if (holder.getName() == required_class || sv_gamemode == "TDM")
		{
			...
			if (inwater)
			{
				sprite.SetEmitSoundSpeed(40.0f + (getGameTime() % 13) * 0.01f);
			}
			else
			{
				sprite.SetEmitSoundSpeed(40.0f);
			}
			...
			const u8 delay_amount = inwater ? 1 : 1;
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (hi.blob !is null) // blob
							{
								...
								if (getNet().isServer())
								{
									...
									// Yield half
									Material::fromBlob(holder, hi.blob, attack_dam * 3.0f);
								}
							...
							else // map
							{
								...
								if (getNet().isServer())
								{
									map.server_DestroyTile(hi.hitpos, 15.0f, this);
									map.server_DestroyTile(hi.hitpos, 15.0f, this);
									map.server_DestroyTile(hi.hitpos, 15.0f, this);
									map.server_DestroyTile(hi.hitpos, 15.0f, this);


									Material::fromTile(holder, tile, 15.0f);
								}
