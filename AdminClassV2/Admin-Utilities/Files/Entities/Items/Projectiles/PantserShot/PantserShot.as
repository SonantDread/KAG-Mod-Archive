...
void onTick(CBlob@ this)
{
	...
	if (isClient())
	{
		...
		CParticle@ p = ParticleAnimated("Particle.png", pos, Vec2f(0,0),  0.0f, 1.0f, 1+XORRandom(3), 0.0f, false);
...
bool DoExplosion(CBlob@ this, Vec2f velocity)
{
	...
	Explode(this, 32.0f, 2.5f);
	LinearExplosion(this, velocity, 60.0f, 16.0f, 9, 4.0f, Hitters::fall);

	this.getSprite().PlaySound("/Mortarshoot");
...
void BallistaHitBlob(CBlob@ this, Vec2f hit_position, Vec2f velocity, const f32 damage, CBlob@ blob, u8 customData)
{
	...
	if (blob.getHealth() > 0.0f)
	{
		...
		if (blob.hasTag("wooden"))
		{
			...
			if (blocks_pierced < 1 && speed > 10.0f)
