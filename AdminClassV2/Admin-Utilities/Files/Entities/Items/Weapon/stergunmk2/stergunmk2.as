...
const uint8 FIRE_INTERVAL = 17; //Used 
const uint8 CLIP        = 9; //Used
const uint8 TOTAL       = 7.; //Used
const uint8 RELOAD_TIME = 30; //Used, reload timer (in ticks)
const uint8 BUL_PER_SHOT= 3; //Shots per bullet | CHANGE B_SPREAD, otherwise both bullets will come out together

//Contact vam for default values
//or keep a fresh copy

//NEW BULLET PROPS
const int8  B_SPREAD = 24; //the higher the value, the more 'uncontrolable' bullets get
const Vec2f B_GRAV   = Vec2f(0,0.015); //Bullet gravity drop \|/
const int8  B_SPEED  = 60; //Bullet speed, STRONGLY AFFECTED/EFFECTS B_GRAV
const int8  B_TTL    = 120; //TTL = Time To Live, bullets will live for 120 ticks before getting destory IF nothing has been hit
const float B_DAMAGE = 3.5; //1 heart
...
const int   B_O_DEATH= 5;
const int   B_O_COINS= 0; //Coins on hitting objects (like tanks, boulders etc)
const int   T_TO_DIE = 150; //30 * how many seconds before gun disspears if it hasnt been picked up
