...
const string rare_ore = "mat_steel";

//balance
const int input = 100;					//input cost in fuel
const int output = 50;					//output amount in ore
...
{
	CSpriteLayer@ belt = this.addSpriteLayer("belt", "steelpress.png", 32, 32);
	if (belt !is null)
	{
		//default anim
		{
		...
		//belt setup
		belt.SetOffset(Vec2f(-4.0f, -4.0f));
		belt.SetRelativeZ(1);
		belt.SetVisible(true);
	}

	CSpriteLayer@ wood = this.addSpriteLayer("wood", "FactorySteel.png", 16, 16);
	if (wood !is null)
	{
...
void onInit(CBlob@ this)
{
	...
	//commands
	this.addCommandID("add resource");
}
