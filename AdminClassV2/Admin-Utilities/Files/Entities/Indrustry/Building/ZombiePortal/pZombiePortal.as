...
void onInit(CBlob@ this)
{
  ...
  CSpriteLayer@ portal = this.getSprite().addSpriteLayer("portal", "pBloodPortal.png" , 64, 64, -1, -1);
...
void onDie(CBlob@ this)
{
  server_DropCoins(this.getPosition() + Vec2f(0, -32.0f), 1000);
  {
	int r = XORRandom(5);
	if (r == 0)
		server_CreateBlob("hayrock", this.getTeamNum(), this.getPosition());
	else if (r == 1)
		server_CreateBlob("hayrock", this.getTeamNum(), this.getPosition());
	else if (r == 2)
		server_CreateBlob("hayrock", this.getTeamNum(), this.getPosition());
		...
		server_CreateBlob("digger", this.getTeamNum(), this.getPosition());
	}
}

void onTick(CBlob@ this)
{
  //int spawnRate = 16 + (184 * this.getHealth() / 42.0); tsilliev - was glitching portals?
  int spawnRate = 16 + (4 * this.getHealth());
  ...
  if (this.get_bool("portalplaybreach")) {
    this.getSprite().PlaySound("pPortalBreach");
    this.set_bool("portalplaybreach", false);
    this.SetLight(true);
    this.SetLightRadius(64.0f);
  }
  ...
  if (this.get_bool("portalbreach")) {
    if ((getGameTime() % spawnRate == 0) && num_zombies < 100) {
      ...
      if (r == 8 && rr < 8)
        ...
        server_CreateBlob("pgregt", -1, sp);
      else if (r==6)
        server_CreateBlob("flamebag", -1, sp);
      else if (r>=3)
...
{
 this.set_bool("shop available", this.isOverlapping(caller) /*&& caller.getName() == "undeadbuilder"*/ );
}
