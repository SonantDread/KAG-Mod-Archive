...
namespace CTFCosts
{
	...
	s32 buildershop_wood, quarters_wood, knightshop_wood, archershop_wood,
		aircraftshop_wood, aircraftshop_gold, boatshop_wood, boatshop_gold, vehicleshop_wood, vehicleshop_gold,
	...
	//Aircraftshop.as
	s32 bomber, bomber_wood, basecraft, basecraft_wood, airship, airship_wood, glider,glider_wood, zeppelin, zeppelin_wood;
	
	//BoatShop.as
	s32 dinghy, dinghy_wood, longboat, longboat_wood, warboat, warboat_gold, uboot, uboot_stone, uboot_gold, caravel_wood, caravel;

	//VehicleShop.as
	s32 car, tank, batterimram, catapult, ballista, ballista_gold, ballista_ammo, ballista_bomb_ammo;
...
void InitCosts()
{
	...
	CTFCosts::aircraftshop_wood =           ReadCost(cfg, "cost_aircraftshop_wood"  , 100);
	CTFCosts::aircraftshop_gold =           ReadCost(cfg, "cost_aircraftshop_gold"  , 50);
	...
	//Aircraftshop.as
	CTFCosts::bomber =                      ReadCost(cfg, "cost_bomber"             , 350);
	CTFCosts::bomber_wood =                 ReadCost(cfg, "cost_bomber_wood"        , 100);
	CTFCosts::basecraft_wood =               ReadCost(cfg, "cost_basecraft_wood"    , 200);
	CTFCosts::basecraft =                    ReadCost(cfg, "cost_basecraft"         , 5000);
	CTFCosts::airship_wood =                ReadCost(cfg, "cost_airship_wood"       , 300);
	CTFCosts::airship =                     ReadCost(cfg, "cost_airship"            , 500);
	CTFCosts::glider_wood =                 ReadCost(cfg, "cost_glider_wood"       	, 200);
	CTFCosts::glider =                      ReadCost(cfg, "cost_glider"            	, 650);
	CTFCosts::zeppelin_wood =               ReadCost(cfg, "cost_zeppelin_wood"      , 400);
	CTFCosts::zeppelin =               		ReadCost(cfg, "cost_zeppelin"      		, 1000);
	...
	CTFCosts::uboot =                      	ReadCost(cfg, "cost_uboot"              , 350);
	CTFCosts::uboot_stone =                 ReadCost(cfg, "cost_uboot_stone"        , 350);
	CTFCosts::uboot_gold =                  ReadCost(cfg, "cost_uboot_gold"         , 150);	
	CTFCosts::caravel_wood =                ReadCost(cfg, "cost_caravel"       		, 400);
	CTFCosts::caravel =                     ReadCost(cfg, "cost_caravel"            , 350);

	//VehicleShop.as
	CTFCosts::tank =                    	ReadCost(cfg, "cost_tank"                   , 800);
	CTFCosts::car =                    		ReadCost(cfg, "cost_car"                   	, 400);
	CTFCosts::batterimram =                 ReadCost(cfg, "cost_batterimram"     		, 1500);
