...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	// tilemap check
	const bool buildSolid = (map.isTileSolid(buildTile) || (blob !is null && blob.isCollidable()) || (buildTile == CMap::tile_steel || buildTile == CMap::tile_mossy_wood || buildTile == CMap::tile_castle_gold || buildTile == CMap::tile_birk_godl));
	...
	if ((buildTile == CMap::tile_wood && backtile.type >= CMap::tile_wood_d1 && backtile.type <= CMap::tile_wood_d0) ||
			...
			//Steel block
			(buildTile == CMap::tile_steel && backtile.type >= CMap::tile_steel_d0 && backtile.type <= CMap::tile_steel_d5) ||
		
		// Mossy Wood
		
		(buildTile == CMap::tile_mossy_wood && backtile.type >= CMap::tile_mossy_wood_d0 && backtile.type <= CMap::tile_mossy_wood_d4) ||
		
		// Castle Gold
		
		(buildTile == CMap::tile_castle_gold && backtile.type >= CMap::tile_castle_gold_d0 && backtile.type <= CMap::tile_castle_gold_d5) || 
		
		// Castle Brick
		
		(buildTile == CMap::tile_birk_godl && backtile.type >= CMap::tile_birk_godl_d0 && backtile.type <= CMap::tile_birk_godl_d3))
...
void SetupBuildDelay(CBlob@ this)
{
	...
	this.set_u32("build delay", 3.5);  // move this to builder init
}
