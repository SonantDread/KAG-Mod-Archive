...
void onInit(CBlob@ this)
{
	...
	this.set_u8("override head", 30);


	}
	else{
		this.set_u8("override head", 32);
	}
}
...
void onTick(CBlob@ this)
{
	...
	bool pressed_a1 = this.isKeyJustPressed(key_action1);
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		...
		{
			//print(""+knight.swordTimer);
			knight.swordTimer++;
		}

		if (knight.state == KnightStates::sword_drawn && !pressed_a1 &&
		        !this.isKeyJustReleased(key_action1) && delta > 2.0f)
		{
			knight.state = KnightStates::normal;
		}
	...
	else if (this.isKeyJustReleased(key_action2) || this.isKeyJustPressed(key_action1) || this.get_u32("knight_timer") <= getGameTime())
	{
		knight.state = KnightStates::normal;
	}
