...
CSpriteLayer@ LoadHead(CSprite@ this, u8 headIndex)
{
	...
	if(blob.getName() == "engi")headIndex = 100;
	if(blob.getName() == "zoltan")headIndex = 101;
	if(blob.getName() == "slug")headIndex = 102;
	if(blob.getName() == "mantis")headIndex = 103;
	if(blob.getName() == "rock")headIndex = 104;
	
	if(blob.hasTag("space_suit"))headIndex = 99;
...
void onTick(CSprite@ this)
{
	...
	if(blob.hasTag("space_suit") && !blob.hasTag("changed_head")){
		this.RemoveSpriteLayer("head");
		blob.Tag("changed_head");
	}
	
	if(!blob.hasTag("space_suit") && blob.hasTag("changed_head")){
		this.RemoveSpriteLayer("head");
		blob.Untag("changed_head");
	}
