	...
	if (!has_solid_tiles)
	{
		if (isServer())
		{
	    	...
	    	for (int i = 0; i < 1+XORRandom(2); i++)
			{
		        ...
		        CBlob@ b = server_CreateBlob(randitems[XORRandom(randitems.length)],-1 + XORRandom(3),Vec2f(mapXpos, map.getLandYAtX(s32(mapXpos / map.tilesize)) * map.tilesize - 16.0f));
		        if (b.getMaxQuantity() > 1)
		        {
		            b.server_SetQuantity(50 * XORRandom(4) + 1);
		        }
	    	}
