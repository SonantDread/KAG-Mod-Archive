...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	print("Body part hit: "+BodyPartHit);
...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu @gridmenu)
{
	...
	CBlob @bag = getEquippedBlob(this,"back");
	if(bag !is null)
	bag.CreateInventoryMenu(Vec2f(gridmenu.getUpperLeftPosition().x + 0.5f * (gridmenu.getLowerRightPosition().x - gridmenu.getUpperLeftPosition().x),gridmenu.getUpperLeftPosition().y+64));
	...
	if (cmd == this.getCommandID("equip_head"))
	{
		if(this.get_string("equipment_head_name") != ""){
			...
			if(item !is null && getNet().isServer()){
				...
				this.set_string("equipment_head_name","");
				item.Sync("head",true);
				item.Sync("equipment_head_name",true);
			}
		} else {
			if(this.getCarriedBlob() !is null){
				...
				if(getEquipedType(carry.getName()) == 0){
					if(getNet().isServer()){
						...
						carry.Sync("equipment_head_name",true);
	...
	if (cmd == this.getCommandID("equip_torso"))
	{
		if(this.get_string("equipment_torso_name") != ""){
			...
			if(item !is null && getNet().isServer()){
				...
				item.Sync("equipment_torso_name",true);
			}
		} else {
			if(this.getCarriedBlob() !is null){
				CBlob @carry = this.getCarriedBlob();
				if(getEquipedType(carry.getName()) == 1){
					...
					if(getNet().isServer()){
						...
						carry.Sync("equipment_torso_name",true);
	...
	if (cmd == this.getCommandID("equip_legs"))
	{
		if(this.get_string("equipment_legs_name") != ""){
			...
			if(item !is null && getNet().isServer()){
				...
				item.Sync("legs",true);
				item.Sync("equipment_legs_name",true);
			}
		} else {
			if(this.getCarriedBlob() !is null){
				CBlob @carry = this.getCarriedBlob();
				if(getEquipedType(carry.getName()) == 2){
					if(getNet().isServer()){
						...
						carry.Sync("equipment_legs_name",true);
	...
	if (cmd == this.getCommandID("equip_main_arm"))
	{
		if(this.get_string("equipment_main_arm_name") != ""){
			...
			if(item !is null && getNet().isServer()){
				...
				item.Sync("equipment_main_arm_name",true);
			}
		} else {
			if(this.getCarriedBlob() !is null){
				CBlob @carry = this.getCarriedBlob();
				if(getEquipedType(carry.getName()) == 3){
					if(getNet().isServer()){
						...
						carry.Sync("equipment_main_arm_name",true);
	...
	if (cmd == this.getCommandID("equip_sub_arm"))
	{
		if(this.get_string("equipment_sub_arm_name") != ""){
			...
			if(item !is null && getNet().isServer()){
				...
				item.Sync("equipment_sub_arm_name",true);
			}
		} else {
			if(this.getCarriedBlob() !is null){
				CBlob @carry = this.getCarriedBlob();
				if(getEquipedType(carry.getName()) == 3){
					if(getNet().isServer()){
						...
						carry.Sync("equipment_sub_arm_name",true);
	...
	if (cmd == this.getCommandID("equip_back"))
	{
		if(this.get_string("equipment_back_name") != ""){
			...
			if(item !is null && getNet().isServer()){
				...
				item.Sync("back",true);
				item.Sync("equipment_back_name",true);
			}
		} else {
			if(this.getCarriedBlob() !is null){
				CBlob @carry = this.getCarriedBlob();
				if(getEquipedType(carry.getName()) == 4){
					if(getNet().isServer()){
						...
						carry.Sync("equipment_back_name",true);
