...
const int MAX_EGGS = 40; //maximum symultaneous eggs
const int MAX_CHICKENS = 20;
const f32 CHICKEN_LIMIT_RADIUS = 20.0f;
		...
		if (getNet().isServer())
		{
			...
			if (g_layEggInterval % 7 == 0)
			{
				...
				if (otherChicken && eggsCount < MAX_EGGS && chickenCount < MAX_CHICKENS)
				{
					...
					int randomnum=XORRandom(128);
					if(randomnum < 10)
					{
						server_CreateBlob("mat_bombs", this.getTeamNum(), this.getPosition() + Vec2f(0.0f, 5.0f));
					}
					else if(randomnum > 15 && randomnum < 30)
					{
						server_CreateBlob("mat_waterbombs", this.getTeamNum(), this.getPosition() + Vec2f(0.0f, 5.0f));
					}
					else if(randomnum > 35 && randomnum < 40)
					{
						server_CreateBlob("satchel", this.getTeamNum(), this.getPosition() + Vec2f(0.0f, 5.0f));
					}
					else if(randomnum > 50 && randomnum < 60)
					{
						server_CreateBlob("mat_bombarrows", this.getTeamNum(), this.getPosition() + Vec2f(0.0f, 5.0f));
					}
					else if(randomnum > 90 && randomnum < 100)
					{
						server_CreateBlob("mat_waterarrows", this.getTeamNum(), this.getPosition() + Vec2f(0.0f, 5.0f));
					}
					else if(randomnum > 120)
					{
						server_CreateBlob("mat_stone", this.getTeamNum(), this.getPosition() + Vec2f(0.0f, 5.0f));
					}
				}
			}
		}
