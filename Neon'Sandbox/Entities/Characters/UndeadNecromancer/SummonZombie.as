...
{
	this.set_u32("last skill", 0 );
	this.set_bool("skill ready", true );
}


void onTick( CBlob@ this ) 
{	
	bool ready = this.get_bool("skill ready");
	...
	if(ready) {
		if(this.isKeyJustPressed( key_action2 )) {
			...
			if(delta.Length() < TELEPORT_DISTANCE){
				this.set_u32("last skill", gametime);
				this.set_bool("skill ready", false );
		...
		u32 lastTeleport = this.get_u32("last skill");
		int diff = gametime - (lastTeleport + 150);
