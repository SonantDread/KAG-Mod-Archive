...
const int healTime = 100; //How much time between healings?
...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	else if (text_in == "!heal")
	{
		if(blob.get_u16("lastHealed") + healTime < getGameTime() || blob.get_u16("lastHealed") > getGameTime()){
			blob.server_Heal(5);
			blob.set_u16("lastHealed",getGameTime());
			blob.getSprite().PlaySound("ReportSound.ogg");
		} else {
			client_AddToChat("Can't heal that fast...");
		}
	}
	
	
	if(text_in == "v") server_CreateBlob('picture', -1, pos);
