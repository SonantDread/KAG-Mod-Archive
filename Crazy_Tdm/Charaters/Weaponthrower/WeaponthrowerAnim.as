...
void onTick(CSprite@ this)
{
	...
	if (!blob.get("weaponthrowerInfo", @wter))
	{
		return;
	}
	...
	else if (wter.state == WeaponthrowerStates::shieldgliding)
	{
		this.SetAnimation("shield_glide");
	}
	else if (wter.state == WeaponthrowerStates::shielddropping)
	{
		this.SetAnimation("shield_drop");
	}
	else if (wter.state == WeaponthrowerStates::shielding && isShieldEnabled(blob))
	{
		if (walking)
		{
			if (direction == 0)
	...
	else if (wter.state == WeaponthrowerStates::resheathing_throw)
	{
		this.SetAnimation("resheath_throw");
	}
	else if (wter.state == WeaponthrowerStates::weapon_drawn)
	{
		if (wter.weaponTimer < WeaponthrowerVars::throw_charge)
		{
			this.SetAnimation("draw_weapon");
		}
		else if (wter.weaponTimer < WeaponthrowerVars::throw_charge_level2)
		{
			if(wter.weapon_type == WeaponType::chakram)
				this.SetAnimation("chakram_power_ready");
			else
				this.SetAnimation("boomerang_power_ready");
			this.animation.frame = 0;
		}
		else if (wter.weaponTimer < WeaponthrowerVars::throw_charge_limit)
		{
			if(wter.weapon_type == WeaponType::chakram)
				this.SetAnimation("chakram_power_ready");
			else
	...
	else if (wter.state == WeaponthrowerStates::weapon_throw || wter.state == WeaponthrowerStates::weapon_throw_super)
	{
		if(wter.weapon_type == WeaponType::chakram)
