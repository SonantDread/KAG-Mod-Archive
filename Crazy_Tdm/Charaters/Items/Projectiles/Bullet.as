...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		HitInfo@[] hitInfos;
		if(this.getMap().getHitInfosFromRay(this.getPosition(), -angle, 24.0f, this, @hitInfos))
		{
			for (uint i = 0; i < hitInfos.length; i++)
			{
				HitInfo@ hi = hitInfos[i];
				CBlob@ b = hi.blob;
				if (b !is null && doesCollideWithBlob(this, b) && b.doesCollideWithBlob(this)) // blob
				{
					ShapePlatformDirection@ plat = b.getShape().getPlatformDirection(0);
					//it sometimes get issue that hit in passable angle
					bool collidable = false;
					if(plat !is null)
					{
						Vec2f hitDirPos = b.getPosition() - this.getPosition();
						hitDirPos.y *= -1;
						float hitDir = hitDirPos.Angle();
						float platDir = b.getAngleDegrees() + plat.direction.Angle();
						float platMaxAngle = platDir + plat.angleLimit / 2;
						float platMinAngle = platDir - plat.angleLimit / 2;
						if(platMinAngle < 0)
						{
							collidable = hitDir <= platMaxAngle || hitDir >= platMinAngle + 360;
						}
						else if(platMaxAngle >= 360)
						{
							collidable = hitDir <= platMaxAngle - 360 || hitDir >= platMinAngle;
						}
						else
						{
							collidable = platMinAngle <= hitDir && hitDir <= platMaxAngle;
						}
					}
					else collidable = true;
					if(collidable)//(plat is null)// non platform
					{
						onCollision(this, b, b.isCollidable(), Vec2f(0,0), this.getPosition());
					}
				}
			}
		}
...
void Pierce(CBlob @this, CBlob@ blob = null)
{
	...
	{
		BulletHitMap(this, end, this.getOldVelocity(), 0.5f, Hitters::shield);
	}
	else if(shape.vellen < 30.0f)
	{
		this.server_Die();
	}
}
