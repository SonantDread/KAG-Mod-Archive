...
shared class DuelistState
{
	...
	bool TickState(CBlob@ this, DuelistInfo@ duel, RunnerMoveVars@ moveVars) { return false; }
	void StateExited(CBlob@ this, DuelistInfo@ duel, u8 next_state) {}
}
...
{
	DuelistInfo@ duel;
	if (!this.get("duelistInfo", @duel)) { return; }

	CBitStream bt;
	bt.write_bool(duel.grappling);

	if (duel.grappling)
	{
		bt.write_u16(duel.grapple_id);
		bt.write_u8(u8(duel.grapple_ratio * 250));
		bt.write_Vec2f(duel.grapple_pos);
		bt.write_Vec2f(duel.grapple_vel);
	}
...
void HandleGrapple(CBlob@ this, CBitStream@ bt, bool apply)
{
	...
	if (apply)
	{
		duel.grappling = grappling;
		if (duel.grappling)
		{
			duel.grapple_id = grapple_id;
			duel.grapple_ratio = grapple_ratio;
			duel.grapple_pos = grapple_pos;
			duel.grapple_vel = grapple_vel;
		}
	}
}
