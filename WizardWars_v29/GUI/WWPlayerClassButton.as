...
class WWPlayerClassButton 
{
	...
	Icon@ rarity, display;
	Button@ classButton, swapButton, unlockButton;
	...
	WWPlayerClassButton(string _name, string _desc, string _configFilename, int _cost, string _imageName, int _icon, int _rarity, string _modName, Vec2f _pos, int _size)
	{
		name = _name;
		modName = _modName;
		description = _desc;
		...
		@swapButton = @Button(Vec2f(100,-24), Vec2f(150,24), "Respawn as "+_name, SColor(255,255,255,255));
		swapButton.name = _configFilename;
		classFrame.addChild(swapButton);
		swapButton.addClickListener(SwapButtonHandler);
		
		@unlockButton = @Button(Vec2f(280,-24), Vec2f(200,24), "Unlock for: "+_cost+" credits", SColor(255,255,255,255));
	...
	bool checkUnlocked()
	{
		if (getNet().isClient())
		{
			string configstr = "../Cache/"+modName+"_Classes"+classesVersion+".cfg";
	...
	void Unlock(){
		if (getNet().isClient())
		{
			...
			ConfigFile cfg = ConfigFile( "../Cache/"+modName+"_Classes"+classesVersion+".cfg" );
			cfg.add_bool(name, true);
			cfg.saveFile(modName+"_Classes"+classesVersion+".cfg");
		}
	}
...
class WWPlayerClassButtonList : GenericGUIItem
{
	...
	WWPlayerClassButtonList(Vec2f _position,Vec2f _size,int _style){
		...
		rules.addCommandID("unlockClass");
		rules.addCommandID("requestClasses");
		rules.addCommandID("sendClasses");
		playerChooser.setCurrentItem("Your Classes");
	}

	void registerWWPlayerClassButton(string _name, string _desc, string _configFilename, int _cost, int _icon = 0, int _rarity = 0,string _modName = "Default", 
		string _imageName = "GUI/ClassIcons.png", int _size = 1)
	{
		WWPlayerClassButton@ classButton = @WWPlayerClassButton(_name, _desc, _configFilename, _cost, _imageName, _icon, _rarity, _modName, position, _size);
	...
	void unlockByName(string _name){
		...
		for(int i = 0; i < list.length; i++){
			if(list[i].name == _name && !list[i].checkUnlocked()){
				...
				rules.SendCommand(rules.getCommandID("unlockClass"),params);
			}
		}
	}
	...
	void renderSmall()
	{
		...
		for(i; i < list.length; i++){
			...
			if (!list[i].gained) GUI::DrawRectangle(list[i].classButton.position, list[i].classButton.position+list[i].classButton.size,SColor(230,150,150,150));
			counterV++;
		}
...
void intitializeClasses()
{
	...
	playerClassButtons.registerWWPlayerClassButton("Wizard", 
													"     A versatile magic-weilding class with a wide variety of spells at their disposal. Harnesses the power of nature to defeat foes and assist allies." +
													"\n\n     Health: 80" +
													"\n     Mana: 100" +
													"\n     Mana Regen: 3 mana/sec", 
													"wizard", 0, 2, 5, "WizardWars");
	...
	playerClassButtons.registerWWPlayerClassButton("Knight", 
													...
													"knight", 1000, 0, 0, "WizardWars");

	playerClassButtons.registerWWPlayerClassButton("Archer", 
													"     Choose me if you want to get rekt!" +
													"\n\n     Health: 90" +
													"\n     Mana: 0" +
													"\n     Mana Regen: 0 mana/sec",
													"archer", 2000, 1, 0, "WizardWars");
}
...
{ 
	...
	if (!localPlayer.get( "hotbarInfo", @hotbarInfo )) 
	{
		return;
	}
	...
	for(int c = 0; c < playerClassButtons.list.length; c++)
	{
		...
		for(int s = 0; s < playerClassButtons.list[c].spellButtons.length; s++)
		{
			...
			if ( sButton.name == sender.name )
			{
				SetCustomSpell(localPlayer, s);
			
				if ( sButton.toggled == false )
					Sound::Play( "MenuSelect2.ogg" );
				
				sButton.toggled = true;
				
				Spell sSpell = WizardParams::spells[s];
	...
	for(int i = 0; i < playerClassButtons.list.length; i++)
	{
		...
		if ( iButton.toggled == true )
		{
			if ( iButton.name == "wizard" )
			{
				...
				bool canCustomizeHotbar = controls.mousePressed1;
				...
				for (uint i = 0; i < 15; i++)	//only 15 total spells held inside primary hotbar
				{
					...
					if ( i < 5 )		//spells 0 through 4
					{
						...
						{
							assignHotkey(localPlayer, i, hotbarInfo.customSpellID, "wizard");
							hotbarClicked = true;
						}			
					}
				...
				GUI::DrawText("Primary Spell - LMB", primaryPos + Vec2f(0,-32), color_white );
				...
				{
					assignHotkey(localPlayer, 15, hotbarInfo.customSpellID, "wizard");	//hotkey 15 is the secondary fire hotkey
					hotbarClicked = true;
				}
				...
				{
					assignHotkey(localPlayer, 16, hotbarInfo.customSpellID, "wizard");	//hotkey 16 is the auxiliary1 fire hotkey
					hotbarClicked = true;
				}
