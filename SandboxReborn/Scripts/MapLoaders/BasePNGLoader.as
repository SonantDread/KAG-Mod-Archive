...
class PNGLoader
{
	...
	bool loadMap(CMap@ _map, const string& in filename)
	{
		...
		getRules().set_bool("blobs created", false);
		@map = _map;
		@map_random = Random();
...
/////////////////////////////
/*
		//print("loaded froooom "+filename);
		ConfigFile cfg = ConfigFile();
		string filenameonly = filename;//getFilenameWithoutExtension(filename);
		//print("loaded froooom2 "+filenameonly);
		string cfgFileName = "danger-test/"+filenameonly+".cfg";

		if (cfg.loadFile("../Cache/" + cfgFileName))
		{
			u32 blob_count = 5; //= cfg.read_u32("blob_count");
			...
			for(u32 i = 0; i < blob_count; i++)
			{
				...
				if(i == blob_count) //print("all extra blobs loaded!");
			}


		}
		else(error("no file exists"));*/
/////////////////////////////
	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		if(alpha < 255)
		{
			...
			{
				CBlob@ blob = spawnBlob(map, "keg", 0, position, 0, false);
				offsets[autotile_offset].push_back(offset);
			}
			// MECHANISMS
			else if(rgb == lever)
		...
		{
			spawnBlob(map, "spawnpoint", offset, 0, false);
			offsets[autotile_offset].push_back( offset );
		}
...
void SaveMap(CMap@ map, const string &in fileName)
{
	...
	//print("saved toooooo "+fileName);
	ConfigFile cfg = ConfigFile();
	string filenameonly = getFilenameWithoutExtension(fileName);
	string cfgFileName = "sbr/Maps/"+filenameonly+".cfg";

	cfg.saveFile(cfgFileName);
	cfg.loadFile("../Cache/" + cfgFileName);
	CBlob@[] all;
	getBlobs(@all);
	cfg.add_u32("blob_count", all.length);
	for(u32 i = 0; i < all.length; i++)
	{
		CBlob@ blob = all[i];
		if(blob !is null)
		{
			cfg.add_f32("pos_x"+i, blob.getPosition().x);
			cfg.add_f32("pos_y"+i, blob.getPosition().y);
			cfg.add_f32("vel_x"+i, blob.getVelocity().x);
			cfg.add_f32("vel_y"+i, blob.getVelocity().y);
			cfg.add_s32("team"+i, blob.getTeamNum());
			cfg.add_f32("angle"+i, blob.getAngleDegrees());
			cfg.add_u32("health"+i, blob.getHealth());
			cfg.add_string("name"+i, blob.getName());
			
			if(blob.getPlayer() !is null)
			{
				CPlayer@ player = blob.getPlayer();
				cfg.add_string("player"+i, player.getUsername());
			}
		}
	}
