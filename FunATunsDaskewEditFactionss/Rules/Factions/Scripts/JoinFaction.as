...
bool onServerProcessChat( CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player )
{
    ...
    if(args.length > 0)
    {
        ...
        else if(args.length > 1 && args[0] == "!invite")
        {
            ...
            if(fact !is null && (fact.leader == player.getUsername() || fact.moderators.find(player.getUsername()) != -1) && fact.members.length < 6)
            {
            ...
            {
              send_chat(this, player, "You are not an admin in this faction or there are too many people in it. (Max 5)", SColor(255, 255, 0, 0) );
            }
        }
        else if(args[0] == "!join")
        {
