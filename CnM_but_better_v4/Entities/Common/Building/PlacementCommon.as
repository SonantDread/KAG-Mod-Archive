...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	// tilemap check
	const bool buildSolid = (map.isTileSolid(buildTile) || (blob !is null && blob.isCollidable()) || (buildTile >= CMap::tile_goldbrick && buildTile <= CMap::tile_goldbrick_d0));
	...
	const bool backSolid = (map.isTileSolid(backtile.type) || (backtile.type >= CMap::tile_goldbrick && backtile.type <= CMap::tile_goldbrick_d0));
	...
	if (blob is null && buildTile == map.getTile(offset).type)
	{
		sameTile = true;
		return false;
	}
	...
	else if (buildTile == CMap::tile_ladder_n && (backtile.type >= CMap::tile_ladder_n && backtile.type <= CMap::tile_ladder_wood_n))
	{
		return false;
	}
	else if ((buildTile >= CMap::tile_fake_dirt && buildTile <= CMap::tile_fake_goldbrick) && (backtile.type >= CMap::tile_fake_dirt && backtile.type <= CMap::tile_fake_goldbrick))
	{
		return false;
	}
	else if (backSolid || map.hasTileSolidBlobs(backtile))
	{
		if (!buildSolid && !map.hasTileSolidBlobsNoPlatform(backtile) && !backSolid)
		{
