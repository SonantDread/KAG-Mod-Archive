...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info)
{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					bool gold_brick = hi.tile >= CMap::tile_goldbrick && hi.tile <= CMap::tile_goldbrick_d0;
					bool ladder_tile = hi.tile >= CMap::tile_ladder_n && hi.tile <= CMap::tile_ladder_wood_n;
					bool wood = map.isTileWood(hi.tile);
					if (ground || wood || dirt_stone || gold || gold_brick)
					{
	...
	if(deltaInt == DELTA_BEGIN_ATTACK + 1) // hit ladders or background
	{
		bool slash = damage > 1.0f;
		float real_radius = radius*(slash ? 1.8f : 1.6f);
		Vec2f aim_pos = this.getAimPos();
		Vec2f aim_vec = aim_pos-blobPos;
		if(aim_vec.Length() > real_radius)
		{
			aim_vec.Normalize();
			aim_vec *= real_radius;
		}
		Vec2f end_pos = blobPos+aim_vec;
		if(!map.rayCastSolid(blobPos, end_pos)) // if not hit solid
		{
			Tile tile = map.getTile(end_pos);
			if(tile.type == CMap::tile_wood_back || tile.type == 207 || (tile.type >= CMap::tile_ladder_n && tile.type <= CMap::tile_ladder_wood_n) || (tile.type >= CMap::tile_fake_dirt && tile.type <= CMap::tile_fake_goldbrick))
			{
				map.server_DestroyTile(end_pos, damage, this);
			}
		}
	}
	...
	/*if(deltaInt == DELTA_BEGIN_ATTACK + 1) // hit ladders
	{
		f32 tilesize = map.tilesize;
		int steps = Maths::Ceil(2 * radius / tilesize);
		int sign = this.isFacingLeft() ? -1 : 1;
		int sign_y = this.getAimPos().y > this.getPosition().y ? 1 : -1;

		for (int y = 0; y < steps; y++)
		{
			for (int x = 0; x < steps; x++)
			{
				Vec2f tilepos = blobPos + Vec2f(x * tilesize * sign, y * tilesize * sign_y);
				TileType tile = map.getTile(tilepos).type;

				if (tile >= CMap::tile_ladder_n && tile <= CMap::tile_ladder_wood_n)
				{
					map.server_DestroyTile(tilepos, damage, this);
