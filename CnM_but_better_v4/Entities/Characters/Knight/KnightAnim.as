...
void onTick(CSprite@ this)
{
	...
	const bool onladder = blob.get_bool("on_ladder");
	bool inair = (!blob.isOnGround() && !onladder);
	...
	else if (inair)
	{
		...
		else
		{
			...
			else
			{
	         ...
	         (onladder && (blob.isKeyPressed(key_up) || blob.isKeyPressed(key_down))))
	{
		this.SetAnimation("run");
	}
	else
...
void onRender(CSprite@ this)
{
	...
	if (blob.isKeyPressed(key_action1))
	{
		...
		if ((map.isTileSolid(tile) || map.isTileGrass(tile.type) || tile.type == CMap::tile_wood_back || tile.type == 207 || (tile.type >= CMap::tile_ladder_n && tile.type <= CMap::tile_ladder_wood_n) || (tile.type >= CMap::tile_fake_dirt && tile.type <= CMap::tile_fake_goldbrick)) && map.getSectorAtPosition(surface_position, "no build") is null && distance < 16.0f)
		{
			DrawCursorAt(surface_position, cursorTexture);
		}
	}

	/*if (blob.isKeyPressed(key_action1)) // later
	{
		CMap@ map = blob.getMap();
		Vec2f position = blob.getPosition();
		Vec2f cursor_position = blob.getAimPos();
		...
		if(map.getSectorAtPosition(surface_position, "no build") is null)
		{
			if ((map.isTileSolid(tile) || map.isTileGrass(tile.type)) && distance < 16.0f)
			{
				DrawCursorAt(surface_position, cursorTexture);
			}
			else if(tile.type == CMap::tile_wood_back || (tile.type >= CMap::tile_ladder_n || tile.type <= CMap::tile_ladder_wood_n) || (tile.type >= CMap::tile_fake_dirt || tile.type <= CMap::tile_fake_goldbrick) && distance < 24.0f)
			{
				DrawCursorAt(surface_position, cursorTexture);
			}
		}
