...
void onTick(CMovement@ this)
{
	...
	float rad = blob.getRadius();
	float half_rad = rad*Maths::Cos(0.25*Maths::Pi);
	Vec2f[] ladder_check = {Vec2f(0, 0), Vec2f(0, rad), Vec2f(rad, 0), Vec2f(-rad, 0), Vec2f(0, -rad), Vec2f(-half_rad, -half_rad), Vec2f(half_rad, -half_rad), Vec2f(half_rad, half_rad), Vec2f(-half_rad, half_rad)};

	for(int i = 0; i < ladder_check.size(); i++)
	{
		Vec2f check_pos = b_pos+ladder_check[i];
		//CParticle@ pix = ParticlePixel(check_pos, Vec2f_zero, 0xFF20AA20, true, 2); if(pix !is null) {pix.gravity = Vec2f_zero;}
		if(map.hasTileFlag(map.getTileOffset(check_pos), Tile::LADDER))
		{
			onladder = true;
			break;
		}
	...
	blob.set_bool("on_ladder", onladder);

	bool onground = blob.isOnGround() || onladder;

	/*if(!onground) // try notch climbing
	{
		if(left && !right)
		{
			Vec2f notch_pos = b_pos + Vec2f(-7.8f, 4.7f);
			Vec2f player_corner_pos = b_pos + Vec2f(-7.8, -2.0f);
			//CParticle@ pix = ParticlePixel(notch_pos, Vec2f_zero, 0xFF20AA20, true, 2); if(pix !is null){pix.gravity = Vec2f_zero;}
			//CParticle@ pix2 = ParticlePixel(player_corner_pos, Vec2f_zero, 0xFFAA2020, true, 2); if(pix2 !is null){pix2.gravity = Vec2f_zero;}
			if(map.isTileSolid(notch_pos) && !map.isTileSolid(player_corner_pos)) onground = true;
		}
		else if(right && !left)
		{
			Vec2f notch_pos = b_pos + Vec2f(7.8f, 4.7f);
	...
	if (onladder && !blob.isAttached() && !blob.isOnGround() && !isknocked)
	{
	...
	if (!blob.isOnCeiling() && !isknocked &&
	        !onladder && (up || left || right))  //key pressed
	{
		//check solid tiles
	...
	bool left_or_right = (left || right);
	{
		...
		bool stand = blob.isOnGround() || onladder;
		Vec2f walkDirection;
