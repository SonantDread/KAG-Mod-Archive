...
bool MouseTalk(string&in text_to_say, string&out text_out, CPlayer@ player)
{
	CPlayer@ local_player = getLocalPlayer();
	if(local_player is null) return true;
	CBlob@ local_blob = local_player.getBlob();
	CBlob@ blob = player.getBlob();
	if(blob !is null && local_blob !is null)
	{
		if(local_blob !is blob)
		{
			if(local_blob.getTeamNum() == 0 && blob.getTeamNum() == 1)
			{
				text_out = "*squeak*"; // lol
				return true;
			}
	...
	text_out = text_to_say;
	return true; // dont change if other
}
...
bool onClientProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	return MouseTalk(text_in, text_out, player);
}
