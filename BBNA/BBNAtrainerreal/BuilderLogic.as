...
void onInit(CBlob@ this)
{
	...
	if (XORRandom(4) == 0)
	{
		this.server_Hit(this, this.getPosition(), Vec2f(0, 0), 1.0f, Hitters::drown, true);
	}
	if (XORRandom(4) == 0)
	{
		this.server_Hit(this, this.getPosition(), Vec2f(0, 0), 0.5f, Hitters::drown, true);
	}
	if (XORRandom(5) == 0)
	{
		this.server_Hit(this, this.getPosition(), Vec2f(0, 0), 0.5f, Hitters::drown, true);
	...
	if (XORRandom(6) == 0)
	{
		CBlob@ food = server_CreateBlob(XORRandom(4) == 0 ? "grain" : "food", this.getTeamNum(), this.getPosition());
		if (food !is null)
		{
	...
	if (XORRandom(3) != 0)
	{
		CBlob@ drill = server_CreateBlob("drill",this.getTeamNum(),this.getPosition());
        if (drill !is null)
		{
			drill.setPosition(this.getPosition());
			this.server_Pickup(@drill);
		}
	}
	...
	{
		//more mat
		CBlob@ blob = server_CreateBlobNoInit('mat_stone');
		...
		if (blob !is null)
		{
			...
			blob.server_SetQuantity(30*(XORRandom(9)+1));
			blob.setPosition(this.getPosition());
		}

		CBlob@ blob2 = server_CreateBlobNoInit('mat_wood');

		if (blob2 !is null)
		{
			blob2.Tag('custom quantity');
			blob2.Init();

			blob2.server_SetQuantity(100*(XORRandom(7)+1));
			blob2.setPosition(this.getPosition());
		}
	}
	else
	{
