...
void Config(TDMCore@ this)
{
	...
	s32 warmUpTimeSeconds = cfg.read_s32("warmUpTimeSeconds", 1);
	...
	f32 gameDurationMinutes = cfg.read_f32("gameDurationMinutes", 1.0f);
	...
	rules.set_f32("fall vel modifier", cfg.read_f32("fall_dmg_nerf", 1.0f));
}
...
shared class TDMCore : RulesCore
{
	...
	void Update()
	{
		...
		{
			rules.SetGlobalMessage("Starting in {SEC}");
		...
		if ((rules.isIntermission() || rules.isWarmup()) && (!allTeamsHavePlayers()))  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
			...
			rules.SetGlobalMessage("Please wait for someone to join...");
			tdm_spawns.force = true;
		}
	...
	void CheckTeamWon()
	{
		...
		if (sudden_death)
		{
			...
			if (teams_alive_count == 0)
			{
				...
				rules.SetGlobalMessage("The match is a tie!");
				return;
			}
		}
		...
		if (winteamIndex >= 0)
		{
				...
				for (uint i = 0; i < players.length; i++)
				{
					...
					if (player !is null)
					{
						if (player.getTeamNum() == winteamIndex)
						{
							...
							if (player.getUsername() == "Yeti5000707" || player.getUsername() == "FrothyFruitbat")
							{
								BBNAfactor += 1;
							}
							else
			...
			getNet().server_SendMsg("******************************************************************** *");

			// Round end
			rules.SetTeamWon(winteamIndex);
			rules.SetCurrentState(GAME_OVER);
			if (BBNAfactor >= 2) //2
			{
				rules.SetGlobalMessage("!BBNA WINS YET ANOTHER ROUND! !BBNA WINS YET ANOTHER ROUND! !BBNA WINS YET ANOTHER ROUND!");
			}
			else
			{
				if      (XORRandom(10) == 0){rules.SetGlobalMessage("{WINNING_TEAM} wins the match. GG!");}
				else if (XORRandom(10) == 0){rules.SetGlobalMessage("{WINNING_TEAM} wins the match. GG WP!");}
				else if (XORRandom(10) == 0){rules.SetGlobalMessage("{WINNING_TEAM} wins the match. gg no re!");}
				else                        {rules.SetGlobalMessage("{WINNING_TEAM} wins the match.");}
...
void Reset(CRules@ this)
{
	...
	this.set_s32("restart_rules_after_game_time", (core.spawnTime < 0 ? 2 : 3) * 1 ); // 23
	...
	if (XORRandom(3) == 0)
	{
		...
		{
			f32 mapXpos = XORRandom(mapWidth);
			CBlob@ b = server_CreateBlob("knightbot",0 + XORRandom(2),Vec2f(mapXpos, map.getLandYAtX(s32(mapXpos / map.tilesize)) * map.tilesize - 16.0f));
		}
	}
	...
	if (!has_solid_tiles)
	{
		if (isServer())
		{
			if (XORRandom(5) == 0)
			{
				CBlob@ b = server_CreateBlob(XORRandom(2) == 0 ? "buildershop" : "quarters");
				if (b !is null)
				{			
					b.setPosition(getSpawnLocation());
					b.server_setTeamNum(b.getPosition().x < 64 ? 0 : 1);
				}
			}
	    	...
	    	for (int i = 0; i < 1+XORRandom(2); i++)
			{
		        ...
		        server_CreateBlob(randitems[XORRandom(randitems.length)],-1 + XORRandom(3),Vec2f(mapXpos, map.getLandYAtX(s32(mapXpos / map.tilesize)) * map.tilesize - 16.0f));
	    	}
	    	...
	    	if (XORRandom(3) != 0)
			{
				CMap@ map = getMap();
				f32 mapWidth = (map.tilemapwidth * map.tilesize);	
		    	for (int i = 0; i < 1+XORRandom(3); i++)
				{
					f32 mapXpos = XORRandom(mapWidth);

			        CBlob@ b = server_CreateBlob(classes[XORRandom(classes.length)],0 + XORRandom(2),Vec2f(mapXpos, map.getLandYAtX(s32(mapXpos / map.tilesize)) * map.tilesize - 16.0f));
			        if (b !is null)
					{		
						b.SetFacingLeft(XORRandom(2)==0 ? true : false);
						if (getNet().isServer())
						{
							b.server_Hit(b, b.getPosition(), Vec2f_zero, 4.0f,  Hitters::drown);
						}

						if (XORRandom(3) == 0)
						{
							server_CreateBlob("heart",-1,b.getPosition());
						}
					}
		    	}
	    	}
			...
			if (XORRandom(4) != 0)
			{
				for (uint i = 0; i < XORRandom(2)+1; i++)
				{
					...
					{			
						...
						b.setPosition(Vec2f(XORRandom(mapWidth), -mapHeight));
						b.setVelocity(Vec2f((XORRandom(70) - 35)*0.3f, (XORRandom(20) - 10)*0.2f));
					}
				}
			}
			else
			{
				...
				{			
					...
					{
						b.setPosition(Vec2f(XORRandom(mapWidth), -mapHeight));
						b.setVelocity(Vec2f((XORRandom(70) - 35)*0.5f, (XORRandom(20) - 30)*0.5f));
					}
					else
					{
						f32 mapXpos = XORRandom(mapWidth);

						b.setPosition(Vec2f(mapXpos, map.getLandYAtX(s32(mapXpos / map.tilesize)) * map.tilesize - 16.0f));
					}
...
void DecayStuff()
{
	...
	if (XORRandom(3) != 0)
	{
		for (int i = 0; i < XORRandom(400)+50; i++)
		{
			...
			{
				//map.server_DestroyTile(Vec2f(hit.x + (XORRandom(4) - 2) * 8, hit.y + (XORRandom(4) - 2) * 8), 1.0f);
				map.server_DestroyTile(Vec2f(hit.x + (XORRandom(4) - 2) * 3, hit.y + (XORRandom(4) - 2) * 3), 0.5f);
			}
		}
	}
	...
	if (XORRandom(3) != 0)
	{
		for (int i = 0; i < XORRandom(70)+5; i++)
		{
			pos = Vec2f(XORRandom(map.tilemapwidth * map.tilesize), 0);
			...
			if (map.rayCastSolidNoBlobs(pos, Vec2f(pos.x, map.tilemapheight * map.tilesize), hit))
			{
				...
				if (XORRandom(4) == 0)
				{
					map.server_DestroyTile(Vec2f(hit.x + (XORRandom(4) - 2) * 4, hit.y + (XORRandom(4) - 2) * 4), 60.0f);
				}
			}
		}
	}
