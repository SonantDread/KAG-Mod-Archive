...
float drawScoreboard(CPlayer@ localplayer, CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	if (rules.exists("Official") && rules.exists("teamElos")){
		int teamEloIndex = (team.getName() == "Blue Team" ? 0 : 1);
		int[] teamElos = teamEloGet(rules);

		GUI::DrawText(getTranslatedString("Team Elo: {eloCount}").replace("{eloCount}", "" + teamElos[teamEloIndex]), Vec2f(bottomright.x - 200, topleft.y), SColor(0xffffffff));
	}
	...
	if(rules.exists("Official")){
		GUI::DrawText(getTranslatedString("Player"), Vec2f(topleft.x, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Username"), Vec2f(bottomright.x - 680, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Ping"), Vec2f(bottomright.x - 560, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Kills"), Vec2f(bottomright.x - 480, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Deaths"), Vec2f(bottomright.x - 400, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Wins"), Vec2f(bottomright.x - 320, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Losses"), Vec2f(bottomright.x - 260, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Elo"), Vec2f(bottomright.x - 180, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("KDR"), Vec2f(bottomright.x - 100, topleft.y), SColor(0xffffffff));
		GUI::DrawText(getTranslatedString("Accolades"), Vec2f(bottomright.x - 960, topleft.y), SColor(0xffffffff));
	}
	if(draw_age)
	{
	...
	for (u32 i = 0; i < players.length; i++)
	{
		...
		if(rules.exists("Official")){
			//float CaptainWL = (getStats(rules, p, "Losing_Captain")  == 0) ? (getStats(rules, p, "Winning_Captain") / 1): (getStats(rules, p, "Winning_Captain") / getStats(rules, p, "Losing_Captain"));
			GUI::DrawText("" + username, Vec2f(bottomright.x - 680, topleft.y), namecolour);
			GUI::DrawText("" + ping_in_ms, Vec2f(bottomright.x - 560, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + p.getKills(), Vec2f(bottomright.x - 480, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + p.getDeaths(), Vec2f(bottomright.x - 400, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + getStats(rules, p.getUsername(), "Wins"), Vec2f(bottomright.x - 320, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + getStats(rules, p.getUsername(), "Losses"), Vec2f(bottomright.x - 260, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + getStats(rules, p.getUsername(), "Elo"), Vec2f(bottomright.x - 180, topleft.y), SColor(0xffffffff));
			GUI::DrawText("" + formatFloat(getKDR(p), "", 0, 2), Vec2f(bottomright.x - 100, topleft.y), SColor(0xffffffff));
		}
	}
...
void onRenderScoreboard(CRules@ this)
	...
	if (this.exists("Official")){
		drawStatCard(hoveredPlayer, hoveredPos);
	}
