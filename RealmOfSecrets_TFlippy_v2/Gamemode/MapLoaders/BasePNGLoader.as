...
const string[] desertMaps = 
{ 
	"TFlippy_Barrens_v6.png",
	"TFlippy_Barrens_v3.png",
	"TFlippy_Barrens_v1.png",
	"OldTower.png",
	"AncientSecrets.png",
};

// const string[] jungleMaps = 
// { 
	// "ClamberingIsles.png",
	// "TFlippy_UncleBen_v5.png"
// };
...
class PNGLoader
{
	...
	bool loadMap(CMap@ _map, const string& in filename)
	{
		...
		string mapname = spl[spl.length - 1];
		
		bool isDesert = false;
		for (int i = 0; i < desertMaps.length; i++)
		{
			if (mapname == desertMaps[i]) isDesert = true;
		}
	
		// print("map is " + mapname + "; is desert: " + isDesert);
		
		if(!getNet().isServer())
		{
			SetupMap(0, 0, isDesert);
			SetupBackgrounds(isDesert);
		...
		if(image.isLoaded())
		{
			SetupMap(image.getWidth(), image.getHeight(), isDesert);
			SetupBackgrounds(isDesert);
	...
	void SetupMap(int width, int height, bool isDesert)
	{
		if (isDesert)
		{
			map.CreateTileMap(width, height, 8.0f, "Desert_world.png");
		}
		else
		{
	...
	void SetupBackgrounds(bool isDesert)
	{	
		if (isDesert)
		{
