...
void onTick(CBlob@ this)
{
	...
	if (this.isAttached())
	{
		...
		bool drill_power = (holder.get_bool("use drill") == true && getGameTime() < holder.get_u32("use drill duration"));

		if (holder.getName() == required_class || sv_gamemode == "TDM" || drill_power)
		{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (hitsomething)
							{
								if (heat < heat_max)
								{
									if (hit_constructed)
									{
										if (drill_power) heat += (heat_add_constructed / 3);
										else heat += heat_add_constructed;
									}
									else if (hitblob)
									{
										if (drill_power) heat += (heat_add_blob / 3);
										else heat += heat_add_blob;
									}
									else
									{
										if (drill_power) heat += (heat_add / 3);
										else heat += heat_add;
									}
								}
								hitsomething = false;
...
void onRender(CSprite@ this)
{
	...
	bool drill_power = (holderBlob.get_bool("use drill") == true && getGameTime() < holderBlob.get_u32("use drill duration"));

	if (holderBlob.getName() != required_class && sv_gamemode != "TDM" && !drill_power ){return;}
