...
void onInit(CBlob@ this)
{
    ...
    this.push("names to activate", "floatkeg");
        this.push("names to activate", "rocketkeg");
    this.push("names to activate", "minikeg");
        this.push("names to activate", "trikeg");
    this.push("names to activate", "bouncykeg");
        this.push("names to activate", "instakeg");
    this.push("names to activate", "archerkeg");
        this.push("names to activate", "teleportbomb");
	this.push("names to activate", "voidkeg");
	...
	if (cmd == this.getCommandID("get bomb"))
	{
		...
		if (hasItem(this, bombTypeName))
		{
			...
			else if (bombType == 3)
			{
				if (getNet().isServer())
				{
					CBlob @blob = server_CreateBlob("firebomb", this.getTeamNum(), this.getPosition());
					if (blob !is null && this.getPlayer() !is null)
					{
						TakeItem(this, bombTypeName);
						this.server_Pickup(blob);
			...
			else if (bombType == 4)
			{
				if (getNet().isServer())
				{
					CBlob @blob = server_CreateBlob("knockbackbomb", this.getTeamNum(), this.getPosition());
					if (blob !is null && this.getPlayer() !is null)
					{
						TakeItem(this, bombTypeName);
						this.server_Pickup(blob);
						...
						else
			{
				if (getNet().isServer())
				{
					CBlob @blob = server_CreateBlob(bombBlobs[bombType], this.getTeamNum(), this.getPosition());
					if (blob !is null)
					{
						TakeItem(this, bombTypeName);
						this.server_Pickup(blob);
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info)
{
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		// start from furthest ones to avoid doing more raycasts than needed
		for (int i = hitInfos.size() - 1; i >= 0; i--)
		{
			...
			if (b !is null)
			{
				...
				// its length is slightly higher than hitvec to make sure it reaches the blob it's directed at
				HitInfo@[] rayInfos;
				...
				for (int j = 0; j < rayInfos.size(); j++)
				{
					...
					bool large = rayb.hasTag("blocks sword") && !rayb.isAttached() && rayb.isCollidable(); // mostly doors, but can also be boats/some mechanisms
					...
					if (large)
					{
						break; // prevent raycasting past the door after we do damage to it
					}
				}
			}
			else  // hitmap
