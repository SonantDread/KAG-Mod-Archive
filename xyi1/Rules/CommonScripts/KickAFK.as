...
void onTick(CRules@ this)
{
	...
	if (!warned)
	{
		if (diff > 0)
		{
			...
			else
			{
				...
				client_AddToChat("Move around or you will be " + (kickImmunity ? "moved to spectator" : "moved to spectator") + " in "+warnToKickSeconds+" seconds!", SColor(255, 255, 100, 32));
				warned = true;
				warnTime = time;
			}
		}
	...
	{
		//so long, sucker
		client_AddToChat("You were " + (kickImmunity ? "moved to spectator" : "moved to spectator") + " for being AFK too long.", SColor(255, 240, 50, 0));
		warned = false;
		if (!kickImmunity)
		{
