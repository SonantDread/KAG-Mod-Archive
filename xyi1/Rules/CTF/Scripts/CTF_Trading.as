	...
	if (victim !is null)
	{
		if (killer !is null)
		{
			if (killer !is victim && killer.getTeamNum() != victim.getTeamNum())
			{
				...
				if (this.hasTag("stats on"))
				{
					string playerteam = ((killer.getTeamNum() == 0) ? "blue" : "red"); 
					this.set_u32(killer.getUsername() + "coins~" + playerteam, this.get_u32(killer.getUsername() + "coins~" + playerteam) + coinsOnKillAdd);
				}
			}
			...
			{ 
				...
				if (this.hasTag("stats on"))
				{
					string playerteam = ((helper.getTeamNum() == 0) ? "blue" : "red"); 
					this.set_u32(helper.getUsername() + "coins~" + playerteam, this.get_u32(helper.getUsername() + "coins~" + playerteam) + coinsOnAssistAdd);
				}
			}
		}
...
f32 onPlayerTakeDamage(CRules@ this, CPlayer@ victim, CPlayer@ attacker, f32 DamageScale)
{
	...
	if (attacker !is null && attacker !is victim && attacker.getTeamNum() != victim.getTeamNum())
	{
		...
		if (this.hasTag("stats on"))
		{
			string playerteam = ((attacker.getTeamNum() == 0) ? "blue" : "red"); 
			this.set_u32(attacker.getUsername() + "coins~" + playerteam, this.get_u32(attacker.getUsername() + "coins~" + playerteam) + (dmg * coinsOnDamageAdd / this.attackdamage_modifier));
		}
	}
	...
	if (cmd == getGameplayEventID(this))
	{
		...
		if (p !is null)
		{
			...
			if (coins > 0)
			{
				...
				if (this.hasTag("stats on"))
				{
					string playerteam = ((p.getTeamNum() == 0) ? "blue" : "red"); 
					this.set_u32(p.getUsername() + "coins~" + playerteam, this.get_u32(p.getUsername() + "coins~" + playerteam) + coins);
				}
