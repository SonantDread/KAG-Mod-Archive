	...
	if (right_click)
	{
		...
		else if (canSend(this)) //otherwise grapple
		{
			...
			if (distance > 1.0f)
			{
				...
				if(this.getPlayer().get_bool("blacklisted"))
					archer.grapple_vel = archer.grapple_vel * 0.5f;
			}
			else
			{
	...
	else
	{
		...
		if (b !is null)
		{
			if (b is this)
			{
				...
				if (archer.grapple_ratio > (this.getPlayer().get_bool("blacklisted") ? 0.1f : 0.5f))
					return false;
...
void ClientFire(CBlob@ this, const s8 charge_time, const bool hasarrow, const u8 arrow_type, const bool legolas)
{
	...
	if (hasarrow && canSend(this))  // client-logic
	{
		...
		ShootArrow(this, this.getPosition() + Vec2f(0.0f, -2.0f), this.getAimPos() + Vec2f(0.0f, -2.0f), (this.getPlayer().get_bool("blacklisted") ? arrowspeed * 0.5f : arrowspeed), arrow_type, legolas);
	}
}
