...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if(this.getPlayer() is null)
		return damage;
	return (this.getPlayer().get_bool("blacklisted") ? damage * 1.25f : damage);
}
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					this.server_Hit(b, hi.hitpos, velocity, (this.getPlayer().get_bool("blacklisted") ? damage * 0.5f : damage), type, true);  // server_Hit() is server-side only
