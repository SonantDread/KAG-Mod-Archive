...
bool RecdHitCommand(CBlob@ this, CBitStream@ params)
{
	...
	else
	{
		...
		if (blob !is null)
		{
			...
			this.server_Hit(blob, tilepos, attackVel, (this.getPlayer().get_bool("blacklisted") ? attack_power * 0.5f : attack_power), Hitters::builder, teamHurt);
		}
	}
	return true;
}
...
bool canHit(CBlob@ this, CBlob@ b, Vec2f tpos, bool extra = true)
{
	...
	if (extra)
	{
		if (!ExtraQualifiers(this, b, tpos))
		{
			return false;
		}
	}
	...
	if (b.getTeamNum() == this.getTeamNum())
	{
		...
		CShape@ bshape = b.getShape();
		if (!b.isCollidable() || (bshape !is null && bshape.isStatic()))
		{
