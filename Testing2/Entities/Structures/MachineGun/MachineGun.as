...
void onInit( CBlob@ this )
{
  ...
  this.set_bool("MG ammo", hasTech(this, "MG ammo"));
...
{
  if (this.hasAttached() || this.getTickSinceCreated() < 30) {//driver, seat or gunner, or just
    VehicleInfo@ v;
    if (!this.get("VehicleInfo", @v)) {
      return;
    }
  ...
  if (this.hasTag("tryReload")) {
    int TimeToReload;
    bool hasMachinegunTech = this.get_bool("MG ammo");
    if (hasMachinegunTech) {
      TimeToReload = 300;
    } else{
      TimeToReload = 150;
    }
