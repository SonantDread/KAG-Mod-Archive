...
const Vec2f upgradeButtonPos(-36.0f, 2.0f);
const Vec2f classButtonPos(0, 2);



void InitWorkshop( CBlob@ this )
{
	this.set_Vec2f("shop offset", upgradeButtonPos);
...
void onInit( CBlob@ this )
{
    ...
    this.CreateRespawnPoint( "base", Vec2f(0.0f, 10.0f) );
    this.SetLight( true );
	...
	AddIconToken( "$mat_bullets2$", "mat_bullets2.png", Vec2f(16,16), 0 );
    AddIconToken( "$mounted_missile$", "mounted_missile.png", Vec2f(16,16), 0 );
	 AddIconToken( "$mat_missile$", "mat_missile.png", Vec2f(16,16), 0 );
	 AddIconToken( "$mounted_bow5$", "mounted_bow5.png", Vec2f(16,16), 0 );
	InitWorkshop( this );
    this.set_TileType("background tile", CMap::tile_castle_back);
    this.getShape().SetStatic(true);
    this.getShape().getConsts().mapCollisions = false;

    server_CreateBlob( "tent", this.getTeamNum(), this.getPosition() );
    ...
    if (!this.exists("upgrade_1_cost")) {
        this.set_u16("upgrade_1_cost", 2400);
    }

    if (!this.exists("upgrade_2_cost")) {
        this.set_u16("upgrade_2_cost", 4200);
    }
  ...
  //  this.addCommandID("class menu");
	...
	this.inventoryButtonPos.Set(50.0f, 2.0f);
}
...
// Copypaste from TunnelTravel.as
...
{
    return (this.get_u8("upgrade_level") >= 0) && isInRadius( this, caller );
}
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
 ...
 if (this.get_u8("upgrade_level") > 0)
    {
    caller.CreateGenericButton( "$seed$", Vec2f(-70, 2), this, this.getCommandID("seed menu"), "Seed nursery", params );
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
    ...
    //    if (caller !is null && caller.isMyPlayer())
    //    {
    //        BuildShopMenu(this, caller, "Build in workbench", Vec2f(150,-50), Vec2f(8,6) );
    //    }
    //}
...
void onInit( CSprite@ this )
{
    ...
    //tower sprites
    {
        ...
        CSpriteLayer@ tower_flagpole = this.addSpriteLayer( "tower_flagpole", "Entities/Special/flag.png" , 16, 32, blob_team, blob_skin );
        ...
        CSpriteLayer@ tower_flag = this.addSpriteLayer( "tower_flag", "Entities/Special/flag.png" , 32, 16, blob_team, blob_skin );
    ...
    //barracks sprites
    {
        ...
        if (barracks_bench !is null)
        {
            ...
            barracks_bench.SetOffset( Vec2f(32.0f, 0.0f));
...
void onGib( CSprite@ this )
{
	...
	//for (int i = 0; i < 20; i++)
		//MakeDustParticle(pos + Vec2f(Random(64) + (sign * 32)), "" );
