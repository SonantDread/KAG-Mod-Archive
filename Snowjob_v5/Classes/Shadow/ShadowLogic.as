			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					if(b.isFacingLeft() == this.isFacingLeft())
						this.server_Hit(b, hi.hitpos, velocity, 4, type, true);  // server_Hit() is server-side only
					else
						this.server_Hit(b, hi.hitpos, velocity, 1, type, true);
