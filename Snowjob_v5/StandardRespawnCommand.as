...
void InitClasses(CBlob@ this)
{
	...
	AddIconToken("$shadowblade_icon$", "ShadowBladeIcon.png", Vec2f(32, 32), 0);
	AddIconToken("$shadow_icon$", "ShadowIcon.png", Vec2f(32, 32), 0);
	...
	addPlayerClass(this, "Shadow Blade", "$shadowblade_icon$", "shadowblade", "Kill and suck");
	addPlayerClass(this, "The Shadow", "$shadow_icon$", "shadow", "Hide and seek");
}
...
void BuildRespawnMenuFor(CBlob@ this, CBlob @caller)
{
	...
	{
		if(getRules().isMatchRunning() || (caller.getPlayer() !is null && caller.getPlayer().getUsername() == "Pirate-Rob")){
			...
			CGridMenu@ menu = CreateGridMenu(caller.getScreenPos() + Vec2f(24.0f, caller.getRadius() * 1.0f - 96.0f), this, Vec2f(1 * CLASS_BUTTON_SIZE, CLASS_BUTTON_SIZE), "Normal Classes");
			if (menu !is null)
			{
