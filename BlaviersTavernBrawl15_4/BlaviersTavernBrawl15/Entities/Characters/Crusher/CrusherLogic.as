...
void onInit(CBlob@ this)
{
	...
	this.set_u8("boulder_throw_count", 0);
...
void onTick(CBlob@ this)
{
	...
	print("time: " + this.get_u8("boulder_throw_count"));
	...
	else // If the entity isn't stunned
	{
		...
		if (this.get_u8("boulder_throw_count") == 16)
		{
			Sound::Play("throw", this.getPosition(), 3.2f);
		}

		if (this.get_u8("boulder_throw_count") == 12)
		{
			// This sets speed for when rightclicking
			// This is the boulder throwing script:
			// =--------------------=

			CBlob@ projectile = server_CreateBlob("boulder",this.getTeamNum(),this.getPosition()+Vec2f((this.isFacingLeft() ? -8 : 8),-4));
			...
			{
				projectile.set_u8("launch team", this.getTeamNum());
				projectile.server_SetTimeToDie(1.5f);
				...
				if (direction == -1)
				{
					Vec2f vel((this.isFacingLeft() ? -3.0f : 3.0f), -6.5f);
				...
				else if (direction == 0)
				{
					...
					projectile.setVelocity(vel * 1.2);
					//this.AddForce(Vec2f((this.isFacingLeft() ? -1.0f : 1.0f) * 660.0f, -580.0f - XORRandom(80)));
				}
				else
				{
					Vec2f vel((this.isFacingLeft() ? -3.0f : 3.0f), 6.5f);
			...
			this.set_u8("boulder_throw_count", 11);
		}
		else if (this.get_u8("boulder_throw_count") > 1)
		{
			this.set_u8("boulder_throw_count", this.get_u8("boulder_throw_count") - 1);
		}
	}
	if (!pressed_a1 && !swordState &&
	         (pressed_a2 || (specialShieldState)))
	{
		if (this.get_u8("boulder_throw_count") == 1)
		{
			this.set_u8("boulder_throw_count", 20);

			Sound::Play("/ArgLong", this.getPosition(), 2.0f);
		}
	}
		         ...
		         knight.state <= KnightStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				if ((!inair) && (knight.state == KnightStates::sword_cut_down))
				{
					...
					Sound::Play("throw", this.getPosition(), 2.8f);
					this.AddForce(Vec2f(0.0f, -30.0f - XORRandom(60)));

					if (Maths::Abs(this.getVelocity().x) > 7.2f)
					{
						sparks(this.getPosition(), 0, 1.0f);
						Sound::Play("ShieldHit", this.getPosition(), 1.0f);
					}
				}
			}
			else if (delta >= 9)
