...
void onTick(CBlob@ this)
{
	...
	if (getNet().isClient() && !this.isInInventory() && myplayer)  //Boomer charge cursor
	{
		BoomerangerCursorUpdate(this, knight);
	}
	...
	{
		// parachute
		this.AddForce(Vec2f(0.0f, -12.5f));
	}
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		...
		moveVars.walkFactor *= (strong ? 0.8f : 1.0f);
		knight.shieldTimer = 0;
		...
		if ((delta >= KnightVars::slash_charge - 1) && (delta < KnightVars::slash_charge + 1))
		{
			if (getNet().isClient())
			{
		...
		else if (knight.state == KnightStates::sword_power)
		{
			...
			Sound::Play("Throwrang", this.getPosition());
			CBlob@ projectile = server_CreateBlob("boomerang",-1,this.getPosition()+Vec2f((this.isFacingLeft() ? -8 : 8), -4));

			this.set_u8("rang_count", this.get_u8("rang_count") - 1);
			...
			if (projectile !is null)
			{
				...
				u16 targetID = 0xffff;
				targetID = this.getNetworkID();

				if (targetID != 0xffff)
				{
					projectile.set_u16("target", targetID);
				}
				...
				if (direction == -1)
				{
					Vec2f vel((this.isFacingLeft() ? -4.5f : 4.5f), -4.5f);
					projectile.setVelocity(vel * 1.1);
				}
				else if (direction == 0)
				{
					if (this.isKeyPressed(key_down)) //short throw
					{
						if (aimpos.y < this.getPosition().y)
						{
							Vec2f vel((this.isFacingLeft() ? -2.2f : 2.2f), 0.0f);
							projectile.setVelocity(vel * 1.05);
						}
						else
						{
							Vec2f vel((this.isFacingLeft() ? -2.2f : 2.2f), 0.0f);
					...
					else //normal throws
					{
						if (aimpos.y < this.getPosition().y)
						{
							Vec2f vel((this.isFacingLeft() ? -6.0f : 6.0f), 0.0f);
							projectile.setVelocity(vel * 1.1);
						}
						else
						{
		...
		else if (knight.state == KnightStates::sword_power_super)
		{
			...
			if (this.isKeyPressed(key_down)) //short throw
			{
				Sound::Play("Throwshock", this.getPosition());

				CBlob@ projectile = server_CreateBlob("shockrang",-1,this.getPosition()+Vec2f((this.isFacingLeft() ? -6 : 6), -3));
				...
				if (direction == -1)
				{
					Vec2f vel((this.isFacingLeft() ? -5.5f : 5.5f), -5.5f);
					projectile.setVelocity(vel * 1.1);
				}
				else if (direction == 0)
				{
					if (aimpos.y < this.getPosition().y)
					{
						Vec2f vel((this.isFacingLeft() ? -8.0f : 8.0f), 0.0f);
						projectile.setVelocity(vel * 1.1);
					}
					else
					{
			...
			{
				Sound::Play("Throwtriple", this.getPosition());
				...
				while (count < 3)
				{
					...
					CBlob@ projectile = server_CreateBlob("boomerang",-1,this.getPosition()+Vec2f((this.isFacingLeft() ? -6 : 6), -3));
					...
					if (projectile !is null)
					{
						...
						Vec2f vel((this.isFacingLeft() ? -3.0f : 3.0f), (count-2.0f)/2);
						projectile.setVelocity(vel * 1.5);
					}	
				}
			}
...
void BoomerangerCursorUpdate(CBlob@ this, KnightInfo@ knight)
{
	...
	{
		if (knight.swordTimer <= KnightVars::resheath_time)
		{
			getHUD().SetCursorFrame(0);
		}
		else
