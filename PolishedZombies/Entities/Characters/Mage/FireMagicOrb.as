...
void onTick(CBlob@ this)
{
	...
	if(count < ORB_LIMIT)
	{
		...
		if(this.isKeyPressed(key_action1) && !this.isKeyPressed(key_inventory) && !this.isKeyPressed(key_use))
		{
			...
			if (diff > 0)
			{
				...
				for (uint i = 0; i < orbTypeNames.length; i++)
				{
					if (this.getBlobCount(orbTypeNames[type]) > 0 && h == 1) 
					{
						if (getNet().isServer())
						{
							@orb = server_CreateBlob("orb", this.getTeamNum(), pos + norm*this.getRadius());
							if (orb !is null)
							{
