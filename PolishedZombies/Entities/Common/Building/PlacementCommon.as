...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	// tilemap check
	const bool buildSolid = (map.isTileSolid(buildTile) || (blob !is null && blob.isCollidable()) || (buildTile == CMap::tile_goldB));
	...
	if ((buildTile == CMap::tile_wood && backtile.type >= CMap::tile_wood_d1 && backtile.type <= CMap::tile_wood_d0) ||
			(buildTile == CMap::tile_castle && backtile.type >= CMap::tile_castle_d1 && backtile.type <= CMap::tile_castle_d0) ||
				(buildTile == CMap::tile_goldB && backtile.type >= CMap::tile_goldB_d1 && backtile.type <= CMap::tile_goldB_d0))
	{
		//repair like tiles
	}
	else if (buildTile == CMap::tile_castle && (backtile.type >= CMap::tile_wood && backtile.type <= CMap::tile_wood_d0 && !map.isInFire(p)) ||
				(backtile.type == CMap::tile_castle_moss))
	{
		// can build stone on moss or wood when not on fire, do nothing
	}
	else if (buildTile == CMap::tile_goldB && (backtile.type >= CMap::tile_castle && backtile.type <= CMap::tile_castle_d0) || 
				(backtile.type >= CMap::tile_wood && backtile.type <= CMap::tile_wood_d0 && !map.isInFire(p)) ||
					(backtile.type == CMap::tile_castle_moss))
	{
		// can build gold on stone, moss or wood when not on fire, do nothing
	}
