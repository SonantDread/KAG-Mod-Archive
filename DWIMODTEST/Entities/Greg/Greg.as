...
void pickRandPlayer(CBlob@ this)
{
    ...
    {
        if(players[i].getTeamNum() != this.getTeamNum())
        {
            targets.insertLast(players[i]);
        }
    }
    ...
    u16 targetId = targets[gregRand.Next()%targets.length].getNetworkID();
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid )
{
    ...
    if (this.hasTag("statue"))
    {
        if (blob !is null && blob.hasTag("flesh") && !this.isOnGround() && vel.y > 10.0f)
        {
            if (getNet().isServer())
            {
                this.server_Hit(blob, blob.getPosition(), vel, 3.0f, Hitters::stomp);
