...
void onCommand(CInventory@ this, u8 cmd, CBitStream@ params)
{
	...
	else if (cmd == Builder::TOOL_CLEAR)
	{
		...
		if (quick_swap_enabled && blob.isMyPlayer() && blob.get_u8("current block") != 255)
		{
			blob.set_u8("prev block", blob.get_u8("current block"));
	...
	else if (cmd >= Builder::PAGE_SELECT && cmd < Builder::PAGE_SELECT + Builder::PAGE_COUNT)
	{
		...
		if (quick_swap_enabled && blob.isMyPlayer() && blob.get_u8("build page") != cmd - Builder::PAGE_SELECT)
		{
...
void onInit(CBlob@ this)
{
	...
	quick_swap_enabled = cfg.read_bool("quickswap", false);
	if (quick_swap_enabled)
	{
		this.set_u8("current block", 255); // 255 for no block
		this.set_u8("prev block", 255);
	}
			...
			for (f32 step_x = 0.0f; step_x < space.x ; ++step_x)
			{
				for (f32 step_y = 0.0f; step_y < space.y ; ++step_y)
				{
					...
					if (!onground || map.getSectorAtPosition(v , "no build") !is null
						|| map.getSectorAtPosition(v, "no_build_new") !is null
						|| map.isTileSolid(v) || blobBlockingBuilding(map, v))
					{
						// draw red
				...
				if (bc.blockActive || bc.blobActive)
				{
					...
					else if (bc.cursorClose)
					{
						...
						if (!bc.buildableAtPos && !bc.sameTileOnBack) //no build indicator drawing
						{
							...
							if (sector is null)
							{
								@sector = map.getSectorAtPosition(middle, "no_build_new");
							}
