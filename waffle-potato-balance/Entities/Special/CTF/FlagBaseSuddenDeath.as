...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (getNet().isServer())
	{
		...
		else if (cmd == this.getCommandID("stop_sudden_death"))
		{
			...
			getRules().set_u16(cooldown_prop, default_zone_cooldown);
			getRules().set_u8(max_expands_prop, default_max_expands);
	...
	if (getNet().isServer())
	{
		...
		else if (this.hasTag(sudden_death_tag))
		{
			...
			if (getGameTime() > this.get_u32(timer_prop))
			{
				...
				if (max_expands < 255 && this.get_u8(expand_count_prop) + 1 < max_expands)
				{
