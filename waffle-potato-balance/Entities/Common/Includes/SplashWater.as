	...
	if (map !is null)
	{
		...
		if (is_server)
		{
			...
			for (uint i = 0; i < blobs.length; i++)
			{
				...
				Vec2f splashvel = shouldStun ? getSplashVelocity(pos, blob) : Vec2f();

				if (shouldStun && (ownerBlob is blob || (this.isOverlapping(blob) && hitHard)))
				{
					this.server_Hit(blob, pos, splashvel, 0.0f, Hitters::water_stun_force, true);
				}
				else if (hitHard)
				{
					this.server_Hit(blob, pos, splashvel, 0.0f, hitter, true);
				}
				else //still have to hit teamies so we can put them out!
				{
					this.server_Hit(blob, pos, splashvel, 0.0f, Hitters::water, true);
				}
			}
		}
	}
...
Vec2f getSplashVelocity(Vec2f pos, CBlob@ hitBlob)
{
	if (hitBlob is null || hitBlob.getShape().isStatic() || SPLASH_FACTOR <= 0)
		return Vec2f();
	...
	const f32 oomph_range = 45.0f; 	// degree range from due left/right over which to scale oomph
	const f32 de_oomph_horizontal = 0.50f; // % of oomph velocity to remove from horizontal
	const f32 upwards_oomph = 0.30f; // % of oomph_factor to add to upward hits
	const f32 upwards_range = 45.0f; // degree range from due up over which to scale upwards oomph

	const f32 stopping_force = 0.75f; // % of antiparallel blob velocity to counteract
	const f32 accel_vel_lost = 0.60f; // % of preexisting parallel blob velocity to ignore
	const f32 perp_velocity_lost = 0.35f; // % of perpendicular blob velocity to counteract
	...
	if (oomph_factor > 0)
	{
		...
		if (upwards_oomph > 0)
		{
			// adjust scale based on how close to 90°
			oomph_scale += Maths::Max(0, (upwards_range - Maths::Abs(90-angle)) / (90-oomph_range))
						   * upwards_oomph;
		}
	...
	if (stopping_force > 0 || accel_vel_lost > 0 || perp_velocity_lost > 0)
	{
		angle = velocity.getAngleDegrees();
		Vec2f blobvel = hitBlob.getVelocity();
		velocity.RotateBy(angle); // align these temporarily so that
		blobvel.RotateBy(angle);  // x = parallel, y = perpendicular
		velocity.y -= blobvel.y * perp_velocity_lost;
		if (blobvel.x < 0)
		{
			velocity.x -= blobvel.x * stopping_force;
		}
		else if (blobvel.x > 0 && accel_vel_lost > 0)
		{
			velocity.x -= blobvel.x * accel_vel_lost;
			if (velocity.x < 0)
			{
				velocity.x = 0.0f;
			}
		...
		velocity += blobvel;
		velocity.RotateBy(-angle); // set velocity back to proper angle
		// blobvel.RotateBy(-1.0 * angle);
	}

	const f32 max_lightness_multiplier = 2.00f; // Very light objects will be pushed further
	if (max_lightness_multiplier > 1)
		velocity *= 1.0f + ((max_lightness_multiplier - 1.0f) / (hitBlob.getMass() + 1));

	return velocity; // Velocity to set blob at
}
