				...
				if (player !is null)
				{
					...
					if(XORRandom(100) < player.get_s16("book_level")*5){
						...
						if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @blobsInRadius)) 
						{
							...
							if(itemType(b.getName()) == 3)makebook = 4;
							if(b.hasTag("player")){
								if(b.hasTag("dead"))makebook = 1;
								else makebook = 2;
							}
							if(b.getName() == "ghost" || b.getName() == "taintedghost")makebook = 1;
						}
						
						if(makebook == 1){
							if(player.get_s16("book_level") > 7){
								CBlob @newBlob = server_CreateBlob("deathbook", 0, this.getPosition());
								if(player is getLocalPlayer())client_AddToChat("You created a fine study of death and life.", SColor(255, 0, 0, 0));
							} else
							if(player is getLocalPlayer())client_AddToChat("You feel you were onto something about death, but lost it.", SColor(255, 0, 0, 0));
						}
						
						if(makebook == 2){
							if(player.get_s16("book_level") > 3){
								CBlob @newBlob = server_CreateBlob("gravbook", 0, this.getPosition());
								if(player is getLocalPlayer())client_AddToChat("You created a fine study of matter manipulation.", SColor(255, 0, 0, 0));
							} else
							if(player is getLocalPlayer())client_AddToChat("You feel you were onto something about gravity, but lost it.", SColor(255, 0, 0, 0));
						}
						
						if(makebook == 3){
							if(player.get_s16("book_level") > 5){
								CBlob @newBlob = server_CreateBlob("elembook", 0, this.getPosition());
								if(player is getLocalPlayer())client_AddToChat("You created a fine study of elemental control.", SColor(255, 0, 0, 0));
							} else
							if(player is getLocalPlayer())client_AddToChat("You feel you were onto something about water and fire, but lost it.", SColor(255, 0, 0, 0));
						}
						
						if(makebook == 4){
							if(player.get_s16("book_level") > 9){
								CBlob @newBlob = server_CreateBlob("runebook", 0, this.getPosition());
								if(player is getLocalPlayer())client_AddToChat("You created a fine study of runes.", SColor(255, 0, 0, 0));
							} else
							if(player is getLocalPlayer())client_AddToChat("You feel you were onto something about runes, but lost it.", SColor(255, 0, 0, 0));
						}
					...
					if(makebook == 0){
						...
						switch(booktype){
							...
							case 5:{
								...
								if(XORRandom(1) == 0)name = "Wards 101";
								if(XORRandom(1) == 0)name = "Wards and You";
								if(XORRandom(1) == 0)name = "Wards for dummies";
								if(XORRandom(1) == 0)name = "Wards for griefers";
							break;}
							...
							case 7:{
								name = "Help! I'm stuck in a book naming factory!";
								...
								if(XORRandom(1) == 0)name = "Griffor!";
								if(XORRandom(1) == 0)name = "Admin aboos!";
								if(XORRandom(1) == 0)name = "My friend is an idit";
								if(XORRandom(1) == 0)name = "This kingdom is a garbadge dump";
								if(XORRandom(1) == 0)name = "Everyone is illiterate";
								if(XORRandom(1) == 0)name = "My lyf story";
							break;}
							...
							case 8:{
								...
								if(XORRandom(1) == 0)name = "Ghosts too spooky four me";
								if(XORRandom(1) == 0)name = "A study on why ghosts smell bad";
								if(XORRandom(1) == 0)name = "Ghosts are just misunderstood";
							break;}
						...
						if(level == -1)if(player is getLocalPlayer())client_AddToChat("You have finished drawing scribbles.", SColor(255, 0, 0, 0));
						else if(player is getLocalPlayer())client_AddToChat("You have completed the book: '"+this.get_string("name")+"'.", SColor(255, 0, 0, 0));
...
int itemType(string name){
	...
	if(name == "mat_wood" || name == "wooden_door" || name == "platform" || name == "ward")return 5;
	
	if(name == "mat_stone" || name == "stone_door")return 6;
	
	if(name == "ghost" || name == "taintedghost" || name == "wraithknight" || name == "wraitharcher")return 8;
