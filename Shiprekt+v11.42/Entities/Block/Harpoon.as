...
const f32 harpoon_grapple_stiffness = 1.0f;
...
{	
		...
		if (left_click)
		{
			if (true && harpoon.grappling == false && harpoon.reeling == false) //otherwise grapple PROBLEM BLOCK
			{
				...
				CParticle@ p = ParticleAnimated( "Entities/Effects/Sprites/WhitePuff.png",
									this.getPosition(),
									thisVel*0.5f + (occupier.getAimPos() - this.getPosition())/(occupier.getAimPos() - this.getPosition()).getLength(),
									1.0f, 0.5f, 
									2, 
									0.0f, true );			
				...
				if (canSend(this))
				{
					...
					if (distance > 1.0f)
					{	
						harpoon.grapple_vel = thisVel + direction * harpoon_grapple_throw_speed;
					}
					else
					{
						harpoon.grapple_vel = thisVel;
					}
	...
	if (harpoon.grappling || !(harpoon.grapple_id == 0xffff))
	{
		...
		if ( (ropeTooLong && harpoon.grapple_id == 0xffff) && harpoon.reeling == false )
		{
		...
		if (occupier !is null)
		{
			if ( occupier.isKeyJustPressed( key_action2 ) && harpoon.reeling == false )
			{
				Sound::Play("HookReel.ogg", this.getPosition());
				harpoon.reeling = true;
			}					
		...
		if( ( (ropeTooLong && harpoon.grapple_id == 0xffff) || harpoon.reeling == true ) && !layer.isAnimation("set") )
		{
				...
				Vec2f retract = retractBaseMin*10.0f;
				Vec2f next = harpoon.grapple_pos - retract;
...
void onCommand( CBlob@ this, u8 cmd, CBitStream @params )
{
	...
	if( cmd == this.getCommandID(grapple_sync_cmd) )
    {
		...
		bool occupierIsClient;
		if ( localBlob !is null )
		{
			occupierIsClient = occupier !is localBlob;
		}
...
bool checkGrappleStep(CBlob@ this, HarpoonInfo@ harpoon, CMap@ map, const f32 dist)
{
	...
	bool hitMap = map.rayCastSolidNoBlobs(this.getPosition(), harpoon.grapple_pos);
								
	if( ropeOutOfBounds || hitMap )
	{
		if (canSend(this))
		{
			harpoon.reeling = true;
		...
		Sound::Play( "HookReel.ogg", harpoon.grapple_pos );
	}
	else
	{
		CBlob@ b = map.getBlobAtPosition(harpoon.grapple_pos);
...
{
	return (occupier.isMyPlayer() || occupier.getPlayer() is null);
}
