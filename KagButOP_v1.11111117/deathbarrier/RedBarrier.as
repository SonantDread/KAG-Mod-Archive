...
SColor randomColour =  SColor(100,XORRandom(255),XORRandom(255), XORRandom(255));
				...
				if (b.getTeamNum() < 100 || b.hasTag("no barrier pass"))
				{
					...
					if (pos.x < middle)
					{
						...
						if(!(b.hasTag("nope")))
						{
							...
							b.server_SetTimeToDie(0.35f);
							CSprite@ sound = b.getSprite();

							sound.SetEmitSoundVolume(1.0f);
							sound.SetEmitSound("nope.ogg");
							sound.SetEmitSoundPaused(false);
							b.Tag("nope");
						}
					}
					else
...
{
	barrier_set = false;
	randomColour = SColor(100,XORRandom(255),XORRandom(255), XORRandom(255));
}
...
void onRender(CRules@ this)
{
	if (shouldBarrier(this))
	{
		f32 x1, x2, y1, y2;
		getBarrierPositions(x1, x2, y1, y2);
		GUI::DrawRectangle(getDriver().getScreenPosFromWorldPos(Vec2f(x1, y1)), getDriver().getScreenPosFromWorldPos(Vec2f(x2, y2)), randomColour);
	}
}
