...
void onTick(CBlob@ this)
{
	...
	bool pressed_a1 = this.isKeyPressed(key_action1) && this.hasTag("has_sword");
	bool pressed_a2 = this.isKeyPressed(key_action2) && this.hasTag("has_shield");
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					bool weakstone = (hi.tile == 63)||(hi.tile == 62);
					if (ground || wood || dirt_stone || gold || weakstone)
					{
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if(itemname == "sword")if(!this.hasTag("has_sword")){
		if (getNet().isServer()){
			blob.server_Die();
			this.Tag("has_sword");
			this.Sync("has_sword",true);
			return;
		}
	}
...
void onDie(CBlob@ this){
	if (getNet().isServer()){
		if(this.hasTag("has_sword")){
			server_CreateBlob("sword", this.getTeamNum(), this.getPosition());
		}
		if(this.hasTag("has_shield")){
