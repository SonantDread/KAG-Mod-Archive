	...
	if (blob.getMap().getBlobsInRadius( pos, SEEK_RANGE, @blobsInRadius ))
	{
		if (strategy == Strategy::find_teammate || strategy == Strategy::idle) 
		{
			...
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				if (b !is blob && b.getTeamNum() == blob.getTeamNum() && !b.isInFlames() && !b.isInWater() && b.hasTag("player")) //(b.hasTag("player") || b.hasTag("bed"))
				{								  
...
CBlob@ getAttacker( CBrain@ this, CBlob @blob )
{
	...
	if (map.getBlobsInRadius( pos, ENEMY_RANGE, @blobsInRadius ))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			CBlob @b = blobsInRadius[i];
			if (b !is blob 
				&& (((b.getTeamNum() != blob.getTeamNum() && (b.hasTag("player") || b.hasTag("zombie")) && !b.hasTag("dead")) || (b.isInFlames() || b.hasTag("animal") || b.hasTag("saw"))) 	// runaway from enemies and from burning stuff
