...
void onInit(CBlob@ this)
{
 	...
 	this.getSprite().SetRelativeZ(-550.0f);
 	this.Tag("tree");
}
...
void onTick(CBlob@ this)
{
	...
	if (getNet().isServer())
	{
		if (map !is null && !this.isAttached()/* && getNet().isServer()*/)
		{
			...
			{			
				@sector_tree = map.server_AddSector(Vec2f(pos.x - radius, pos.y - radius * 20), Vec2f(pos.x + radius, pos.y + radius), "tree", "", this.getNetworkID());

				this.getSprite().SetFrameIndex( map_random.NextRanged(35) < 21 ? 1 : 2 );				
			}		
		}
		else
		{
