...
void onTick(CMovement@ this)
{
	...
	const bool justup = blob.isKeyJustPressed(key_up);

	const bool isknocked = isKnocked(blob);
	
	const bool isknight = blob.getName() == "knight";
	...
	if (!blob.hasTag("double jump") && isknight && onground)
	{
		blob.Tag("double jump");
		blob.Untag("has double jumped");
	}
	...
	if (moveVars.jumpFactor > 0.01f && !isknocked)
	{
		...
		else if (blob.hasTag("double jump") && justup && !onground)
		{
			moveVars.jumpCount = -1;
			blob.setVelocity(Vec2f(vel.x, 0.0f));
			blob.Untag("double jump");
			blob.Tag("has double jumped");
		}
		else
		{
			moveVars.jumpCount++;
		...
		if ((up || blob.hasTag("has double jumped")) && vel.y > -moveVars.jumpMaxVel)
		{
