...
void addShopItemsToMenu(CBlob@ this, CGridMenu@ menu, CBlob@ caller)
{
	...
	if (this.get(SHOP_ARRAY, @shop_items))
	{
		for (uint i = 0 ; i < shop_items.length; i++)
		{
			...
			if(i == 0) //A hack cause I'm whack
			{
				AddIconToken("$kegs_icon$", "KEGS_icon.png",Vec2f(96, 24), 0);
				CGridButton@ separator = menu.AddButton("$kegs_icon$", "", Vec2f(4,1));
			...
			if(i == 12)
			{
				AddIconToken("$bombs_icon$", "BOMBS_icon.png",Vec2f(96, 24), 0);
				CGridButton@ separator = menu.AddButton("$bombs_icon$", "", Vec2f(4,1));
				if (separator !is null)
				{
					separator.clickable = false;
				}
