...
void onTick(CRules @ this){
	...
	if(!this.isWarmup()){
		...
		if(Maths::FMod(Minutes+Hours*60, 8) == 7)this.SetGlobalMessage("(WARNING: Event Soon) Game Time: "+ time + " (WARNING: Event Soon)");
		...
		if(getNet().isServer()){
			if(Maths::FMod(Minutes+Hours*60, 8) == 0 && Seconds == 0){
				if(!this.hasTag("hit")){
					this.Tag("hit");
				
					this.Tag("firstpass"); //Hack cause it seems not needed
					if(!this.hasTag("firstpass")){
						this.Tag("firstpass");
						print("firstpass");
					} else {
						switch(XORRandom(4)){
