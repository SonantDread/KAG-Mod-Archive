...
CBlob@ getNewTargetStandGround( CBrain@ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false )
{
	...
	Vec2f pos = blob.getPosition();
	int radius = 0;
	if (blob.getName() == "knight_bot")
		radius = 100;
	if (blob.getName() == "archer_bot")
		radius = 300;	
...
void SearchTarget( CBrain@ this, const bool seeThroughWalls = false, const bool seeBehindBack = true )
{
	...
	if (target is null)
	{
		...
		if (standground)
		{
			//printf("getNewTargetStandGround ");
			@target = getNewTargetStandGround(this, blob, seeThroughWalls, seeBehindBack);
		}
		else
		{
			//printf("getNewTarget ");
