...
//property
const string SPAWN_ITEMS_TIMER = "GR SpawnItems:";
...
bool GiveSpawnResources( CRules@ this, CBlob@ blob, CPlayer@ player, GRPlayerInfo@ info )
{
	bool ret = false;
...
void onPlayerDie( CRules@ this, CPlayer@ victim, CPlayer@ attacker, u8 customData )
{
	if (victim !is null)
	{
		...
		if (core !is null)
		{
			GRPlayerInfo@ info = cast<GRPlayerInfo@>( core.getInfoFromPlayer( victim ) );
			if (info !is null)
			{
				info.items_collected &= ~ItemFlag::Archer;
			}
...
s32 getGRTimer(CRules@ this, CPlayer@ p)
{
...
{
	s32 next_items = getGRTimer(this, p);
	s32 gametime = getGameTime();
	
	GRPlayerInfo@ info = cast<GRPlayerInfo@>( core.getInfoFromPlayer( p ) );
...
void SetGRTimer(CRules@ this, CPlayer@ p, s32 time)
{
	string property = SPAWN_ITEMS_TIMER+p.getUsername();
...
void doGiveSpawnMats(CRules@ this, CPlayer@ p, CBlob@ b, RulesCore@ core)
{
	if(canGetSpawnmats(this, p, core))
	{
		...
		if (gotmats)
		{
			SetGRTimer(this, p, gametime + (this.isWarmup()?materials_wait_warmup:materials_wait)*getTicksASecond());
		}
	}
}
