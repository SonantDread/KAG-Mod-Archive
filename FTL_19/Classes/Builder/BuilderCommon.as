...
Vec2f getBuildingOffsetPos(CBlob@ blob, CMap@ map, Vec2f required_tile_space)
{
	...
	offsetPos.x = int(pos.x / 24);
	offsetPos.x *= 24;
	offsetPos.y = int(pos.y / 24);
	offsetPos.y *= 24;
...
CBlob@ server_BuildBlob(CBlob@ this, BuildBlock[]@ blocks, uint index)
{
	...
	if(canBuild(this, blocks, index))
	{
		...
		if(b.size != Vec2f(0,0))
		{
			const bool onground = this.isOnGround() || !b.buildOnGround;
			...
			if(!fail)
			{
				...
				for(f32 step_x = -1.0f; step_x < space.x+1; ++step_x)
				{
					for(f32 step_y = -1.0f; step_y < space.y+1; ++step_y)
					{
						Vec2f temp = (Vec2f(step_x + 0.5, step_y + 0.5) * map.tilesize);
						Vec2f v = offsetPos + temp;
						if (map.getSectorAtPosition(v , "room spacing") !is null)
						{
							fail = true;
							break;
						}
