...
void onTick(CMovement@ this)
{
	...
	const bool hasGravity = (blob.getShape().getGravityScale() > 0);
	...
	// No gravity - overrides other movement completely
	if (!hasGravity && !blob.isAttached() && !isknocked)
	{
		if(blob.getShape().isOverlappingTileBackground(false) || blob.hasTag("space_suit")){
			Vec2f backgroundforce;

			if (up)
			{
				backgroundforce.y -= 0.1f;
			}
			...
			blob.AddForce(backgroundforce * moveVars.overallScale * 100.0f);
			//damp vel
			Vec2f vel = blob.getVelocity();
			vel *= 0.95f;
			blob.setVelocity(vel);
