...
void onTick(CBlob@ this)
{
	...
	else if (!pressed_a1 && !swordState && (pressed_a2 || (specialShieldState)))
	{
		...
		{
			bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f) && moveVars.fallCount > KnightVars::glide_down_time);
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding) //no attacking when sliding
	{
		...
		if (!inair)
		{
			this.AddForce(Vec2f(vel.x * -5.0, 0.0f)); // Horizontal slowing force
		}

		if (knight.state == KnightStates::normal || this.isKeyJustPressed(key_action1) && (!inMiddleOfAttack(knight.state) || shieldState))
		{
		...
		if (knight.swordTimer < 128) {knight.swordTimer++;}
		...
		else if (knight.state >= KnightStates::sword_cut_mid && knight.state <= KnightStates::sword_cut_down)
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				// Pole vault
				if ((this.wasOnGround() || this.isOnGround() || !inair) && (knight.state == KnightStates::sword_cut_down) && Maths::Abs(this.getVelocity().x) < 7.0f)
				{
					this.setVelocity(vel * 1.72);
				}
			}
			else if (delta >= 12)
			{
	...
	if (myplayer)
	{
		if (this.isKeyJustPressed(key_action3)) // Space
		{
	...
	if (shieldState)
	{
		...
		else if (knocked < 1 && (this.isKeyPressed(key_left) || this.isKeyPressed(key_right)))
		{
...
{
	if (blob is null || !solid || this.getTeamNum() == blob.getTeamNum()) {return;}
	...
	if (this.getShape().vellen > SHIELD_KNOCK_VELOCITY || onground)
	{
		...
		if (knight.state == KnightStates::shielddropping &&
		        ...
		        (blob.getShape() !is null && !blob.getShape().isStatic())) // && getKnocked(blob) == 0
		{
			Vec2f pos = this.getPosition();
...
f32 onHit( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData )
{
    ...
    CPlayer@ otherPlayer;//Player that damaged this blob
    @otherPlayer = @hitterBlob.getPlayer();//Get the player that attacked this
    
    if(otherPlayer == null)//The blob that damaged this blob doesn't have a player controlling it
        @otherPlayer = @hitterBlob.getDamageOwnerPlayer();//Does it have a player that shot it or used it in some way?
    
    if(otherPlayer == null)//If a player has nothing to do with the damage done to this blob
        return damage;//Stop calculations

    CPlayer@ ownPlayer = this.getPlayer();//Get the current blobs player
    if(ownPlayer != null && ownPlayer.getNetworkID() == otherPlayer.getNetworkID())//If this blob has a player, and that player is the person who did the damage to themself
        return damage;//Don't make them a bad player, hurting yourself is a completely different problem.

    Vec2f blobPos = this.getPosition();
    CRules@ rules = getRules();
    uint16 g_count = rules.get_u16("g_count");//This is done for performance sake, checking for this every loop in the for loop is overkill

    for(uint16 i = 0; i < g_count; i += 2)
    {
        Vec2f g_pos1 = rules.get_Vec2f("g_pos" + i);//Get area point
        Vec2f g_pos2 = rules.get_Vec2f("g_pos" + (i + 1));//Get area2 point, the second point we check between
        
        if(g_pos1.x < blobPos.x && g_pos2.x > blobPos.x && g_pos1.y < blobPos.y && g_pos2.y > blobPos.y)//If the damage took place within this area
        {
            string username = otherPlayer.getUsername();
            rules.set_u16(username + "_bad", 20 * 30);//20 seconds of argro time, and this player is now a bad one
            rules.Sync(username + "_bad", true);//Make sure the client knows this too
        }
    }
