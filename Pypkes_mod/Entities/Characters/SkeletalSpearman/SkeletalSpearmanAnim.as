...
void onInit(CSprite@ this)
{
	string texname = this.getBlob().getSexNum() == 0 ?
	                 "Entities/Characters/SkeletalSpearman/SkeletalSpearman.png" :
	                 "Entities/Characters/SkeletalSpearman/SkeletalSpearman.png";
...
void onTick(CSprite@ this)
{
	...
	if (!blob.get("SkeletalSpearmanInfo", @knight))
	{
		return;
	}
	...
	/*else if (knight.state == SkeletalSpearmanStates::shieldgliding)
	{
		this.SetAnimation("shield_glide");
	}*/
	else if (knight.state == SkeletalSpearmanStates::shielddropping)
	{
		this.SetAnimation("shield_drop");
	}
	else if (knight.state == SkeletalSpearmanStates::shielding)
	{
		if (walking)
		{
			if (direction == 0)
	...
	else if (knight.state == SkeletalSpearmanStates::spear_drawn)
	{
		if (knight.spearTimer < SkeletalSpearmanVars::slash_charge)
		{
			this.SetAnimation("draw_sword");
		}
		else if (knight.spearTimer < SkeletalSpearmanVars::slash_charge_level2)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 0;
		}
		else if (knight.spearTimer < SkeletalSpearmanVars::slash_charge_limit)
		{
			this.SetAnimation("strike_power_ready");
			this.animation.frame = 1;
			shinydot = true;
	...
	else if (knight.state == SkeletalSpearmanStates::spear_cut_mid)
	{
		this.SetAnimation("strike_mid");
	}
	else if (knight.state == SkeletalSpearmanStates::spear_cut_up)
	{
	...
	if (shiny !is null)
	{
		...
		if (shinydot)
		{
			f32 range = (SkeletalSpearmanVars::slash_charge_limit - SkeletalSpearmanVars::slash_charge_level2);
			f32 count = (knight.spearTimer - SkeletalSpearmanVars::slash_charge_level2);
