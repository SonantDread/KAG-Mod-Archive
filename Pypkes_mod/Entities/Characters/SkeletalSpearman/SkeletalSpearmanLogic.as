...
#include "SkeletalSpearmanCommon.as";
...
{
	SkeletalSpearmanInfo knight;

	knight.state = SkeletalSpearmanStates::normal;
	knight.spearTimer = 0;
	...
	this.set("SkeletalSpearmanInfo", @knight);
...
void onTick(CBlob@ this)
{
	...
	SkeletalSpearmanInfo@ knight;
	if (!this.get("SkeletalSpearmanInfo", @knight))
	{
		return;
	}
	...
	{
		knight.state = SkeletalSpearmanStates::normal; //cancel any attacks or shielding
		knight.spearTimer = 0;
		knight.shieldTimer = 0;
		knight.slideTime = 0;
	         ...
	         (pressed_a2 || (specialShieldState)))
	{
		...
		{
			if (knight.state != SkeletalSpearmanStates::normal)
			{
		...
		else
		{
			bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f) &&
			                  moveVars.fallCount > SkeletalSpearmanVars::glide_down_time);

			if (pressed_a2 && inair && !this.isInWater())
			{
				if (direction == -1 && !forcedrop && !getMap().isInWater(pos + Vec2f(0, 16)) && !moveVars.wallsliding)
				{
					//knight.state = SkeletalSpearmanStates::shieldgliding;
					knight.shieldTimer = 1;
				}
				else if (forcedrop || direction == 1)
				{
					knight.state = SkeletalSpearmanStates::shielddropping;
					knight.shieldTimer = 5;
					knight.slideTime = 0;
				}
				else //remove this for partial locking in mid air
				{
					knight.state = SkeletalSpearmanStates::shielding;
				}
			}
			 if (knight.state == SkeletalSpearmanStates::shielddropping)
			{
				if (this.isInWater())
				{
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		if (getNet().isClient())
		{
			if (knight.spearTimer == SkeletalSpearmanVars::slash_charge_level2)
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
			else if (knight.spearTimer == SkeletalSpearmanVars::slash_charge)
			{
				Sound::Play("SwordSheath.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
		}

		if (knight.spearTimer > SkeletalSpearmanVars::slash_charge_limit)
		{
			Sound::Play("/Stun", pos, 1.0f, this.getSexNum() == 0 ? 1.0f : 2.0f);
			SetKnocked(this, 15);
		}

		bool strong = (knight.spearTimer > SkeletalSpearmanVars::slash_charge_level2);
		...
		{
			knight.state = SkeletalSpearmanStates::spear_drawn;
			knight.spearTimer = 0;
		}

		if (knight.state == SkeletalSpearmanStates::spear_drawn && getNet().isServer())
		{
			knight_clear_actor_limits(this);
		}

		//responding to releases/noaction
		s32 delta = knight.spearTimer;
		if (knight.spearTimer < 128)
			knight.spearTimer++;

		if (knight.state == SkeletalSpearmanStates::spear_drawn && !pressed_a1 &&
		        !this.isKeyJustReleased(key_action1) && delta > SkeletalSpearmanVars::resheath_time)
		{
			knight.state = SkeletalSpearmanStates::normal;
		}
		else if (this.isKeyJustReleased(key_action1) && knight.state == SkeletalSpearmanStates::spear_drawn)
		{
			knight.spearTimer = 0;

			if (delta < SkeletalSpearmanVars::slash_charge)
			{
			   if (direction == 0)
				{
					if (aimpos.y < pos.y)
					{
						knight.state = SkeletalSpearmanStates::spear_cut_mid;
					}
				}
			}
			else if (delta < SkeletalSpearmanVars::slash_charge_level2)
			{
				knight.state = SkeletalSpearmanStates::spear_power;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				knight.slash_direction = aiming_direction;
			}
			else if (delta < SkeletalSpearmanVars::slash_charge_limit)
			{
				knight.state = SkeletalSpearmanStates::spear_power_super;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				knight.slash_direction = aiming_direction;
		...
		else if (knight.state >= SkeletalSpearmanStates::spear_cut_mid) // cut state
		{
		...
		else if (knight.state == SkeletalSpearmanStates::spear_power ||
		         knight.state == SkeletalSpearmanStates::spear_power_super)
		{
			//setting double
			if (knight.state == SkeletalSpearmanStates::spear_power_super &&
			        this.isKeyJustPressed(key_action1))
			{
		...
		if ((knight.state == SkeletalSpearmanStates::spear_power ||
		        knight.state == SkeletalSpearmanStates::spear_power_super) &&
		        delta < SkeletalSpearmanVars::slash_move_time)
		{

			if (Maths::Abs(vel.x) < SkeletalSpearmanVars::slash_move_max_speed &&
			        vel.y > -SkeletalSpearmanVars::slash_move_max_speed)
			{
	...
	if (shieldState)
	{
		...
		if ((knight.state == SkeletalSpearmanStates::shielding && direction == -1))
		{
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, SkeletalSpearmanInfo@ info)
{
	if (!getNet().isServer())
	{
		return;
...
bool isSliding(SkeletalSpearmanInfo@ knight)
{
