...
void Config(ZombiesCore@ this)
{
    ...
    int target_number_of_days = cfg.read_s32("target_number_of_days", 20);
    getRules().set_s32("target_number_of_days", target_number_of_days);
}
...
shared class ZombiesSpawns : RespawnSystem
{
    ...
    void Update()
    {
        ...
        getBlobsByTag("zombie-portal", @zombiePortal_blobs);
        int num_portals = zombiePortal_blobs.length;
        ...
        getRules().set_s32("num_portals", num_portals);
        getRules().SetGlobalMessage("Day "+ dayNumber + ". Zombie Portals Left: " + num_portals);
    }
...
shared class ZombiesCore : RulesCore
{
  ...
  void CheckTeamWon()
  {
    ...
    int num_portals = rules.get_s32("num_portals");
    if (num_portals == 0) {
      rules.SetCurrentState(GAME_OVER);
    ...
    if (dayNumber > 20) {
      rules.SetCurrentState(GAME_OVER);
      rules.SetGlobalMessage("Hurray! You survived 20 days!");
      getRules().set_bool("everyones_dead", false);
    }
  }
