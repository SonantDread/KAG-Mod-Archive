...
void Config(ChallengeCore@ this)
{
	...
	string configstr = "../Mods/ArcherRaces/Rules/Challenge/challenge_vars.cfg";

	ConfigFile cfg = ConfigFile(configstr);

	//spawn after death time
	s32 warmUpTimeSeconds = cfg.read_s32("warmup_time", 15);
...
shared class ChallengeCore : RulesCore
{
	...
	void Update()
	{
		...
		if (rules.isIntermission() || rules.isWarmup() || !hasPlayers())
		{
			...
			if(warmUpTime == ticksASecond * 3)
			{
				rules.set_u8("warmupSecond", 3);
				rules.Sync("warmupSecond", true);
			}
			else if(warmUpTime == ticksASecond * 2)
			{
				rules.set_u8("warmupSecond", 2);
				rules.Sync("warmupSecond", true);
			}
			else if(warmUpTime == ticksASecond)
			{
				rules.set_u8("warmupSecond", 1);
				rules.Sync("warmupSecond", true);
			}
			else if(rules.get_u8("warmupSecond") != 0)
			{
				rules.set_u8("warmupSecond", 0);
				rules.Sync("warmupSecond", true);
			}
			...
			if (warmUpTime <= 0)
			{
				...
				//CBlob@[] blobs;
				//getBlobsByName("start_gate", @blobs);
