...
void onInit(CBlob@ this)
{
	...
	Vehicle_SetupWeapon(this, v,
	                    ...
	                    "cannonball", // bullet ammo config name
	                    "", // bullet config name
	                    "", // fire sound
	...
	this.set_string("autograb blob", "cannonball");
	...
	if (getNet().isServer())
	{
		CBlob@ ammo = server_CreateBlob("cannonball");
		if (ammo !is null)
		{
...
void onTick(CBlob@ this)
{
	if (this.hasAttached() || this.getTickSinceCreated() < 30)
	{
		...
		if (this.hasTag("fuse"))
		{
			// u32 timer = Maths::Min(this.get_u32("nextFire") - getGameTime(), 0);
			// print("" + timer);
		
			// Vec2f vel = getRandomVelocity(180 + angle, 2.0f, 45.0f);
			// SColor col = SColor(255, 255, 200 + timer, 0);
			
			// ParticlePixel(this.getPosition(), vel, col, true, 119);
		
			if (this.get_u32("nextFire") < getGameTime())
			{
				this.Untag("fuse");
				Shoot(this);
			}
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _unused)
{
	if (bullet !is null)
	{
		this.getSprite().PlaySound("Sparkle", 3.0f, 4.00f);
		
		this.set_u32("nextFire", getGameTime() + 70);
		this.Tag("fuse");
	}
}
...
void Shoot(CBlob@ this)
{
	...
	Vec2f vel = Vec2f(25 * (this.isFacingLeft() ? -1 : 1), 0.0f).RotateBy(angle);
	
	f32 angleRad = ((this.isFacingLeft() ? 180 : 0) + angle - (10 * (this.isFacingLeft() ? -1 : 1))) * 0.0174533f;
	
	Vec2f pos = this.getPosition() + Vec2f(Maths::Cos(angleRad) * 10, Maths::Sin(angleRad) * 10);
	// print("Angle: " + angle + " "  + Maths::Cos(angleRad) + " and " + Maths::Sin(angleRad));
	
	this.getSprite().PlaySound("KegExplosion", 3.0f, 1.00f);
	makeSmallExplosionParticle(pos);
	
	
	CBlob@ ball = server_CreateBlob("cannonball", this.getTeamNum(), pos);
