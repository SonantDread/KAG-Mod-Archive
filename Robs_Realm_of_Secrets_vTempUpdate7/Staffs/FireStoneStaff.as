...
void onTick(CBlob@ this)
{
	...
	if(this.isInWater()){
		if(getNet().isServer()){
			CBlob @staff = server_CreateBlob("stone_staff", this.getTeamNum(), this.getPosition());
	...
	if (this.isAttached())
	{
		...
		if (holder.get_u8("knocked") <= 0)
		{
			...
			if(point.isKeyPressed(key_action1))if(getNet().isServer()){
				CBlob @blob = server_CreateBlob("fire_blob", holder.getTeamNum(), Vec2f(holder.getAimPos().x,0));
				if (blob !is null)
				{
					blob.setVelocity(Vec2f(0,50));
					blob.SetDamageOwnerPlayer(holder.getPlayer());
					blob.set_u16("size", 50);
				}
				this.set_u16("timer",0);
			}
			
			if(point.isKeyPressed(key_action2)){
				ControlElements(this.get_f32("power"),holder.getAimPos(),true,false,false,false,false,false,false,false,false,true,false);
			}
			...
			if(point.isKeyPressed(key_action3)){
				for(uint i = 0; i < 20; i += 1){
					CBlob @blob = server_CreateBlob("fire_blob", holder.getTeamNum(), Vec2f(holder.getPosition().x-80+i*8,XORRandom(20)));
					if (blob !is null)
					{
						blob.setVelocity(Vec2f(0,20+XORRandom(10)));
						blob.SetDamageOwnerPlayer(holder.getPlayer());
						blob.set_u16("size", 100);
					}
				}
