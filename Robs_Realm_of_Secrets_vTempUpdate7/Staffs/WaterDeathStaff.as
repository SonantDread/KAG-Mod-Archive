	...
	if (this.isAttached())
	{
		...
		if (holder.get_u8("knocked") <= 0)
		{
			...
			if(point.isKeyPressed(key_action2)){
				ControlElements(this.get_f32("power"),holder.getAimPos(),false,true,false,false,false,false,false,true,false,false,false);
			}
			
			if(this.get_u16("super_timer") < 200)this.set_u16("super_timer",this.get_u16("super_timer")+1);
			else
			if(point.isKeyPressed(key_action3)){
				...
				if (this.getMap().getBlobsInRadius(this.getPosition(), 32.0f, @blobsInRadius)) 
				{
					for (uint i = 0; i < blobsInRadius.length; i++)
					{
						...
						if(b.getName() == "ghost")
						{
							CBlob @wraith = ChangeClass(b,"wraithknight",b.getPosition(),holder.getTeamNum());
							wraith.Tag("water");
							break;
						}
					}
