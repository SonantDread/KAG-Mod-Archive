...
void onTick(CBlob@ this)
{
	...
	bool pressed_a1 = this.isKeyPressed(key_action1) || (XORRandom(10) == 0);
	bool pressed_a2 = this.isKeyPressed(key_action2) || (XORRandom(10) == 0);
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         ghoul.state <= GhoulStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				DoAttack(this, 1.5f, attackAngle, attackarc, Hitters::sword, delta, ghoul);
			}
			else if (delta >= 9)
			{
				ghoul.swordTimer = 0;
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					this.server_Hit(b, hi.hitpos, velocity, damage*DamageMultiplier(this), type, true);  // server_Hit() is server-side only
					
					if(b.hasTag("flesh"))OverHeal(this,0.5);
