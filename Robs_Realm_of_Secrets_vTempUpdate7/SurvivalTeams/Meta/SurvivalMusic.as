...
string base_blob_name = "flag_base";
...
void AddGameMusic(CBlob@ this, CMixer@ mixer)
{
	...
	mixer.AddTrack(getFilePath(getCurrentScriptName())+"/Trollmusic.ogg", troll_music);
	mixer.AddTrack(getFilePath(getCurrentScriptName())+"/Rain.ogg", world_rain);
}
...
void GameMusicLogic(CBlob@ this, CMixer@ mixer)
{
	...
	if (timer % 2 == 0)
	{
		...
		bool isRaining = rules.get_bool("raining");
		
		if (isRaining)
		{
			changeAmbience(mixer, world_rain, 1.0f, 1.0f);
		}
	...
	if(getLocalPlayer() !is null){
		if(getLocalPlayer().getUsername() == "BarsukEughen555"){
			changeMusic(mixer, troll_music);
			return;
		}
	}
