...
{
	for (f32 step = 0.0f; step < (width / scale - 64)+(blob.getHealth()-2)*64.0f*scale; step += 64.0f * scale)
	{
...
void renderFrontStone(CBlob@ blob, Vec2f farside, f32 width, f32 scale)
{
	...
	{
		GUI::DrawIcon("Entities/Common/GUI/BaseGUI.png", 2, Vec2f(16, 32), farside + Vec2f(-step * scale - 32 * scale+(blob.getHealth()-2)*64.0f*scale, 0), scale);
	}

	if (width > 16)
	{
		GUI::DrawIcon("Entities/Common/GUI/BaseGUI.png", 2, Vec2f(16, 32), farside + Vec2f(-width+(blob.getHealth()-2)*64.0f*scale, 0), scale);
	}

	GUI::DrawIcon("Entities/Common/GUI/BaseGUI.png", 0, Vec2f(16, 32), farside + Vec2f(-width - 32 * scale+(blob.getHealth()-2)*64.0f*scale, 0), scale);
	GUI::DrawIcon("Entities/Common/GUI/BaseGUI.png", 3, Vec2f(16, 32), farside + Vec2f((blob.getHealth()-2)*64.0f*scale, 0), scale);
}
