...
string[] age_description = {
	...
	//cake day
	"Cake Day - it's this player's KAG Birthgay!",
	//(gap in the sheet)
	"", "", "", "", "", "",
	//established player
	...
	"This player has over 9000 years of experience",
...
float drawScoreboard(CPlayer@[] players, Vec2f topleft, CTeam@ team, Vec2f emblem)
{
	...
	GUI::DrawText(getTranslatedString("Username"), Vec2f(bottomright.x - 560, topleft.y), SColor(0xffffffff));
	GUI::DrawText(getTranslatedString("Stocks"), Vec2f(bottomright.x - 400, topleft.y), SColor(0xffffffff));
	...
	for (u32 i = 0; i < players.length; i++)
	{
		...
		GUI::DrawText("" + username, Vec2f(bottomright.x - 560, topleft.y), namecolour);
		GUI::DrawText("" + rules.get_u8("playerStocks"+username), Vec2f(bottomright.x - 400, topleft.y), SColor(0xffffffff));
