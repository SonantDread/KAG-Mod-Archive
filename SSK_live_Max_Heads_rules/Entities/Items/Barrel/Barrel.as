	...
	if (map.getHitInfosFromArc(pos, -angle, 30, vellen, this, false, @hitInfos))
	{
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			else if (team != u8(hi.blob.getTeamNum()))
			{
				...
				server_fighterHit(this, hi.blob, pos, Vec2f(vel.x, vel.y*0.4f), dmg, Hitters::cata_boulder, true, fighterHitData);
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (solid && blob !is null)
	{
		...
		{
			FighterHitData fighterHitData(6, 4.0f, 0.04f);
			server_fighterHit(this, blob, point1, Vec2f(hitvel.x, hitvel.y*0.4f), dmg, Hitters::cata_boulder, true, fighterHitData);			
		}
