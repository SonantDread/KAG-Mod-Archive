...
{
	if ( (getSawOn(this) && caller.getTeamNum() != this.getTeamNum()) || this.getDistanceTo(caller) > 16) return;
...
{
	if (this is tobeblended || tobeblended.hasTag("invincible") || !getSawOn(this))
	{
		return;
	}
	...
	{
		CBlob @wood = server_CreateBlob("mat_wood", this.getTeamNum(), pos + Vec2f(0, 12));
		if (wood !is null)
		{
	...
	if (tobeblended.getPlayer() !is null)
	{
		CBitStream params;
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	...
	if (canSaw(this, blob))
	{
		...
		Vec2f bpos = blob.getPosition();
		//this.Tag("sawed");
		FighterHitData fighterHitData(12, 10.0f, 0.02f);
		server_fighterHit(this, blob, bpos, bpos - pos, 100.0f, Hitters::saw, true, fighterHitData);
	}
}
...
bool canBePickedUp(CBlob@ this, CBlob@ byBlob)
{
	return (byBlob.getTeamNum() == this.getTeamNum() ||
	        byBlob.getPosition().y > this.getPosition().y + 4) || !getSawOn(this);
}
