...
const s32 bomb_fuse = 20;
const f32 arrowMediumSpeed = 1.0f;
const f32 arrowFastSpeed = 1.5f;
//maximum is 15 as of 22/11/12 (see ArcherCommon.as)

const f32 ARROW_PUSH_FORCE = 0.1f;
...
void onInit(CBlob@ this)
{
	...
	// in ArrowHitMap
	this.server_SetTimeToDie(50);

	const u8 arrowType = this.get_u8("arrow type");

	if (arrowType == ArrowType::bomb)			 // bomb arrow
	{
		SetupBomb(this, bomb_fuse, 20.0f, 5.0f, 10.0f, 0.12f, true);
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		else
		{
			...
			if (dmg > 0.0f)
			{
				FighterHitData fighterHitData(0, dmg*0.2f, 0.06f);

				if (arrowType == ArrowType::fire)
					server_fighterHit(this, blob, point1, initVelocity, dmg, Hitters::fire, true, fighterHitData);
				else
					server_fighterHit(this, blob, point1, initVelocity, dmg, Hitters::arrow, true, fighterHitData);
			}
		}
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		if (speed > ArcherParams::shoot_max_vel * 0.5f)
		{
			        ...
			        hitBlob.hasTag("player"))
			{
				...
				{
					hitBlobFighterVars.tumbleTime = 20;

					if (getNet().isServer())
						SyncTumbling(hitBlob);
				}
