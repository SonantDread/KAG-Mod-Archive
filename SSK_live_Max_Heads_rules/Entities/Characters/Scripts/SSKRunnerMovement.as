...
void onTick(CMovement@ this)
{
	...
	bool upJustPressed			= blob.isKeyJustPressed(key_up);
	bool leftJustPressed		= blob.isKeyJustPressed(key_left);
	bool rightJustPressed		= blob.isKeyJustPressed(key_right);
	...
	if (inMoveAnimation)
	{
		...
		moveVars.stoppingFactor *= 0.4f;
		moveVars.walkFactor *= 0.1f;
	}

	const bool isTumbling = fighterVars.tumbleTime > 0;
	...
	if (is_client && getGameTime() % 3 == 0)
	{
		...
		/* unfortunately, this doesn't work with superArcher bow draw stuff;
	...
	if (blob.isOnLadder() && !blob.isAttached() && !blob.isOnGround() && !isTumbling)
	{
		Vec2f ladderforce;
	...
	if (!blob.isOnCeiling() && !isTumbling &&
	...
	if (carryBlob !is null)
	{
		...
		else if (carryBlob.hasTag("heavy weight") || carryBlob.hasTag("flesh"))
		{
	...
	//jumping
	if (moveVars.jumpFactor > 0.01f && !isTumbling)
	{
		...
		if (upJustPressed && moveVars.numJumps < 2)
		{
			const int FIRST_JUMP_MARGIN = 4;
			if (!onground && moveVars.jumpCount > FIRST_JUMP_MARGIN)
			{
				moveVars.numJumps = 2; // prevents double jumping if falling of a ledge
			}
			else
			{
				if (moveVars.numJumps == 0)
					moveVars.jumpCount = 0;

				moveVars.numJumps++;
			}

			const f32 DOUBLE_JUMP_SPEED = 7.0f;
			f32 jumpSpeed = 0.0f;
			if (moveVars.numJumps == 2 && moveVars.jumpCount > FIRST_JUMP_MARGIN)	// jumpcount used to prevent accidental double jumps
			{
				jumpSpeed = DOUBLE_JUMP_SPEED;

				if (is_client)
				{
					ParticleAnimated("ring2.png", pos, Vec2f(vel.x*0.5f, 0), 0, 1.0f, 2, 0.0f, true);
					blob.getSprite().PlaySound("jump2.ogg", 1.5f);
				}
			}

			fighterVars.fastFalling = false;

			blob.setVelocity(Vec2f(vel.x, -jumpSpeed*moveVars.jumpFactor));
		}

		if (up)
		{
			moveVars.jumpStart = 1.7f;
			...
			// jump
			if (moveVars.jumpCount <= moveVars.jumpMaxCount && vel.y > -moveVars.jumpMaxVel)
				force.y -= 2.0f*(1.0f - 1.0f*moveVars.jumpCount/moveVars.jumpMaxCount);		// decrease jump force over time

			force *= moveVars.jumpFactor * moveVars.overallScale * 50.0f;
			...
			{
				blob.getSprite().PlaySound("jump1.ogg", 1.5f);

				//CParticle@ p = ParticleAnimated("impact1.png", pos - Vec2f(0, 22.0f), Vec2f(0, 0), 0.0f, 1.0f, 2, 0.0f, false);
			}
		}
	}
	...
	bool left_or_right = (left || right);
	{
		...
		const f32 backwardsspeed = 0.5f;
		const f32 fastspeed = 1.5f;
		...
		if (right)
		{
			...
			{
				if (moveVars.dashing && onground)
					walkDirection.x += fastspeed;
				else
					walkDirection.x += normalspeed;
			}
		}
		...
		{
			// boost start speed and show effects when changing direction while dashing
			const f32 DASH_START_SPEED = 1.0f;
			if (moveVars.dashing && onground)
			{
				if (rightJustPressed)
				{
					blob.setVelocity(Vec2f(Maths::Max(vel.x, DASH_START_SPEED), vel.y));

					ParticleAnimated("dash1.png", pos + Vec2f(-24, -8), Vec2f(vel.x*0.5f, 0), 0, 1.0f, 2, 0.0f, true);
					sprite.PlaySound("dashstart1.ogg", 1.5f);
				}
				else if (leftJustPressed)
				{
					blob.setVelocity(Vec2f(Maths::Min(vel.x, -DASH_START_SPEED), vel.y));

					ParticleAnimated("dash2.png", pos + Vec2f(24, -8), Vec2f(vel.x*0.5f, 0), 0, 1.0f, 2, 0.0f, true);
					sprite.PlaySound("dashstart1.ogg", 1.5f);
				}
			}
			...
			if (left_or_right)
			{
				...
				else if (down && !moveVars.dashing)
				{
					moveVars.dashing = true;

					if (right)
					{
						blob.setVelocity(Vec2f(Maths::Max(vel.x, DASH_START_SPEED), vel.y));
						ParticleAnimated("dash1.png", pos + Vec2f(-16, -8), Vec2f(0, 0), 0, 1.0f, 2, 0.0f, true);
					}
					else if (left)
					{
						blob.setVelocity(Vec2f(Maths::Min(vel.x, -DASH_START_SPEED), vel.y));
						ParticleAnimated("dash2.png", pos + Vec2f(16, -8), Vec2f(0, 0), 0, 1.0f, 2, 0.0f, true);
					}
			...
			else if (moveVars.dashing)
			{
				moveVars.dashing = false;

				if (onground)
					sprite.PlaySound("dashstop1.ogg");
			}
			...
			if (!isTumbling && ((absx < lim) || left && greater || right && !greater))
			{
...
void CleanUp(CMovement@ this, CBlob@ blob, SSKRunnerMoveVars@ moveVars)
{
