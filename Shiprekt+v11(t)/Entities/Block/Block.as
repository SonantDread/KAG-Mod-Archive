...
f32 getRam( int blockType,int cost )
{ 
  f32 gettt=cost;
  ...
  if ( blockType == Block::RAMCHAIR )
    gettt*=0.1;
  if ( blockType == Block::CANNON )
    gettt*=0.3;
  if ( blockType == Block::HARVESTER )
    gettt*=0.3;
  if ( blockType == Block::MACHINEGUN )
    gettt*=0.3;
  if ( blockType == Block::POINTDEFENSE )
    gettt*=0.3;
  if ( blockType == Block::LAUNCHER )
    gettt*=0.3;
  if ( blockType == Block::FLAK )
    gettt*=0.3;
...
void onCollision( CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1 )
{
	...
	if (color > 0 && other_color > 0 && color != other_color) // block vs block
	{
		...
		if (getNet().isServer() && !(blockType == Block::STATION || other_blockType == Block::STATION) )
		{
			...
			{
				/*
				print(this_health+" "+other_health);
				if(this_health>other_health)
				{
					this.server_Hit( this, point1, Vec2f_zero, other_health, 0, true );
					Die( blob );
					print("Die blob");
				}else
				if(this_health<other_health)
				{
					this.server_Hit( blob, point1, Vec2f_zero, this_health, 0, true );
					Die( this );
					print("Die this");
				}else
				if(this_health==other_health)
				...
				*/
		        int costa = Block::getCost( blockType );  
		        if ( blockType == Block::MOTHERSHIP1 || blockType == Block::MOTHERSHIP2 || blockType == Block::MOTHERSHIP3
		            || blockType == Block::MOTHERSHIP4 || blockType == Block::MOTHERSHIP6
		            || blockType == Block::MOTHERSHIP7 || blockType == Block::MOTHERSHIP8 || blockType == Block::MOTHERSHIP9
		            || blockType == Block::PLATFORM2 )
		        {
		          ...
		          u16 costb = Block::getCost( other_blockType );  
		        if ( other_blockType == Block::MOTHERSHIP1 || other_blockType == Block::MOTHERSHIP2 || other_blockType == Block::MOTHERSHIP3
		            || other_blockType == Block::MOTHERSHIP4 || other_blockType == Block::MOTHERSHIP6
		            || other_blockType == Block::MOTHERSHIP7 || other_blockType == Block::MOTHERSHIP8 || other_blockType == Block::MOTHERSHIP9
		            || other_blockType == Block::PLATFORM2 )
		        {
		          costb = 15;
		        }

		        f32 modif=1;
		        //modif = modif * getRam(blockType,costa) / getRam(other_blockType,costb);
		        modif = modif * getRam(blockType,costa);
		        modif = modif / getRam(other_blockType,costb);

		        //print(modif);

		        if(this_health*modif>other_health)
		        {
		          this.server_Hit( this, point1, Vec2f_zero, other_health/modif, 0, true );
		          Die( blob );
		          print("Die blob");
		        }else
		        if(this_health*modif<other_health)
		        {
		          this.server_Hit( blob, point1, Vec2f_zero, this_health*modif, 0, true );
		          Die( this );
		          print("Die this");
		        }else
		        if(this_health*modif==other_health)
