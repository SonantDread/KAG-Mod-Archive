...
bool GiveSpawnResources(CRules@ this, CBlob@ blob, CPlayer@ player, RulesCore@ core)
{
	...
	{
		s32 extra_players = -5 + getTeamSize(core.teams, player.getTeamNum());

		u16 wood;
		u16 stone;
		if (this.isWarmup())
		{
			wood = 300 - (15 * extra_players);
			stone = 100 - (5 * extra_players);
		}
		else
		{
			wood = 100 - (5 * extra_players);
			stone = 30 - (extra_players < 6 ? 2 * extra_players
											: 1 * extra_players + 5);
		}

		ret = SetMaterials(blob, "mat_wood", wood) || ret;
		ret = SetMaterials(blob, "mat_stone", stone) || ret;
...
void doGiveSpawnMats(CRules@ this, CPlayer@ p, CBlob@ b, RulesCore@ core)
{
	if (canGetSpawnmats(this, p, core))
	{
		...
		bool gotmats = GiveSpawnResources(this, b, p, core);
		if (gotmats)
		{
