...
void Config(CTFCore@ this)
{
	...
	{
		configstr = rules.get_string("ctfconfig");
	}

	ConfigFile cfg = ConfigFile(configstr);

	//how long to wait for everyone to spawn in?
	s32 warmUpTimeSeconds = rules.exists("custom_warmup_time") ? rules.get_s32("custom_warmup_time")
															   : cfg.read_s32("warmup_time", 30);
	...
	s32 gameDurationMinutes = rules.exists("custom_game_time") ? rules.get_s32("custom_game_time")
															   : cfg.read_s32("game_time", -1);

	// Can overtime happen when there's a timer?
	if (!rules.hasTag("overtime_disabled") && cfg.exists("overtime")
		&& cfg.read_bool("overtime", false))
	{
		rules.Tag("overtime_possible");
		rules.Sync("overtime_possible", true);
	}

	if (cfg.exists("sudden_death_vote_time"))
	{
		s32 sudden_death_vote_time = cfg.read_s32("sudden_death_vote_time");
		if (sudden_death_vote_time > 0)
		{
			rules.Tag("suddendeath_vote_allowed");
			rules.Sync("suddendeath_vote_allowed", true);
			this.suddendeathVoteTime = (getTicksASecond() * 60 * sudden_death_vote_time);
		}
		else
		{
			rules.Untag("suddendeath_vote_allowed");
			rules.Sync("suddendeath_vote_allowed", true);
			this.suddendeathVoteTime = 0;
		}
	}
...
shared class CTFSpawns : RespawnSystem
{
	...
	{
		int tickets = getRules().get_s16((info.team == 0 ? "blue" : "red") + "Tickets");
		if (info !is null && tickets > 0)
		{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (canSpawnPlayer(p_info))
		{
			...
			if (playerBlob !is null)
			{
				...
				// [koi]: decrease tickets on successful respawn
				CRules@ rules = getRules();
				if(rules.isMatchRunning())
				{
					decrementTickets(rules, p_info.team);
				}
			}
		}
	}
	...
	bool canSpawnPlayer(PlayerInfo@ p_info)
	{
		...
		// [koi]: don't let the player respawn if there are no tickets left
		if (ticketsRemaining(getRules(), p_info.team) <= 0) { return false; }
...
shared class CTFCore : RulesCore
{
	...
	void Update()
	{
		...
		if ((rules.isIntermission() || rules.isWarmup()) && (!allTeamsHavePlayers()))  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
			...
			rules.set_u16("sudden_death_vote_time", gamestart + suddendeathVoteTime);
	...
	void CheckTeamWon()
	{
		...
		if (winteamIndex >= 0)
		{
			...
			if (rules.isGameOver())
			{
				if (getRules().hasTag("Official"))
				{
					officialMatchHandling(getRules(), Maths::Abs(winteamIndex - 1));
				}
			}
		}
	}
...
void Reset(CRules@ this)
{
	...
	this.set_u32("game_end_time", getGameTime() + core.gameDuration + core.warmUpTime);
	this.set_u16("sudden_death_vote_time", getGameTime() + core.suddendeathVoteTime);
	// for TimeToEnd.as
}
