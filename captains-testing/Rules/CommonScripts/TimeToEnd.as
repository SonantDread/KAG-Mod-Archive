...
void onTick(CRules@ this)
{
	...
	if (getGameTime() > gameEndTime)
	{
		...
		if (!hasWinner)
		{
			...
			else if (!this.hasTag("overtime_active"))
			{
				// Trigger overtime stuff
				server_StartSuddenDeathFlags();
				this.Tag("overtime_active");
			}
		}
	}
}
...
void onRender(CRules@ this)
{
	...
	if (end_in > 0)
	{
		...
		drawRulesFont(getTranslatedString((this.hasTag("overtime_possible") ? "Sudden death in " 
																			: "Time left: ") + "{MIN}:{SEC}")
