...
const string votesuddendeath_id = "vote: suddendeath";
...
void onInit(CRules@ this)
{
	...
	this.addCommandID(votesuddendeath_id);
}
...
class VoteNextmapFunctor : VoteFunctor
{
	...
	{
		playername = getNameToDisplay(player);
	}
...
//VOTE SUDDENDEATH -------------------------------------------------------------
//suddendeath functors

class VoteSuddendeathFunctor : VoteFunctor
{
	VoteSuddendeathFunctor() {} //dont use this
	VoteSuddendeathFunctor(CPlayer@ player)
	{
		team = player.getTeamNum();
	...
	void Pass(bool outcome)
	{
		if (outcome)
		{
			...
			getRules().Untag("suddendeath_vote_allowed");

			client_AddToChat(
				getTranslatedString("Sudden Death has begun! Watch your flags..."),
				vote_message_colour()
			);
		}
		else
...
class VoteSuddendeathCheckFunctor : VoteCheckFunctor
{
	VoteSuddendeathCheckFunctor() {}
	// VoteSuddendeathCheckFunctor(s32 _team)
	// {
	// 	team = _team;
	// }
...
class VoteSuddendeathTallyFunctor : VoteTallyFunctor
{
	VoteSuddendeathTallyFunctor() {}

	// tally[0] = teamnum, tally[1] = yes, tally[2] = no
	int[][] team_tallies;

	// If either team had a majority vote yes, the vote will pass
	bool getOutcome(VoteObject@ vote)
	{

		CRules@ rules = getRules();
		bool foundteam;
		for (int i = 0; i < vote.yesplayers.length; ++i)
		{
			CPlayer@ player = getPlayerByNetworkId(vote.yesplayers[i]);
			if (player is null) continue;
			if (player.getTeamNum() == rules.getSpectatorTeamNum()) continue;
			foundteam = false;
			for (int i = 0; i < team_tallies.length; ++i)
			{
				if (player.getTeamNum() == team_tallies[i][0])
				{
					foundteam = true;
					team_tallies[i][1]++;
				}
			...
			if (!foundteam)
			{
				int[] tally = {player.getTeamNum(), 1, 0};
				team_tallies.push_back(tally);
			}
		}
		for (int i = 0; i < vote.noplayers.length; ++i)
		{
			CPlayer@ player = getPlayerByNetworkId(vote.noplayers[i]);
			if (player is null) continue;
			if (player.getTeamNum() == rules.getSpectatorTeamNum()) continue;
			foundteam = false;
			for (int i = 0; i < team_tallies.length; ++i)
			...
			if (!foundteam)
			{
				int[] tally = {player.getTeamNum(), 0, 1};
				team_tallies.push_back(tally);
			}
		}

		// Figure out which teams voted yes
		bool outcome = false;
		for (int i = 0; i < team_tallies.length; ++i)
		{
			if (doesTallyPass(team_tallies[i][1], team_tallies[i][2]))
			{
				outcome = true;
			}
		}
	...
	// Should be called after getting outcome
	string getMessage(VoteObject@ vote, bool outcome)
	{
		string message = "";
		if (outcome == false) return message;

		const array<string> TEAM_NAME =
		{
			"blue", "red", "green", "purple", "orange", "aqua", "teal", "gray"
		};
		for (int i = 0; i < team_tallies.length; ++i)
		{
			if (doesTallyPass(team_tallies[i][1], team_tallies[i][2]))
			{
				u8 teamnum = team_tallies[i][0];
				if (message != "")
					message += "\n";
				message += "--- Team {TEAM} has passed the vote: {YESCOUNT} vs {NOCOUNT} ---";
				message = getTranslatedString(message).replace(
							"{TEAM}", (teamnum < TEAM_NAME.length) ? TEAM_NAME[teamnum] : "#" + teamnum).replace(
							"{YESCOUNT}", "" + team_tallies[i][1]).replace(
							"{NOCOUNT}", "" + team_tallies[i][2]);
			}
		}
		return message;
	}

	bool doesTallyPass(int yes_count, int no_count)
	{
		return (yes_count > no_count);
	}
};

//setting up a vote suddendeath object
VoteObject@ Create_VoteSuddendeath(CPlayer@ byplayer)
{
	VoteObject vote;

	@vote.onvotepassed = VoteSuddendeathFunctor(byplayer);
	@vote.canvote = VoteSuddendeathCheckFunctor();
	// @vote.canvote = VoteSuddendeathCheckFunctor(byplayer.getTeamNum());
	@vote.tallyvote = VoteSuddendeathTallyFunctor();

	vote.title = "Begin sudden death?";
	vote.reason = "";
	vote.byuser = byplayer.getUsername();
	vote.forcePassFeature = "nextmap";		// TODO: sudden death force vote feature?
	vote.cancel_on_restart = true;
...
string getNameToDisplay(CPlayer@ player)
{
	...
	if (
		...
		return charname + " (" + player.getUsername() + ")";
	}
	else
	{
		return charname;
...
void onMainMenuCreated(CRules@ this, CContextMenu@ menu)
{
	...
	if (this.hasTag("suddendeath_vote_allowed")) // only add sudden death menu if it's enabled
	{
		CContextMenu@ suddendeathmenu = Menu::addContextMenu(votemenu, getTranslatedString("Sudden Death"));

		//suddendeath menu - also uses nextmap counter
		if (!(this.hasTag("suddendeath_vote_allowed")
				&& getGameTime() > this.get_u16("sudden_death_vote_time")))
		{
			Menu::addInfoBox(
				suddendeathmenu,
				getTranslatedString("Can't Start Vote"),
				getTranslatedString(
					"Voting for sudden death\n" +
					"isn't available yet...\n\n" +
					"The night is young!"
				)
			);
				 ...
				 && (!can_skip_wait || g_haveStartedVote))
		{
			...
			Menu::addInfoBox(suddendeathmenu, getTranslatedString("Can't Start Vote"), cantstart_info);
		}
		...
		else // Sudden death allowed
		{
			Menu::addInfoBox(
				suddendeathmenu,
				getTranslatedString("Vote sudden death"),
				getTranslatedString(
					"Vote to trigger sudden death\nto help end a stalemate.\n\n" +
					"- report any abuse of this feature.\n" +
					"\nTo Use:\n\n" +
					"- select the button if you're sure.\n" +
					"- everyone votes.\n"
				)
			);

			Menu::addSeparator(suddendeathmenu);
			CBitStream params;
			Menu::addContextItemWithParams(
				suddendeathmenu,
				getTranslatedString("Sudden Death! (I'm sure)"),
				"DefaultVotes.as", "Callback_Suddendeath",
				params
			);
		}
		Menu::addSeparator(suddendeathmenu);
	...
	//scramble menu - also shares nextmap counter
...
void Callback_Suddendeath(CBitStream@ params)
{
	CloseMenu(); //definitely close the menu
	...
	getRules().SendCommand(getRules().getCommandID(votesuddendeath_id), params2);
	onPlayerStartedVote();
}
...
void onCommand(CRules@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID(votesuddendeath_id))
	{
		u16 byplayerid;
		...
		if (byplayer !is null)
		{
			print(byplayer.getUsername() + " suddendeath vote");
			Rules_SetVote(this, Create_VoteSuddendeath(byplayer));
		}
