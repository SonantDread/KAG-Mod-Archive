...
class VoteTallyFunctor
{
	VoteTallyFunctor() {}
	bool getOutcome(VoteObject@ vote) { return vote.current_yes > vote.required_percent * vote.maximum_votes; }
	string getMessage(VoteObject@ vote, bool outcome) { return ""; }
};
...
void PassVote(VoteObject@ vote)
{
	...
	bool outcome;
	string message = "";
	if (vote.tallyvote !is null)
	{
		outcome = vote.tallyvote.getOutcome(@vote);
		message = vote.tallyvote.getMessage(@vote, outcome);
	}
	else
		outcome = vote.current_yes > vote.current_no + 1;//vote.required_percent * vote.maximum_votes;

	if (message == "")
		message = getTranslatedString("--- Vote {OUTCOME}: {YESCOUNT} vs {NOCOUNT} (out of {MAXVOTES}) ---").replace(
										"{OUTCOME}", getTranslatedString(outcome ? "passed" : "failed")).replace(
										"{YESCOUNT}", vote.current_yes + "").replace(
										"{NOCOUNT}", vote.current_no + "").replace(
										"{MAXVOTES}", vote.maximum_votes + "");

	client_AddToChat(message, vote_message_colour());
...
void Vote(VoteObject@ vote, CPlayer@ p, bool favour)
{
	...
	else
	{
		...
		if (favour)
		{
			vote.yesplayers.push_back(p_id);
			vote.current_yes++;
		}
		else
		{
			vote.noplayers.push_back(p_id);
			vote.current_no++;
		}
