...
void Unpack(CBlob@ this)
{
	...
	if (blob !is null && blob.getShape() !is null)
	{
		...
		if (blob.getName() == "ballista" && isServer())
		{
			Vec2f space = this.get_Vec2f(required_space);
			if (space != Vec2f(5, 4))
			{
				space *= getMap().tilesize;
				// nasty hack, bottom of ballista is ~20 down, height isn't accurate
				Vec2f ul(-space.x/2 + 2, 22 - space.y);
				Vec2f lr(space.x/2 - 2, 20);
				getMap().server_AddMovingSector(ul, lr, "no_build_new", blob.getNetworkID());
			}
		}
	}
...
Vec2f crate_getOffsetPos(CBlob@ blob, CMap@ map)
{
	...
	pos.y -= (space.y % 2 == 0) ? 2 : 6; // prevents wiggling

	// Make tall crates unpackable
	if (space.y > 4)
	{
		pos.y -= 4 * (space.y - 4);
	}

	Vec2f offsetPos = map.getAlignedWorldPos(pos); // center
	offsetPos.x -= int(space.x/2) * map.tilesize; // half space
	offsetPos.y -= int(space.y/2) * map.tilesize;
	return offsetPos;
}
...
void onRender(CSprite@ this)
{
	...
	if (blob.isAttached())
	{
		...
		if (local !is null && local.getBlob() is holder)
		{
			...
			if (space != Vec2f(5, 4))
			{
				Vec2f lr(aligned.x + space.x*8 * zoom*2, aligned.y + space.y*8 * zoom*2);
				SColor rect_color = color_white;
				rect_color.setAlpha(128);
				GUI::DrawRectangle(aligned, lr, rect_color);
				GUI::DrawLine2D(aligned, Vec2f(lr.x, aligned.y), color_white);
				GUI::DrawLine2D(aligned, Vec2f(aligned.x, lr.y), color_white);
				GUI::DrawLine2D(Vec2f(aligned.x, lr.y), lr, color_white);
				GUI::DrawLine2D(Vec2f(lr.x, aligned.y), lr, color_white);
			}
			else
			{
