...
void DoKnockedUpdate(CBlob@ this)
{
	...
	// Tick down both knock / dazzle timers
	...
	// Now determine effect - knocked > dazzled
	if (wasKnocked || frozen)
	{
		if (knockedRemaining < 2)
		{
			takekeys = key_action1 | key_action2 | key_action3;

			if (this.isOnGround() && !isDazzled(this))
			{
	...
	else if (wasDazzled)
	{
		...
		else if (this.hasTag("dazzle_sponged"))
		{
			this.getSprite().PlaySound("Sounds/wetfall2.ogg", 2.5f);
			this.Untag("dazzle_sponged");
		}
	}
