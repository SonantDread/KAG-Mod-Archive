...
void ManageBow(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	bool hasarrow = true;//archer.has_arrow;
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("cycle"))  //from standardcontrols
	{
		...
		while (count < arrowTypeNames.length)
		{
			...
			if (type == 0) //if reg arrows, dont care if u has zilch
			{
				archer.arrow_type = type;
				if (this.isMyPlayer())
				{
	...
	if (menu !is null)
	{
		...
		for (uint i = 0; i < arrowTypeNames.length; i++)
		{
			...
			if (button !is null)
			{
				...
				if (i == 0) //terrible modding speak for I DONT FUCKING GIVE 2 SHITS IF U DONT HAVE REG ARROW
				{
					enabled = true;
				}
