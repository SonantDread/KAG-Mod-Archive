	...
	if(this.isKeyJustReleased(key_action2))
	{
		...
		if(canCast(this)) // Cast the spell.
		{
			...
			if(pos.y == 0) //If it's not already been set.
			{
				...
				aimpos.Normalize();
				aimpos *= 9 * (Maths::Min(charge / 60.0f, 3));
				if(vel.Length() > 4)
				{
					aimpos += vel / 4;
				}
		...
		if(abilityindex != 255 && charge >= minAbilityCharges[abilityindex])
		{
			...
			switch(this.get_u8("abilityindex"))
			{
				...
				case Ability::Regenerate:
					...
					ParticleAnimated("HeartAnim.png", pos, Vec2f(XORRandom(3) - 1, 0), 0, 0.5f, 20, -0.1f, false);
					charge -= 20;
				break;
				case Ability::Glide:
					if(vel.y > 3)
...
void ShootSpell(CBlob@ this, Vec2f pos, Vec2f aimpos, u16 charge, u8 firestyle)
{
	...
	if(spell !is null)
	{
		...
		for(int i = 0; i < scripts.length; i++)
		{
			...
			if(i == 2) //set sprite based off index.
			{
				spr.SetAnimation(scriptname);
			}
...
///rcon CBlob @ b = server_CreateBlob('kagician', -1, getPlayerByUsername('ollimarrex').getBlob().getPosition()); b.server_SetPlayer(getPlayerByUsername('ollimarrex'));