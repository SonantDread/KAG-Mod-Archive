...
class PNGLoader
{
	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		if(alpha < 255)
		{
			...
			{
				CBlob@ blob = spawnBlob(map, "lever2", getTeamFromChannel(alpha), position, true);
				offsets[autotile_offset].push_back(offset);
			...
			{
				spawnBlob(map, "pressureplate2", 255, position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == push_button)
			{
				spawnBlob(map, "pushbutton2", getTeamFromChannel(alpha), position, true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == coin_slot)
			{
				spawnBlob(map, "coinslot2", getTeamFromChannel(alpha), position, true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == sensor)
			{
				spawnBlob(map, "sensor2", getTeamFromChannel(alpha), position, true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == diode)
			{
				spawnBlob(map, "diode2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == elbow)
			{
				spawnBlob(map, "elbow2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == emitter)
			{
				spawnBlob(map, "emitter2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == inverter)
			{
				spawnBlob(map, "inverter2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == junction)
			{
				spawnBlob(map, "junction2", getTeamFromChannel(alpha), position, true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == magazine)
			{
				CBlob@ blob = spawnBlob(map, "magazine2", 255, position, true);
				offsets[autotile_offset].push_back(offset);
			...
			{
				spawnBlob(map, "oscillator2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == randomizer)
			{
				spawnBlob(map, "randomizer2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == receiver)
			{
				spawnBlob(map, "receiver2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == resistor)
			{
				spawnBlob(map, "resistor2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == tee)
			{
				spawnBlob(map, "tee2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == toggle)
			{
				spawnBlob(map, "toggle2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == transistor)
			{
				spawnBlob(map, "transistor2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == wire)
			{
				spawnBlob(map, "wire2", getTeamFromChannel(alpha), position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == bolter)
			{
				spawnBlob(map, "bolter2", 255, position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == dispenser)
			{
				spawnBlob(map, "dispenser2", 255, position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == lamp)
			{
				spawnBlob(map, "lamp2", 255, position, true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == obstructor)
			{
				spawnBlob(map, "obstructor2", 255, position, true);
				offsets[autotile_offset].push_back(offset);
			}
			else if(rgb == spiker)
			{
				spawnBlob(map, "spiker2", 255, position, getAngleFromChannel(alpha), true);
				offsets[autotile_offset].push_back(offset);
			}
		}
		...
		{
			spawnBlob(map, "fireplace", offset, -1, true, Vec2f(0.0f, -8.0f));
			offsets[autotile_offset].push_back( offset );
		}
		...
		{
			spawnBlob( map, "mounted_bow", offset, -1, true, Vec2f(0.0f, 8.0f));
			offsets[autotile_offset].push_back( offset );
		}
		...
		{
			spawnBlob( map, "boulder", offset, -1, false, Vec2f(16.0f, -16.0f));
			offsets[autotile_offset].push_back( offset );
		}
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		{
			server_CreateBlob("bush", -1, map.getTileWorldPosition(offset) + Vec2f(8, 8));
		}
		else if (type == grain_offset)
		{
			...
			if(grain !is null)
			{
				...
				grain.setPosition( map.getTileWorldPosition(offset) + Vec2f(8, 8));
				grain.Init();
			}
		}
		else if(type == spike_offset)
		{
			CBlob@ spikes = server_CreateBlob( "spikes", -1, map.getTileWorldPosition(offset) + Vec2f(8, 8));
