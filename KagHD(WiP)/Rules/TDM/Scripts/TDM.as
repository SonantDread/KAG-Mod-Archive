...
shared class TDMCore : RulesCore
{
	...
	void SetupBases()
	{
		...
		if (map !is null)
		{
			...
			if (!getMap().getMarkers("blue main spawn", respawnPositions))
			{
				...
				respawnPos = Vec2f(300.0f, map.getLandYAtX(300.0f / map.tilesize) * map.tilesize - 64.0f);
				respawnPos.y -= 32.0f;
			...
			if (!getMap().getMarkers("red main spawn", respawnPositions))
			{
				...
				respawnPos = Vec2f(map.tilemapwidth * map.tilesize - 300.0f, map.getLandYAtX(map.tilemapwidth - (300.0f / map.tilesize)) * map.tilesize - 64.0f);
				respawnPos.y -= 32.0f;
	...
	void SpawnPowerup()
	{
		CBlob@ powerup = server_CreateBlob("powerup", -1, Vec2f(getMap().tilesize * 0.5f * getMap().tilemapwidth, 100.0f));
	}
	...
	void GiveSpawnResources(CBlob@ blob, CPlayer@ player)
	{
		...
		if (blob.getName() == "archer")
		{
			...
			CMap@ map = getMap();
			bool found = false;
			if (map.getBlobsInRadius(blob.getPosition(), 120.0f, @blobsInRadius))
			{
