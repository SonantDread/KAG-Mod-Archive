...
void GatherPickupBlobs(CBlob@ this)
{
	...
	if (this.getMap().getBlobsInRadius(this.getPosition(), this.getRadius() + 100.0f, @blobsInRadius))
	{
...
bool canBlobBePickedUp(CBlob@ this, CBlob@ blob)
{
	...
	return (((pos2 - pos).getLength() < (this.getRadius() + blob.getRadius()) + 40.0f)
	        && !blob.isAttached()
...
void onRender(CSprite@ this)
{
	...
	if (blob.isKeyPressed(key_inventory))
	{
		...
		if (pickBlob !is null)
		{
			pickBlob.RenderForHUD((blob.getAimPos() + Vec2f(0.0f, 16.0f)) - blob.getPosition() , RenderStyle::normal);
		}
	}
		...
		if (blob.get("pickup blobs", @pickupBlobs))
		{
			...
			for (uint i = 0; i < pickupBlobs.length; i++)
			{
				...
				if (b is closestBlob)
				{
					...
					GUI::DrawText(b.getInventoryName(), getDriver().getScreenPosFromWorldPos(b.getPosition() - Vec2f(0, -b.getHeight() / 2)) - Vec2f(dimensions.x / 2, -16.0f), color_white);
