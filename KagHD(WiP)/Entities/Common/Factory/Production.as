				...
				while (!item.requirements.isBufferEnd())
				{
					...
					if (caller.hasBlob( name, 1 ))
					{
						...
						if (!added)
						{
							...
							caller.CreateGenericButton( "$"+name+"$", Vec2f(-8.0f*item_count + 24.0f*i, -count), this, this.getCommandID("add req"), "Put in " + friendlyName, params );
							count++;
