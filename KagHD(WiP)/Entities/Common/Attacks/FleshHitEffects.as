	...
	if (damage > 0.1f && (hitterBlob !is this || customData == Hitters::crush))  //sound for anything actually painful
	{
		...
		if (showblood)
		{
			...
			if (capped_damage > 0.25f)
			{
				...
				{
					ParticleBloodSplat(worldPoint + getRandomVelocity(0, 1.5f + capped_damage * 2.0f * XORRandom(2), 360.0f), false);
				}
			}
			...
			if (capped_damage > 0.01f)
			{
				...
				{
					Vec2f vel = getRandomVelocity(angle, 1.0f + 0.6f * capped_damage * 0.1f * XORRandom(40), 60.0f);
					vel.y -= 3.0f * capped_damage;
