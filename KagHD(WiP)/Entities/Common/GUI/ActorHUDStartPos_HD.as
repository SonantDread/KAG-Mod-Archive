...
void DrawInventoryOnHUD( CBlob@ this, Vec2f tl )
{
	...
	for (int i = 0; i < inv.getItemsCount(); i++)
	{
		...
		if (drawn.find(name) == -1)
		{
			...
			GUI::DrawIcon( item.inventoryIconName, item.inventoryIconFrame, item.inventoryFrameDimension, tl+Vec2f(0 + (drawn.length-1) * 32,0), 0.5f );
...
void DrawCoinsOnHUD( CBlob@ this, const int coins, Vec2f tl, const int slot )
{
	if (coins > 0)
	{
		GUI::DrawIconByName( "$COIN$", tl+Vec2f(-8 + slot * 32, -16) );
