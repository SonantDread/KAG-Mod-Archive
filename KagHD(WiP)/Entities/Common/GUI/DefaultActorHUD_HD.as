...
#include "ActorHUDStartPos_HD.as";
...
void renderHPBar( CBlob@ blob, Vec2f origin)
{
    ...
    for (f32 step = 0.0f; step < blob.getInitialHealth(); step += 0.5f)
    {
        ...
        if (thisHP > 0)
        {
            Vec2f heartoffset = (Vec2f(-8,10) * 1);
            Vec2f heartpos = origin+Vec2f(segmentWidth*HPs,0)+heartoffset;

            if (thisHP <= 0.125f)
            {
                GUI::DrawIcon(heartFile, 4, Vec2f(24,24), heartpos);
            }
            else if (thisHP <= 0.25f)
            {
                GUI::DrawIcon(heartFile, 3, Vec2f(24,24), heartpos);
            }
            else if (thisHP <= 0.375f)
            {
                GUI::DrawIcon(heartFile, 2, Vec2f(24,24), heartpos);
            }
            else
            {
                GUI::DrawIcon(heartFile, 1, Vec2f(24,24), heartpos);
            }
        }
