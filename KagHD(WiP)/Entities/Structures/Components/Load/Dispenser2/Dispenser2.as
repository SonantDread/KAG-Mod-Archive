...
class Dispenser : Component
{
	...
	void Activate(CBlob@ this)
	{
		...
		{
			CBlob@[] blobs;
			getMap().getBlobsAtPosition((offset * -1) * 16 + position, @blobs);

			for(uint i = 0; i < blobs.length; i++)
			{
				CBlob@ blob = blobs[i];
				if(blob.getName() != "magazine2" || !blob.getShape().isStatic()) continue;
				...
				if(getMap().rayCastSolid(position + offset * 10, position + offset * 22)) break;

				blob.server_PutOutInventory(item);

				item.setPosition(position + (offset * 16));
				item.AddForce(offset * (item.getMass() * 9.6f));
