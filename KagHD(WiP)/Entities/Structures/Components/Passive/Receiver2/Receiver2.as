	...
	u8 Special(MapPowerGrid@ grid, u8 power_old, u8 power_new)
	{
		...
		if(this.get(EMITTER, @id))
		{
			for(u8 i = 0; i < id.length; i++)
			{
				...
				Component@ emitter2 = null;
				if(!blob.get("component", @emitter2))
				{
					this.removeAt(EMITTER, i);
					continue;
				}

				if(grid.getPower(emitter2.x, emitter2.y) > 0)
				{
					if(i > 0)
					{
						this.removeAt(EMITTER, i);
						this.push(EMITTER, grid.getID(emitter2.x, emitter2.y));
					}
					return power_source;
				}
			}
...
void onSetStatic(CBlob@ this, const bool isStatic)
{
	...
	const Vec2f POSITION = this.getPosition() / 16;
	...
	if(getNet().isServer())
	{
		...
		for(u8 i = 1; i < signal_strength; i++)
		{
			...
			Component@ emitter2 = null;
			if(!blob.get("component", @emitter2)) continue;
	...
	CSpriteLayer@ layer = sprite.addSpriteLayer("background", "Receiver2.png", 32, 32);
