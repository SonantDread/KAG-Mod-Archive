...
{
	if(this.getDistanceTo(caller) > 32.0f || !this.getShape().isStatic()) return;

	CBlob@ carried = caller.getAttachments().getAttachmentPointByName("PICKUP").getOccupied();
	if(carried !is null)
	{
		if(carried.getRadius() > 16.0f || !carried.canBePutInInventory(this)) return;
