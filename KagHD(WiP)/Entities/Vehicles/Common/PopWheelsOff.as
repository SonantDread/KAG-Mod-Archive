...
{
	if ( caller.getDistanceTo(this) < this.getRadius() && !this.hasTag("immobile") )
	{
		caller.CreateGenericButton( 2, Vec2f(0.0f, 32.0f), this, this.getCommandID("pop_wheels"), "Immobilise");
	}
}
...
void PopWheels(CBlob@ this, bool addparticles = true)
{
	...
	for(int i = 0; i < sprite.getSpriteLayerCount(); ++i)
	{
		...
		if(wheel !is null && wheel.name.substr(0,2) == "!w")
		{
			if(addparticles)
			{
				...
				makeGibParticle( "Entities/Vehicles/Common/WoodenWheels.png", pos+wheel.getOffset(), vel + getRandomVelocity( 90, 5, 80 ), 0, 0, Vec2f (32,32), 2.0f, 20, "/material_drop", 0 );
			}
	...
	//add chocks
	CSpriteLayer@ chocks = sprite.addSpriteLayer( "!chocks", "Entities/Vehicles/Common/WoodenChocks.png", 64, 32 );

    if (chocks !is null)
    {
        Animation@ anim = chocks.addAnimation( "default", 0, false );
        anim.AddFrame(0);
        chocks.SetOffset( Vec2f(0,this.getHeight() * 0.5f - 5.0f) );
    }
}