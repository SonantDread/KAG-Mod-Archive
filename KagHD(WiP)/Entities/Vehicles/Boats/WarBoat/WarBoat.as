...
void onInit(CBlob@ this)
{
	...
	this.getShape().SetOffset(Vec2f(-12, 32));
	...
	//front bits
	{
		Vec2f[] shape = { Vec2f(86.0f,  8.0f) - pos_off,
		                  Vec2f(146.0f,  14.0f) - pos_off,
		                  Vec2f(186.0f,  72.0f) - pos_off,
		                  Vec2f(138.0f,  48.0f) - pos_off
		                };
		this.getShape().AddShape(shape);
	}
	...
	//back bit
	{
		Vec2f[] shape = { Vec2f(16.0f,  51.0f) - pos_off,
		                  Vec2f(28.0f, 51.0f) - pos_off,
		                  Vec2f(28.0f, 372.0f) - pos_off,
		                  Vec2f(22.0f, 72.0f) - pos_off
		                };
		this.getShape().AddShape(shape);
	}
	//rudder
	...
	CSpriteLayer@ front = sprite.addSpriteLayer("front layer", sprite.getConsts().filename, 192, 112);
	if (front !is null)
	{
	...
	CSpriteLayer@ flag = sprite.addSpriteLayer("flag", sprite.getConsts().filename, 80, 112);
	if (flag !is null)
	{
		...
		flag.SetOffset(Vec2f(56, -48));
	}

	this.set_f32("oar offset", 108.0f);

	// add pole ladder
	getMap().server_AddMovingSector(Vec2f(-56.0f, -64.0f), Vec2f(-24.0f, 0.0f), "ladder", this.getNetworkID());
	// add back ladder
	getMap().server_AddMovingSector(Vec2f(-100.0f, 0.0f), Vec2f(-70.0f, 40.0f), "ladder", this.getNetworkID());
