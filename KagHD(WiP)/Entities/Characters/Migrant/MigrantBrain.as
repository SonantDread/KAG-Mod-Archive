...
void GoToBlob( CBrain@ this, CBlob @target )
{
	...
	if (targetDistance < 80.0f && target.hasTag("player")) // keep distance from player
	{
		return;
	}
    ...
    if (!justGo && blob.get_bool("justgo"))
    {
        ...
        if (XORRandom(100) == 0 && (blob.get_Vec2f("target spot") - targetpos).getLength() > 100.0f)
        {
            Repath( this );
        }
	...
	if (!justGo)
    {
        ...
        case CBrain::wrong_path:
			...
			if (XORRandom(200) == 0)
			{
				if (Maths::Abs( targetpos.x - mypos.x) < 100.0f)
					set_emote( blob, targetpos.y > mypos.y ? Emotes::down : Emotes::up);
				else
					set_emote( blob, targetpos.x > mypos.x ? Emotes::right : Emotes::left);
			}
...
void JumpOverObstacles( CBlob@ blob )
{
	Vec2f pos = blob.getPosition();	  
	if (!blob.isOnLadder())
	if ( (blob.isKeyPressed( key_right ) && (getMap().isTileSolid( pos + Vec2f( 2.6f*blob.getRadius(), blob.getRadius())*1.0f ) || blob.getShape().vellen < 0.1f) ) ||
		 (blob.isKeyPressed( key_left )  && (getMap().isTileSolid( pos + Vec2f(-2.6f*blob.getRadius(), blob.getRadius())*1.0f ) || blob.getShape().vellen < 0.1f) ) )
	{
		blob.setKeyPressed( key_up, true );
	}
}
...
bool Runaway( CBrain@ this, CBlob@ blob, CBlob@ attacker )
{
	...
	//out of sight?
	if ((mypos - hispos).getLength() > 400.0f)
	{
		return false;
	}
