...
void onTick(CMovement@ this)
{
	...
	if (blob.isInWater() && !isknocked)
	{
		...
		if (up)
		{
			if (vel.y > -swimspeed)
			{
				if (!map.isInWater(pos + Vec2f(0, -16)))
				{
					waterForce.y -= 1.2f;
				}
				else
	        ...
	        !blob.isOnLadder() && (up || left || right))  //key pressed
	{
		        ...
		        !onground)
		{
			...
			if (moveVars.jumpCount > 5) //wait some time to be properly in the air
			{
				...
				{
					f32 walljumpforce = 8.0f;
					vel.Set(surface_right ? -walljumpforce : walljumpforce, -4.0f);
					blob.setVelocity(vel);
			...
			if (dust)
			{
				Vec2f dust_pos = (Vec2f(right ? 8.0f : -8.0f, 0.0f) + pos);
	...
	if (moveVars.jumpFactor > 0.01f && !isknocked)
	{
		...
		{
			moveVars.jumpStart = 1.4f;
			moveVars.jumpMid = 0.4f;
			moveVars.jumpEnd = 0.2f;
			...
			{
				TileType tile = blob.getMap().getTile(blob.getPosition() + Vec2f(0.0f, blob.getRadius() + 8.0f)).type;
	...
	bool left_or_right = (left || right);
	{
		...
		Vec2f walkDirection;
		const f32 turnaroundspeed = 2.6f;
...
bool canVault(CBlob@ blob, CMap@ map, f32 movingside)
{
	        ...
	        map.isTileSolid(Vec2f(pos.x + movingside * tilesize, pos.y + tilesize * (offsetheight + 2))))
	{
		...
		bool hasRayFace = map.rayCastSolid(pos + Vec2f(0, -12), pos + Vec2f(movingside * 24, -12));
		if (hasRayFace)
			return false;

		bool hasRayFeet = map.rayCastSolid(pos + Vec2f(0, 12), pos + Vec2f(movingside * 24, 12));
		...
		{
			return !checkForSolidMapBlob(map, pos + Vec2f(movingside * 24, -12)) &&
			       checkForSolidMapBlob(map, pos + Vec2f(movingside * 24, 12));
		}
	}
	return false;
}
