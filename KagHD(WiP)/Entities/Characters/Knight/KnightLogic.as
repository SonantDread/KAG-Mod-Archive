...
void onInit(CBlob@ this)
{
    ...
    addShieldVars(this, SHIELD_BLOCK_ANGLE, 4.0f, 10.0f);
...
void onTick(CBlob@ this)
{
             ...
             (pressed_a2 || (specialShieldState)))
    {
        ...
        {
            bool forcedrop = (vel.y > Maths::Max(Maths::Abs(vel.x), 4.0f) &&
                ...
                else
                {
                    ...
                    if (!inair)
                    {
                        ...
                        if (knight.slideTime > 0)
                        {
                            ...
                            {
                                Vec2f velr = getRandomVelocity(!this.isFacingLeft() ? 70 : 110, 8.6f, 80.0f);
            ...
            else if (bombType == 1)
            {
                if (getNet().isServer())
                {
                    ...
                    if (blob !is null)
                    {
                        ...
                        blob.set_f32("explosive_radius", 184.0f);
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info)
{
    ...
    Vec2f pos = blobPos - thinghy * 12.0f + vel + Vec2f(0, -8);
    vel.Normalize();

    f32 attack_distance = Maths::Min(DEFAULT_ATTACK_DISTANCE + Maths::Max(0.0f, 3.50f * this.getShape().vellen * (vel * thinghy)), MAX_ATTACK_DISTANCE);
        ...
        for (uint i = 0; i < hitInfos.length; i++)
        {
            ...
            if (b !is null && !dontHitMore) // blob
            {
                ...
                if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
                {
                    ...
                    {
                        Vec2f tpos = map.getTileWorldPosition(hi.tileOffset) + Vec2f(8, 8);
