...
const u8 GRID_SIZE = 24;
const u8 GRID_PADDING = 52;
...
void onInit(CInventory@ this)
{
	...
	AddIconToken("$BUILDER_CLEAR$", "BuilderIcons.png", Vec2f(32, 32), 8);
...
void MakeBlocksMenu(CInventory@ this, const Vec2f &in INVENTORY_CE)
{
	...
	if(menu !is null)
	{
		...
		const Vec2f TOOL_POS = menu.getUpperLeftPosition() + Vec2f(-2, 2) * GRID_SIZE / 2;
		...
		if(time + SHOW_NO_BUILD_TIME > getGameTime())
		{
			...
			for (f32 step_x = 0.0f; step_x < space.x ; ++step_x)
			{
				for (f32 step_y = 0.0f; step_y < space.y ; ++step_y)
				{
					...
					Vec2f pos = aligned + (temp - Vec2f(0.5f,0.5f)* map.tilesize) * 4 * zoom;
					if (!onground || map.getSectorAtPosition(v , "no build") !is null || map.isTileSolid(v) || blobBlockingBuilding(map, v))
					{
						// draw red
						GUI::DrawIcon( "CrateSlots.png", 5, Vec2f(16,16), pos, zoom );
					}
					else
					{
						// draw white
						GUI::DrawIcon( "CrateSlots.png", 9, Vec2f(16,16), pos, zoom );
					}
				}
			}
		}
