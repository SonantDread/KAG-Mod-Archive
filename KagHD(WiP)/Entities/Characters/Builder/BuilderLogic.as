...
{
	this.set_f32("pickaxe_distance", 20.0f);
...
void onTick(CBlob@ this)
{
	...
	if(ismyplayer && this.isKeyPressed(key_action1) && !this.isKeyPressed(key_inventory)) //Don't let the builder place blocks if he/she is selecting which one to place
	{
		...
		hitdata.blobID = 0;
		hitdata.tilepos = bc.buildable ? bc.tileAimPos : Vec2f(-16, -16);
	}
...
void Pickaxe(CBlob@ this)
{
	...
	Vec2f pos = blobPos - Vec2f(4, 0).RotateBy(-aimangle);
...
bool ExtraQualifiers(CBlob@ this, CBlob@ b, Vec2f tpos)
{
	...
	if (bshape.isStatic())
	{
		bool bigenough = bshape.getWidth() >= 16 &&
		                 bshape.getHeight() >= 16;
		...
		else
		{
			...
			Vec2f tileCenterPos = Vec2f(s32(bpos.x / 16), s32(bpos.y / 16)) * 16 + Vec2f(8, 8);
