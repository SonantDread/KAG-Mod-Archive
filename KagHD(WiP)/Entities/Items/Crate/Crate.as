...
void onInit( CBlob@ this )
{
    ...
    if (this.exists("frame"))
    {
		...
		{
			CSpriteLayer@ icon = this.getSprite().addSpriteLayer( "icon", "/MiniIcons.png" , 32, 32, this.getTeamNum(), -1 );
			if(icon !is null)
			{
			...
			AddIconToken( "$crate_"+packed+"$", "/MiniIcons.png", Vec2f(32,32), frame );
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	...
	if (this.hasTag("unpackall"))
	{
		caller.CreateGenericButton( 12, Vec2f(0,-16), this, this.getCommandID("unpack"), "Unpack all" );
	}
	else if (hasSomethingPacked(this) && !canUnpackHere(this))
	{
		...
		CButton@ button = caller.CreateGenericButton( 12, Vec2f(0,-16), this, 0, msg );
		if (button !is null) {
    ...
    {
        caller.CreateGenericButton( "$DISABLED$", Vec2f(0,-16), this, this.getCommandID("stop unpack"), "Stop " + this.get_string("packed name") );
    }
    else if (hasSomethingPacked(this))
    {
		caller.CreateGenericButton( 12, Vec2f(0,-16), this, this.getCommandID("unpack"), "Unpack " + this.get_string("packed name") );
    }
...
void ShowParachute( CBlob@ this )
{
    ...
    CSpriteLayer@ parachute = sprite.addSpriteLayer( "parachute",   64, 64 );

    if (parachute !is null)
    {
        Animation@ anim = parachute.addAnimation( "default", 0, true );
        anim.AddFrame(4);
        parachute.SetOffset( Vec2f(0.0f, - 34.0f) );
    }
}
...
void onDie(CBlob@ this)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        CParticle@ temp = makeGibParticle( fname, pos, vel + getRandomVelocity( 90, 1 , 120 ), 9, 2+i, Vec2f (32,32), 2.0f, 20, "Sounds/material_drop.ogg", 0 );
    }
}
...
bool canUnpackHere( CBlob@ this )
{
	...
	if (this.isAttached())
	{
		...
		if (parent !is null)
		{
			return ((!water && parent.isOnGround()) || (water && map.isInWater(parent.getPosition() + Vec2f(0.0f, 16.0f))));	  
		}
	}
...
void onRender( CSprite@ this )
{
    ...
    if (blob.isAttached())
    {
        ...
        if (local !is null && local.getBlob() is holder)
		{
			...
			GUI::DrawIcon( "CrateSlots.png", 0, Vec2f(80,64), aligned, zoom);
