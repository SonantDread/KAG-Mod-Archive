...
void onTick(CSprite@ this)
{
	...
	if (heatlayer !is null)
	{
		...
		if (heatPercent > 0.1f)
		{
			...
			blob.SetLight(true);
			blob.SetLightRadius(heatPercent * 48.0f);
...
void makeSteamParticle(CBlob@ this, const Vec2f vel, const string filename = "SmallSteam")
{
	...
	Vec2f random = Vec2f(XORRandom(256) - 128, XORRandom(256) - 128) * 0.031250f * rad;
...
void makeSteamPuff(CBlob@ this, const f32 velocity = 1.0f, const int smallparticles = 10, const bool sound = true)
{
	...
	{
		f32 randomness = (XORRandom(64) + 64) * 0.031250f * 0.5f + 0.75f;
	...
	if (this.isAttached())
	{
		...
		if (holder.getName() == required_class)
		{
			...
			// delay drill
			{
				...
				const f32 attack_distance = 12.0f;
