...
{
	CSpriteLayer@ bed = this.addSpriteLayer("bed", "Quarters.png", 60, 24);
	if (bed !is null) {
		{
		...
		bed.SetOffset(Vec2f(8, 12));
		bed.SetVisible(true);
	}

	CSpriteLayer@ zzz = this.addSpriteLayer("zzz", "Quarters.png", 16, 16);
	if (zzz !is null) {
		{
		...
		zzz.SetOffset(Vec2f(-6, -12));
		zzz.SetLighting(false);
		zzz.SetVisible(false);
	}

	CSpriteLayer@ backpack = this.addSpriteLayer("backpack", "Quarters.png", 32, 32);
	if (backpack !is null) {
		{
		...
		backpack.SetOffset(Vec2f(-24, 14));
...
void onInit(CBlob@ this)
{
	...
	// ICONS
	AddIconToken("$quarters_beer$", "Quarters.png", Vec2f(48,48), 7);
	AddIconToken("$quarters_meal$", "Quarters.png", Vec2f(128,48), 2);
	AddIconToken("$quarters_egg$", "Quarters.png", Vec2f(48,48), 8);
	AddIconToken("$quarters_burger$", "Quarters.png", Vec2f(48,48), 9);
...
void onAttach(CBlob@ this, CBlob@ attached, AttachmentPoint@ attachedPoint)
{
	...
	if (sprite !is null)
	{
		...
		if (getNet().isClient())
		{
			CSpriteLayer@ bed_head = sprite.addSpriteLayer("bed head", texName, 32, 32, attached.getTeamNum(), attached.getSkinNum());
			if (bed_head !is null)
			{
