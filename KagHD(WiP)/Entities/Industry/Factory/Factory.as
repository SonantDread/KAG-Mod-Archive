...
void onInit( CBlob@ this )
{
	...
	AddIconToken( "$take_scroll$", "GUI/InteractionIcons.png", Vec2f(64,64), 19, 1 );
	...
	AddIconToken( "$kill_children$", "GUI/InteractionIcons.png", Vec2f(64,64), 10, 1 );
	...
	this.set_Vec2f("production offset", Vec2f( -16.0f, 0.0f) );																		
}
...
void AddButtonsForSet( CBlob@ this, CGridMenu@ menu, ScrollSet@ set )
{
	...
	for (uint i = 0; i < set.names.length; i++)
	{
		...
		{	
			...
			CGridButton@ button = menu.AddButton( "MiniIcons.png", def.scrollFrame, Vec2f(32,32), def.name, this.getCommandID("upgrade factory"), Vec2f(1,1), params );
			if (button !is null)
			{
