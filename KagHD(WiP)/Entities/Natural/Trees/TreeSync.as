...
void InitVars( CBlob@ this )
{
	...
	AddIconToken( "$Tree$", "Entities/Common/GUI/HelpIcons.png", Vec2f(32,32), 1 );
	AddIconToken( "$Axe$", "Entities/Common/GUI/HelpIcons.png", Vec2f(32,32), 13 );
    ...
    if (vars.height < vars.max_height)
    {
		...
		if (map !is null)
		{
			...
			if(raycast)
			{
				...
				if(!unbreakable && (partpos - endpos).Length() > 8.0f)
				{
					unbreakable = !treeBreakableTile(map, map.getTile(endpos + Vec2f(0,-16)).type );
					killtwo = true;
				}
			}
		}
		...
		else
		{
			if(raycast && map !is null)
			{
				...
				if(killtwo)
				{
					map.server_DestroyTile(endpos + Vec2f(0,-16), 100.0f, this);
				}
			}
