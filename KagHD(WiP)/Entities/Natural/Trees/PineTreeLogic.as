...
void GrowSprite( CSprite@ this, TreeVars@ vars )
{
    ...
    for (uint i = 0; i < segments.length; i++)
    {
        ...
        if (segment !is null && !segment.gotsprites)
        {
            ...
            {
                CSpriteLayer@ newsegment = this.addSpriteLayer( "segment "+i, "Entities/Natural/Trees/Trees.png" , 32, 32, 0, 0 );
            ...
            {
                CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves "+i, "Entities/Natural/Trees/Trees.png" , 64, 64, 0, 0 );
            ...
            else if (segment.grown_times == 5)
            {
				...
				{
					bool flip = false;
					CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves side "+i, "Entities/Natural/Trees/Trees.png" , 64, 64, 0, 0 );
					...
					if (newsegment !is null)
					{
						...
						newsegment.SetOffset(segment.start_pos + Vec2f(((vars.max_height - i*2) + XORRandom(8))*0.5 + 16.0f , 16.0f));
					}

					if (XORRandom(2) == 0)
					{
						CSpriteLayer@ secondnewsegment = this.addSpriteLayer( "leaves doubleside "+i, "Entities/Natural/Trees/Trees.png" , 64, 64, 0, 0 );
						...
						if (secondnewsegment !is null)
						{
							...
							secondnewsegment.SetOffset(segment.start_pos + Vec2f(((vars.max_height - i*2) + XORRandom(8))*0.5 + 16.0f , 16.0f));
						}
					}
				}
			}
