...
void GrowSprite( CSprite@ this, TreeVars@ vars )
{
    ...
    for (uint i = 0; i < segments.length; i++)
    {
        ...
        if (segment !is null && !segment.gotsprites)
        {
            ...
            {
                CSpriteLayer@ newsegment = this.addSpriteLayer( "segment "+i, "Entities/Natural/Trees/Trees.png" , 64, 32, 0, 0 );
                ...
                if (newsegment !is null)
                {
					...
					Vec2f offset = segment.start_pos + Vec2f(-16,0);
            ...
            else if (segment.grown_times == 3 && i > 0 && i != vars.max_height-1)
            {
                ...
                {
                    CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves "+i+" "+spriteindex, "Entities/Natural/Trees/Trees.png" , 64, 64, 0, 0 );
                    ...
                    if (newsegment !is null)
                    {
                        ...
                        newsegment.RotateBy( segment.angle, Vec2f(0,0) );
                        Vec2f offset = Vec2f(10*scalex,-16);

                        if (spriteindex == 1) {
                            offset = Vec2f(-20*scalex, 0);
                        }
                        else if ( spriteindex == 2 ) {
                            offset = Vec2f(32*scalex, -8);
                        }
			...
			{
				f32 flipsign = 1.0f;
				CSpriteLayer@ newsegment = this.addSpriteLayer( "roots", "Entities/Natural/Trees/Trees.png" , 64, 32, 0, 0 );
				...
				if (newsegment !is null)
				{
					...
					newsegment.ResetTransform();
					newsegment.SetRelativeZ( -16.0f );
					newsegment.RotateBy( segment.angle, Vec2f(0,0) );
					newsegment.SetOffset(segment.start_pos + Vec2f(0,16.0f));
            ...
            {
                CSpriteLayer@ newsegment = this.addSpriteLayer( "extra leaves top", "Entities/Natural/Trees/Trees.png" , 64, 64, 0, 0 );
            ...
            else if (segment.grown_times == 5 && i == vars.max_height-1) //top of the tree
            {
                ...
                for (int spriteindex = 0; spriteindex < 3; spriteindex++)
                {
                    ...
                    if (newsegment !is null)
                    {
                        ...
                        newsegment.SetRelativeZ( z );
                        newsegment.RotateBy( segment.angle, Vec2f(0,0) );
                        Vec2f offset = Vec2f(32*scalex,8);

                        if (spriteindex == 1) {
                            offset = Vec2f(-32*scalex, -8);
                        }
                        else if ( spriteindex == 2 ) {
                            offset = Vec2f(44*scalex, -24);
                        }
                ...
                {
                    CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves "+i+" "+3, "Entities/Natural/Trees/Trees.png" , 128, 64, 0, 0 );
                ...
                {
                    CSpriteLayer@ newsegment = this.addSpriteLayer( "leaves "+i+" "+4, "Entities/Natural/Trees/Trees.png" , 128, 64, 0, 0 );
            ...
            if (segment.grown_times < 5)
            {
                ...
                if (segmentlayer !is null)
                {
                    ...
                    {
                        segmentlayer.SetOffset(segmentlayer.getOffset()+Vec2f(16,0));
                    }
                }
            }
        }
