...
void onInit(CBlob@ this)
{
	...
	//for aquatic animal
	this.set_f32(terr_rad_property, 128.0f);
	this.set_f32(target_searchrad_property, 192.0f);
	...
	this.getCurrentScript().runProximityRadius = 640.0f;
...
void onTick(CBlob@ this)
{
	...
	if(getNet().isServer() && getGameTime() % 10 == 0)
	{
		...
		if(this.get_u8(state_property) == MODE_TARGET )
		{
			...
			if(b !is null && this.getDistanceTo(b) < 112.0f)
			{
				this.Tag(chomp_tag);
			}
			else
	...
	bool significantvel = (vel.LengthSquared() > 2.0f && Maths::Abs(vel.x) > 0.4f);
