	...
	if(getNet().isServer())
	{
		if(this.getTickSinceCreated() > 30)
		{
			if(!this.hasTag("nobuild sector added") && this.getTickSinceCreated() > 30)
			{
				...
				map.server_AddSector( pos+Vec2f(-24,-64), pos+Vec2f(24,32), "no build", "", this.getNetworkID() );

				map.server_SetTile( pos+Vec2f(-16,24), CMap::tile_bedrock );
				map.server_SetTile( pos+Vec2f( 0,24), CMap::tile_bedrock );
				map.server_SetTile( pos+Vec2f( 16,24), CMap::tile_bedrock );
		...
		if(!this.hasAttached())
		{
			...
			else
			{
				...
				this.getMap().server_AddSector( pos+Vec2f(-24,-16), pos+Vec2f(24,32), "no build", "", this.getNetworkID() );
			}
		}
		else
		{
