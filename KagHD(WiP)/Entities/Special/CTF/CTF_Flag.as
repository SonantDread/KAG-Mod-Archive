...
{
	this.SetZ(-10.0f);
    CSpriteLayer@ flag = this.addSpriteLayer( "flag_layer", "/CTF_Flag.png", 64, 32, this.getBlob().getTeamNum(), this.getBlob().getSkinNum() );

    if (flag !is null)
    {
		flag.SetOffset(Vec2f(30,-16));
...
void onRender(CSprite@ this)
{
	...
	if (returncount > 0)
	{
		...
		if (returncount < return_time) {
			GUI::DrawProgressBar( Vec2f(pos2d.x - 60.0f, pos2d.y - 20.0f + wave), Vec2f(pos2d.x + 60.0f, pos2d.y + 10.0f + wave), 1.0f - float(returncount)/float(return_time) );
			
			if (getGameTime() % 15 > 10)
			{
				if(!shouldFastReturn(blob))
				{
					GUI::DrawIconByName( "$ALERT$", Vec2f(pos2d.x-64.0f, pos2d.y-60.0f + wave) );
				}
				else
				{
					//maybe a check icon?
