...
{
	this.set_Vec2f("shop offset", Vec2f(-220, 20));
...
{
    this.CreateRespawnPoint( "base", Vec2f(0.0f, 32.0f) );
    //set up classes
	AddIconToken( "$WAR_BASE$", "Rules/WAR/WarGUI.png", Vec2f(96,64), 10+2 );
    AddIconToken( "$builder_class_icon$", "GUI/MenuItems.png", Vec2f(64,64), 8 );
    AddIconToken( "$knight_class_icon$", "GUI/MenuItems.png", Vec2f(64,64), 12 );
    AddIconToken( "$archer_class_icon$", "GUI/MenuItems.png", Vec2f(64,64), 16 );
    //AddIconToken( "$upgrade_base$", "GUI/TechnologyIcons.png", Vec2f(16,16), 8 );
    //AddIconToken( "$food$", "Grain.png", Vec2f(8,8), 0 );
    AddIconToken( "$seed$", "Seed.png", Vec2f(16,16), 0 );
    AddIconToken( "$change_class$", "GUI/InteractionIcons.png", Vec2f(64,64), 12, 2 );
    InitClasses( this );
	//setup shop
	InitWorkshop( this );
    //set up shape
	...
	this.inventoryButtonPos.Set(-32.0f, 40.0f);
}
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
    ...
    if (this.get_u8("upgrade_level") >= 1)
    {
		...
		{			
			f32 offsetx = classButtonPos.x + 20.0f;
			if ((this.getPosition() + Vec2f(this.isFacingLeft() ? -offsetx : offsetx, classButtonPos.y) - caller.getPosition()).Length() < 36.0f) {
