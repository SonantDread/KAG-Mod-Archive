...
const SColor color_bunn_main_spawn(255, 254, 0, 0);
...
const SColor color_carrot(255, 227, 138, 22);
const SColor color_rocknut(255, 114, 114, 114);
const SColor color_shrub(255, 60, 232, 43);
//fauna
const SColor color_shark(0xff2cafde);
const SColor color_fish(0xff79a8a3);
const SColor color_bison(0xffb75646);
const SColor color_chicken(0xff941b1b);
const SColor color_piglet(255, 255, 125, 255);
//ostia et pedicas
...
const SColor color_coal(255, 44, 43, 43);
const SColor color_scandium(255, 190, 3, 0);
const SColor color_cobalt(255, 44, 43, 103);
const SColor color_angelite(255, 245, 245, 245);
...
class PNGLoader
{
	...
	if (pixel == color_tile_ground) {
        ...
        else if (pixel == color_carrot)
		{
            AddMarker( map, offset, "bf_spawncarrot" );
			offsets[autotile_offset].push_back( offset );
		} 
		else if (pixel == color_rocknut)
		{
            spawnBlob( map, "bf_rocknutplant", offset, -1);
			map.SetTile(offset, CMap::tile_ground_back );
			offsets[autotile_offset].push_back( offset );
		} 
        else if (pixel == color_shrub)
		{
            offsets[bf_shrub_offset].push_back( offset );
			offsets[autotile_offset].push_back( offset );
            AddMarker( map, offset, "bf_spawnshrub" );
        }
		// Fauna
		...
		else if (pixel == color_piglet)
		{
            AddMarker( map, offset, "bf_spawnpiglet" );
			offsets[autotile_offset].push_back( offset );
		} 
		//DOORS AND TRAPS
		...
		else if (pixel == color_coal)
		{
			spawnBlob( map, "bf_coaldepo", offset, -1, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_cobalt)
		{
			spawnBlob( map, "bf_cobaltdepo", offset, -1, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_scandium)
		{
			spawnBlob( map, "bf_scandiumdepo", offset, -1, true );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_angelite)
		{
			spawnBlob( map, "bf_angelitedepo", offset, -1, true );
			offsets[autotile_offset].push_back( offset );
		}
		...
		else if (pixel == color_bunn_main_spawn)
		{
			spawnBlob( map, "bf_townhall", offset, 255 );
			offsets[autotile_offset].push_back( offset );
		}
	}
	...
	void handleOffset(int type, int offset, int position, int count)
	{
        ...
        else if (type == bf_shrub_offset)
        {
            CBlob@ bf_shrubplant = server_CreateBlobNoInit("bf_shrubplant");
            if (bf_shrubplant !is null)
            {
                bf_shrubplant.Tag("instant_grow");
                bf_shrubplant.setPosition(map.getTileWorldPosition(offset));
                bf_shrubplant.Init();
            }
        }
	}
