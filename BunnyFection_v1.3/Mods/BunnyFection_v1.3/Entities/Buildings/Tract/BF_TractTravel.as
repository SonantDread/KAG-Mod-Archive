//BF_TractTravel. Based on TunnelTravel.as
...
void onInit( CBlob@ this )
{
	...
	this.Tag("travel tunnel");
	 ...
	 AddIconToken( "$HATCHERY$", "BF_Hatchery.png", Vec2f(40, 24), 0 );

	 if (!this.exists("travel button pos" ))
		this.set_Vec2f("travel button pos", Vec2f(0, 20) );
}

void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	if (caller.getTeamNum() == this.getTeamNum() && this.hasTag("travel tunnel") 
		&& getGameTime() > this.get_u16( "lastHit" ) + 300)
	{
		MakeTravelButton( this, caller, this.get_Vec2f("travel button pos"), "Travel", "Travel (requires Travel Tracts)" );
	}
}
...
void Travel( CBlob@ this, CBlob@ caller, CBlob@ tunnel )
{
	if (caller !is null && tunnel !is null)
	{
		...
		{
			Sound::Play( "SparkleShort.ogg" );
		}
		else
		{
			Sound::Play( "SparkleShort.ogg", this.getPosition() );
			Sound::Play( "SparkleShort.ogg", caller.getPosition() );
		}
	}
}
