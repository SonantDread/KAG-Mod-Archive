...
void onInit( CBrain@ this )
{
	...
	{
		moveVars.walkSpeed *= 0.2f;
		moveVars.jumpMaxVel *= 0.1f;
	}
}
...
{	
	...
	else
	{
		...
		if (blob.get_u8( "spawnTimer" ) >= 300 )
		{
			u16 realowner = blob.getNetworkID();
			print("po");
			CBlob@ minion = server_CreateBlobNoInit( "bf_minion2" );
			CBlob@ minion2 = server_CreateBlobNoInit( "bf_minion2" );
			if ( minion !is null && minion2 !is null)
			{
				minion.server_setTeamNum( blob.getTeamNum());
				minion.setPosition( pos );
				minion.set_netid( "owner", realowner );
				minion.Init();
				minion2.server_setTeamNum( blob.getTeamNum());
				minion2.setPosition( pos );
				minion2.set_netid( "owner", realowner );
				minion2.Init();
				blob.set_u8( "spawnTimer",0 );
			}
		}
		...
		{
			blob.set_u8( "spawnTimer", blob.get_u8( "spawnTimer" ) + 1 );
			print( " TARGET" );
		}
	}
...
void Attack( CBlob@ this, CBlob@ target )
{
	...
	if ( attackTimer == 50 )
	{	
		this.server_Hit( target, this.getPosition(), Vec2f_zero, 1.0f, 40, true );
