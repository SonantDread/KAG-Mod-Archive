...
void onTick( CMovement@ this )
{
	...
	if (onground) //also reset when vaulting
	{
		moveVars.walljumped_side = Walljump::NONE;
		moveVars.wallrun_start = pos.y;
		moveVars.wallrun_current = pos.y;
	}
    else if (!blob.isOnCeiling() && !isknocked && 
		!blob.isOnLadder() && (up || left || right) ) //key pressed
    {
		//check solid tiles
		const f32 ts = map.tilesize;
		const f32 y_ts = ts * 0.1f;
		const f32 x_ts = ts * 0.8f;
		
		bool surface_left = map.isTileSolid(pos + Vec2f(-x_ts, 0)) || checkForSolidMapBlob( map, pos + Vec2f(-x_ts, 0));	
		bool surface_right = map.isTileSolid(pos + Vec2f(x_ts, 0)) || checkForSolidMapBlob(map, pos + Vec2f(x_ts, 0));
			...
			!onground )											
		{
			...
			if(moveVars.jumpCount > 5) //wait some time to be properly in the air
			{
					...
					((left && surface_left && !jumpedLEFT) || (right && surface_right && !jumpedRIGHT) || set_contact) )
				{
						...
						pos.y + 1.0f > moveVars.wallrun_start - map.tilesize * moveVars.wallrun_length))
					{
						...
						if(set_contact || getGameTime() % 5 == 0)
						{
							...
							if(!set_contact)
							{
								blob.getSprite().PlaySound("/EarthJump");
							}
						}
					}
					else
				...
				//walljump
							...
							!surface_below && !surface_above && !jumpedLEFT && !jumpedRIGHT)
				{
		...
		//wall sliding
		{
				...
				Maths::Abs(groundNormal.y) <= 0.01f) //sliding on wall
			{
				...
				if (vel.y >= slidespeed && ( blob.isFacingLeft() ? groundNormal.x > 0 : groundNormal.x < 0 ))
				{
					...
					if (getNet().isClient()) // effect
					{
						if (!moveVars.wallsliding) {
							blob.getSprite().PlaySound( "/Scrape" );
						}
    ...
    if (moveVars.jumpFactor > 0.01f && !isknocked && !blob.hasTag( "crawling" ) )
    {
		...
		if (up && vel.y > -moveVars.jumpMaxVel )
		{
			...
			if (moveVars.jumpCount == 1 && getNet().isClient())
			{
	...
	bool left_or_right = (left || right);
	{
		...
		
		{
			...
			{
				f32 hpFactor = blob.getName() == "bf_bunny" ? Maths::FastSqrt( blob.getHealth() / 1.5f ) : 1.0f;
				force *= moveVars.walkFactor * moveVars.overallScale * 30.0f * hpFactor;
