...
void onTick(CRules@ this)
{
	...
	getBlobsByName( "bf_rocknut", @carrotPlants );//actually gets the carrots
	...
	for( int s = 0; s < carrotSpots.length(); s++ )
	{
		...
		{                  
			if ( blobsAtTile[b].getName() ==  "bf_rocknutplant" || blobsAtTile[b].getName() ==  "bf_carrotplant" || blobsAtTile[b].isCollidable() )
				clear = false;
		}

		//randomly spawn carrot if tile is clear
		if ( clear && XORRandom( 11 ) * SPAWN_CHANCE_FACTOR >= 5  )
				server_CreateBlob( "bf_rocknutplant", 0, spot );
...
bool isPosFertile( CMap@ this, Vec2f spot )
{
		...
		return !this.hasTileSolidBlobs( tile ) && !this.isTileSolid( tile );
}