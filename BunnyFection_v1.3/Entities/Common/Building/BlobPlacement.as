...
void onInit( CBlob@ this )
{
	...
	//this.getCurrentScript().runFlags |= Script::tick_onscreen;	  wont run on server
...
void onTick( CBlob@ this )
{
    ...
    {
        if (!carryBlob.isSnapToGrid()) // hide block in hands when placing close
        {
    ...
    if (carryBlob !is null)
    {
        ...
        if (!getHUD().hasButtons() && !carryBlob.hasTag("custom drop") )
        {
			if (this.isKeyPressed( key_action1 ))
			{
				...
				/*else if (!getHUD().hasButtons() && !this.isKeyPressed( key_pickup ) && bc.buildable && bc.supported && carryBlob.hasTag("place45"))
				{	
					CBitStream params;
					params.write_u16( carryBlob.getNetworkID() );
					params.write_Vec2f( this.getAimPos() );
					this.SendCommand( this.getCommandID("settleLadder"), params );
					
					SetBuildDelay( this, 40 );
				}*/
