...
void onTick(CMovement@ this)
{
	...
	if (brain !is null)
	{
		if (blob.getTeamNum() == 12) //seperate team
		{
			CBlob@ target = brain.getTarget();
			if (target !is null)
			{
				if ((target.getPosition() - blob.getPosition()).getLength() < blob.getRadius() * 2.0f && target.getPosition().y < blob.getPosition().y - blob.getRadius() && !blob.isOnMap())
				{
					blob.AddForce(Vec2f(blob.getVelocity().x * 4.0f, -1000.0f));
				}
			}
		}
	}
