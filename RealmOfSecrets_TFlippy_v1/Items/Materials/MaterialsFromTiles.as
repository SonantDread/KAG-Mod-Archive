	...
	if (getNet().isServer())
	{
		...
		if (map.isTileStone(tile))
		{
			if (map.isTileThickStone(tile)){
				...
				if (depth < 0.80f && XORRandom(100) < 70) MakeMat(this, worldPoint, "mat_copper", (3 + XORRandom(3)) * Multiplier);
				if (depth < 0.60f && XORRandom(100) < 60) MakeMat(this, worldPoint, "mat_iron", (5 + XORRandom(10)) * Multiplier);
			...
			else 
			{
				...
				if (depth < 0.80f && XORRandom(100) < 50) MakeMat(this, worldPoint, "mat_copper", (2 + XORRandom(2)) * Multiplier);
				if (depth < 0.60f && XORRandom(100) < 30) MakeMat(this, worldPoint, "mat_iron", (3 + XORRandom(4)) * Multiplier);
			}
		}
		else if (map.isTileGold(tile))
		{
			...
			if (depth < 0.25f && XORRandom(100) < 25) MakeMat(this, worldPoint, "mat_mithril", (1 + XORRandom(3)) * Multiplier);
			if (depth < 0.20f && XORRandom(100) < 10) MakeMat(this, worldPoint, "mat_wilmet", (2 + XORRandom(8)) * Multiplier);
		}
