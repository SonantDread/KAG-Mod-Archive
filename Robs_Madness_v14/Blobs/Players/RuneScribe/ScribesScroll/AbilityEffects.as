...
void Root(CBlob @this, CBlob @Holder, f32 Power, int Flow)
{
	...
	if (this.getMap().getBlobsInRadius(this.getPosition(), 128.0f, @blobsInRadius)) 
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			{
				if(!b.hasTag("Rooted"))b.AddScript("Root.as");
				b.set_u32("root_timer",getGameTime()+30.0f*(10.0f*Power*((10-Flow)/10)));
			}
		}
	}
}
...
void Blink(CBlob @this, CBlob @Holder, f32 Power){
	...
	for(int i=0;i<200*Power;i+=10)
	ParticleAnimated("RuneParticle.png", Holder.getPosition()+direction*i, Vec2f(0,0), 0, 1.0f, 3, 0, true);
...
void Slow(CBlob @this, CBlob @Holder, f32 Power, int Flow)
{
	...
	if (this.getMap().getBlobsInRadius(this.getPosition(), 128.0f, @blobsInRadius)) 
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			if(b.getTeamNum() != Holder.getTeamNum() && b.hasTag("flesh"))
			{
				if(!b.hasTag("Slowed"))b.AddScript("Slow.as");
...
void SecondaryAbility(CBlob@ this, CBlob @Holder, int AbilityID){
...
{
	if(!Holder.hasTag("LivingFlame"))Holder.AddScript("LivingFlame.as");
}

void AirBubble(CBlob @this, CBlob @Holder)
{
	if(!Holder.hasTag("AirBubble"))Holder.AddScript("AirBubble.as");
}

void StoneShield(CBlob @this, CBlob @Holder)
{
	if(!Holder.hasTag("StoneShield"))Holder.AddScript("StoneShield.as");
}
...
{
	if(!Holder.hasTag("RegenVines"))Holder.AddScript("RegenVines.as");
}
...
{
	if(!Holder.hasTag("Haste"))Holder.AddScript("Haste.as");
}
...
{
	if(!Holder.hasTag("Barrier"))Holder.AddScript("Barrier.as");
}
...
{
	if(!Holder.hasTag("Invisible"))Holder.AddScript("Invisibility.as");
}

void SelfSacrifice(CBlob @this, CBlob @Holder)
{
	if(Holder.hasTag("evil"))this.server_Hit(Holder, this.getPosition(), Vec2f(0,0), 0.25f, Hitters::suddengib, true);
...
{
	if(!Holder.hasTag("Plagued"))Holder.AddScript("Plague.as");
}