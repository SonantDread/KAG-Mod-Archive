...
shared class ZombiesSpawns : RespawnSystem
{
  ...
  void Update()
  {
    ...
    if (getRules().isMatchRunning()) {
      ...
      if (getGameTime() % (10 * getTicksASecond()) == 0)
        warn("ED:" + everyone_dead + " TC:" + total_count);
    }
  }
  ...
  void RemovePlayerFromSpawn(PlayerInfo@ p_info)
  {
    ...
    if (info is null) {
      warn("Zombies LOGIC: Couldn't get player info (in void RemovePlayerFromSpawn(PlayerInfo@ p_info))");
      return;
    }
  ...
  void AddPlayerToSpawn(CPlayer@ player)
  {
    ...
    if (info is null) {
      warn("Zombies LOGIC: Couldn't get player info (in void AddPlayerToSpawn(CPlayer@ player))");
      return;
    }
