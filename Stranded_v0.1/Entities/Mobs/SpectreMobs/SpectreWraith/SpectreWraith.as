...
void onInit(CBlob@ this)
{
    ...
    this.getSprite().PlaySound("EvilWraithSpawn.ogg");

    this.getSprite().SetEmitSound("EvilWraithFly.ogg");
	...
	// explosiveness
	this.set_f32("explosive_radius", 30.0f);
	this.set_f32("explosive_damage", 10.0f);
	this.set_string("custom_explosion_sound", "Entities/Items/Explosives/KegExplosion.ogg");
	this.set_f32("map_damage_radius", 25.0f);
	this.set_f32("map_damage_ratio", 2.5f);
...
void onTick(CBlob@ this)
{
	if (this.hasTag("enraged"))
	{
		...
		{
            this.SetLight( true );
            this.SetLightRadius(this.get_f32("explosive_radius") * 1.5f);
...
void onDie(CBlob@ this)
{
	if (getNet().isServer())
	{
		for (int i = 0; i < (4) + XORRandom(10) ; i++)
		{
			CBlob@ blob = server_CreateBlob("evilfog", -1, this.getPosition());
			blob.setVelocity(Vec2f(2 - XORRandom(16), 2 - XORRandom(16)));
		}
	}
}
