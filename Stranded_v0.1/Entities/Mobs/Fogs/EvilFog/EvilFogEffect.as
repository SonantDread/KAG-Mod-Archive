...
void onInit(CBlob@ this)
{
	...
	if (this.hasTag("evilfoged")) this.getCurrentScript().runFlags |= Script::remove_after_this;
	this.Tag("evilfoged");
}
...
void onTick(CBlob@ this)
{
	...
	if (this.get_u8("evilfog value") < 5) return;
	
	const int ticks = getGameTime() - this.get_u32("evilfog time");
	const f32 mod = f32(this.get_u8("evilfog value") / 64.0f);
	...
	if (ticks % u32(5 + 200 * mod_inv) == 0) 
	{
		...
		int end_it = XORRandom(3);
		if (end_it == 2)
		{
			this.RemoveScript("EvilFogEffect.as");
		}
	}	
...
void onRender(CSprite@ this)
{
	...
	const int ticks = getGameTime() - blob.get_u32("evilfog time");
	const f32 mod = f32(blob.get_u8("evilfog value") / 100.0f);
