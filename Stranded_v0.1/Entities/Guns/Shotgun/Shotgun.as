...
const u8 FIRE_INTERVAL = 40; // How long do we wait before firing again in ticks
const u8 CLIP        = 2;  // Clip size
const u8 TOTAL       = 32;  // Total ammo count
const u8 RELOAD_TIME = 30;   // How long do we wait in ticks to reload
const u8 BUL_PER_SHOT= 5;    // Bullets per shot | CHANGE B_SPREAD, otherwise both bullets will come out together
const u8 B_SPREAD	 = 6;    // The higher the value, the more 'uncontrolable' bullets get | USEFUL FOR BUL_PER_SHOT INCREASE 
const u8 B_SPEED	 = 60;   // Bullet speed, STRONGLY AFFECTED/EFFECTS B_GRAV FOR NOW
const u8 B_TTL		 = 60;  // Time To Live, bullets will live for 120 ticks before getting destory IF nothing has been hit
...
const s16 G_RECOIL = -7; // 0 is off, add's recoil based on number given (try -10 for example)

const float B_DAMAGE = 2.5f; // 1.0f = 1 heart

const Vec2f B_GRAV   = Vec2f(0,0.015); // Bullet gravity drop
...
const string FIRE_SOUND    = "Shotgun_Fire.ogg";   // Sound we make when pulling the trigger
