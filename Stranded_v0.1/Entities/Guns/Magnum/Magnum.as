...
const u8 FIRE_INTERVAL = 18; // How long do we wait before firing again in ticks
const u8 CLIP        = 6;  // Clip size
const u8 TOTAL       = 32;  // Total ammo count
const u8 RELOAD_TIME = 35;   // How long do we wait in ticks to reload
...
const u8 B_SPEED	 = 30;   // Bullet speed, STRONGLY AFFECTED/EFFECTS B_GRAV FOR NOW
...
const u16 T_TO_DIE = 100; // how many seconds before gun disspears if it hasnt been picked up in seconds
...
const s16 G_RECOIL = -1; // 0 is off, add's recoil based on number given (try -10 for example)

const float B_DAMAGE = 6.0f; // 1.0f = 1 heart

const Vec2f B_GRAV   = Vec2f(0,0.05); // Bullet gravity drop
const Vec2f B_KB     = Vec2f(0.5,0);     // KnockBack velocity on hit

const string C_TAG         = "Pistol";         // Custom TAG, so you can mark different weapons if needed.
...
const string FIRE_SOUND    = "PistolFire.ogg";   // Sound we make when pulling the trigger
