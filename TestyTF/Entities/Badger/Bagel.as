...
void EatMap(CBlob@ this)
{
	...
	Vec2f offset = Vec2f(this.getSprite().isFacingLeft() ? -12 : 12, 0);
	
	Vec2f m_pos = ((this.getPosition()) / map.tilesize);
	m_pos = (m_pos * map.tilesize) + Vec2f(map.tilesize / 2, map.tilesize / 2);
	
	getMap().server_DestroyTile(m_pos + offset, 0.1f, this);
	getMap().server_DestroyTile(m_pos + Vec2f(0, 0), 0.1f, this);
}
