...
void onTick(CBlob@ this)
{
	...
	if(this.getHealth()>1.0f) {
		...
		sprite.SetEmitSoundVolume(v.soundVolume);
...
void BomberHandling(CBlob@ this,VehicleInfo@ v)
{
	...
	if(driverSeat !is null){
				...
				if(itemCount>0) {
					if(getNet().isServer()) {
						...
						if(item.maxQuantity>8) { // To prevent spamming 
							...
							CBlob@ blob=server_CreateBlob(item.getName(),this.getTeamNum(),this.getPosition());
		...
		float goalSpeed=	fakeCrash ? -300.0f : ((up ? 35.0f : 0.0f)+(down ? 220.75f : 310.15f));
...
void onCollision(CBlob@ this,CBlob@ blob,bool solid)
{
	...
	if(power>1.5f &&(solid ||(blob !is null && blob.isCollidable() && blob.getTeamNum()!=this.getTeamNum()))){
		if(getNet().isClient()){
