...
void GunTick(CBlob@ this)
{
	...
	if((point.isKeyPressed(key_action1) || holder.isKeyPressed(key_action1)) && !holder.isAttached()) {
...
void Shoot(CBlob@ this)
{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if(getNet().isServer()) 
			{
				if(blobHit) 
				{
					...
					for(u32 i=0;i<hitInfos.length;i++) 
					{
						...
						{	
							...
							if((blob.isCollidable() || blob.hasTag("flesh")) && (blob.getTeamNum()!=holder.getTeamNum() || (blob.getTeamNum()<0 || blob.getTeamNum()>=7))) 
							{
								if (!blob.hasTag("isWeapon"))
								{
									f32 dmg = damage*Maths::Max(0.1,falloff)*(blob.hasTag("door") ? 0.2f : 1.0f);
								
									holder.server_Hit(blob,hitInfos[i].hitpos,Vec2f(0,0),dmg,Hitters::arrow);
				...
				if(mapHit)
				{
					...
					if((map.isTileWood(tile) || damage > 1.5f) && !map.isTileBedrock(tile))
					{
	...
	else
	{
		...
		if(getNet().isServer()) 
		{
			...
			Vec2f dir=		Vec2f((this.isFacingLeft() ? -1 : 1),0.0f).RotateBy(angle);
			Vec2f startPos=	this.getPosition()+this.get_Vec2f("gun_projOffset").RotateBy(angle);
			
			CBlob@ blob=	server_CreateBlob(fireProj,holder.getTeamNum(),startPos);
			blob.setVelocity(dir*8);
...
void DoRecoil(CBlob@ this)
{
	...
	f32 len = dir.Length();
	f32 angle = dir.Angle() - (0.25f * modifier * (this.isFacingLeft() ? 1.0f : -1.0f));
...
u32 TakeAmmo(CInventory@ inv,const string ammoType,u32 amount)
{
	u32 taken=	0;
	int size=	inv.getItemsCount();
	for(int i=0;i<size;i++){
