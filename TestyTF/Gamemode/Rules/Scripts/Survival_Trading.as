...
void onPlayerDie(CRules@ this, CPlayer@ victim, CPlayer@ killer, u8 customData)
{
	if (victim !is null)
	{
		...
		if (killer !is null)
		{
			...
			{
				if(!victim.hasTag("coin cheater")) killer.server_setCoins(killer.getCoins() + (victim.getCoins() * 0.1f));
				// if (!victim.hasTag("coin cheater")) server_DropCoins(victim.getBlob().getPosition(), victim.getCoins() * 0.1f);
			}
		}
		else if (!victim.hasTag("coin cheater") && victimBlob !is null) server_DropCoins(victimBlob.getPosition(), victim.getCoins() * 0.1f);
		
		// if (!victim.hasTag("coin cheater") && victimBlob !is null) server_DropCoins(victimBlob.getPosition(), victim.getCoins() * 0.1f);
		victim.Untag("coin cheater");
		victim.server_setCoins(victim.getCoins() * 0.9f);
	}
}
