...
shared class TDMSpawns : RespawnSystem
{
	...
	void UpdateSpawnTime(TDMPlayerInfo@ info, int i)
	{
		...
		bool no_respawn = false;//TDM_core.rules.isMatchRunning() ? huge_respawn : false;
		if (no_respawn)
		{
			spawn_property = 253;
		}
...
shared class TDMCore : RulesCore
{
	...
	void CheckTeamWon()
	{
		...
		CBlob@[] Blobs;	   
		getBlobsByName("knight", @Blobs);
		getBlobsByName("archer", @Blobs);
		for (uint i = 0; i < Blobs.length; i++)
		{
			CBlob@ b = Blobs[i];
			if(b.get_s16("crown_amount") >= 500){
				rules.SetTeamWon(b.getTeamNum());   //game over!
				rules.SetCurrentState(GAME_OVER);
				if(b.getPlayer() !is null)
				rules.SetGlobalMessage(b.getPlayer().getUsername() + " wins the game!");
				break;
			}
		}
	}
...
{
	CBlob@[] Blobs;	   
	getBlobsByName("crown", @Blobs);
	if(Blobs.length <= 0)
	server_CreateBlob("crown",-1,Vec2f((getMap().tilemapwidth/2) * getMap().tilesize,0));
}