...
void Explode(CBlob@ this, f32 radius, f32 damage)
{
	...
	bool should_killowner = !(this.exists("explosive_killowner") && !this.get_bool("explosive_killowner"));
	...
	if (getNet().isServer())
	{
		...
		for (uint i = 0; i < blobs.length; i++)
		{
			...
			HitBlob(this, hit_blob, radius, damage, hitter, true, should_teamkill, should_killowner);
		}
	}
...
void LinearExplosion(CBlob@ this, Vec2f _direction, f32 length, const f32 width,
                     ...
                     bool should_teamkill = false, bool should_killowner = false)
{
	Vec2f pos = this.getPosition();
	CMap@ map = this.getMap();
	...
	for (uint i = 0; i < blobs.length; i++)
	{
		...
		{
			HitBlob(this, hit_blob, length, damage, hitter, false, should_teamkill, should_killowner);
		}
	}
}
...
void BombermanExplosion(CBlob@ this, f32 radius, f32 damage, f32 map_damage_radius,
                        ...
                        const bool should_teamkill = false, const bool should_killowner = false)
{
	Vec2f pos = this.getPosition();
	CMap@ map = this.getMap();
	...
	//up
	LinearExplosion(this, Vec2f(0, -1), radius, ray_width, steps, damage, hitter, blobs, should_teamkill, should_killowner);
	//down
	LinearExplosion(this, Vec2f(0, 1), radius, ray_width, steps, damage, hitter, blobs, should_teamkill, should_killowner);
	//left and right
	LinearExplosion(this, Vec2f(-1, 0), radius, ray_width, steps, damage, hitter, blobs, should_teamkill, should_killowner);
	LinearExplosion(this, Vec2f(1, 0), radius, ray_width, steps, damage, hitter, blobs, should_teamkill, should_killowner);
...
bool HitBlob(CBlob@ this, CBlob@ hit_blob, f32 radius, f32 damage, const u8 hitter,
			 ...
			 const bool should_killowner = true)
{
	Vec2f pos = this.getPosition();
	CMap@ map = this.getMap();
	Vec2f hit_blob_pos = hit_blob.getPosition();
	...
	this.server_Hit(hit_blob, hit_blob_pos,
	                ...
	                isOwnerBlob(this, hit_blob) && should_killowner ||	//allow selfkill with bombs
