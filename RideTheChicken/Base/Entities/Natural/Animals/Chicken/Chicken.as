...
const int MAX_EGGS = 6; //maximum symultaneous eggs
const int MAX_CHICKENS = 24;
...
void onInit(CBlob@ this)
{
    ...
    // Now the chicken's attachment point can listen for player's WASD actions
    AttachmentPoint@ ap = this.getAttachments().getAttachmentPointByName("DRIVER");
    if (ap !is null)
    {
        ap.SetKeysToTake(key_up | key_left | key_right | key_down);
    }
   
    AnimalVars@ vars;
    if (!this.get("vars", @vars))
        return;
    vars.walkForce.Set(8.5f, -0.25f);
    vars.runForce.Set(10.0f, -2.5f);
    vars.slowForce.Set(3.5f, 0.0f);
    vars.jumpForce.Set(0.0f, -8.0f);
...
void onTick(CBlob@ this)
{
    ...
    // Attachment point, will be needed for movement logic
    AttachmentPoint@ ap = this.getAttachments().getAttachmentPointByName("DRIVER");

    // Null check because KAG's weird
    if (ap !is null && ap.getOccupied() !is null)
    {
   //     print("derp");
    }
    else // Does default chicken code when the point isn't occupied
    {
