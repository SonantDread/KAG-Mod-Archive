...
void Config(ZombiesCore@ this)
{
	...
	s32 max_pzombies = cfg.read_s32("max_pzombies",55);
	s32 max_migrantbots = cfg.read_s32("max_migrantbots",2);
	s32 max_wraiths = cfg.read_s32("max_wraiths",10);
	s32 max_gregs = cfg.read_s32("max_gregs",10);
	...
	getRules().set_s32("curse_day", cfg.read_s32("curse_day", 33));
...
shared class ZombiesCore : RulesCore
{
    ...
    void Update()
    {
		...
		*/
		...
		rules.SetGlobalMessage("Day: " + dayNumber + "\nPortals: " + num_zombiePortals  + "\nZombies: " + (num_zombies+num_pzombies));

		//Difficulty settings
		int timeElapsed = getGameTime()-gamestart;
		float difficulty = dayNumber*1.75; //default 1.7
		float zombdiff = dayNumber*1.25; //default 3.7
		...
		if (zombdiff>33) //default zombdiff>50
		{ 
			zombdiff=33; //difficulty=25; 
		} 
		...
		int spawnRate = getTicksASecond() * (6-(difficulty/6.0)); //default 2.0
		...
		if (getGameTime() % (spawnRate) == 0) //zombies spawn more often as days pass by, up to 1 sec
        {
			...
			if (map !is null)
			{
				...
				if ((map.getDayTime()>0.8 || map.getDayTime()<0.1) && num_zombies<max_zombies)
                {
                    ...
                    if (r>=30 && num_wraiths<max_wraiths) //hardcap for wraiths
                    server_CreateBlob( "wraith", -1, sp);
					
                    else if (r>=26 && num_gregs<max_gregs) //hardcap for gregs 
                    server_CreateBlob( "greg", -1, sp);
					
					else if (r>=23 && num_immol<8) //hardcap for immolators
					...
					if (transition == 1 && (dayNumber % 25) == 0) //Every 25 days!
					{
						transition=0;
						rules.set_s32("transition",0);
						Vec2f sp = zombiePlaces[XORRandom(zombiePlaces.length)];
						int megaboss = XORRandom(9);
						...
						if (megaboss <= 6) 
						{
							...
							getNet().server_SendMsg("3x Abomination"); 
							server_CreateBlob("megabossmessage");
						}
						if (megaboss <= 9) 
						{
							server_CreateBlob( "rabiddog", -1, sp);
							server_CreateBlob( "rabiddog", -1, sp);
							
							getNet().server_SendMsg("2x Rabid Dog"); 
							server_CreateBlob("megabossmessage");
						}
					}	
					...
					else if (transition == 1 && (dayNumber % 10) == 0) //Every 10 days!
					{
						transition=0;
						rules.set_s32("transition",0);
						Vec2f sp = zombiePlaces[XORRandom(zombiePlaces.length)];
						int boss = XORRandom(12);
						if (boss <= 3) 
						{
							server_CreateBlob( "abomination", -1, sp);
						...
						else if (boss <= 6)
						{
							...
							getNet().server_SendMsg("4x Horrors"); 
							server_CreateBlob("bossmessage");
						}
						else if (boss <= 9)
						{
							...
							getNet().server_SendMsg("5x Banshee"); 
							server_CreateBlob("bossmessage");
						}
						else if (boss <= 12)
						{
							...
							getNet().server_SendMsg("1x Rabid Dog"); 
							server_CreateBlob("bossmessage");
						}
					}	
