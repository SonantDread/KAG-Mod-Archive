...
const float required_minutes = 5; //time you have to wait after joining w/o skip_votewait.
...
class VoteKickLeaveFunctor : VotePlayerLeaveFunctor
{
	...
	void PlayerLeft(VoteObject@ vote, CPlayer@ player)
	{
		...
		{
			client_AddToChat(player.getUsername() + " left before being kicked.", vote_message_colour());
			if (getNet().isServer())
			{
...
class VoteNextmapFunctor : VoteFunctor
{
	...
	void Pass(bool outcome)
	{
		...
		{
			client_AddToChat(playername + "'s vote failed, Play on!", vote_message_colour());
		}
	}
};
...
void onMainMenuCreated(CRules@ this, CContextMenu@ menu)
{
	...
	{
		Menu::addContextItem(menu, "(Vote currently in progress)", "DefaultVotes.as", "void CloseMenu()");
