	...
	if(this.isKeyPressed(key_action2))
	{
		...
		if (this.getMap().getBlobsInRadius(this.getPosition(), 96.0f, @blobsInRadius)) 
		{
			for (uint i = 0; i < blobsInRadius.length; i++)
			{
				...
				{
					CBlob@[] enemies;	   
					if (this.getMap().getBlobsInRadius(this.getPosition(), 64.0f, @enemies)) 
					{
						for (uint i = 0; i < enemies.length; i++)
						{
							CBlob@ e = enemies[i];
							if(e.getTeamNum() != this.getTeamNum() && e.hasTag("player"))
							{
								SetupBomb(b, 0, 64.0f, 4.0f, 24.0f, 0.4f, true);
							}
						}
					}
				}
