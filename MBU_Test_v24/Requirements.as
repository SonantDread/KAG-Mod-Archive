...
CBlob@ server_TakeRequirementsAndReturn(CInventory@ inv1, CBitStream &inout bs)
{
	if (!getNet().isServer())
	{
		return null;
	...
	while (!bs.isBufferEnd())
	{
		...
		if (req == "blob")
		{
			...
			if (inv1 !is null)
			{
				for(int i = 0; i < inv1.getItemsCount();i++){
					CBlob @item = inv1.getItem(i);
					
					if(item.getName() == blobName){
						...
						if(item.getQuantity() <= 1){
							...
							item.server_SetQuantity(item.getQuantity()-Maths::Min(item.getQuantity(),quantity-taken));
							
							taken += Maths::Min(item.getQuantity(),quantity-taken);
