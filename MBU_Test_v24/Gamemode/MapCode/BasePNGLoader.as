...
class PNGLoader
{
	...
	{
		SColor color_ruin(0xff808000);
		if(color_ruin == pixel){
			server_CreateBlob("ruins", -1, getMap().getTileWorldPosition(offset)+Vec2f(4,0));
			autotile(offset);
		}
		SColor color_blackmarket(0xff130d1d);
		if(color_blackmarket == pixel){
			server_CreateBlob("gs", -1, getMap().getTileWorldPosition(offset));
			autotile(offset);
		}
		SColor color_mechhall(0xffffbe0a);
		if(color_mechhall == pixel){
			server_CreateBlob("construction_yard", -1, getMap().getTileWorldPosition(offset)+Vec2f(0,-8));
			autotile(offset);
			
			if(XORRandom(4) == 0){
				server_CreateBlob("catapult", -1, getMap().getTileWorldPosition(offset));
			} else
			if(XORRandom(4) == 0){
				server_CreateBlob("ballista", -1, getMap().getTileWorldPosition(offset));
			} else
			if(XORRandom(4) == 0){
				server_CreateBlob("bomber", -1, getMap().getTileWorldPosition(offset));
			} else
			if(XORRandom(4) == 0){
				server_CreateBlob("air_castle", -1, getMap().getTileWorldPosition(offset));
			}
		}
	}
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		...
		case tree_offset:
		{
			...
			string tree_s = "tree_bushy";
			switch(map_random.NextRanged(4)){
				case 0: tree_s = "tree_pine"; break;
				case 1: tree_s = "tree_large"; break;
				default: tree_s = "tree_bushy"; break;
			}
			
			CBlob@ tree = server_CreateBlobNoInit(tree_s);
			if(tree !is null)
			{
				tree.Tag("startbig");
		...
		break;
		case bush_offset:
			if(XORRandom(2) == 0)server_CreateBlob("bush", -1, map.getTileWorldPosition(offset) + Vec2f(4, 4));
			else server_CreateBlob("big_bush", -1, map.getTileWorldPosition(offset) + Vec2f(4, 4));
		break;
		case grain_offset:
		{
