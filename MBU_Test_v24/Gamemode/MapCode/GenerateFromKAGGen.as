...
bool loadMap(CMap@ _map, const string& in filename)
{
	...
	for(int i = 0; i < width; i += 1) //The Abyss
		for(int j = 0; j < height; j += 1){ getNet().server_KeepConnectionsAlive();
			if(j == height-1){
				World[i][j] = CMap::tile_ground_back;
			}
			...
			if(j == height-3){
				if(XORRandom(2) > 0)World[i][j] = CMap::tile_gold;
				else World[i][j] = CMap::tile_ground_back;
			}
			if(j == height-4){
		...
		for(int j = 0; j < height-1; j += 1){ getNet().server_KeepConnectionsAlive();
			if(World[i][j] == 0 && World[i][j+1] == CMap::tile_ground){
				if(biome[i] == BiomeType::Swamp){
					...
					if(XORRandom(2) == 0){
						if(XORRandom(2) == 0){
							CBlob@ plant1 = server_CreateBlobNoInit("big_bush");
							if (plant1 !is null)
							{
				...
				if(j < SeaLevel){
					...
					if(XORRandom(6) == 0){
						...
						if(XORRandom(5) == 0 && tree_str == "tree_bushy")tree_str = "tree_large";
						CBlob@ tree = server_CreateBlobNoInit(tree_str);
						if (tree !is null)
						{
							tree.Tag("startbig");
					...
					if(biome[i] == BiomeType::Desert)if(XORRandom(10) == 0){ //Grain grows in the desert cause it's hipster like that.
...
bool LoadMap(CMap@ map, const string& in fileName)
{
	...
	bool success = loadMap(map, fileName);
