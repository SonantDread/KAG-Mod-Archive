...
const string tcdevs = "tflippy;pirate-rob;";
const string contributors = "cesar0;sylw;sjd360;";


void onTick(CRules@ this)
{
	//if(getGameTime() == 300)getMap().MakeMiniMap();
}
...
void onRenderScoreboard(CRules@ this)
{
	...
	GUI::DrawText("Status", Vec2f(bottomright.x - 200, topleft.y), SColor(0xffffffff));
	...
	for (u32 i = 0; i < sortedplayers.length; i++)
	{
		...
		GUI::DrawText("" + getStatus(p), Vec2f(bottomright.x - 200, topleft.y), SColor(0xffffffff));
	}
}
...
string getRank(CPlayer@ p)
{
	...
	else if (tcdevs.find(username) != -1) return "TC Developer";
...
string getStatus(CPlayer@ p)
{
	...
	if(blob is null)return "Non-exsitant";
	
	if(blob.getName() == "humanoid"){
		if(blob.hasTag("alive"))return "Alive";
		else return "Dead";
	}
