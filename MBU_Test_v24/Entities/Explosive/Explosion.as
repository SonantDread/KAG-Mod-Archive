...
void Explode(CBlob@ this, f32 radius, f32 damage){
	Explode(this, radius, damage, this.getPosition());
}

void Explode(CBlob@ this, f32 radius, f32 damage, Vec2f pos)
{
	pos += this.get_Vec2f("explosion_offset");
	...
	if (this.isInInventory())
	{
		...
		{
			this.server_Hit(doomed, pos, Vec2f(), damage, Hitters::explosion, true);
		}
	}
             ...
             const bool bother_raycasting = true, const bool should_teamkill = false)
{
	...
	f32 scale;
	hitvec.Normalize();
	Vec2f bombforce = hitvec*3.0f;
	
	getBombForce(this, radius, hit_blob_pos, pos, hit_blob.getMass(), scale);
