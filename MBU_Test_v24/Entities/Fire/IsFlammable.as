...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if ((isIgniteHitter(customData) && !this.hasTag("ghost")) ||					 	   // Fire arrows
...
{
	Vec2f p = pos + Vec2f((_r.NextFloat() - 0.5f) * 32.0f, (_r.NextFloat() - 0.5f) * 32.0f);
	getMap().server_setFireWorldspace(p, true);
	
	CBlob@[] blobsInRadius;
	if (getMap().getBlobsInRadius(pos, 24.0f, @blobsInRadius)) 
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			CBlob@ b = blobsInRadius[i];
			
			if(b !is null && !b.hasTag(burning_tag) && XORRandom(10) == 0 && b.getShape().getConsts().isFlammable){

				b.server_Hit(b, b.getPosition(), Vec2f(0,0), 0.25f, Hitters::fire, true);
			}
		}
	}
}
...
void onTick(CBlob@ this)
{
	...
	if (burn_time == 0 || this.isInWater() || (this.hasTag("fire_ability") && !this.hasTag("pyromaniac")))
	{
		server_setFireOff(this);
	...
	else if (burn_time > 0)
	{
		...
		if ((burn_time % 8) == 0)
		{
			if(!this.hasTag("soul"))BurnRandomNear(pos);
		}
		...
		if ((burn_time % 7) == 0)
		{
			...
			blob.server_Hit(this, pos, Vec2f(0, 0), 0.5f, this.get_u8(burn_hitter), true);
		}
...
void onCollision( CBlob@ this, CBlob@ b, bool solid, Vec2f normal)
{
	if(this.hasTag(burning_tag))
	if(b !is null && !b.hasTag(burning_tag) && XORRandom(2) == 0 && b.getShape().getConsts().isFlammable){

		this.server_Hit(b, b.getPosition(), Vec2f(0,0), 0.25f, Hitters::fire, true);
	}
}