...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
	if (blob !is null)
	{
		...
		if(getNet().isServer()){
			if (blob.getName() == "steak")
			{
				server_CreateBlob("cooked_steak",-1,blob.getPosition());
				blob.server_Die();
			}
			if (blob.getName() == "fishy")
			{
				server_CreateBlob("fish_stick",blob.get_u8("colour"),blob.getPosition());
				blob.server_Die();
			}
			if (blob.getName() == "emj")
			{
				server_CreateBlob("eaj",-1,blob.getPosition());
				blob.server_Die();
			}
		}
