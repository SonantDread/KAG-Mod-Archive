...
{
	if (cmd == this.getCommandID("fire_arrow")){
...
{
	if (blob !is null && !this.hasTag("collided") && !this.hasTag("hit_once"))
	{
		if(doesCollideWithBlob(this,blob) || (blob.hasTag("flesh") && checkAlly(this.getTeamNum(), blob.getTeamNum()) != Team::Ally))
		{
			...
			f32 dmg = 0.0f;
			if (checkAlly(this.getTeamNum(), blob.getTeamNum()) != Team::Ally)
...
bool doesCollideWithBlob(CBlob@ this, CBlob@ blob)
{
	...
	if(shape.isStatic() && shape.getConsts().collidable)if(blob.getName() == "wooden_door" || blob.getName() == "stone_door" || blob.getName() == "trap_block")return true;
...
f32 getArrowDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	return  this.get_f32("damage")*0.5f;
}
