...
void ManagePick(CBlob @this, CBlob @item, bool holding, string type){

	string drawback_string = type+"_drawback";

	float damage = 1;
	int hitter = Hitters::pick;
	int speed = 5;
	...
	if(holding){
		if(this.get_s16(drawback_string) < 75)this.add_s16(drawback_string,speed);
			...
			DoPick(this,item,damage, hitter);
...
void DoPick(CBlob@ this, CBlob @item, f32 damage, u8 hitter)
{
...
bool canHitWorldPick(CBlob@ this, Vec2f position, CBlob@ item){
	...
	if(name == "hammer"){if(map.isTileCastle(tile) || map.isTileWood(tile) || (map.isTileBackground(map.getTile(position)) && !map.isTileGroundBack(tile)) || tile > 300)return true;}
	else if(map.isTileSolid(tile) || (map.isTileBackground(map.getTile(position)) && !map.isTileGroundBack(tile)) || tile > 300)return true;
