...
void onTick(CMovement@ this)
{
	...
	// flying - overrides other movement completely
	if (blob.hasTag("flying") && !blob.isAttached())
	{
		...
		blob.AddForce(flyingforce * moveVars.overallScale * 100.0f);
		//damp vel
		Vec2f vel = blob.getVelocity();
		vel *= 0.9f;
	...
	if (blob.isOnLadder() && !blob.isOnGround() && !isknocked && !blob.isAttached())
	{
		shape.SetGravityScale(0.0f);
		Vec2f ladderforce;
	...
	if (blob.isInWater() && !isknocked)
	{
		...
		if (up)
		{
			if (vel.y > -swimspeed)
			{
				if (!map.isInWater(pos + Vec2f(0, -4)))
				{
					waterForce.y -= 0.6f;
				}
				else
	...
	bool Effects = blob.getSprite().isVisible();
	...
	// carrying heavy
	{
		...
		for(int i = 0; i < inv.getItemsCount();i++){
			...
			if(item !is null){
				...
				else if (item.hasTag("heavy weight"))
				{
					moveVars.walkFactor *= 0.6f;
					moveVars.jumpFactor *= 0.5f;
				}
