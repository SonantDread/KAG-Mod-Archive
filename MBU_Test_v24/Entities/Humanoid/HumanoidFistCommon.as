...
void ManageFist(CBlob @this, bool holding, string type){ //Inb4 FIST! clan thinks I made this class for them
	...
	f32 damage = getStrength(this,type+"_arm");

	if(holding){
		if(this.get_s16(type+"_drawback") < 30)this.add_s16(type+"_drawback",2);
	} else {
		if(this.get_s16(type+"_drawback") > 10){
			DoPunch(this,(f32(this.get_s16(type+"_drawback"))/30.0f*0.75f+0.25f)*damage,-getAimAngle(this),40);
			this.set_s16(type+"_drawback",-20);
		}
		if(this.get_s16(type+"_drawback") > 0)this.set_s16(type+"_drawback",0);
		if(this.get_s16(type+"_drawback") < 0)this.add_s16(type+"_drawback",3);
	}
	if(this.get_s16(type+"_drawback") != 0)this.set_u8(type+"_implement", 1);
}
