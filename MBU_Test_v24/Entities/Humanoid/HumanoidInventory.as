...
void MakeBlocksMenu(CInventory@ this, const Vec2f &in INVENTORY_CE)
{
	...
	if(menu !is null)
	{
		...
		for(u8 i = 0; i < blocks[PAGE].length; i++)
		{
			...
			CGridButton@ button = menu.AddButton(b.icon, getTranslatedString("Build"), Builder::make_block + i);
...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu@ menu)
{
	...
	if(!this.hasTag("no hands")){
		...
		const Vec2f INVENTORY_CE = Vec2f(9,3) * GRID_SIZE / 2 + menu.getUpperLeftPosition();
		this.set_Vec2f("backpack position", INVENTORY_CE);

		MakeBlocksMenu(inv, INVENTORY_CE);
