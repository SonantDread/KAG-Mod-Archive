...
void onInit(CBlob@ this)
{
	...
	this.set_u16("heat_amount",50);
	this.Tag("heat_infused");
}
...
void onTick(CBlob @this){
		...
		if(this.get_u32("last_heated") < getGameTime()-(30*20)){
			...
			if(!this.hasTag("cooled")){
				...
				if(getMap().isTileSolid(this.getPosition()+Vec2f(-6,0)) && getMap().isTileSolid(this.getPosition()+Vec2f(6,0))){
					if(this.getName() == "molten_metal")@result = server_CreateBlob("metal_bar",-1,this.getPosition());
					if(this.getName() == "molten_metal_dirty")@result = server_CreateBlob("metal_drop_dirty",-1,this.getPosition());
					if(this.getName() == "molten_gold")@result = server_CreateBlob("gold_bar",-1,this.getPosition());
				} else {
					if(this.getName() == "molten_metal")@result = server_CreateBlob("metal_drop",-1,this.getPosition());
					if(this.getName() == "molten_metal_dirty")@result = server_CreateBlob("metal_drop_dirty",-1,this.getPosition());
					if(this.getName() == "molten_gold")@result = server_CreateBlob("gold_drop",-1,this.getPosition());
				}
				
				if(result !is null){
					if(this.hasTag("light_infused"))result.Tag("light_infused");
					result.set_u16("heat_amount",this.get_u16("heat_amount"));
				}
	...
	if(getMap().isTileGroundStuff(getMap().getTile(this.getPosition()+Vec2f(0,5)).type)){
		...
		if(!this.hasTag("merged") && !this.hasTag("dirtied")){
			this.Tag("dirtied");
			if(getNet().isServer()){
				CBlob @result = server_CreateBlob("molten_metal_dirty",-1,this.getPosition());
				if(result !is null){
					if(this.hasTag("light_infused"))result.Tag("light_infused");
					result.set_u16("heat_amount",this.get_u16("heat_amount"));
				}
