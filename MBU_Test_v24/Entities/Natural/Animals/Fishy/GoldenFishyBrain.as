...
void onTick(CBrain@ this)
{
	...
	{
		Vec2f earthpos = blob.getPosition();

		blob.set_Vec2f(terr_pos_property, earthpos);
		//printf("set territory " + earthpos.x + " " + earthpos.y );
	}
	
	if(getGameTime() % 8 == 3){
		CBlob@[] t;
		getBlobsByName("goldenbeing", t);
		if(t.length > 0){
			CBlob @ gb = t[XORRandom(t.length)];
			blob.set_Vec2f(terr_pos_property, gb.getPosition()+Vec2f(XORRandom(96)-48,XORRandom(96)-48));
		}
	}
					...
					for (uint step = 0; step < blobs.length; ++step)
					{
						...
						if (personality & AGGRO_BIT != 0)  //aggressive
						{
							...
							if (other.getName() != name && //dont eat same type of blob
							        other.getName() == "humanoid"//attack humanoid blobs
									&& other.hasTag("flesh")
									&& !other.hasTag("ghost")) 
							{
								mode = MODE_TARGET;
