	...
	for (uint i = 0; i < segments.length; i++)
	{
		...
		if (getNet().isServer())
		{
			...
			if (log !is null)
			{
				...
				if (this.hasTag(burning_tag))this.server_Hit(log, pos, Vec2f(0, 0), 0.25f, Hitters::fire, true);
			}
			if(XORRandom(2) == 0){
				CBlob@ stick = server_CreateBlob("stick", this.getTeamNum(), pos);
				if (stick !is null)
				{
					stick.setAngleDegrees(fall_angle);
					if (this.hasTag(burning_tag))this.server_Hit(stick, pos, Vec2f(0, 0), 0.25f, Hitters::fire, true);
				}
			}
		}
	}
