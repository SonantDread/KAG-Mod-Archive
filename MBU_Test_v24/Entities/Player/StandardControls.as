...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("putin"))
	{
		...
		{
			if(!putInInventory(owner,pick)){
				if(pick.hasTag("inventory") && owner.getName() == "humanoid"){
					CBitStream par;
					par.write_u16(pick.getNetworkID());
					owner.SendCommand(owner.getCommandID("equip_back"),par);
				}
			}
			if(owner.hasTag("inventory")){
				owner.server_PutInInventory(pick);
			}
		}
	}
...
bool ClickGridMenu(CBlob@ this, int button)
{
	...
	if (this.ClickGridMenu(button, gmenu, gbutton))   // button gets pressed here - thing get picked up
	{
		if (gmenu !is null)
		{
			...
			{
				//if (pickBlob is null)print("pickblob is null");
				//else print("Pickblob is:"+pickBlob.getName());
				CBlob @ owner = gmenu.getOwner();
				//if (owner is null)print("Owner is null");
				//else print("Owner is:"+owner.getName());
...
void onTick(CBlob@ this)
{
	...
	if (this.isKeyJustPressed(key_use))
	{
		...
		this.ClearMenus();
		if(!this.hasTag("no hands"))this.ShowInteractButtons();
	...
	if (this.isInInventory())
	{
		...
		return; // no more stuff possible while in crate...
	}
	...
	{
		if ((control.isKeyJustPressed(KEY_LBUTTON) /*|| getControls().isKeyJustPressed(KEY_LBUTTON)*/) && !this.isKeyPressed(key_pickup))
		{
