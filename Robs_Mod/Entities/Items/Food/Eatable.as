...
void Heal(CBlob@ this, CBlob@ blob)
{
	...
	if (getNet().isServer() && blob.hasTag("player") && !this.hasTag("healed") && exists)
	if (this.getName() == "goldheart")	    // HACK
	{
		if(blob.getHealth() < blob.getInitialHealth()*2.5f)
		if(blob.getHealth() >= blob.getInitialHealth()){
			blob.server_SetHealth(blob.getHealth()+0.5f);
			this.Tag("healed");
			this.server_Die();
		}
	}
