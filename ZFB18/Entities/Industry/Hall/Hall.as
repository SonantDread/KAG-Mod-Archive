...
void onInit( CBlob@ this )
{
	...
	this.Tag("respawn"); // allow spawning in WAR
...
void GetButtonsFor( CBlob@ this, CBlob@ caller )
{
	...
	if (this.getTeamNum() != 255)
	{
		...
		{
			Vec2f buttonpos = this.hasTag("script added") ? Vec2f(0.0f, -14.0f) : Vec2f(-24.0f, -14.0f);
			
			const u16 goldCount = caller.getBlobCount("mat_gold");
			if( goldCount >= MIGRANT_COST )
			{
				//buy migrant button
				CButton@ button = caller.CreateGenericButton( "$migrant$", buttonpos, this, this.getCommandID(buymigrantcmd), "Buy Migrant For "+MIGRANT_COST+" Gold" , params);
			}
			else
			{
				CButton@ button = caller.CreateGenericButton( "$migrant$", buttonpos, this, 0, "Buy Migrant: Requires "+MIGRANT_COST+" Gold" );
				if (button !is null) {
