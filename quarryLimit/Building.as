...
void onInit(CBlob@ this)
{
	...
	if(getNet().isServer())
	{
		if(rules.exists("quarryTeamLimit"))
			return;
		string user_configstr   = "../Cache/quarryConfig.cfg";

		ConfigFile user_cfg = ConfigFile(user_configstr);
		rules.set_u8("quarryTeamLimit", user_cfg.read_u8("quarryTeamLimit"));
		rules.Sync("quarryTeamLimit", true);
		print(""+rules.get_u8("quarryTeamLimit"));
	}
	...
	if(rules.exists("quarryTeamLimit") && rules.get_u8(this.getTeamNum()+"quarryAmount") < rules.get_u8("quarryTeamLimit"))
	{
		print(""+rules.get_u8(this.getTeamNum()+"quarryAmount"));
		{
