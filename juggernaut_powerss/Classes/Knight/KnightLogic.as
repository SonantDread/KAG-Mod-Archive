...
#include "PowerVampirismCommon.as"; // new
...
void onTick(CBlob@ this)
{
	...
	// Quickfix for Quickattack ;)
	s32 resheath_cut_time = KnightVars::resheath_cut_time;
        s32 resheath_slash_time = KnightVars::resheath_slash_time;

        s32 slash_charge = KnightVars::slash_charge;
        s32 slash_charge_level2 = KnightVars::slash_charge_level2;
        s32 slash_charge_limit = slash_charge_level2 + slash_charge + 10;
        s32 slash_move_time = KnightVars::slash_move_time;
        s32 slash_time = KnightVars::slash_time;
        s32 double_slash_time = KnightVars::double_slash_time;
        ...
        f32 resheath_time = 2.0f;

	if (hasPower(this, Powers::QUICK_ATTACK)) {
		resheath_cut_time /= 2;
		slash_charge /= 2;
		slash_charge_level2 /= 2;
		resheath_time /= 2;
	}
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		...
		if (knight.state == KnightStates::sword_drawn && !pressed_a1 &&
		       !(this.isKeyJustReleased(key_action1) && !this.hasTag("noLMB")) && delta > resheath_time)
		{
			knight.state = KnightStates::normal;
		}
		         ...
		         knight.state <= KnightStates::sword_cut_down) // cut state
		{
			...
			int end = DELTA_END_ATTACK;
			if (hasPower(this, Powers::QUICK_ATTACK)) {
				end /= 2;
			}

			if (delta > DELTA_BEGIN_ATTACK && delta < end)
			{
				f32 attackarc = 90.0f;
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			else if (delta >= slash_time ||
			         (knight.doubleslash && delta >= double_slash_time))
			{
				knight.swordTimer = 0;
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore)
				{
					...
					if (hasPower(this, Powers::VAMPIRISM)) { // new for vampirism heal superpower
				...
				if (!dontHitMoreMap && (deltaInt == DELTA_BEGIN_ATTACK + 1))
				{
					...
					bool castle = map.isTileCastle(hi.tile); // new, for strength check in next line
					if (ground || wood || dirt_stone || gold || hasPower(this, Powers::STRENGTH) && castle) // new check for strength power
					{
						...
						if (dif < 20.0f)
						{
							...
							if (canhit)
							{
								float dmg = hasPower(this, Powers::STRENGTH) ? 0.3f : 0.1f; // new support for strength superpower
		        ...
		        getKnocked(blob) == 0)
		{
			...
			if (vel * normal < 0.0f && knight_hit_actor_count(this) == 0) //only bash one thing per tick
			{
				...
				if (shieldVars.direction * normal < 0.0f)
				{
					...
					if (hasPower(this, Powers::KNOCKBACK)) {
					blob.AddForce(force * 1.5);
					} else {
					blob.AddForce(force);
					}
	...
	if (hasPower(this, Powers::QUICK_ATTACK)) {
		...
		resheath_time /= 2;
                ...
                else if (knight.swordTimer < slash_charge && knight.state == KnightStates::sword_drawn)
                {
                        ...
                        if (knight.swordTimer <= resheath_cut_time) //prevent from appearing when jabbing/jab spamming
                        {
                                getHUD().SetCursorFrame(0);
                        }
                        else
