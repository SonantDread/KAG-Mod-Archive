...
{
	if (hasPower(this, Powers::FIRE_LORD)) // new
	{
        return damage;
	}
...
void onTick(CBlob@ this)
{
	...
	bool fireLord = hasPower(this, Powers::FIRE_LORD); // new
	s16 burn_time = this.get_s16(burn_timer);
	//check if we should be getting set on fire or put out
	if (fireLord || burn_time < (burn_thresh / fire_wait_ticks) && this.isInFlames()) // new, added is firelord check
	{
		server_setFireOn(this);
	...
	if (!fireLord && (burn_time == 0 || this.isInWater())) // new, added not firelord check
	{
		server_setFireOff(this);
	...
	else if (burn_time > 0)
	{
		...
		if ((burn_time % 7) == 0)
		{
			...
			if (!hasPower(this, Powers::FIRE_LORD)) // new, added not firelord check
			{
