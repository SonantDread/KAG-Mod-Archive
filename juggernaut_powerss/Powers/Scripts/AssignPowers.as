...
void onTick(CRules@ this) {
    if (lastOnSetPlayerTick < getGameTime() && blobs.length > 0) {
		...
		{
			// new
			// perhaps later on make a hasPowers(blob) custom function for this instead
			// old line: 46
			// hasPower(blob, Powers::TRIPLE_JUMP) || hasPower(blob, Powers::MONKEY) || hasPower(blob, Powers::MIDAS) || hasPower(blob, Powers::FIRE_LORD) ||
			// hasPower(blob, Powers::SPEED) || hasPower(blob, Powers::QUICK_ATTACK) || hasPower(blob, Powers::MOUNTAIN) || hasPower(blob, Powers::FEATHER) ||
			if (!(hasPower(blob, Powers::BOUNCE) || hasPower(blob, Powers::HEALER) || hasPower(blob, Powers::STRENGTH) || hasPower(blob, Powers::VAMPIRISM) || hasPower(blob, Powers::DRAIN) ||
					hasPower(blob, Powers::SPEED) || hasPower(blob, Powers::QUICK_ATTACK) || hasPower(blob, Powers::FLIGHT) ||
					hasPower(blob, Powers::TRIPLE_JUMP) || hasPower(blob, Powers::MONKEY) || hasPower(blob, Powers::FIRE_LORD) ||
					hasPower(blob, Powers::FORCE) || hasPower(blob, Powers::GHOST) || hasPower(blob, Powers::KNOCKBACK) || hasPower(blob, Powers::TELEPORT)))
			{
