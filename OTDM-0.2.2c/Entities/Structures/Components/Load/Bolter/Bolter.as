	...
	void Activate(CBlob@ this)
	{
		...
		if(getNet().isServer())
		{
			...
			for(uint i = 0; i < blobs.length; i++)
			{
				...
				velocity.RotateBy(XORRandom(-4));
				velocity *= 27.59f;
				Vec2f velocity2 = offset;
				velocity2.RotateBy(deviation);
				velocity2.RotateBy(XORRandom(4));
				velocity2 *= 27.59f;

				// spawn projectile
				CBlob@ projectile = server_CreateBlobNoInit("bullet");
				...
				CBlob@ projectile2 = server_CreateBlobNoInit("bullet");

				projectile2.server_setTeamNum(this.getTeamNum());
				projectile2.Init();

				projectile2.IgnoreCollisionWhileOverlapped(this);
				projectile2.setPosition(position);
				projectile2.setVelocity(velocity2);
