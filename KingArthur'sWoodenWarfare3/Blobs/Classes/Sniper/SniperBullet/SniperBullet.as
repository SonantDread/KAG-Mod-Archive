...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		if (this.hasTag("shotgunned"))
		{
			...
			{
				this.server_Hit(this, this.getPosition(), Vec2f(), 0.5f, Hitters::crush);
			}
		}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	{
		if ((blob.getName() == "m60" || blob.getName() == "t10") && blob.getTeamNum() != this.getTeamNum())
		{
			this.Tag("dead");
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		const f32 scale = specialArrowHit(hitBlob) ? SPECIAL_HIT_SCALE : 0.5f;
...
f32 getArrowDamage(CBlob@ this, f32 vellen = -0.5f)
{
	return 4.25f;
}
