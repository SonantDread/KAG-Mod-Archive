...
const u8 cooldown_time = 16; //1440

// 0 == up, 90 == sideways
const f32 high_angle = 9.0f;
const f32 low_angle = 47.0f;
...
void onInit(CBlob@ this)
{
	...
	Vehicle_Setup(this,
	              1.0f, // move speed
	              0.1f,  // turn speed
	              Vec2f(0.0f, 0.55f), // jump out velocity
	...
	Vehicle_SetupWeapon(this, v,
	                    ...
	                    "mat_heatwarhead", // bullet ammo config name
	                    "mortarshell", // bullet config name   //ballista_bolt
	                    "MortarFire", // fire sound
	...
	this.set_string("autograb blob", "mat_heatwarhead");

	// auto-load on creation
	if (getNet().isServer())
	{
		CBlob@ ammo = server_CreateBlob("mat_heatwarhead");
		if (ammo !is null)
		{
	...
	CSpriteLayer@ arm = sprite.addSpriteLayer("arm", sprite.getConsts().filename, 40, 40);
	...
	CSpriteLayer@ indicator = sprite.addSpriteLayer("indicator", sprite.getConsts().filename, 10, 10);
	if (indicator !is null)
	{
		indicator.addAnimation("default", 0, false);
		int[] frames = { 8, 9, 10, 11, 20, 21, 22 };
		indicator.animation.AddFrames(frames);
		indicator.SetRelativeZ(0.8f);
		indicator.SetOffset(Vec2f(19.0f, -13.0f));
	}
		...
		if (getNet().isClient())
		{
			...
			if (p !is null)
			{
				...
				if (local !is null)
				{
					if (arm !is null)
					{	
						arm.SetVisible(local.isAttachedTo(this));

						arm.ResetTransform();
						f32 floattime = getGameTime();
						arm.RotateBy((angle - angle%6), Vec2f(-0.5f, 15.5f));
						arm.SetOffset(Vec2f(-4.0f, -19.0f));   //10.0f, -6.0f
					}
				}
			}
		}
...
void onRender(CSprite@ this)
{
	...
	f32 angle = getAngle(blob, v.charge, v);
	...
	if (getNet().isClient())
	{
		...
		if (p !is null)
		{
			...
			if (local !is null)
			{
				if (indicator !is null)
				{	
					indicator.SetVisible(local.isAttachedTo(blob));

					if (blob.isFacingLeft())
					{
						indicator.animation.SetFrameIndex(-1*((angle - angle%6)/6)-1);
					}
					else
					{
						indicator.animation.SetFrameIndex(((angle - angle%6)/6)-1);
					}
				}
			}
		}
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _charge)
{
	if (bullet !is null)
	{
		...
		f32 charge = 23.0f + 15.0f * (float(charge_prop) / float(v.max_charge_time));

		f32 angle = getAngle(this, _charge, v) + this.getAngleDegrees();
		angle = (angle - angle%6);
		Vec2f vel = Vec2f(0.0f, -charge).RotateBy(angle);
		bullet.setVelocity(vel);
		bullet.setPosition(bullet.getPosition() + vel + Vec2f((this.isFacingLeft() ? -1 : 1)*12.0f, 0.0f)); //here1   2.5
	}
