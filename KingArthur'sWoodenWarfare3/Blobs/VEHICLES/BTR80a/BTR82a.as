...
const u8 cooldown_time = 19; //1440

// 0 == up, 90 == sideways
const f32 high_angle = 70.0f;
const f32 low_angle = 94.0f;
...
void onInit(CBlob@ this)
{
	...
	Vehicle_SetupWeapon(this, v,
	                    ...
	                    "btrshell", // bullet config name   //ballista_bolt
	                    "BTRFire", // fire sound
	...
	if (arm !is null)
	{
		...
		if (arm !is null)
		{
			arm.SetRelativeZ(-6.5f); //0.5f
			//arm.RotateBy(angle, Vec2f(-0.5f, 15.5f));
			arm.SetOffset(Vec2f(-90.0f, -14.0f));
		}
	}
...
void onTick(CBlob@ this)
{
	if (this.hasAttached() || this.getTickSinceCreated() < 30)
	{
		...
		if (arm !is null)
		{
			...
			arm.RotateBy(angle, Vec2f(-0.5f, 15.5f));
			arm.SetOffset(Vec2f(3.0f, -26.0f));
		}
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _charge)
{
	if (bullet !is null)
	{
		...
		f32 charge = 21.0f + 15.0f * (float(charge_prop) / float(v.max_charge_time));
