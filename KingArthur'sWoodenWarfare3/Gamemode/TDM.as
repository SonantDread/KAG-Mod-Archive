...
void Config(TDMCore@ this)
{
	...
	s32 warmUpTimeSeconds = cfg.read_s32("warmUpTimeSeconds", 10);
	...
	f32 gameDurationMinutes = cfg.read_f32("gameDurationMinutes", 20.0f);
	...
	rules.set_f32("fall vel modifier", cfg.read_f32("fall_dmg_nerf", 1.2f));
}
...
shared class TDMCore : RulesCore
{
	...
	void Update()
	{
		...
		{
			rules.SetGlobalMessage("Battle starts in {SEC}");
		...
		if ((rules.isIntermission() || rules.isWarmup()) && (!allTeamsHavePlayers()))  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
			...
			rules.SetGlobalMessage("Not enough players!\nPlease wait for someone to join...");
			tdm_spawns.force = true;
		}
