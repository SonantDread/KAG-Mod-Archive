...
/*
TradeItem@ addItemForBlob(CBlob@ this, const string &in name, int cost, const bool instantShipping, const string &in iconName, const string &in configFilename, const string &in description)
{
	if (cost <= 0) {
		return null;
	}
...
void MakeTradeMenu(CBlob@ trader)
{
	...
	s32 cost_keg = cfg.read_s32("cost_keg", 60);
	s32 cost_mine = cfg.read_s32("cost_mine", 30);

	s32 cost_arrows = cfg.read_s32("cost_arrows", 15);
	s32 cost_waterarrows = cfg.read_s32("cost_waterarrows", 20);
	s32 cost_firearrows = cfg.read_s32("cost_firearrows", 20);
	s32 cost_bombarrows = cfg.read_s32("cost_bombarrows", 40);

	//s32 cost_boulder = cfg.read_s32("cost_boulder", -1);
	s32 cost_burger = cfg.read_s32("cost_burger", 40);
	//s32 cost_sponge = cfg.read_s32("cost_sponge", -1);
	s32 cost_saw = cfg.read_s32("cost_saw", 60);

	s32 cost_mountedbow = cfg.read_s32("cost_mountedbow", -1);
	s32 cost_drill = cfg.read_s32("cost_drill", -1);
	s32 cost_catapult = cfg.read_s32("cost_catapult", 120);
	//s32 cost_ballista = cfg.read_s32("cost_ballista", -1);

	//AMMO
	s32 cost_7mmround = cfg.read_s32("cost_7mmround", 19);
	s32 cost_14mmround = cfg.read_s32("cost_14mmround", 26);
	s32 cost_bolts = cfg.read_s32("cost_bolts", 24);

	s32 menu_width = cfg.read_s32("trade_menu_width", 4);
	s32 menu_height = cfg.read_s32("trade_menu_height", 4);
	...
	//knighty stuff
	addItemForCoin(trader, "Bomb", cost_bombs, true, "$mat_bombs$", "mat_bombs", "Used to blow people up.");
	addItemForCoin(trader, "Water Bomb", cost_waterbombs, true, "$mat_waterbombs$", "mat_waterbombs", "Stun yer' mates.");
	addItemForCoin(trader, "Keg", cost_keg, true, "$keg$", "keg", "A big boomer, be careful!");
	addItemForCoin(trader, "Mine", cost_mine, true, "$mine$", "mine", "A sneaky mine, vonerable to the keen eye.");
	...
	addItemForCoin(trader, "Bomb Arrow", cost_bombarrows, true, "$mat_bombarrows$", "mat_bombarrows", "A bomb on a stick, deadly.");

	addItemForCoin(trader, "7.62mm Rounds", cost_7mmround, true, "$mat_7mmround$", "mat_7mmround", "Ammo for a machinegun.");
	addItemForCoin(trader, "14.5mm Rounds", cost_14mmround, true, "$mat_14mmround$", "mat_14mmround", "Ammo for an APC's heavy machinegun.");
	addItemForCoin(trader, "105mm Shells", cost_bolts, true, "$mat_bolts$", "mat_bolts", "Ammo for a main tank gun.");
	...
	addItemForCoin(trader, "Burger", cost_burger, true, "$food$", "food", "Delicious food to keep you fighting.");
}
