...
void onTick(CBlob@ this)
{
	...
	{			
		...
		if (this !is null)
		{
			...
			if (getNet().isClient()&&!getNet().isServer())
			{
				print("Client15555555555");
			}
			if (!getNet().isClient()&&getNet().isServer())
			{
				print("Server1555555555555555555");
			}
			if (getNet().isClient()&&getNet().isServer())
			{
				print("Both15555555555555555555");
			}
			this.getSprite().PlaySound("thud.ogg");
			CBlob@ bullet = server_CreateBlob("bullet", this.getTeamNum(), this.getPosition());

			//print("Running on: " + getNet().isServer()&&getNet().isClient()?"Both!":getNet().isServer()?"Server only":getNet().isClient()?"Client only");
			if (getNet().isClient()&&!getNet().isServer())
			{
				print("Client222222222");
			}
			if (!getNet().isClient()&&getNet().isServer())
			{
				print("Server2222222");
