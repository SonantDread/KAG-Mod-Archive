...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	if (sv_test)
	{
		...
		{
			getMap().server_SetTile(pos + Vec2f(0, 6), CMap::tile_ground);
		}
		else if (text_in == "!bedrock")
		{
			getMap().server_SetTile(pos + Vec2f(0, 6), CMap::tile_bedrock);
		}
		...
		else if (text_in.substr(0, 1) == "!")
		{
			...
			if (tokens.length > 1)
			{
				...
				else if (tokens[0] == "!sign")
				{
					...
					createSign(pos - Vec2f(0, 6), text);
				}
				else if (tokens[0] == "!morph")
				{
					string classname = tokens[1];

					if(!admin && classname != "knight" && classname != "archer" && classname != "builder")
						return true;
