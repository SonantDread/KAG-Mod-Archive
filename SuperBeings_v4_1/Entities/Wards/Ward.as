		...
		if(getNet().isServer()){
			...
			if (this.getMap().getBlobsInRadius(this.getPosition(), 24.0f, @blobsInRadius)) 
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					...
					if(b.hasTag("player") && !b.hasTag("dead"))if(b.getTeamNum() == this.getTeamNum())
					{
						if(XORRandom(power*10) == 0)b.server_Heal(0.25);
					}
				}
			}
		}
			...
			if(getNet().isServer()){
				...
				if (this.getMap().getBlobsInRadius(this.getPosition(), 24.0f, @blobsInRadius)) 
				{
					for (uint i = 0; i < blobsInRadius.length; i++)
					{
						...
						{
							if(XORRandom(power*10) == 0)this.server_Hit(b, b.getPosition(), Vec2f(), 0.25f, Hitters::fire);
						}
					}
				}
			}
