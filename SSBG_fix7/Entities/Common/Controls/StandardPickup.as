...
void onTick( CBlob@ this )
{
	...
	if (carryBlob !is null)
	{
		if (this.getCarriedBlob().getPlayer() !is null)
		{
			...
			u32 lastPickupTime = this.get_u32("last pickup time");
			int ticksTillThrow = 80;
			int diff = gametime - lastPickupTime;
			if (diff > ticksTillThrow)
			{
				client_SendThrowCommand( this );

				lastPickupTime = gametime;
				this.set_u32("last pickup time", lastPickupTime);
			}				
		}
	}
	else
	...
	{
		this.DisableKeys(key_pickup);
		
		const u32 gametime = getGameTime();
		u32 lastGrabTime = this.get_u32("last grabbed time");
		int ticksTillGrabAgain = 100;
		int diff = gametime - (lastGrabTime + ticksTillGrabAgain);
		if (diff > 0)
		{
			this.set_u8( "state", 0 );
		}
...
{  
	...
	{
		this.set_u8( "state", 1 );
		this.set_u32("last grabbed time", getGameTime());
	}
}
