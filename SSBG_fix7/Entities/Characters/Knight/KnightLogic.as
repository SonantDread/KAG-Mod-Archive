...
void onInit( CBlob@ this )
{
    ...
    addShieldVars(this, 220.0f, 2.0f, 5.0f);
...
void onTick( CBlob@ this )
{
                ...
                if ((!inair && this.getShape().vellen < 1.0f) || !pressed_a2) {
					...
					if (!inair) {
						...
						if (knight.slideTime > 0)
						{
							...
							{
								this.getSprite().PlaySound( "/Scrape" );
							}
    ...
    else if ( (pressed_a1 || swordState) && !moveVars.wallsliding ) //no attacking during a slide
    {
        ...
        else if (knight.state == KnightStates::sword_power || knight.state == KnightStates::sword_power_super)
        {
            ...
            else if (delta > DELTA_BEGIN_ATTACK && delta < 10)
            {
				if (knight.state == KnightStates::sword_power)
					...
					DoAttack(this, 10.0f, -(vec.Angle()), 120.0f, Hitters::sword, delta, knight );
            }
        ...
        if ( ((knight.state == KnightStates::sword_power) &&
			 ...
			 Maths::Abs(vel.x) < KnightVars::slash_move_max_speed &&
			 vel.y > -KnightVars::slash_move_max_speed )
        {
			if (delta == 0)
			{                              
				Vec2f aimDirection = this.getAimPos() - this.getPosition();
				aimDirection.Normalize();
				Vec2f slash_vel = aimDirection * 500.0f;
				this.AddForce(slash_vel);
				
				ParticleAnimated( "Entities/Effects/Sprites/MediumSteam2.png",
								this.getPosition(),
								-aimDirection*0.5f,
								1.0f, 2.0f, 
								3, 
			 ...
			 vel.y > -KnightVars::slash_move_max_speed )
        {
			...
			{                              
				Vec2f aimDirection = this.getAimPos() - this.getPosition();
				aimDirection.Normalize();
				Vec2f slash_vel = aimDirection * 10.0f;
				this.set("curSlashVel",slash_vel);
				
				this.getSprite().PlaySound( "Woosh" );
			}
			if (delta < KnightVars::slash_move_time * 0.7)
			{
				Vec2f slash_vel;
				this.get("curSlashVel",slash_vel);
				this.setVelocity(slash_vel);
				
				MakeParticle(this.getPosition(),
					"Slice.png", 10.0, -this.getVelocity()*0.05 );
			}
			else
			{
				this.setVelocity(Vec2f(0,0)); //stop the knight in place
			}
		}
    ...
    if (shieldState)
    {
        ...
        if (knight.state == KnightStates::shielding && direction == -1 && !inair)
        {
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info )
{
	...
	{
		if (this.getCarriedBlob().getPlayer() !is null)
			{
				Vec2f velocity = this.getCarriedBlob().getPosition() - this.getPosition();
				this.server_Hit( this.getCarriedBlob(), this.getCarriedBlob().getPosition(), velocity, 0.5f, type, true);				
			}
	}
	...
	if (map.getHitInfosFromArc( pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos ))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			{					
				...
				const bool large = b.hasTag("blocks sword") && b.isCollidable();
