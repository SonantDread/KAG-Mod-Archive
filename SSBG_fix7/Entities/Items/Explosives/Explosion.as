...
void Explode ( CBlob@ this, f32 radius, f32 damage )
{
	...
	if (this.hasTag("nuke"))
	{
		ParticleAnimated( "Entities/Effects/Sprites/Nuclear.png",
		...
		Sound::Play("Blast3.ogg");
	}
	
	if (this.hasTag("fire bolt"))
	{
		ParticleAnimated( "Entities/Effects/Sprites/Hit3.png",
			this.getPosition(),
			Vec2f(0.0,0.0f),
			1.0f, 1.0f, 
			3, 
                        ...
                        if (dist < map_damage_radius)
                        {
                            ...
                            if (canHit)
                            {
								...
								if (map.getSectorAtPosition( tpos, "no build") is null)
								{
									...
									{
										if (dist >= rad_thresh || (hitter == Hitters::bomb && map.isTileGround(tile ))  ) 										{
