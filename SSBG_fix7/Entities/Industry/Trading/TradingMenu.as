...
{		
    ...
    else if (cmd == this.getCommandID("buy"))
    {
        ...
        if (caller !is null)
        {
            ...
            TradeItem@ item = AddItemToShop( this, caller, itemIndex, goldCount );
			...
			if (item is null) // reload menu
			{
				...
				//	CBitStream params;
				//	params.write_u16(callerid);
				//	this.server_SendCommandToPlayer( this.getCommandID("reload menu"), params, caller.getPlayer() ); // use this for nicer reloading menu - we dont need it for scrolls
				//}
				caller.ClearMenus();
				BuildTradingMenu( this, caller );
			}
...
TradeItem@ AddItemToShop( CBlob@ this, CBlob@ caller, const uint itemIndex, const u16 goldCount )
{
	if (caller is null)
		return null;
