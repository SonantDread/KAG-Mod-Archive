...
const SColor color_tradingpost(255, 34, 255, 0);
const SColor color_emitter(255, 243, 54, 255);
...
class PNGLoader
{
	...
	if (pixel == color_tile_ground) {
		...
		else if (pixel == color_tradingpost)
		{
			spawnBlob( map, "tradingpost", offset, -1 );
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_emitter)
		{
			spawnBlob( map, "emitter", offset, -1 );
			offsets[autotile_offset].push_back( offset );
		}			
		...
		{
			CBlob@ ruins = spawnBlob( map, "ruins", offset, 2 );
			if (ruins !is null) // add research to first hall
			{
				ruins.AddScript("Researching.as");
				ruins.Tag("script added");
			}
			offsets[autotile_offset].push_back( offset );
		}
		else if (pixel == color_purple_main_spawn)
		{
			CBlob@ ruins = spawnBlob( map, "ruins", offset, 3 );
			if (ruins !is null) // add research to first hall
			{
				ruins.AddScript("Researching.as");
				ruins.Tag("script added");
		...
		{
			CBlob@ ruins = spawnBlob( map, "ruins", offset, 4 );
			if (ruins !is null) // add research to first hall
			{
				ruins.AddScript("Researching.as");
				ruins.Tag("script added");
		...
		{
			CBlob@ ruins = spawnBlob( map, "ruins", offset, 5 );
			if (ruins !is null) // add research to first hall
			{
				ruins.AddScript("Researching.as");
				ruins.Tag("script added");
		...
		{
			CBlob@ ruins = spawnBlob( map, "ruins", offset, 6 );
			if (ruins !is null) // add research to first hall
			{
				ruins.AddScript("Researching.as");
				ruins.Tag("script added");
		...
		{
			CBlob@ ruins = spawnBlob( map, "ruins", offset, 7 );
			if (ruins !is null) // add research to first hall
			{
				ruins.AddScript("Researching.as");
				ruins.Tag("script added");
	...
	{
		string mapName = getFilenameWithoutExtension( getFilenameWithoutPath( getMapInParenthesis() ) );
		print("::::getMapName = " + getFilenameWithoutExtension( getFilenameWithoutPath( getMapInParenthesis() ) ) + "::");
		if (XORRandom(2) == 1)
			map.CreateTileMap( width, height, 8.0f, "Sprites/Sonic/Sonic_tiles.png" );
		else
	...
	void SetupBackgrounds()
	{
		...
		if (XORRandom(2) == 1)
		{
			map.AddBackground( "Sprites/Sonic/Sonic_Plains.png", Vec2f(0.0f, -18.0f), Vec2f(0.3f, 0.3f), color_white ); 
			map.AddBackground( "Sprites/Sonic/Sonic_Trees.png", Vec2f(0.0f,  -5.0f), Vec2f(0.4f, 0.4f), color_white ); 
			map.AddBackground( "Sprites/Sonic/Sonic_Island.png", Vec2f(0.0f, 0.0f), Vec2f(0.6f, 0.6f), color_white ); 
		}
		else if (XORRandom(2) == 1)
		{ 
			map.AddBackground( "Sprites/Space/Space_Island.png", Vec2f(0.0f, 0.0f), Vec2f(0.6f, 0.6f), color_white ); 
		}
		else
		{
