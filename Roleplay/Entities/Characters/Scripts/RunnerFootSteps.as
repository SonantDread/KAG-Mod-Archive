    ...
    if (blob.isKeyPressed(key_left) || blob.isKeyPressed(key_right))
    {
		...
		if ((blob.getNetworkID() + getGameTime()) % (moveVars.walkFactor < 1.0f ? 14 : 8) == 0)
		{
			...
			if (map.isTileGrass(tile))
			    this.PlaySound("../Mods/Roleplay/Entities/Characters/Sounds/Footsteps/GrassStep" + (1 + XORRandom(5)) + ".ogg", volume);
			else if (map.isTileWood(tile))
				this.PlaySound("../Mods/Roleplay/Entities/Characters/Sounds/Footsteps/WoodStep" + (1 + XORRandom(4)) + ".ogg", volume);
			else if (map.isTileCastle(tile))
				this.PlaySound("../Mods/Roleplay/Entities/Characters/Sounds/Footsteps/StoneStep" + (1 + XORRandom(5)) + ".ogg", volume);
			else
