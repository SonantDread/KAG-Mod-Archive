...
class PNGLoader
{
	...
	void handlePixel(SColor pixel, int offset)
	{
		...
		if (alpha < 255)
		{
			...
			{
				const string team = alpha & 0x01 == 0 ? "green" : "red";
		...
		{
			spawnBlob(map, "wooden_door", offset, 2, true);
			offsets[autotile_offset].push_back(offset);
		}
		else if (pixel == color_wooden_door_v_blue)
		{
			CBlob@ blob = spawnBlob(map, "wooden_door", offset, 2, false);
			offsets[autotile_offset].push_back(offset);
			CShape@ shape = blob.getShape();
			blob.setAngleDegrees(90.0f);
			shape.SetStatic(true);
		...
		{
			spawnBlob(map, "stone_door", offset, 2, true);
			offsets[autotile_offset].push_back(offset);
		}
		else if (pixel == color_stone_door_v_blue)
		{
			CBlob@ blob = spawnBlob(map, "stone_door", offset, 2, false);
			offsets[autotile_offset].push_back(offset);
			CShape@ shape = blob.getShape();
			blob.setAngleDegrees(90.0f);
			shape.SetStatic(true);
		...
		{
			spawnBlob(map, "trap_block", offset, 2, true);
			offsets[autotile_offset].push_back(offset);
		}
