...
bool GiveSpawnResources(CRules@ this, CBlob@ blob, CPlayer@ player, CTFPlayerInfo@ info)
{
	...
	{
		CBlob@[] possible_plutus;
		getBlobsByName("resupplyaltar", @possible_plutus);
		
		u8 matmul = 0;

		for(int i = 0; i < possible_plutus.size(); i++)
		{
			CBlob@ check = possible_plutus[i];
			if (check.getTeamNum() == blob.getTeamNum())
			{
				matmul ++;
			}
		}
		...
		if (this.isWarmup())
		{
			ret = SetMaterials(blob, "mat_wood", 300 + (matmul*50)) || ret;
			ret = SetMaterials(blob, "mat_stone", 100 + (matmul*25)) || ret;
		}
		else
		{
			ret = SetMaterials(blob, "mat_wood", 100 + (matmul*50)) || ret;
			ret = SetMaterials(blob, "mat_stone", 30 + (matmul*25)) || ret;
		}
	...
	{
		CBlob@[] possible_plutus;
		getBlobsByName("plutusaltar", @possible_plutus);
