...
void onTick(CMovement@ this)
{
	...
	if (blob.isInWater() && !isknocked)
	{
		...
		//up & down, left & right
		if (up && vel.y > -(swimspeed * 1.6))
		{
			waterForce.y -= 1;
		}
		if (down && vel.y < (swimspeed * 1.6))
		{
			waterForce.y += 1;
		}
		if (left && vel.x > -(swimspeed * 1.6))
		{
			waterForce.x -= 1;
		}
		if (right && vel.x < (swimspeed * 1.6))
		{
			waterForce.x += 1;
		}
	...
	bool left_or_right = (left || right);
	{
		...
		Vec2f walkDirection;
		const f32 turnaroundspeed = 1.15f;
		const f32 normalspeed = 1.0f;
		const f32 backwardsspeed = 0.9f;
