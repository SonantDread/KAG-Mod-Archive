...
{
AddIconToken( "$Trap$", "KnightIcons.png", Vec2f(16,32), 6 );
AddIconToken( "$ClusterBomb$", "KnightIcons.png", Vec2f(16,32), 7 );
	AssassinInfo knight;
	...
	if (cmd == this.getCommandID("get bomb"))
	{
		...
		if (hasItem(this, bombTypeName))
		{
			...
			else if (bombType == 1)
			{
				if (getNet().isServer())
				{
					CBlob @blob = server_CreateBlob("trap", this.getTeamNum(), this.getPosition());
					if (blob !is null)
					{
						TakeItem(this, bombTypeName);
						this.server_Pickup(blob);
