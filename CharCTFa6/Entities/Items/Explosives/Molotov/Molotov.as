...
{
    if ((blob.hasTag("solid") || blob.hasTag("plat") || blob.hasTag("door") || ( blob.hasTag("player")) && blob.getTeamNum() != this.getTeamNum()))
    this.server_Die();
    return false;
}
...
{
	CBlob@[] blobsInRadius;
			if (this.getMap().getBlobsInRadius(this.getPosition(), 5.0f, @blobsInRadius))
			{
				for (uint i = 0; i < blobsInRadius.length; i++)
				{
					CBlob @b = blobsInRadius[i];
						if (getNet().isServer())
						this.server_Hit(b, this.getPosition(), Vec2f(0, 0), 0.1f, Hitters::fire, true);
				}
			}
