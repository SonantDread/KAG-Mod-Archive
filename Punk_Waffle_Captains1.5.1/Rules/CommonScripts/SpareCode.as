	...
	for (int i = 0; i < getPlayersCount(); i++){
		...
		if (statPlayer !is null){
			if (statPlayer.getTeamNum() == 0 || statPlayer.getTeamNum() == 1){
				...
				tcpr("[USCaptains] updateBackend: " + statPlayer.getUsername() + "+" + backendUpdate(this, statPlayer.getUsername()));
			}
		}
	}
...
shared string backendUpdate(CRules@ this, string playerName){
	string statList;
	string[] checklist = {"Wins", "Losses", "Winning_Captain", "Losing_Captain", "Flags", "Kills", "Deaths", "Elo"};
	for(int i=0; i < checklist.size(); i++){
		statList += this.get_u32(playerName + " " + checklist[i]) + (i != (checklist.size() - 1) ? " " : "");
	}
	return statList;
}
