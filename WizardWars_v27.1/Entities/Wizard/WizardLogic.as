...
void ManageSpell( CBlob@ this, WizardInfo@ wizard, RunnerMoveVars@ moveVars )
{
	...
	Vec2f tilepos = pos + normal * Maths::Min(aimVec.Length() - 1, MAX_ATTACK_DIST);
	Vec2f surfacepos;
	CMap@ map = this.getMap();
	bool aimPosBlocked = map.rayCastSolid(pos, tilepos, surfacepos);
	Vec2f spellPos = surfacepos - normal*8.0f;

    Spell spell = WizardParams::spells[wizard.primarySpellID];
    s32 mana = wizard.mana;
    ...
    {
        spell = WizardParams::spells[wizard.secondarySpellID];
    ...
    {
        spell = WizardParams::spells[wizard.aux1SpellID];
    ...
    else if (getControls().isKeyJustPressed( KEY_KEY_U )) // teleport using u
    {
        ...
        charge_state = WizardParams::cast_3;
        if (wizard.mana >= spell.mana && (this.isMyPlayer() || this.getPlayer() is null || this.getPlayer().isBot()))
        {
            CBitStream params;
    ...
    {
        if (wizard.mana >= spell.mana && charge_state > WizardParams::charging && not (spell.needs_full && charge_state < WizardParams::cast_3) &&
            (this.isMyPlayer() || this.getPlayer() is null || this.getPlayer().isBot()))
        {
			...
			u8 castSpellID;
			if ( is_aux1 )
				castSpellID = wizard.aux1SpellID;
			else if ( is_secondary )
				castSpellID = wizard.secondarySpellID;
			else
				castSpellID = wizard.primarySpellID;
	...
	if ( !is_pressed )
	{
		...
		int currHotkey = wizard.primaryHotkeyID;
		int nextHotkey =  wizard.hotbarAssignments.length;
		...
		if ( nextHotkey <  wizard.hotbarAssignments.length )
		{
			wizard.primaryHotkeyID = nextHotkey;
			wizard.primarySpellID = wizard.hotbarAssignments[nextHotkey];
...
void onTick( CBlob@ this )
{
    ...
    ManageSpell( this, wizard, moveVars );
}
...
void CastSpell(CBlob@ this, const s8 charge_state, const Spell spell, Vec2f aimpos )
{
		...
		else
		{
			if (getNet().isServer())
			{
				...
				if (orb !is null)
				{
					...
					orb.set_f32("heal_amount", healAmount*2.0f);
    ...
    else if (spellName == "magic_barrier")
    {
        ...
        dirNorm.Normalize();
		Vec2f orbPos = this.getPosition() + Vec2f(24.0f,0.0f).RotateBy(-dirNorm.Angle());	
...
{ 
    ...
    CGridMenu@ spellMenu = CreateGridMenu( Vec2f(pos.x, pos.y - 32), this, Vec2f( 10, 2 ), "Choose a spell above and click on spell icons in the hotbar to customize it." );
	const u8 customSpell = wizard.customSpellID;

    if (spellMenu !is null)
    {
		spellMenu.deleteAfterClick = false;	

        for (uint i = 0; i < WizardParams::spells.length; i++)
        {
            Spell spell = WizardParams::spells[i];

            CBitStream params;
            params.write_u8(i);
            params.write_bool(false);
            CGridButton @button = spellMenu.AddButton("SpellIcons.png", spell.iconFrame, Vec2f(16, 16), spell.name, this.getCommandID( "pick spell" ), Vec2f(1, 1), params);
			
            string hoverText = spell.spellDesc + "\n\nMana required: "+spell.mana;
            if (spell.needs_full)
