...
void onTick(CBlob@ this)
{
	...
	if (this.getInventory() !is null && this.getTickSinceCreated() > 10)
	{
		if (this.isKeyJustPressed(key_inventory))
		{
			...
			Vec2f invPos = getDriver().getScreenCenterPos() + Vec2f(0,288);
			if (this.exists("inventory offset"))
			{
				this.CreateInventoryMenu(invPos + this.get_Vec2f("inventory offset"));
			}
			else
			{
				this.CreateInventoryMenu(invPos);
			}

			//getControls().setMousePosition( invPos );
		}
