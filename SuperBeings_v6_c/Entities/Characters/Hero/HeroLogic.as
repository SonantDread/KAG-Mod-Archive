...
void onTick(CBlob@ this)
{
	...
	getBlobsByName("hero", @fg);
	for(uint i = 0; i < fg.length; i++)
	{
		if(fg[i].getPlayer() !is null)
		if(fg[i].getPlayer().getUsername() == this.get_string("owner")){
			power = fg[i].get_s16("power");
		}
		if(fg[i].get_string("owner") == this.get_string("owner")){
			powermod += 1;
		}
	}
	
	if(powermod > 0)this.set_f32("damage",(power/500+1)/powermod);
	else this.set_f32("damage",(power/500+1));
	
	this.set_s16("power",power/powermod);
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		         ...
		         knight.state <= KnightStates::sword_cut_down) // cut state
		{
			...
			if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
			{
				...
				DoAttack(this, this.get_f32("damage"), attackAngle, attackarc, Hitters::sword, delta, knight);
			}
			else if (delta >= 9)
			{
				knight.swordTimer = 0;
		         ...
		         knight.state == KnightStates::sword_power_super)
		{
			...
			{
				DoAttack(this, this.get_f32("damage")*2.0f, -(vec.Angle()), 120.0f, Hitters::sword, delta, knight);
			}
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if(this.get_s16("power") > 0){
		...
		else if(this.get_s16("power") > 750){
			damage -= 1.5f;
		}
		else if(this.get_s16("power") > 500){
			damage -= 1.0f;
		}
		else if(this.get_s16("power") > 250){
			damage -= 0.5f;
		}
