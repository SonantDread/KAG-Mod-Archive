...
void onCreateInventoryMenu(CBlob@ this, CBlob@ forBlob, CGridMenu @gridmenu)
{
	...
	CGridMenu@ menu = CreateGridMenu(pos, this, Vec2f(4, 2), "Abilities");
	
	AddIconToken("$herosword$", "HeroSword.png", Vec2f(16, 16), 0);
	AddIconToken("$holybow$", "HolyCrossbow.png", Vec2f(16, 16), 0);
	AddIconToken("$goldenorb$", "GoldenOrb.png", Vec2f(16, 16), 0);
	AddIconToken("$goldenfish$", "GoldenFish.png", Vec2f(16, 16), 0);
	AddIconToken("$well$", "WellIcon.png", Vec2f(16, 16), 0);
	...
	if (menu !is null)
	{
		...
		{
			CGridButton@ b = menu.AddButton("$well$", "A powerful well spewing out pure energy. Costs 1000 golden essence.", this.getCommandID("makewell"));
			if(this.get_s16("power") <= 1000)b.SetEnabled(false);
		}
		
		{
			CGridButton@ b = menu.AddButton("$holybow$", "Create a bow for some to fight with. Costs 50 golden essence.", this.getCommandID("makebow"));
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("makebow"))if(this.get_s16("power") > 50){
		if (getNet().isServer())
		{
			CBlob @blob = server_CreateBlob("holycrossbow", -1, this.getPosition());
		}
	...
	if(this.get_s16("power") > 750)
	if (cmd == this.getCommandID("makewell")){
		if (getNet().isServer())
		{
			CBlob @blob = server_CreateBlob("goldwell", -1, this.getPosition());
		}
