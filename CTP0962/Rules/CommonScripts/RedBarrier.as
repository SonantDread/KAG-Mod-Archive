   ...
   // if(getNet().isServer()) {
     //  this.set_s32("biome", XORRandom(12)); //s32 might be overkill, but anyways
	  ...
	  // }
	   ...
	   if(getNet().isServer()) {
       this.set_s8("biome", XORRandom(12)); //s32 might be overkill, but anyways
       this.Sync("biome", true);
	   }
...
void onInit(CRules@ this)
{
	...
	//getRules().set_s32("biome",0);
	//s32 biome = this.get_s32("lastReloadTime");
	//s32 biome = this.set_s32("biome", XORRandom(3));
...
void addBarrier()
{
	...
	if (map.getSectorAtPosition((ul + lr) * 0.5, "barrier") is null)
		...
		//s32 biome = XORRandom(2);
	//getRules().set_s32("biome",biome);
