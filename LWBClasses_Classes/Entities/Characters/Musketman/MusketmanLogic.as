		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (!dontHitMore && (b.getName() != "log" || !dontHitMoreLogs))
				{
					...
					this.server_Hit(b, hi.hitpos, velocity, hitLogNow ? 0.5f : 0.25f, Hitters::shovel, true);  // server_Hit() is server-side only
...
void ClientFire(CBlob@ this, const bool hasbullet)
{
	...
	if (hasbullet && canSend(this))  // client-logic
	{
		...
		ShootBullet(this, this.getPosition() + offset, this.getAimPos(), MusketmanParams::shoot_max_vel);
	}
}
