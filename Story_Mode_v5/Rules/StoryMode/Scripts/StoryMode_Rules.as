...
shared class StoryModeCore : RulesCore
{
				...
				if(blob !is null){
					...
					if(inv !is null){
						for (int i = 0; i < inv.getItemsCount(); i++)
						{
							...
							if(item !is null){
								p.set_string("item"+i,item.getName());
								p.set_s16("itemamount"+i,blob.getBlobCount(item.getName()));
	...
	void NextMap()
	{
		...
		if(getMap().getMapName() == "Graveyard.png" || getMap().getMapName() == "SkyLands.png"){
			LoadMap("Forkagumsville.png");
			return;
		}
		
		if(getMap().getMapName() == "Forkagumsville.png"){
			clearAdventure();
			LoadMap(FirstMap());
			return;
		}
