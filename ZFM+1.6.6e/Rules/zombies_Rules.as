...
shared class ZombiesCore : RulesCore
{
    ...
    void Update()
    {
		...
		const int wraiteRate = 10; //lowed from 15 to 10 
			...
			if (map !is null)
			{
               ...
               // if(map.getDayTime()>0.6 && map.getDayTime()<0.8)
				...
				if (map.getDayTime()>0.8 || map.getDayTime()<0.1)
                {
                    ...
                    if (transition == 1 && (dayNumber % 5) == 0) //day 5 wave
					{
						transition=0;
    ...
    void CheckTeamWon( )
    {
		...
		/*if(getRules().get_bool("everyones_dead")) //Old win-lose condition check, doesn't bug with migrants
		{
            rules.SetTeamWon(1);
		...
		if(num_survivors == 0) //Seems to be more eficient, although migrant corpses count as survivors
		{
			rules.SetTeamWon(1);
			rules.SetCurrentState(GAME_OVER);
