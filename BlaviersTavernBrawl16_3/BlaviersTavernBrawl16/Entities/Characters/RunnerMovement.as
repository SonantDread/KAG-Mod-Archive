...
void onTick(CMovement@ this)
{
	        ...
	        !blob.isOnLadder() && (up || left || right))  //key pressed
	{
		...
		const f32 slidespeed = 1.6f;

		// stop climb
		if (blob.hasTag("climbing"))
		{
			...
			if (surface_left && left)
			{
				blob.SetFacingLeft(true);
			}
			else if (surface_right && right)
			{
		        ...
		        !onground)
		{
			...
			bool dust = false;
			bool rogue = blob.getName() == "rogue";

			if (rogue && ((left && surface_left) || (right && surface_right)))
			{
				//climb mode
				blob.Tag("climbing");
			}
			...
			if (!blob.hasTag("climbing"))
			{
				if (moveVars.jumpCount > 5) //wait some time to be properly in the air
				{
					...
					//wallrun
					if ((!surface_above && vel.y < slidespeed || rogue) &&
					        ((left && surface_left && !jumpedLEFT) || (right && surface_right && !jumpedRIGHT) || set_contact))
					{
						         ...
						         pos.y + 1.0f > moveVars.wallrun_start - map.tilesize * moveVars.wallrun_length))
						{
							...
							if (set_contact || getGameTime() % 5 == 0)
							{
								...
								f32 wallrun_speed = moveVars.jumpMaxVel * 1.7f;
	...
	if (moveVars.jumpFactor > 0.01f && !isknocked)
	{
		...
		if (up && vel.y > -moveVars.jumpMaxVel)
		{
			...
			if (moveVars.jumpCount == 1 && is_client && blob.getName() != "rogue")
			{
