...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	if (damage > 0.1f && (hitterBlob !is this || customData == Hitters::crush))  //sound for anything actually painful
	{
		...
		switch (customData)
		{
			...
			case Hitters::rang:
				if (this.getHealth() - damage/2 <= 0.0f && !this.hasTag("dead"))
					Sound::Play("Playerdie", this.getPosition());
				else
					Sound::Play("Playerhitrang", this.getPosition());
				break;
			...
			case Hitters::drill:
				if (this.getHealth() - damage/2 <= 0.0f && !this.hasTag("dead"))
					...
					Sound::Play("Playerhitflail", this.getPosition());
				break;

			case Hitters::hyper_rang:
				if (this.getHealth() - damage/2 <= 0.0f && !this.hasTag("dead"))
					Sound::Play("Playerdie", this.getPosition());
				else
					Sound::Play("Playerhithyperrang", this.getPosition());
				break;

			case Hitters::muscles:
				if (this.getHealth() - damage/2 <= 0.0f && !this.hasTag("dead"))
					Sound::Play("Playerhithammer", this.getPosition(), 1.0f, 0.8f + XORRandom(10)*0.1);
				else
					Sound::Play("Playerhithammer", this.getPosition(), 1.0f, 0.8f + XORRandom(10)*0.1);
				break;
			...
			case Hitters::sword:
				if (this.getHealth() - damage/2 <= 0.0f && !this.hasTag("dead"))
					...
					Sound::Play("Playerhitmelee", this.getPosition());
				break;

			case Hitters::arrow:
				if (this.getHealth() - damage/2 <= 0.0f && !this.hasTag("dead"))
					Sound::Play("Playerdiearrow", this.getPosition());
				else
					Sound::Play("ArrowHitFlesh", this.getPosition());
				break;

			case Hitters::boulder:
				if (this.getHealth() - damage/2 <= 0.0f && !this.hasTag("dead"))
					Sound::Play("Playerdieboulder", this.getPosition());
				else
					Sound::Play("Playerhitboulder", this.getPosition());
