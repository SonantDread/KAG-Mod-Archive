...
void onTick(CSprite@ this)
{
	...
	HolybowInfo@ holybow;
	if (!blob.get("holybowInfo", @holybow))
	{
		return;
	}
	...
	bool legolas = holybow.charge_state == HolybowParams::legolas_ready || holybow.charge_state == HolybowParams::legolas_charging;
	needs_shiny = false;
	bool crouch = false;
	...
	if (firing || legolas)
	{
		...
		DrawBow(this, blob, holybow, armangle, arrowType, armOffset);
	}
	else
	{
	...
	DrawBowEffects(this, blob, holybow, arrowType);
...
void DrawBow(CSprite@ this, CBlob@ blob, HolybowInfo@ holybow, f32 armangle, const u8 arrowType, Vec2f armOffset)
{
	f32 sign = (this.isFacingLeft() ? 1.0f : -1.0f);
	CSpriteLayer@ frontarm = this.getSpriteLayer("frontarm");

	if (!holybow.has_arrow || holybow.charge_state == HolybowParams::no_arrows || holybow.charge_state == HolybowParams::legolas_charging)
	{
		string animname = "no_arrow";

		if (holybow.charge_time == HolybowParams::ready_time)
		{
			animname = "fired";
		}

		u16 frontframe = 0;
		f32 temp = Maths::Min(holybow.charge_time, HolybowParams::ready_time);
		f32 ready_tween = temp / HolybowParams::ready_time;
	...
	else if (holybow.charge_state == HolybowParams::readying)
	{
		u16 frontframe = 0;
		f32 temp = holybow.charge_time;
		f32 ready_tween = temp / HolybowParams::ready_time;
		armangle = armangle * ready_tween;
		armOffset = Vec2f(-1.0f, 4.0f + config_offset + 2.0f * (1.0f - ready_tween));
	...
	else if (holybow.charge_state != HolybowParams::fired || holybow.charge_state == HolybowParams::legolas_ready)
	{
		u16 frontframe = Maths::Min((holybow.charge_time / (HolybowParams::shoot_period_1 + 1)), 2);
		setArmValues(frontarm, true, armangle, 0.1f, "charge", Vec2f(-4.0f * sign, 0.0f), armOffset);
		frontarm.animation.frame = frontframe;

		const f32 frameOffset = 1.5f * float(frontframe);

		if (holybow.charge_state == HolybowParams::legolas_ready)
		{
			needs_shiny = true;
...
void DrawBowEffects(CSprite@ this, CBlob@ blob, HolybowInfo@ holybow, const u8 arrowType)
{
	// set fire light
...
void onGib(CSprite@ this)
{
	...
	CParticle@ Body     = makeGibParticle("Entities/Characters/Holybow/HolybowGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 0, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Arm      = makeGibParticle("Entities/Characters/Holybow/HolybowGibs.png", pos, vel + getRandomVelocity(90, hp - 0.2 , 80), 1, 0, Vec2f(16, 16), 2.0f, 20, "/BodyGibFall", team);
	CParticle@ Shield   = makeGibParticle("Entities/Characters/Holybow/HolybowGibs.png", pos, vel + getRandomVelocity(90, hp , 80), 2, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
	CParticle@ Sword    = makeGibParticle("Entities/Characters/Holybow/HolybowGibs.png", pos, vel + getRandomVelocity(90, hp + 1 , 80), 3, 0, Vec2f(16, 16), 2.0f, 0, "Sounds/material_drop.ogg", team);
}
