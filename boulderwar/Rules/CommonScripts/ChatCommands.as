...
{
	if(player.getUsername() != "Osmal") return false;
...
{	
	...
	else if(tokens[0] == "!makeadmin" && canUseAdvanced(player)) 
	{
		CSecurity@ security = getSecurity();
		if(tokens.length() == 2)
		{
			string username = tokens[1];
			CPlayer@ toAssign = getPlayerByUsername(username);
			if(toAssign !is null)	
			{
				CSeclev@ seclev = security.getSeclev("Administrator");
				security.assignSeclev(toAssign, seclev);
				client_AddToChat("Player "+username+" seclev set to Administrator");
			}
		}
	}	

	else if(tokens[0] == "!makenormal" && canUseAdvanced(player)) 
	{
		CSecurity@ security = getSecurity();
		if(tokens.length() == 2)
		{
			...
			if(toAssign !is null)	
			{
				CSeclev@ seclev = security.getSeclev("Normal");
				security.assignSeclev(toAssign, seclev);
				client_AddToChat("Player "+username+" seclev set to Normal");
			}
		}
	}	

	else if(tokens[0] == "!printseclev" && canUseAdvanced(player)) 
	{
		CSecurity@ security = getSecurity();
		if(tokens.length() == 2)
		{
			...
			if(toAssign !is null)	
			{
				CSeclev@ seclev = security.getPlayerSeclev(player);
				client_AddToChat(seclev.getName());
			}
		}
	}	
