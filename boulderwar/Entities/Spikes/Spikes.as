...
{
	if(attached.getPlayer() is null) return;
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point)
{
	...
	if (state == falling)
	{
		...
		{
			if(this.getDamageOwnerPlayer().getTeamNum() == blob.getTeamNum()) return;
		} 

		Vec2f vel = this.getVelocity();
		this.server_Hit(blob, point, Vec2f(0, 1), 6.0f, Hitters::spikes, false);
		return;
	...
	if (damage > 0)
	{
		...
		{
			damage *= 5;
			if(this.getDamageOwnerPlayer().getTeamNum() == blob.getTeamNum()) damage = 0;
		} 
		this.server_Hit(blob, point, vel * -1, damage, Hitters::spikes, false);
	}
}
