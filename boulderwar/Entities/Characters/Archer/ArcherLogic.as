...
void onTick(CBlob@ this)
{
	...
	u16 rock_wait = this.get_u16("rock_wait");
	if(rock_wait > 0) 
	{
		rock_wait--;
		this.set_u16("rock_wait", rock_wait);
	}
	...
	rock_wait = this.get_u16("rock_wait");
	if(this.isKeyJustPressed(key_action3) && this is getLocalPlayerBlob() && !getHUD().hasMenus() && rock_wait == 0 && this.getCarriedBlob() is null)
	{
		this.SendCommand(this.getCommandID("create_rock"));
		this.set_u16("rock_wait", 50);
		this.Sync("rock_wait", false);
	}
...
{
	if(cmd == this.getCommandID("create_rock") && this.getPlayer() !is null)
	{
		CBlob@ rock = server_CreateBlob("rock", this.getTeamNum(), this.getPosition());
		if(rock !is null)
		{
			rock.Tag(this.getPlayer().getUsername());
			this.server_Pickup(rock);
			CBlob@[] rocks; 

			if(getBlobsByTag(this.getPlayer().getUsername(), @rocks))
			{
				for(uint i = 0; i < rocks.length; i++)
				{
					CBlob@ other_rock = rocks[i];
					if(other_rock !is null && !other_rock.hasTag("dying") && !other_rock.isInInventory())
					{
						Vec2f diff = other_rock.getPosition() - this.getPosition();
						if(diff.Length() > 100)
						{
							other_rock.Tag("dying");
							other_rock.server_SetTimeToDie(4);
						}
					}
				}
			}
