...
void onTick(CBlob@ this)
{
	...
	this.server_HitMap(pos, Vec2f(5, 5), 100, Hitters::builder);
	pos.y += 1;
	this.server_HitMap(pos, Vec2f(5, 5), 100, Hitters::builder);
	pos.y += 8;
	this.server_HitMap(pos, Vec2f(5, 5), 100, Hitters::builder);
...
{
	print("ok");
	this.getSprite().ReloadSprites(this.getTeamNum(), this.getSkinNum());
	this.getSprite().ScaleBy(Vec2f(2, 2));
}
...
void Slam(CBlob @this, f32 angle, Vec2f vel, f32 vellen)
{
	...
	if (map.getHitInfosFromArc(pos, -angle, 30, vellen, this, false, @hitInfos))
	{
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			{
				this.server_Hit(hi.blob, pos, vel, dmg*0.5f, Hitters::cata_boulder, true);
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (solid && blob !is null)
	{
		...
		//hurt
		this.server_Hit(blob, point1, hitvel, dmg*0.5f, Hitters::boulder, true);
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if(hitterBlob is null) return 0;
	return damage*0.5f;
}
...
{
	if(this.getBlob() !is null) this.SetFrame(XORRandom(4));//this.animation.frame = (this.getBlob().getNetworkID() % 4);
