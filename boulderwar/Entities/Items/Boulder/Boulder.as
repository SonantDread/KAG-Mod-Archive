...
void onDie(CBlob@ this)
{
	for(uint i = 0; i < 10; i++)
	{
		...
		if(rock !is null)
		{
			...
			if(this.getDamageOwnerPlayer() !is null) rock.SetDamageOwnerPlayer(this.getDamageOwnerPlayer());
			Vec2f vel;
			vel.y -= 400+XORRandom(50);
			vel.RotateBy(-90+XORRandom(180));
			vel+= this.getVelocity();
			rock.AddForce(vel);
			rock.Tag("dying");
			rock.Tag("cluster");
			rock.server_SetTimeToDie(3);
		}
	}
}
...
void Slam(CBlob @this, f32 angle, Vec2f vel, f32 vellen)
{
	...
	if (map.getHitInfosFromArc(pos, -angle, 30, vellen, this, false, @hitInfos))
	{
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			{
				this.server_Hit(hi.blob, pos, vel, dmg*2, Hitters::cata_boulder, true);
...
{
	print("vel: "+velocity.x+", "+velocity.y);
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (solid && blob !is null)
	{
		...
		//hurt
		this.server_Hit(blob, point1, hitvel, dmg*2, Hitters::boulder, true);
		this.server_Die();
