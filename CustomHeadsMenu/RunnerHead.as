...
bool hasCustomHead(CPlayer@ player)
{
	return player.get_bool("is_custom_head_set");
}

void onPlayerInfoChanged(CSprite@ this)
{
	CPlayer@ player = this.getBlob().getPlayer();
	if (player !is null && hasCustomHead(player))
		LoadCustomHead(this, player.get_u16("custom_head_index"));
	else
...
CSpriteLayer@ LoadCustomHead(CSprite@ this, int headIndex)
{
	this.RemoveSpriteLayer("head");
	// add head
	string texture_filename = getRules().get_string("custom_heads_filename");
	printf(texture_filename);
	CSpriteLayer@ head = this.addSpriteLayer("head",
						texture_filename, 16, 16,
						(this.getBlob().getTeamNum()),
						(this.getBlob().getSkinNum()));
	CBlob@ blob = this.getBlob();

	// set defaults
	s32 headFrame = headIndex * 4;
	if (head !is null)
	{
		Animation@ anim = head.addAnimation("default", 0, false);
		anim.AddFrame(headFrame);
...
void onGib(CSprite@ this)
{
	...
	{
		string texture_filename = getHeadTexture(blob.getHeadNum());
		int frame = blob.get_s32("head index");
		int framex = frame % FRAMES_WIDTH;
		int framey = frame / FRAMES_WIDTH;

		//Handling custom heads
		CPlayer@ player = this.getBlob().getPlayer();
		if (player !is null && player.exists("custom_head_index") && hasCustomHead(player))
		{
			texture_filename = getRules().get_string("custom_heads_filename");
			frame = player.get_u16("custom_head_index") * 4;
		}
		...
		makeGibParticle(texture_filename,
