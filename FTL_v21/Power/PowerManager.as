...
void onInit(CBlob @this){
	...
	this.set_u8("Zoltan_Shields",0);
	this.set_u8("Zoltan_Engines",0);
	this.set_u8("Zoltan_Oxygen",0);
	this.set_u8("Zoltan_Medical",0);
	this.set_u8("Zoltan_Cloning",0);
...
void UpdateSystemAmount(CBlob@ this){
	...
	for(int s = 0;s < system_Amount;s+=1)
	{
		...
		if(this.get_u8("Bars_"+systemNames[s]) < this.get_u8("Power_"+systemNames[s])+this.get_u8("Zoltan_"+systemNames[s]))ZoltanExcessPower += this.get_u8("Zoltan_"+systemNames[s]);
		...
		for (u32 k = 0; k < blobs.length; k++)
		{
			...
			for (u32 l = 0; l < rooms.length; l++)
			{
				...
				if(Maths::Sqrt(Maths::Pow(blob.getPosition().x-room.getPosition().x, 2)+Maths::Pow(blob.getPosition().y-room.getPosition().y, 2)) < 20){
					...
					if(room.get_f32("Power") > room.get_u8("Level"))room.set_f32("Power",room.get_u8("Level"));
