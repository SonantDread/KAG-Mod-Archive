...
void onInit(CSprite@ this)
{
	...
	if(this.getBlob().getName() == "engi")texname = "Engi.png";
	if(this.getBlob().getName() == "rock")
	{
		texname = "RockMan.png";
	}
	if(this.getBlob().getName() == "mantis")texname = "Mantis.png";
	if(this.getBlob().getName() == "zoltan")texname = "Zoltan.png";
	if(this.getBlob().getName() == "slug")texname = "Slug.png";
	
	this.ReloadSprite(modpath+texname);

	this.getCurrentScript().runFlags |= Script::tick_not_infire;
	
	CSpriteLayer@ heatsig = this.addSpriteLayer("heatsig", "SlugVision.png" , 16, 16, 0, 0);
	if (heatsig !is null)
	{
		heatsig.SetRelativeZ(1001);
		heatsig.SetVisible(false);
		heatsig.setRenderStyle(RenderStyle::additive);
	}
}
...
void onTick(CSprite@ this)
{
	...
	CSpriteLayer@ heatsig = this.getSpriteLayer("heatsig");
	if (heatsig !is null)
	{
		if(getLocalPlayer() !is null)
		if(getLocalPlayer().getBlob() !is null)
		if(getLocalPlayer().getBlob() !is blob){
			CBlob @local = getLocalPlayer().getBlob();
			if(local.getName() == "slug"){
				if(Maths::Sqrt(Maths::Pow(blob.getPosition().x-local.getPosition().x, 2)+Maths::Pow(blob.getPosition().y-local.getPosition().y, 2)) > 60){
					heatsig.SetVisible(true);
					heatsig.setRenderStyle(RenderStyle::additive);
					blob.SetLight(true);
					blob.SetLightRadius(8);
	...
	if (!blob.hasTag(burning_tag)) //give way to burning anim
	{
		...
		const bool inair = ((!blob.isOnGround() && !blob.isOnLadder()) || (blob.getShape().getGravityScale() <= 0));
		...
		else if (action2 || ((this.isAnimation("strike") || this.isAnimation("shoot")) && !this.isAnimationEnded()))
		{
			if(!blob.hasTag("alternate_mode")){
				this.SetAnimation("strike");
			} else {
				this.SetAnimation("shoot");
			}
		}
		else if (action1  || ((this.isAnimation("build") || this.isAnimation("repair")) && !this.isAnimationEnded()))
		{
			if(!blob.hasTag("alternate_mode")){
