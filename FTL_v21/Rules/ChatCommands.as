...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
	...
	if (sv_test)
	{
		...
		{
			blob.Tag("space_suit");
			blob.set_u16("air_tank",1500);
			if(getNet().isServer()){
				blob.Sync("space_suit",true);
				blob.Sync("air_tank",true);
			}
		}
		...
		else if (text_in.substr(0, 1) == "!")
		{
			...
			if (tokens.length > 1)
			{
				...
				{
					loadEvent(getMap(),"event"+tokens[1]+".png",1);
				}
				else if (tokens[0]  == "!weapon")
				{
					CBlob@ b = server_CreateBlob("weapon", team, pos);

					if (b !is null)
					{
						b.set_u16("type",parseInt(tokens[1]));
					}
				}
