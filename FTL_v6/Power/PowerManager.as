...
void AssignPower(CBlob@ this){

	{
		...
		for (u32 k = 0; k < blobs.length; k++)
		{
			CBlob@ blob = blobs[k];
			if(blob.getTeamNum() == this.getTeamNum()){
				blob.set_f32("Power",(this.get_u8("Power_Oxygen")*1.0f)/Amount*(blob.get_u8("Level")*1.0f));
			}
		}
	}
	
	{
		...
		for (u32 k = 0; k < blobs.length; k++)
		{
			CBlob@ blob = blobs[k];
			if(blob.getTeamNum() == this.getTeamNum()){
				blob.set_f32("Power",(this.get_u8("Power_Cloning")*1.0f)/Amount*(blob.get_u8("Level")*1.0f));
			}
		}
	}

	{
		...
		for (u32 k = 0; k < blobs.length; k++)
		{
			CBlob@ blob = blobs[k];
			if(blob.getTeamNum() == this.getTeamNum()){
				blob.set_f32("Power",(this.get_u8("Power_Engines")*1.0f)/Amount*(blob.get_u8("Level")*1.0f));
			}
		}
	}
	
	{
		...
		for (u32 k = 0; k < blobs.length; k++)
		{
			CBlob@ blob = blobs[k];
			if(blob.getTeamNum() == this.getTeamNum()){
				blob.set_f32("Power",(this.get_u8("Power_Shields")*1.0f)/Amount*(blob.get_u8("Level")*1.0f));
			}
		}
	}
}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("ftl_jump"))
	{
		...
		if(this.get_u16("FTLDrive") >= this.get_u16("FTLDriveMax")){
			this.set_u16("FTLDrive",0);
			if(isServer)this.Sync("FTLDrive",true);
			
			loadEvent(getMap(),"event"+XORRandom(2)+".png",1);
		}
	}
}
...
void onRender(CSprite@ this)
{
	...
	if(FTLCharge >= 1.0f)GUI::DrawIcon("JUMP.png", 0, Vec2f(96,48), Vec2f(getScreenWidth()/2-48*GUIScale,Y));
