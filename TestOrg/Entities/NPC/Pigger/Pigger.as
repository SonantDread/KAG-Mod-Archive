		...
		if (netid != 0)
		{
			...
			if (blob !is null && !blob.hasTag("dead"))
			{
				...
				if (this.getTickSinceCreated() % 15 == 0) 
				{
					...
					if (getNet().isServer()) this.server_Hit(blob, blob.getPosition(), Vec2f(0, 0), 0.05f, Hitters::bite, true);
				}
...
{
	if (blob !is null && getGameTime() >= this.get_u32("next attach") && this.get_u16("succ netid") == 0 && blob.hasTag("human") && !blob.hasTag("dead") && !this.hasTag("dead"))
	{
