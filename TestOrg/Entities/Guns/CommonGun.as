				...
				if(blobHit) 
				{
					...
					for(u32 i=0;i<hitInfos.length;i++) 
					{
						...
						{	
							...
							if((blob.isCollidable() || blob.hasTag("flesh") || force_nonsolid) && (!blob.hasTag("invincible") && blob.getTeamNum() != holder.getTeamNum() || blob.getTeamNum() != 255)) 
							{
								if (!blob.hasTag("isWeapon"))
								{
									...
									holder.server_Hit(blob, hitInfos[i].hitpos, dir, dmg, this.get_u8("gun_hitter"));
