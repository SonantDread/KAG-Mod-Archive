...
void renderJugernautHPBar(CBlob@ blob, Vec2f origin)
{
	...
	{		
		...
		// Health
		renderBar(Vec2f(192, 19), Vec2f(-128, 16), blob.getHealth() / initialHealth, SColor(0xffb73333), origin);

		// Nova timer
		{
			u32 startTime = blob.get_u32("nova time");
			u32 timeSinceLast = getGameTime() - startTime;
			f32 percent = timeSinceLast / blob.get_f32("nova delay");
			if(startTime == -1)
				percent = 1.0f;
			
			renderBar(Vec2f(192, 11), Vec2f(-128, 37), percent, SColor(0xff00A3BA), origin);
		}

		/*// Rocket timer
		{
			u32 startTime = blob.get_u32("rocket time");
			u32 timeSinceLast = getGameTime() - startTime;
			f32 percent = timeSinceLast / blob.get_f32("rocket delay");
			if(startTime == -1)
				percent = 1.0f;		
				
			renderBar(Vec2f(95, 11), Vec2f(-31, 37), percent, SColor(0xffFF8F00), origin);
		}*/
	}
}

void renderBar(Vec2f size, Vec2f offset, f32 percent, SColor colour, Vec2f origin)
{	
	GUI::DrawRectangle(Vec2f(offset.x + origin.x, offset.y + origin.y), Vec2f(offset.x + origin.x + size.x, offset.y + origin.y + size.y));
	
	if (percent >= 0.0f)
	{
		GUI::DrawRectangle(Vec2f(offset.x + origin.x + 4, offset.y + origin.y + 4), Vec2f(offset.x + origin.x + (size.x * percent) - 4, offset.y + origin.y + size.y - 4), colour);
	}
}
