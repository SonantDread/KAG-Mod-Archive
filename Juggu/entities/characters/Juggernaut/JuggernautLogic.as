...
void onInit(CBlob@ this)
{
	...
	this.set_u32("nova time", -1);
	this.set_f32("nova delay", 20.0f * getTicksASecond());
	this.addCommandID("nova");
	
	//this.set_u32("rocket time", -1);
	//this.set_f32("rocket delay", 10.0f * getTicksASecond());
}
...
void onTick(CBlob@ this)
{
	...
	if(this.get_u32("nova time") != -1)
	{
		u32 timeSinceLast = getGameTime() - this.get_u32("nova time");
		u32 useTime = this.get_f32("nova delay");
		if(timeSinceLast >= useTime)
		{
			this.set_u32("nova time", -1);
			Sound::Play("/switch.ogg", pos, 1.0f, 0.75f);
			addHoverMessage(this, "Nova", 1, SColor(0xffE649FF));
		}
	}
	
	/*if(this.get_u32("rocket time") != -1)
	{
		u32 timeSinceLast = getGameTime() - this.get_u32("rocket time");
		u32 useTime = this.get_f32("rocket delay");
		if(timeSinceLast >= useTime)
		{
			this.set_u32("rocket time", -1);
			Sound::Play("/switch.ogg");
			addHoverMessage(this, "Rocket", 1, SColor(0xffFF8F00));
		}
	}*/
	
	CSpriteLayer@ nova = sprite.getSpriteLayer("nova");
	Animation@ novaAnim = nova.getAnimation("default");
	if(novaAnim.ended())
	{
	...
	if(myplayer)
	{
		if(getControls().isKeyJustPressed(KEY_LSHIFT))
		{
			if(this.get_u32("nova time") == -1)
			{
				this.set_u32("nova time", getGameTime());
				
				CBitStream params;
				this.SendCommand(this.getCommandID("nova"), params);
			}
			else
			{
		...
		/*if(getControls().isKeyJustPressed(KEY_KEY_R)) // Rocket shooting stuff
		{	
			if(this.get_u32("rocket time") == -1)
			{			
				...
				if (arrow !is null)
				{
					...
					arrow.getShape().SetGravityScale(0.0f);
					print("" + arrow.getShape().getGravityScale());
					
					Vec2f vel;
					this.getAimDirection(vel);
					
					arrow.setVelocity(vel * 10.0f);
				}
				
				//this.TakeBlob("mat_bombarrows", 1);
				this.set_u32("rocket time", getGameTime());
			}
			else
			{
				Sound::Play("/NoAmmo.ogg");
...
void addHoverMessage(CBlob@ this, string name, int quantity, SColor colour)
{
	...
	HoverMessage m(name, quantity, colour);
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	...
	else if (cmd == this.getCommandID("nova"))
	{
		Vec2f pos = this.getPosition();
		...
		for(uint i = 0; i < blobs.length; i++)
		{
			...
			Vec2f vel = hit_blob.getPosition() - pos;
			vel.Normalize();
			hit_blob.AddForce(vel * 800.0f);
			
			SetKnocked(hit_blob, 40);
		}
		
		if(getNet().isClient())
		{
			Sound::Play("/WoodHeavyHit2.ogg", this.getPosition(), 2.0f);
			ShakeScreen2(20, 10, this.getPosition());
			
			//nova animation

			CSpriteLayer@ nova = this.getSprite().getSpriteLayer("nova");
			Animation@ novaAnim = nova.getAnimation("default");
			if (nova !is null && novaAnim !is null)
			{
				novaAnim.SetFrameIndex(0);
