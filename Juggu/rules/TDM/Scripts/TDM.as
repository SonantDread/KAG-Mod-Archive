...
shared class TDMSpawns : RespawnSystem
{
	...
	void DoSpawnPlayer(PlayerInfo@ p_info)
	{
		if (force || canSpawnPlayer(p_info))
		{
			...
			{
				if(p_info.blob_name == "juggernaut")
				{
					// if you're a juggernaut then become a knight
			...
			if (playerBlob !is null)
			{
				...
				if(blobName == "knight")
				{
					...
					playerBlob.server_PutInInventory(server_CreateBlob("mat_bombs"));
					playerBlob.server_PutInInventory(server_CreateBlob("mine"));
					playerBlob.server_PutInInventory(server_CreateBlob("food"));
				}
				else if(blobName == "archer")
				{
...
shared class TDMCore : RulesCore
{
	...
	{
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), "knight");
		players.push_back(p);
	...
	void CheckTeamWon()
	{
		...
		if (winteamIndex >= 0)
		{
			...
			rules.SetCurrentState(GAME_OVER);
			rules.SetGlobalMessage(winteam.name + " win the game!");
		}
	}
