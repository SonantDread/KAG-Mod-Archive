...
void onPlayerDie(CRules@ this, CPlayer@ victim, CPlayer@ killer, u8 customData)
{
	...
	if (victim !is null)
	{
		if (killer !is null)
		{
			...
			{
				killer.server_setCoins(killer.getCoins() + (coinsOnKillAdd * Maths::Pow(killstreakFactor, killer.get_u8("killstreak"))) * getMultiplier());
			}
		}
...
f32 onPlayerTakeDamage(CRules@ this, CPlayer@ victim, CPlayer@ attacker, f32 DamageScale)
{
	...
	if (attacker !is null && attacker !is victim && attacker.getTeamNum() != victim.getTeamNum())
	{
		...
		attacker.server_setCoins(attacker.getCoins() + (dmg * coinsOnDamageAdd / this.attackdamage_modifier) * getMultiplier());
	}
		...
		if (p !is null)
		{
			...
			switch (g.getType())
			{
...

				{
					...
					{
						coins = coinsOnBuild * getMultiplier();
					}
					else if (tile == CMap::tile_wood)
					{
						coins = coinsOnBuildWood * getMultiplier();
					}
				}
...

				{
					...
					{
						coins = coinsOnBuildWorkshop * getMultiplier();
					}
				}
...

				{
					...
					coins = (coinsOnHitSiege * damage) * getMultiplier();
				}

				break;

				case GE_kill_vehicle:
					coins = coinsOnKillSiege * getMultiplier();
					break;
