...
{
	//CBlob @spawnblob = server_CreateBlob("tradingpost", this.getTeamNum(), this.getPosition());
	//CBlob @pistol = server_CreateBlob("revolver", this.getTeamNum(), this.getPosition());
...
{
	if (this.getTickSinceCreated() > 20 && this.getTickSinceCreated() < 50)
	{
		CPlayer@ playa = this.getPlayer();
		if (playa is null && !this.hasTag("botti"))
		{
			this.getBrain().server_SetActive(true);
			this.Tag("botti");
			this.set_string("autograb blob", "ak");
		}
	}
...
{
	if(blob !is null && this !is null && !blob.isAttached())
 	{
		if (blob.getName()=="ak")
		{
			this.server_Pickup(blob);
