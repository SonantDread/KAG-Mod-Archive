...
namespace NecroParams
{
	...
	const ::Spell[] spells = {
		Spell("skeleton", "Skeleton", "$Skeleton$", SpellType::summoning, 1, 10, 0, true),
		Spell("lantern", "Lantern", "$Lantern$", SpellType::summoning, 1, 10, 0, true),
		Spell("zombie", "Zombie", "$Zombie$", SpellType::summoning, 4, 25, 30, true),
		Spell("wraith", "Wraith", "$Wraith$", SpellType::summoning, 10, 40, 30, true),
		Spell("greg", "Greg", "$Greg$", SpellType::summoning, 10, 40, 30, true),
...
shared class NecromancerInfo
{
	...
	NecromancerInfo()
	{
		...
		charge_state = 0;
		maxMana = 100;
		mana = (getGameTime() > 2500 ? 0 : maxMana);
		fletch_cooldown = 0;
		primarySpellID = 2;
		secondarySpellID = 6;
	}
