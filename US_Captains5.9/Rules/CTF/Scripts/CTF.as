...
{
	string configstr = CFileMatcher("captain_vars.cfg").getFirst();
		...
		if (victim !is null)
		{
			if (killer !is null && killer.getTeamNum() != victim.getTeamNum())
			{
				if(getRules().hasTag("Official")){
					print("Kills: " + killer.getUsername() + " " + "Deaths " + victim.getUsername());
				}
			}
		}
		...
		if (winteamIndex >= 0)
		{
			...
			if (rules.isGameOver()){
				if (!getRules().hasTag("Official")){
					...
					string[] deleter;
					print('tagged');
					rules.SetGlobalMessage("{WINNING_MSG} to victory");
					string WinningCaptain = getRules().get_string("captain" + " " + (winteamIndex == 1 ? "red" : "blue"));
					string LosingCaptain = getRules().get_string("captain" + " " + (winteamIndex == 0 ? "red" : "blue"));
					string[]@ WinningTeam;
					string[]@ LosingTeam;
					getRules().get((winteamIndex == 1 ? "reds" : "blues"), @WinningTeam); 
					getRules().get((winteamIndex == 0 ? "reds" : "blues"), @LosingTeam); 
					rules.AddGlobalMessageReplacement("WINNING_MSG", WinningCaptain + " lead " + winteam.name);
					tcpr("[USCaptains] Win: " + Flatten(WinningTeam));
					tcpr("[USCaptains] Loss: " + Flatten(LosingTeam));
					tcpr("[USCaptains] Winning_Captain: " + WinningCaptain);
					tcpr("[USCaptains] Losing_Captain: " + LosingCaptain);
					tcpr("[Update]: " + Flatten(WinningTeam) + " " + Flatten(LosingTeam));
					getRules().set_string("captain blue", "");
		    		getRules().set_string("captain red", "");
					getRules().Untag("Official");
					getRules().set("reds", @deleter);
					getRules().set("blues", @deleter);
