...
{
	string configstr = "Rules/survival/survival_vars.cfg";
	...
	this.minimum_players_in_team = cfg.read_s32("100", 2);
...
shared class CTFCore : RulesCore
{
	...
	void Update()
	{
		...
		if ((rules.isIntermission() || rules.isWarmup()) && (!allTeamsHavePlayers()))  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
			...
			rules.SetGlobalMessage("Welcome to Oblivion!.\nThanks, Aphelion foe your help with the mod!");
			ctf_spawns.force = true;
		}
	...
	void SetupBases()
	{
		...
		if (map !is null)
		{
            ...
            {
                warn("l: blue spawn added");
                respawnPos = Vec2f(1200.0f, map.getLandYAtX(100.0f/map.tilesize)*map.tilesize - 45.0f);
            }
            ...
            {
                warn("l: red spawn added");
                respawnPos = Vec2f(675.0f, map.getLandYAtX(100.0f/map.tilesize)*map.tilesize - 100.0f);
            }
            ...
            {
                warn("l: green spawn added");
                respawnPos = Vec2f(110.0f, map.getLandYAtX(100.0f/map.tilesize)*map.tilesize - 6.0f);
            }
            ...
            {
                warn("l: purple spawn added");
                respawnPos = Vec2f(1010.0f, map.getLandYAtX(100.0f/map.tilesize)*map.tilesize - -110.0f);
            }
