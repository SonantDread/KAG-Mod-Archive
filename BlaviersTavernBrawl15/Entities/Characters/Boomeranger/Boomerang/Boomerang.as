...
void onInit(CBlob@ this)
{
	...
	consts.mapCollisions = false; //turn off?
...
void onTick(CBlob@ this)
{
			...
			if (b !is null)
			{
				...
				if (b.isKeyPressed(key_action2))
				{
					...
					if ((this.getDistanceTo(b) < 12.0f) && (getKnocked(b) <= 0))
					{
						...
						b.AddForce(Vec2f((b.isFacingLeft() ? -1 : 1)*220.0f, -400.0f));
						SetKnocked(b, 3);
					}
				}
				else
	...
	// Delete if on edge of map (slowly so its not unfair)
	{
		Vec2f pos = this.getPosition();
		if (pos.x < 0.1f || pos.x > (getMap().tilemapwidth * getMap().tilesize) - 0.1f)
		{
			this.server_Hit(this, this.getPosition(), Vec2f(), 0.03f, Hitters::crush);
			return;
		}
	}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	if (blob !is null && doesCollideWithBlob(this, blob) && !this.hasTag("collided"))
	{
		...
		if (blob.getTeamNum() != this.getTeamNum())
		{
			...
			if ((this.getTickSinceCreated() >= 6) && (this.getTickSinceCreated() < 11))
			{
				dmg = 0.5f;
			}
			if ((this.getTickSinceCreated() < 22) && (this.getTickSinceCreated() >= 11))
			{
				dmg = 0.75f;
			}
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		if (speed > ArcherParams::shoot_max_vel * 0.5f)
		{
			...
			if (this.getTickSinceCreated() < 22 && hitBlob.hasTag("player"))
			{
