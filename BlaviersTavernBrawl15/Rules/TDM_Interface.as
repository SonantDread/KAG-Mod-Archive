...
void onRender(CRules@ this)
{
	...
	if (serialised_team_hud.getBytesUsed() > 10)
	{
		...
		if (serialised_team_hud.saferead_u16(check) && check == 0x5afe)
		{
			...
			while (!serialised_team_hud.isBufferEnd())
			{
				...
				string kills = getTranslatedString("WARMUP TIME");
				...
				{
					kills = getTranslatedString("TAVERN BRAWL");
				}
	...
	if (p.getBlob() is null && this.exists(propname))
	{
		...
		if (spawn != 255)
		{
			...
			{
				GUI::DrawText(getTranslatedString("Waiting for the round to end.") , Vec2f(getScreenWidth() / 2 - 180, getScreenHeight() / 3 + Maths::Sin(getGameTime() / 3.0f) * 5.0f), SColor(255, 255, 255, 55));
			}
			else
			{
