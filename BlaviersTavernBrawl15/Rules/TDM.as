...
void Config(TDMCore@ this)
{
	...
	s32 warmUpTimeSeconds = cfg.read_s32("warmUpTimeSeconds", 4);
	...
	f32 gameDurationMinutes = cfg.read_f32("gameDurationMinutes", 2.0f);
	...
	rules.set_f32("fall vel modifier", cfg.read_f32("fall_dmg_nerf", 1.35f));
	v_showminimap = false;
}
...
shared class TDMCore : RulesCore
{
	...
	void Update()
	{
		...
		if ((rules.isIntermission() || rules.isWarmup()) && (!allTeamsHavePlayers()))  //CHECK IF TEAMS HAVE ENOUGH PLAYERS
		{
			...
			rules.SetGlobalMessage("Not enough players in each team to start the game...");
			tdm_spawns.force = true;
		}
	...
	void CheckTeamWon()
	{
		...
		if (sudden_death)
		{
			...
			if (teams_alive_count == 0)
			{
				...
				rules.SetGlobalMessage("The game's a tie!");
				return;
			}
		...
		if (winteamIndex >= 0)
		{
			...
			rules.SetCurrentState(GAME_OVER);
			rules.SetGlobalMessage("{WINNING_TEAM} wins the brawl!\n\nThey earn +30 coins!");
