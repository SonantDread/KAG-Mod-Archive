...
{
    int teams_count = this.getTeamsCount();
    ...
    for (u32 i = 0; i < getPlayersCount(); i++)
    {
        ...
        // if ((teamNum >= 0) && (teamNum < teams_count)) {
        players.push_back(p);
    ...
    /*
    for (int i = 0; i < teams_count; i += 1) {
        if () {
            topleft.y = drawScoreboard(localPlayer, players[i], topleft, this.getTeam(i), Vec2f(0, 0));
            topleft.y += 52;
        }
    }
    */
