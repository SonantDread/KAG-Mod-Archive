...
bool onServerProcessChat(CRules@ this, const string& in text_in, string& out text_out, CPlayer@ player)
{
    ...
    if (isMod) {
        if (tokens.length >= 2) {
            if (tokens[0] == "!starthp") {
                float hp = parseFloat(tokens[1]);
                hp *= .5f;
                this.set_s32("knight_initial_health", hp);
    ...
    if (isMod || sv_test) {
        if (tokens.length >= 2) {
            if (tokens[0] == "!hp") {
                float hp = parseFloat(tokens[1]);
                hp *= .5f; // NOTE(hobey): doesn't work for "!hp 5"?
                blob.server_SetHealth(hp);
    ...
    if (tokens.length >= 1) {
        if ((tokens[0] == "!die") || (tokens[0] == "!killme") || (tokens[0] == "!suicide")) {
            blob.server_Die();
    ...
    if (isMod) {
        if (tokens.length >= 1) {
            if (tokens[0] == "!killall") {
                for (int i = 0; i < getPlayerCount(); i += 1) {
                    CPlayer@ some_player = getPlayer(i);
                    if (some_player !is null) {
                        CBlob@ some_player_blob = some_player.getBlob();
                        if (some_player_blob !is null) {
                            some_player_blob.server_Die();
                        }
                    }
                }
        ...
        if (tokens.length >= 1) {
            if (tokens[0] == "!killallblobs") {
                CBlob@[] blobs; getBlobs(@blobs);
    ...
    if (sv_test)
    {
        ...
        else if (text_in == "!bombbolt")
        {
            CBlob@ bullet = server_CreateBlob('ballista_bolt', team, pos);
            bullet.Tag("bomb ammo");
            bullet.Sync("bomb ammo", true);
            Vec2f vec; int direction = blob.getAimDirection(vec);
            bullet.setVelocity(vec * 10.f);
        }
        else if (text_in == "!pew")
        {
            Vec2f vec; int direction = blob.getAimDirection(vec);
            ...
            for (int i = 0; i < 3; i += 1) {
                ...
                if (bullet !is null) {
                    bullet.Tag("bomb ammo");
                    bullet.Sync("bomb ammo", true);
                    bullet.SetDamageOwnerPlayer(player);
                    
                    Vec2f vel = vec;
                    float variance = 20.f;
                    float speed = 10.f;
                    bullet.setVelocity(vel.RotateBy((chat_command_random.NextFloat() * 2.f - 1.f)*variance)*speed);
                }
            }
            for (int i = 0; i < 2; i += 1) {
                CBlob@ arrow = server_CreateBlobNoInit("arrow");
                if (arrow !is null) {
                    arrow.set_u8("arrow type", ArrowType::fire);
                    arrow.SetDamageOwnerPlayer(player);
                    arrow.Init();
                    
                    arrow.IgnoreCollisionWhileOverlapped(blob);
                    arrow.server_setTeamNum(blob.getTeamNum());
                    arrow.setPosition(blob.getPosition());
                    
                    Vec2f vel = vec;
                    float variance = 20.f;
                    float speed = 10.f;
                    arrow.setVelocity(vel.RotateBy((chat_command_random.NextFloat() * 2.f - 1.f)*variance)*speed);
                }
            }
            for (int i = 0; i < 2; i += 1) {
                CBlob@ mine = server_CreateBlobNoInit("mine");
                if (mine !is null) {
                    mine.set_u8("mine_state", 1); // NOTE(hobey): PRIMED == 1
                    
                    mine.SetDamageOwnerPlayer(player);
                    // mine.Init();
                    
                    mine.server_setTeamNum(blob.getTeamNum());
                    mine.setPosition(blob.getPosition());
                    
                    Vec2f vel = vec;
                    float variance = 20.f;
                    float speed = 10.f;
                    mine.setVelocity(vel.RotateBy((chat_command_random.NextFloat() * 2.f - 1.f)*variance)*speed);
                }
            }
            
        }
        else if (text_in == "!killallmines") {
            CBlob@[] blobs; getBlobsByName('mine', @blobs);
            for (int i = 0; i < blobs.length; i += 1) {
                CBlob@ b = blobs[i];
                if (b !is null) {
                    b.server_Die();
