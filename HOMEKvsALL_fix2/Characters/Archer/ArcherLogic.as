...
{
	AddIconToken("$HomingArrow$", "Entities/Characters/Archer/ArcherIcons.png", Vec2f(16, 32), 4);
	AddIconToken("$RocketArrow$", "Entities/Characters/Archer/ArcherIcons.png", Vec2f(16, 32), 5);
	...
	if (archer.grappling)
	{
		...
		else
		{
			...
			else //stuck -> pull towards pos
			{
				...
				if (b !is null && !b.hasTag("can grapple"))
...
bool checkGrappleStep(CBlob@ this, ArcherInfo@ archer, CMap@ map, const f32 dist)
{
	...
	else
	{
		...
		if (b !is null)
		{
			...
			else if (b.hasTag("can grapple") ||b.isCollidable() && b.getShape().isStatic() && !b.hasTag("ignore_arrow"))
			{
	...
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		if (legolas)
		{
			...
			for (int i = 0; i < ArcherParams::legolas_arrows_volley; i++)
			{
				if (getNet().isServer())
				{
					for (int k = 0; k < ((arrowType == ArrowType::rocket) ? ArcherParams::rocket_arrow_multishot : 1); k++)
					{
						CBlob@ arrow = CreateArrow(this, arrowPos, arrowVel, arrowType);
						this.TakeBlob(arrowTypeNames[ arrowType ], (arrowType == ArrowType::rocket) ? ArcherParams::rocket_arrow_multishot : 1);
		...
		else
		{
			for (int k = 0; k < ((arrowType == ArrowType::rocket) ? ArcherParams::rocket_arrow_multishot : 1); k++)
			{
				...
				if (k == 0) this.getSprite().PlaySound("Entities/Characters/Archer/BowFire.ogg");
