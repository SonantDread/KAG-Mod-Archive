...
void onTick(CMovement@ this)
{
	...
	if (!blob.isInWater() && !blob.isOnGround() && blob.get_bool("flight") == true && getGameTime() < blob.get_u32("flight duration"))
	{
		if (up)
		{
			blob.setVelocity(Vec2f(blob.getVelocity().x, -3));
		}
	}
	        ...
	        !blob.isOnLadder() && (up || left || right || down))  //key pressed
	{
		        ...
		        !onground)
		{
			...
			bool infinite_wallrun = (blob.get_bool("infinite wallrun") == true && getGameTime() < blob.get_u32("infinite wallrun duration"));
			...
			if (moveVars.jumpCount > 5) //wait some time to be properly in the air
			{
				...
				{
					//within range
					if (set_contact || infinite_wallrun ||
