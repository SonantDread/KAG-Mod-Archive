const string STATS_DIR = "HomekVsAllStats/";
...
class Stats
{
    ...
    u32 m_coins;
 	...
 	Stats(string username, int teamnum)
    {
    	ConfigFile file;

        string playerc_team = ((teamnum == 0) ? "blue" : "red"); 
    
    	if(file.loadFile("../Cache/" + STATS_DIR + username + "~" + playerc_team)) 
    	{ 
            ...
            this.m_coins = file.read_u32("coins");
  		}
    }
    ...
    // without wins/losses
    Stats(u32 a, u32 b, u32 e, u32 f, u32 g, u32 h, u32 i, u32 j, u32 coins, string username)
    {
    	this.m_kills = a;
    	this.m_deaths = b;
    	this.m_username = username;
    ...
    Stats(CBitStream@ params)
    {
        ...
        u32 temp_coins;
        if (!params.saferead_u32(temp_coins)) printf("somethings not quite right");
    	...
    	if(file.loadFile("../Cache/" + STATS_DIR + "match" + count)) 
    	{
	    	...
	    	if(blue_players.size() > 0)
	    	{
	    		for(int i = 0; i < blue_players.size(); ++i)
	    		{
	    			...
	    			u32 p_kills = file.read_u32(p_user + "_kills~blue");
	    			u32 p_deaths = file.read_u32(p_user + "_deaths~blue");

	    			this.m_blue_kills += file.read_u32(p_user + "_kills~blue");
	    			this.m_blue_deaths += file.read_u32(p_user + "_deaths~blue");

	    			u32 p_k_kills = file.read_u32(p_user+ "_k_kills~blue");
	    			u32 p_k_deaths = file.read_u32(p_user + "_k_deaths~blue");

	    			u32 p_b_kills = file.read_u32(p_user+ "_b_kills~blue");
	    			u32 p_b_deaths = file.read_u32(p_user + "_b_deaths~blue");

	    			u32 p_a_kills = file.read_u32(p_user + "_a_kills~blue");
	    			u32 p_a_deaths = file.read_u32(p_user + "_a_deaths~blue");

                    u32 p_coins = file.read_u32(p_user + "_coins~blue");

	    			Stats@ temporary = Stats(p_kills, p_deaths, p_k_kills, p_k_deaths, p_b_kills, p_b_deaths, p_a_kills, p_a_deaths, p_coins, p_user);
	    	...
	    	if(red_players.size() > 0)
	    	{
	    		for(int i = 0; i < red_players.size(); ++i)
	    		{
	    			...
	    			u32 p_kills = file.read_u32(p_user + "_kills~red");
	    			u32 p_deaths = file.read_u32(p_user + "_deaths~red");

	    			this.m_red_kills += file.read_u32(p_user + "_kills~red");
	    			this.m_red_deaths += file.read_u32(p_user + "_deaths~red");

	    			u32 p_k_kills = file.read_u32(p_user + "_k_kills~red");
	    			u32 p_k_deaths = file.read_u32(p_user + "_k_deaths~red");

	    			u32 p_b_kills = file.read_u32(p_user+ "_b_kills~red");
	    			u32 p_b_deaths = file.read_u32(p_user + "_b_deaths~red");

	    			u32 p_a_kills = file.read_u32(p_user + "_a_kills~red");
	    			u32 p_a_deaths = file.read_u32(p_user + "_a_deaths~red");

                    u32 p_coins = file.read_u32(p_user + "_coins~red");
