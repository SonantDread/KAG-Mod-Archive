...
f32 getPriorityPickupScale(CBlob@ this, CBlob@ b)
{
	...
	//// MILITARY ////
	{
		...
		if (name == "mine" || name == "bomb" || name == "waterbomb" || name == "clustercharge")
		{
		...
		if (b.hasScript("KegVoodoo.as") || b.hasScript("BouncyKegVoodoo.as"))
		{
			return exploding ? factor_very_important : factor_military_important;
		}

		// Regular military stuff
		if (name == "boulder" || name == "saw" || name == "leviathancannon")
		{
	...
	if (material)
	{
		...
		if (name == "mat_bombs" || name == "mat_waterbombs" || name == "mat_teleportbombs")
		{
...
CBlob@ getClosestAimedBlob(CBlob@ this, CBlob@[] available)
{
	...
	for (int i = 0; i < available.length; ++i)
	{
		...
		if (radius > 3.0f && cursorDistance > current.getRadius() * (current.hasTag("dead") ? 0.5f : 1.5f))
		{
			continue;
		}
...
CBlob@ getClosestBlob(CBlob@ this)
{
	...
	if (this.get("pickup blobs", @pickupBlobs))
	{
		...
		for (uint i = 0; i < available.length; ++i)
		{
			...
			// which makes its priority skyrocket if they're standing too close 
