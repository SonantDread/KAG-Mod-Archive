	...
	f32 speedmin = this.get_f32("gun_speedmin");
	 ...
	 for (int i = 0; i < 1; ++i) // tweak for the shotgun effect by Asu
	 {
       ...
       if (bullet !is null)
       {
		 ...
		 //this.server_Hit(holder, holder.getPosition(), Vec2f(0,0), 0.5f, Hitters::fall, true); - each shoot take 0.5 heart no I don't think it would work in this form, need separate for each class
			if (this.hasTag("reloading"))
			{
			if (this.getName() == "leviathancannon")
			{
				this.server_Hit(this, this.getPosition(), Vec2f(0,0), 0.75f, Hitters::fire, true);
