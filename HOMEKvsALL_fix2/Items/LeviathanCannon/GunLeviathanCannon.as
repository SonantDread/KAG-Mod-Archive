void onInit(CBlob@ this)
{
	...
	string config_filename = this.get_string("gunleviathancannon_config");
	if(config_filename == "") {
	...
	//reloading
	this.set_u16("gun_clip_size", cfg.read_u16("gun_clip_size", 10));
	...
	this.set_f32("gun_speedmin", cfg.read_f32("gun_speedmin", 15.0f)); // was 25.0f
	this.set_f32("gun_speedmax", cfg.read_f32("gun_speedmax", 16.0f)); // was 25.0f apparently the cfg doesn't work for the speed? - nein, it was because of line 195
...
void onTick(CBlob@ this)
{
	if (this.isAttached())
	{
		...
		// this.set_f32("aim", angle);

		AimAtMouse(this, holder);
		// reset frame (set by fire event)
		if (sprite.isAnimationEnded()) sprite.SetAnimation("default");
		...
		if (holder.isMyPlayer())
		{
			...
			if (hud.hasMenus())
			{
				// don't take keys if players has menus open
				this.getAttachments().getAttachmentPointByName("PICKUP").SetKeysToTake(0);
				return;
			}
			else
			{
				AttachmentPoint@ att = this.getAttachments().getAttachmentPointByName("PICKUP");
				if (att.getKeysToTake() == 0 && this.get_u16("gun_fire_delay_timer") < 5)
				{
					// add cooldown so player doesn't shoot himself right after menu closes
					this.set_u16("gun_fire_delay_timer", 5);
				}
	...
	if (cmd == this.getCommandID("shoot"))
   	{
		...
		Vec2f aimpos = holder.getAimPos();
		Vec2f force = Vec2f(1, 0);
		force = force.RotateByDegrees(angle);

		//take a bullet
		TakeBullet(this);
		if (this.hasTag("dying"))
		{
			// if out of ammo, throw the gun out of player's hands
			force.Normalize();
			force *= this.getMass() * 4 * 1.5;
			this.AddForce(-force);
		}
		
		// apply recoil to the holder
		force.Normalize();
		force *= holder.getMass() * 3 * 2;
		...
		//effects before we try to create anything
		{
			...
			sprite.SetAnimation("post_shoot");
			// pull back
		...
		for (int i = 0; i < 1; ++i) // tweak for the shotgun effect by Asu
		{
			...
			CBlob@ bullet = server_CreateBlobNoInit("leviathanshell");
			if (bullet !is null)
			{
				float cangle = angle;
				
				Vec2f ray = aimpos - pos;
				ray.Normalize();
				ray *= 16;
				Vec2f rayEnd;
				bool rcast = rayCheck(pos, pos + ray, rayEnd);
				bullet.setPosition(rayEnd);
				...
				bullet.getShape().SetGravityScale(this.get_f32("gun_bullet_gravity"));
			}
		}
	}
}
...
void AimAtMouse(CBlob@ this, CBlob@ holder)
{
	...
	if (!holder.isFacingLeft() && !this.isFacingLeft()) mouseAngle += 180;
