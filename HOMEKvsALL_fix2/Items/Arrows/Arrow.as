...
void onInit(CBlob@ this)
{
	...
	if (arrowType == ArrowType::rocket)
	{
		this.getShape().SetGravityScale(0.0f);
		this.getShape().setDrag(0.0f);
		this.server_SetTimeToDie(5.0f);
		SetupBomb(this, bomb_fuse, 40.0f, 1.0f, 24.0f, 0.4f, true);
		this.set_u8("custom_hitter", Hitters::bomb_arrow);

		this.set_f32("target_velocity", 4.0f + XORRandom(40) / 10.0f);
		this.set_f32("rotation_amount", 0.0f + XORRandom(69) / 10.0f);
		this.set_u8("rotation_flip_interval", 20 + XORRandom(15));
		this.set_bool("flipped_rotation", false);
	}
	...
	if (arrowType == ArrowType::homing)
	{
		...
		this.getShape().setDrag(0.0f);
		this.server_SetTimeToDie(5.0f);
		this.set_f32("homing_range", 192.0f);
	}
	...
	{
		Animation@ anim = sprite.addAnimation("homing arrow", 0, false);
		anim.AddFrame(16);
		if (arrowType == ArrowType::homing)
			sprite.SetAnimation(anim);
	}
	...
	{
		Animation@ anim = sprite.addAnimation("rocket arrow", 0, false);
		anim.AddFrame(17);
		if (arrowType == ArrowType::rocket)
			sprite.SetAnimation(anim);
	}
...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		if (arrowType == ArrowType::homing)
		{
			...
			if (getMap().getHitInfosFromArc(this.getPosition(), -this.getVelocity().getAngleDegrees(), 180.0f, this.get_f32("homing_range"), this, h))
			{
				for (int i = 0; i < h.size(); i++)
				{
					...
					if (blob !is null && blob.hasTag("player") && blob.getTeamNum() != this.getTeamNum())
					{
						...
						Vec2f dist = blob.getPosition() - this.getPosition();
						if (vel.Length() < 8.0f)
						{
							dist.Normalize();
							dist *= 0.5f;
						...
						vel.RotateByDegrees(vel.AngleWith(dist) / 2.3f);
		...
		{
			Vec2f vel = this.getVelocity();
			f32 target_vel = this.get_f32("target_velocity");
			vel.Normalize();
			vel *= target_vel;
			Vec2f force = vel;

			f32 rotation = this.get_f32("rotation_amount") + XORRandom(20) / 10.0f;
			u8 flip_interval = this.get_u8("rotation_flip_interval");
			bool flip = this.get_bool("flipped_rotation");

			if (getGameTime() % flip_interval == 0)
			{
				this.set_bool("flipped_rotation", !flip);
				flip = !flip;
			}

			vel.RotateByDegrees(flip ? -rotation : rotation);
			this.setVelocity(vel);
			this.AddForce(force * this.getMass());
		}
		...
		{
			if (arrowType != ArrowType::homing && arrowType != ArrowType::rocket)
			{
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	{
		// print("collided with " + blob.getName());
		const u8 arrowType = this.get_u8("arrow type");
		...
		if (arrowType == ArrowType::water || arrowType == ArrowType::bomb || arrowType == ArrowType::rocket)
		{
		...
		else if (arrowType == ArrowType::rocket)
		{
			dmg = 1.0f;
		}
		else
		...
		if (dmg > 0.0f)
		{
			...
			const u8 hit_type =
				...
				(arrowType == ArrowType::bomb || arrowType == ArrowType::rocket) ? Hitters::bomb_arrow :
				Hitters::arrow;
		...
		if (arrowType == ArrowType::bomb || arrowType == ArrowType::rocket)
		{
			if (!this.hasTag("dead"))
			{
...
f32 ArrowHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType)
{
	if (hitBlob !is null)
	{
		...
		else if (arrowType != ArrowType::normal || arrowType != ArrowType::homing)
		{
			damage = 0.0f;
		}
...
{
	// print("arrow died on " + getGameTime());
	if (getNet().isClient())
	{
		Vec2f pos = this.getPosition();
		if (pos.x >= 1 && pos.y >= 1)
...
{
	if (this.get_u8("arrow type") == ArrowType::homing)
	{
		return 1.5f;
	}
