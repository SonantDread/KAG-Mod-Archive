...

		{
			...
			else //mode == idle
			{
				if (personality != 0) //we have a special personality
				{
					...
					CBlob@ low_priority_target = @null;
					bool found_better_target = false;
					...
					for (uint step = 0; step < blobs.length; ++step)
					{
						...
						if (personality & AGGRO_BIT != 0)  //aggressive
						{
							...
							{
								if (other.getName() == "chicken" || other.hasTag("dead") && low_priority_target is null)
								{
									@low_priority_target = @other;
									continue;
								}
								mode = MODE_TARGET;
								blob.set_netid(target_property, other.getNetworkID());
								this.SetTarget(low_priority_target);
								found_better_target = true;
								break;
							}
						}
					}

					if (!found_better_target && low_priority_target !is null)
					{
						blob.set_netid(target_property, low_priority_target.getNetworkID());
						this.SetTarget(low_priority_target);
					}
				}
