...
class Piston : Component
{
	...
	Piston(Vec2f position, u16 netID, Vec2f _offset)
	{
		x = position.x;
		y = position.y;
	...
	void Activate(CBlob@ this)
	{
		...
		CBlob@ PistonHead = mechanism.getOccupied();
		if(PistonHead is null) return;
	...
	void Deactivate(CBlob@ this)
	{
		...
		if(PistonHead.get_u8("state") != 0)
			sprite.PlaySound("In.ogg");

		PistonHead.set_u8("state", 0);
...
void onSetStatic(CBlob@ this, const bool isStatic)
{
	...
	Piston component(position, this.getNetworkID(), offset);
	...
	if(getNet().isServer())
	{
		...
		CBlob@ PistonHead = server_CreateBlob("pistonhead", this.getTeamNum(), this.getPosition());
		PistonHead.setAngleDegrees(this.getAngleDegrees());
		PistonHead.set_u8("state", 0);

		ShapeConsts@ consts = PistonHead.getShape().getConsts();
		consts.mapCollisions = false;
		consts.collideWhenAttached = true;

		this.server_AttachTo(PistonHead, "MECHANISM");
	}
	...
	CSpriteLayer@ layer = sprite.addSpriteLayer("background", "Piston.png", 8, 16);
...
void onDie(CBlob@ this)
{
	...
	CBlob@ PistonHead = this.getAttachments().getAttachmentPointByName("MECHANISM").getOccupied();
	if(PistonHead is null) return;

	PistonHead.server_Die();
}