			...
			if (getNet().isServer())
			{
				...
				if (caller !is null && canChangeClass(this, caller))
				{
					...
					if (newBlob !is null)
					{
						...
						// Transfer Superpowers - new
						// preferrably make a custom getPower(blob) function later on to shorten this bit of the file massively
						// No else ifs due to that resulting in only transfering 1 power over even if the player had more than 1 before
						if (hasPower(caller, Powers::BOUNCE))
						{
							givePower(newBlob, Powers::BOUNCE);
						}
						if (hasPower(caller, Powers::KNOCKBACK))
						{
							givePower(newBlob, Powers::KNOCKBACK);
						}
						if (hasPower(caller, Powers::STRENGTH))
						{
							givePower(newBlob, Powers::STRENGTH);
						}
						if (hasPower(caller, Powers::VAMPIRISM))
						{
							givePower(newBlob, Powers::VAMPIRISM);
						}
						if (hasPower(caller, Powers::DRAIN))
						{
							givePower(newBlob, Powers::DRAIN);
						}
						if (hasPower(caller, Powers::SPEED))
						{
							givePower(newBlob, Powers::SPEED);
						}
						if (hasPower(caller, Powers::QUICK_ATTACK))
						{
							givePower(newBlob, Powers::QUICK_ATTACK);
						}
						//if (hasPower(caller, Powers::MOUNTAIN))
						//{
						//	givePower(newBlob, 7);
						//}
						if (hasPower(caller, Powers::FLIGHT))
						{
							givePower(newBlob, Powers::FLIGHT);
						}
						if (hasPower(caller, Powers::TRIPLE_JUMP))
						{
							givePower(newBlob, Powers::TRIPLE_JUMP);
						}
						if (hasPower(caller, Powers::MONKEY))
						{
							givePower(newBlob, Powers::MONKEY);
						}
						//if (hasPower(caller, Powers::MIDAS))
						//{
					//		givePower(newBlob, 11);
					//	}
						if (hasPower(caller, Powers::FIRE_LORD))
						{
							givePower(newBlob, Powers::FIRE_LORD);
						}
						if (hasPower(caller, Powers::FORCE))
						{
							givePower(newBlob, Powers::FORCE);
						}
						if (hasPower(caller, Powers::GHOST))
						{
							givePower(newBlob, Powers::GHOST);
						}
						if (hasPower(caller, Powers::TELEPORT))
						{
							givePower(newBlob, Powers::TELEPORT);
						}
						if (hasPower(caller, Powers::HEALER))
						{
							givePower(newBlob, Powers::HEALER);
						}
