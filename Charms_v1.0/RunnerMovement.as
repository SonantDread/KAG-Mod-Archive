...
void onTick(CMovement@ this)
{
	...
	if(blob.getPlayer() !is null && blob.getShape() !is null)
	{
		if (getRules().get_bool("lightercharm_" + blob.getPlayer().getUsername()))
		{
			blob.getShape().SetGravityScale(0.85f);
		}
	}

	if (blob.hasTag("act 3'd")) 
	{
	...
	if (blob.hasTag("summoning arrows"))
	{
		shape.SetGravityScale(1.5f);
		moveVars.walkFactor *= 0.5f;
	}

	if(blob.getPlayer() !is null && getRules() !is null)
	{
		CRules@ rules = getRules();

		string username = blob.getPlayer().getUsername();

		if(rules.get_bool("speedonkillcharm_" + username))
		{
			if (getGameTime() - rules.get_u32("killtime_" + username) < (5 * 30) && rules.get_u32("killtime_" + username) > 30)
			{
		...
		/*if(rules.get_bool("speedonkillcharm_" + username))
		{
			if (getGameTime() < rules.get_u32("killtime_" + username) || !rules.exists("killtime_" + username))
			{
				rules.set_u32("killtime_" + username, getGameTime());
				rules.Sync("killtime_" + username, true);
				rules.set_bool("speed_" + username, false);
				rules.Sync("speed_" + username, true);
			}

			else if (getGameTime() - rules.get_u32("killtime_" + username) < (5 * 30) && rules.get_u32("killtime_" + username) > 30)
			{
				if (rules.get_bool("speed_" + username) == false)
				{
				...
				rules.set_bool("speed_" + username, true);
				rules.Sync("speed_" + username, true);
			}
			else if (getGameTime() - rules.get_u32("killtime_" + username) > (5 * 30))
			{
				if (rules.get_bool("speed_" + username) == true)
				{
					moveVars.walkSpeed /= 1.5f;
					moveVars.walkSpeedInAir /= 1.5f;
					moveVars.swimspeed /= 1.5f;
					...
					if(blob !is null && blob.getPlayer() !is null)
					{
						if (set_contact ||
						        ((pos.y - 1.0f < moveVars.wallrun_current &&
						         pos.y + 1.0f > moveVars.wallrun_start - map.tilesize * moveVars.wallrun_length) || getRules().get_bool("infinitewallruncharm_"+blob.getPlayer().getUsername())))
						{
