...
void onNewPlayerJoin(CRules@ this, CPlayer@ player )
{
	...
	if (blob.get("playercharms", @charms))
	{
		for (uint i = 0 ; i < charms.length; i++)
		{
			...
			string charm_user = currentcharm.configFilename + "_" + player.getUsername();

			string charm_user_slots = "charmslots_" + player.getUsername();

			this.set_bool(charm_user, false);
			this.Sync(charm_user, true);

			this.set_u8(charm_user_slots, 10);
			this.Sync(charm_user_slots, true);

			this.set_u8("iterator_" + player.getUsername(), 0);
			this.Sync("iterator_" + player.getUsername(), true);
		}
	}
}
