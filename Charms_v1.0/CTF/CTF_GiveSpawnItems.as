...
bool GiveSpawnResources(CRules@ this, CBlob@ blob, CPlayer@ player, CTFPlayerInfo@ info)
{
	...
	if (blob.getName() == "builder" || blob.getName() == "builder2")
	{
		if (this.isWarmup())
		{
	...
	else if (blob.getName() == "archer" || blob.getName() == "archer2")
	{
	...
	else if (blob.getName() == "knight" || blob.getName() == "builder2")
	{
		if (ret)
		{
...
bool canGetSpawnmats(CRules@ this, CPlayer@ p, RulesCore@ core)
{
	...
	else //trying to get new class items, give a guy a break
	{
		...
		if (name == "builder" || name == "builder2")
			flag = ItemFlag::Builder;
		else if (name == "knight" || name == "knight2")
			flag = ItemFlag::Knight;
		else if (name == "archer" || name == "archer2")
		...
		for (uint step = 0; step < spots.length; ++step)
		{
			...
			if (spot !is null && spot.getOverlapping(overlapping))
			{
				...
				for (uint o_step = 0; o_step < overlapping.length; ++o_step)
				{
					...
					{
						string haha = overlapped.getName();
						haha.resize(haha.size() - 1);

						if (!isShop || name.find(overlapped.getName()) != -1 || name.find(haha) != -1)
						{
