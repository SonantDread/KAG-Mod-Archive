...
void onPlayerDie(CRules@ this, CPlayer@ victim, CPlayer@ killer, u8 customData)
{
	...
	f32 coinIncreaseCharmFactor = 1.0f;

	f32 deathCoinLossReduceFactor = 1.0f;

	f32 deathCoinDropFactor = 1.0f;

	if (killer !is null)
	{
		if(getRules().get_bool("coinincreasecharm_" + killer.getUsername()))
		{
			coinIncreaseCharmFactor = 1.5f;
			deathCoinLossReduceFactor = 0.75f;
			deathCoinDropFactor = 1.25f;
		}
	}
	...
	if (victim !is null)
	{
		if (killer !is null)
		{
			...
			{
				killer.server_setCoins(killer.getCoins() + coinsOnKillAdd * coinIncreaseCharmFactor);
			}
		...
		{
			s32 lost = victim.getCoins() * (coinsOnDeathLosePercent * 0.01f * deathCoinLossReduceFactor);
			...
			if (blob !is null)
				server_DropCoins(blob.getPosition(), XORRandom(lost) * deathCoinDropFactor);
		}
	}
}
...
f32 onPlayerTakeDamage(CRules@ this, CPlayer@ victim, CPlayer@ attacker, f32 DamageScale)
{
	...
	if (attacker !is null && attacker !is victim && attacker.getTeamNum() != victim.getTeamNum())
	{
		...
		if(getRules().get_bool("coinincreasecharm_" + attacker.getUsername()))
		{
			coinIncreaseCharmFactor = 1.5f;
		}
		...
		attacker.server_setCoins(attacker.getCoins() + dmg * coinsOnDamageAdd / this.attackdamage_modifier * coinIncreaseCharmFactor);
	}
