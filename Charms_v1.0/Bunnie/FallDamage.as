	...
	if (damage != 0.0f) //interesting value
	{
		...
		if (damage > 0.0f)
		{
			...
			if (getRules() !is null)
			{
				...
				{
					if (getRules().get_bool("falldmgcharm_" + this.getPlayer().getUsername()) && damage > 0.1f)
					{
						setKnocked(this, 8);
		...
		else if (getRules() !is null)
		{
			if (this.getPlayer() !is null)
			{
				if (getRules().get_bool("falldmgcharm_" + this.getPlayer().getUsername()))
				{
					setKnocked(this, 8);
					Sound::Play("/BreakBone", this.getPosition());
					return;
