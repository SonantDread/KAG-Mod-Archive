...
void SetKnocked(CBlob@ blob, int ticks, bool sync = false)
{
	...
	if (getRules() !is null && blob.getPlayer() !is null)
	{
		getRules().set_u8("lastknocktime" + blob.getPlayer().getUsername(), getGameTime());
		getRules().Sync("lastknocktime" + blob.getPlayer().getUsername(), true);
		printf("hehe");
	}
}
