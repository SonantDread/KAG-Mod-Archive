...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	if (damage == 0 || force)
	{
			...
			|| customData == Hitters::water_stun_force)
		{
			...
			f32 ifwaterheal = 1.0f;

			bool iswaterheal = false;

			if (this.getPlayer() !is null)
			{
				if (getRules().get_bool("waterhealcharm_" + this.getPlayer().getUsername()))
				{
					ifwaterheal = 0.5f;
					iswaterheal = true;
				}
			...
			if(iswaterheal && this.getInitialHealth() - this.getHealth() < 0.5f)
			{
				this.server_SetHealth(this.getHealth() + (this.getInitialHealth() - this.getHealth()));
			}

			else if(iswaterheal && this.getHealth() < this.getInitialHealth() && (this.getInitialHealth() - this.getHealth() >= 0.5f))
			{
