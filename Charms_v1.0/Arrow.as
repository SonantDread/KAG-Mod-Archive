...
void onTick(CBlob@ this)
{
	...
	if (!this.hasTag("stopped") && !this.hasTag("should add force") && !this.hasTag("add force again")) this.add_u32("lifetime", 1);
	if (!this.hasTag("collided")) //we haven't hit anything yet!
	{
		...
		Pierce(this);   //map
		if (!this.getShape().isStatic()) this.setAngleDegrees(-angle);
		...
		if (this.hasTag("act 3'd")) shape.SetGravityScale(6.0f);
	...
	// sticking
	processSticking = processSticking || this.hasTag("stopped");
	bool nolock = !this.hasTag("stopped") && !this.hasTag("should add force") && !this.hasTag("was in a field");
	if (processSticking && !this.hasTag("should add force") && getGameTime() - this.get_u32("field exit time") > 2)
	{
		//no collision
		...
		if (nolock)this.setPosition(this.get_Vec2f("lock"));
		shape.SetStatic(true);
	}
	else shape.getConsts().collidable = true;
	...
	//this.setVelocity(this.getVelocity() + Vec2f(2,-2));
}
	...
	if (this !is hitBlob && customData == Hitters::arrow)
	{
		...
		if (speed > ArcherParams::shoot_max_vel * 0.5f)
		{
			...
			if (
				...
				hitBlob.hasTag("player") && !this.hasTag("arrow rain"))
			{
...
f32 getArrowDamage(CBlob@ this, f32 vellen = -1.0f)
{
	...
	f32 damage = 1.0f;
	if (this.hasTag("arrow rain")) damage*=1.5f;
