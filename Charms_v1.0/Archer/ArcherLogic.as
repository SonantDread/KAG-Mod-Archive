	...
	else if (this.isKeyPressed(key_action1))
	{
		...
		else if (charge_state == ArcherParams::readying)
		{
			...
			if (getLocalPlayer() !is null)
			{
				...
				if (rules.get_bool(username + "fclick") == false)
				{
					charge_time++;
				}
				else if (rules.get_bool(username + "fclick") == true)
				{
					charge_time = 0;
				}
			}
...
void onTick(CBlob@ this)
{
	...
	if (isKnocked(this) || this.isInInventory() || rules.get_bool(username + "fclick"))
	{
		archer.grappling = false;
...
bool checkGrappleStep(CBlob@ this, ArcherInfo@ archer, CMap@ map, const f32 dist)
{
	...
	else
	{
		...
		if (b !is null)
		{
			...
			else if (b.isCollidable() && b.getShape().isStatic() && !b.hasTag("stopped") && !b.hasTag("ignore_arrow"))
			{
...
f32[] four_angles = {0.5f, -0.5f, 2.0f, -2.0f};
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		{
			int r = 0;
			int arrow_count = ArcherParams::legolas_arrows_volley;
			Vec2f arrowVel1 = arrowVel;
			Vec2f arrowVel2 = arrowVel;
			CPlayer@ player = this.getPlayer();
			bool do_4x = false;
			if (player !is null)
				do_4x = getRules().get_bool("4xcharm_" + this.getPlayer().getUsername());
			if (do_4x) arrow_count++;
			for (int i = 0; i < arrow_count; i++)
			{
				if (getNet().isServer())
				{
					Vec2f vel;
					arrowVel1 = arrowVel2;
					//Vec2f vel = (i<2 && do_4x) ? arrowVel1 : arrowVel;
					//CBlob@ arrow = CreateArrow(this, arrowPos, vel, arrowType);
					if (do_4x) vel = arrowVel1.RotateBy(ArcherParams::legolas_arrows_deviation * four_angles[i]);
					else vel = arrowVel;
					CBlob@ arrow = CreateArrow(this, arrowPos, vel, arrowType);
					if (do_4x && i>1 && arrow !is null)
					{
						arrow.Tag("4x");
					}
					else if (i>0 && arrow !is null && !do_4x)
					{
				...
				{
					rmod = 0.4f;
					arrowVel1 = arrowVel1.RotateBy(ArcherParams::legolas_arrows_deviation * r *rmod, Vec2f());
				}
				*/
