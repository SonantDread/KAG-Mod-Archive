...
void onTick(CBlob@ this)
{
    ...
    if (this.isInInventory() || rules.get_bool(username + "fclick"))
    {
...
class CutState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		else if (delta > DELTA_BEGIN_ATTACK && delta < DELTA_END_ATTACK)
		{
			...
			DoAttack(this, 1.0f, attackAngle, attackarc, Hitters::sword, delta, knight, false);
		}
		else if (delta >= 9)
		{
...
class SlashState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		bool do_360 = false;
		if (player !is null)
			do_360 = hasCharm(this.getPlayer(), getCharmByName("360slashcharm"));

		if (knight.state == KnightStates::sword_power_super
			&& (this.isKeyJustPressed(key_action1) || do_360))
		{
		...
		else if (delta > DELTA_BEGIN_ATTACK && delta < 10)
		{
			...
			f32 angle = 120.0f;
			if (do_360 && knight.doubleslash)
			{
				DoAttack(this, 2.0f, -(vec.Angle()), angle, Hitters::sword, delta, knight, true, do_360 && knight.doubleslash);
				DoAttack(this, 2.0f, -(vec.Angle()-120.0f), angle, Hitters::sword, delta, knight, true, do_360 && knight.doubleslash);
				DoAttack(this, 2.0f, -(vec.Angle()+120.0f), angle, Hitters::sword, delta, knight, true, do_360 && knight.doubleslash);
			}
			else DoAttack(this, 2.0f, -(vec.Angle()), angle, Hitters::sword, delta, knight, true, false);
		}
			...
			|| (knight.doubleslash && delta >= KnightVars::double_slash_time))
		{
			...
			{
				knight.doubleslash = false;
				if (!do_360) knight.state = KnightStates::sword_power; else knight.state = KnightStates::resheathing_slash;
			}
			else
			{
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, KnightInfo@ info, bool is_slash, bool huge_slash = false)
{
	if (!getNet().isServer())
	{
		return;
	...
	bool do_360 = getRules().get_bool("360slashcharm_" + this.getPlayer().getUsername());
	Vec2f blobPos = this.getPosition();
	Vec2f vel = this.getVelocity();
	Vec2f thinghy(1, 0);
	//if (do_360) thinghy = Vec2f(0,0);
	...
	if (do_360 && is_slash && huge_slash) attack_distance = 30.0f;
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			CBlob@ b = hi.blob;
			if (b !is null && (!dontHitMore || do_360)) // blob
			{
				...
				if (!dontHitMore || (do_360 && b.hasTag("blocks sword")))
				{
					Vec2f velocity = b.getPosition() - pos;
					if (!b.hasTag("flesh") && do_360) damage *= 1.5f;
