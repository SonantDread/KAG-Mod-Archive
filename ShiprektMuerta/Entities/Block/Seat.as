...
void onInit( CBlob@ this )
{
	...
	if ( getNet().isServer() )
	{
		...
		u16[] left_propellers, strafe_left_propellers, strafe_right_propellers, right_propellers, up_propellers, down_propellers, machineguns, cannons, weapons;					
		...
		if ( isCaptain || canHijack )
		{
			...
			if ( !space && !Human::isHoldingBlocks( occupier ) && !Human::wasHoldingBlocks( occupier ) )
			{
				...
				if ( left_click )
				{
					...
					for (uint i = 0; i < machineguns.length; ++i)
					{
						...
						if ( Maths::Abs( dirFacing.AngleWith( aim ) ) < 40 )
						{
							...
							weap.set_u32("fire time", gameTime);
						}
					}
				}
				...
				if ( reload )
				{
					...
					{
						CBlob@ weap = getBlobByNetworkID( weapons[i] );
						if ( weap is null )
							continue;
						
						if ( weap.get_bool( "reloading" ) == true )
							continue;
...
void updateArrays( CBlob@ this, Island@ island )
{
	...
	for (uint b_iter = 0; b_iter < island.blocks.length; ++b_iter)
	{
		...
		if (block.hasTag("weapon"))
			weapons.push_back(block.getNetworkID());
