...
{
	string configstr = "../Mods/MarcelServer/99TeamColor1/Rules/TCCTF/ctf_vars.cfg";
	...
	this.minimum_players_in_team = cfg.read_s32("minimum_players_in_team", 0);
...
shared class CTFCore : RulesCore
{
	...
	{
		for (uint i = 1; i < teams.length; i++)//alterado
		{
	...
	void SetupBases()
	{
		...
		if (map !is null)
		{
			...
			//blue flags
			if (getMap().getMarkers("green spawn", flagPlaces))
			{
				for (uint i = 0; i < flagPlaces.length; i++)
				{
					//print("Green base spawned");
					server_CreateBlob(flag_spawn_name(), 2, flagPlaces[i] + Vec2f(0, map.tilesize));
				}
			...
			{
				warn("CTF: Green flag added");
				respawnPos = Vec2f(300.0f, (map.getLandYAtX((300.0f / map.tilesize)) - 2) * map.tilesize);
				SetupBase(server_CreateBlob(flag_spawn_name(), 2, respawnPos));
			}
	...
	{
		if (team >= 1 && team < int(teams.length))
		{
