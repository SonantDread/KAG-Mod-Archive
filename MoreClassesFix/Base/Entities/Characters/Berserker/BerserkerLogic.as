...
void berserker_actorlimit_setup(CBlob@ this)
{
	u16[] networkIDs;
	this.set("LimitedActors", networkIDs);
}

bool berserker_has_hit_actor(CBlob@ this, CBlob@ actor)
{
	u16[]@ networkIDs;
...
u32 berserker_hit_actor_count(CBlob@ this)
{
	u16[]@ networkIDs;
	this.get("LimitedActors", @networkIDs);
...
void berserker_add_actor_limit(CBlob@ this, CBlob@ actor)
{
	this.push("LimitedActors", actor.getNetworkID());
}

void berserker_clear_actor_limits(CBlob@ this)
{
...
{
	BerserkerInfo berserker;

	berserker.state = BerserkerStates::normal;
	berserker.swordTimer = 0;
	berserker.slideTime = 0;
	berserker.doubleslash = false;
	berserker.tileDestructionLimiter = 0;

	berserker.rage = 0;

	this.set("berserkerInfo", @berserker);

	this.set_f32("gib health", -3.0f);
	addShieldVars(this, SHIELD_BLOCK_ANGLE, 2.0f, 5.0f);
	berserker_actorlimit_setup(this);
	...
	SetHelp(this, "help self action", "berserker", "$Jab$Jab        $LMB$", "", 4);
	SetHelp(this, "help self action2", "berserker", "$Shield$Shield    $KEY_HOLD$$RMB$", "", 4);
...
void onTick(CBlob@ this)
{
	...
	BerserkerInfo@ berserker;
	if (!this.get("berserkerInfo", @berserker))
	{
		return;
	}

	berserker.rage = (this.getInitialHealth() - this.getHealth()) / 0.5;
	...
	bool swordState = isSwordState(berserker.state);
	...
	//we'll see if berserkers dmging stuff while in menus is a real issue and go from there
	...
	else if ((pressed_a1 || swordState) && !moveVars.wallsliding)   //no attacking during a slide
	{
		if (getNet().isClient())
		{
			if (berserker.swordTimer == BerserkerVars::slash_charge_level2)
			{
				Sound::Play("AnimeSword.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
			else if (berserker.swordTimer == BerserkerVars::slash_charge)
			{
				Sound::Play("SwordSheath.ogg", pos, myplayer ? 1.3f : 0.7f);
			}
		}

		if (berserker.swordTimer > BerserkerVars::slash_charge_limit)
		{
			Sound::Play("/Stun", pos, 1.0f, this.getSexNum() == 0 ? 1.0f : 2.0f);
			SetKnocked(this, 15);
		}

		bool strong = (berserker.swordTimer > BerserkerVars::slash_charge_level2);
		...
		if (berserker.state == BerserkerStates::normal ||
		        this.isKeyJustPressed(key_action1) &&
		        (!inMiddleOfAttack(berserker.state)))
		{
			berserker.state = BerserkerStates::sword_drawn;
			berserker.swordTimer = 0;
		}

		if (berserker.state == BerserkerStates::sword_drawn && getNet().isServer())
		{
			berserker_clear_actor_limits(this);
		}

		//responding to releases/noaction
		s32 delta = berserker.swordTimer;
		if (berserker.swordTimer < 128)
			berserker.swordTimer++;

		if (berserker.state == BerserkerStates::sword_drawn && !pressed_a1 &&
		        !this.isKeyJustReleased(key_action1) && delta > BerserkerVars::resheath_time)
		{
			berserker.state = BerserkerStates::normal;
		}
		else if (this.isKeyJustReleased(key_action1) && berserker.state == BerserkerStates::sword_drawn)
		{
			berserker.swordTimer = 0;

			if (delta < BerserkerVars::slash_charge)
			{
				if (direction == -1)
				{
					berserker.state = BerserkerStates::sword_cut_up;
				}
				else if (direction == 0)
				{
					if (aimpos.y < pos.y)
					{
						berserker.state = BerserkerStates::sword_cut_mid;
					}
					else
					{
						berserker.state = BerserkerStates::sword_cut_mid_down;
					}
				}
				else
				{
					berserker.state = BerserkerStates::sword_cut_down;
				}
			}
			else if (delta < BerserkerVars::slash_charge_level2)
			{
				berserker.state = BerserkerStates::sword_power;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				berserker.slash_direction = aiming_direction;
			}
			else if (delta < BerserkerVars::slash_charge_limit)
			{
				berserker.state = BerserkerStates::sword_power_super;
				Vec2f aiming_direction = vel;
				aiming_direction.y *= 2;
				aiming_direction.Normalize();
				berserker.slash_direction = aiming_direction;
		...
		else if (berserker.state >= BerserkerStates::sword_cut_mid &&
		         berserker.state <= BerserkerStates::sword_cut_down) // cut state
		{
			...
			{
				f32 attackarc = 90.0f;
				f32 attackAngle = getCutAngle(this, berserker.state);

				if (berserker.state == BerserkerStates::sword_cut_down)
				{
					attackarc *= 0.9f;
				}

				DoAttack(this, 0.5f + 0.5f * berserker.rage, attackAngle, attackarc, Hitters::sword, delta, berserker);
			}
			else if (delta >= 9)
			{
				berserker.swordTimer = 0;
		...
		else if (berserker.state == BerserkerStates::sword_power ||
		         berserker.state == BerserkerStates::sword_power_super)
		{
			//setting double
			if (berserker.state == BerserkerStates::sword_power_super &&
			        this.isKeyJustPressed(key_action1))
			{
				berserker.doubleslash = true;
			}
			...
			{
				DoAttack(this, 1.0f + 0.5f * berserker.rage, -(vec.Angle()), 120.0f, Hitters::sword, delta, berserker);
			}
			else if (delta >= BerserkerVars::slash_time ||
			         (berserker.doubleslash && delta >= BerserkerVars::double_slash_time))
			{
				berserker.swordTimer = 0;

				if (berserker.doubleslash)
				{
					berserker_clear_actor_limits(this);
					berserker.doubleslash = false;
					berserker.state = BerserkerStates::sword_power;
		...
		if ((berserker.state == BerserkerStates::sword_power ||
		        berserker.state == BerserkerStates::sword_power_super) &&
		        delta < BerserkerVars::slash_move_time)
		{

			if (Maths::Abs(vel.x) < BerserkerVars::slash_move_max_speed &&
			        vel.y > -BerserkerVars::slash_move_max_speed)
			{
				Vec2f slash_vel =  berserker.slash_direction * this.getMass() * 0.5f;
	...
	else if (this.isKeyJustReleased(key_action2) || this.isKeyJustReleased(key_action1) || this.get_u32("berserker_timer") <= getGameTime())
	{
		berserker.state = BerserkerStates::normal;
	}
	...
	if (myplayer)
	{
		...
		{
			SetHelp(this, "help self action", "berserker", "$Slash$ Slash!    $KEY_HOLD$$LMB$", "", 13);
		}
	}
...
{
	// play cling sound if other berserker attacked us
	// dmg could be taken out here if we ever want to

	if (hitterBlob.getPosition().x < this.getPosition().x && hitterBlob.getName() == "berserker") // berserker and the left one (to play only once)
	{
...
void DoAttack(CBlob@ this, f32 damage, f32 aimangle, f32 arcdegrees, u8 type, int deltaInt, BerserkerInfo@ info)
{
	if (!getNet().isServer())
	{
		return;
	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				if (berserker_has_hit_actor(this, b))
				{
					if (large)
						dontHitMore = true;
				...
				berserker_add_actor_limit(this, b);
				if (!dontHitMore)
				{
...
bool isSliding(BerserkerInfo@ berserker)
{
	return (berserker.slideTime > 0 && berserker.slideTime < 45);
}
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (this.getShape().vellen > SHIELD_KNOCK_VELOCITY || onground)
	{
		...
		//printf("berserker.stat " + berserker.state );
	}
}
...
bool hasItem(CBlob@ this, const string &in name)
{
	...
	{
		warn("our inventory was null! BerserkerLogic.as");
	}
...
void TakeItem(CBlob@ this, const string &in name)
{
	...
	if (inv !is null)
	{
		...
		{
			warn("took a bomb even though we dont have one! BerserkerLogic.as");
		}
	}
	else
	{
...
void onAddToInventory(CBlob@ this, CBlob@ blob)
{
	...
	if (this.isMyPlayer() && this.getInventory().getItemsCount() > 1)
	{
		for (uint j = 1; j < bombTypeNames.length; j++)
		{
			...
			{
				SetHelp(this, "help inventory", "berserker", "$Help_Bomb1$$Swap$$Help_Bomb2$         $KEY_TAP$$KEY_F$", "", 2);
				break;
			}
		}
	}
