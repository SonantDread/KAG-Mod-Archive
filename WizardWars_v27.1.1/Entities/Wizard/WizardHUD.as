...
void onInit( CSprite@ this )
{
	...
	loadHotbarAssignments( thisBlob );
}
...
void DrawSpellBar(CBlob@ this)
{
	...
	f32 wizMana = wizard.mana;
	
	u8[] primaryHotkeys = wizard.hotbarAssignments;
	
	//PRIMARY SPELL HUD
    Vec2f primaryPos = Vec2f( 16.0f, getScreenHeight()-128.0f );

	const u8 primarySpell = wizard.hotbarAssignments[wizard.primaryHotkeyID];
	const u8 primaryHotkey = wizard.primaryHotkeyID;
	
	bool canCustomizeHotbar = this.isKeyPressed(key_inventory) && this.isKeyJustPressed(key_action1);
	...
	for (uint i = 0; i < 15; i++)	//only 15 total spells held inside primary hotbar
	{
		...
		if ( i < 5 )		//spells 0 through 4
		{
			...
			{
				assignHotkey(this, i, wizard.customSpellID);
				hotbarClicked = true;
			}			
		}
	...
	Spell secondarySpell = WizardParams::spells[wizard.secondarySpellID];
	...
	{
		assignHotkey(this, 15, wizard.customSpellID);	//hotkey 15 is the secondary fire hotkey
		hotbarClicked = true;
	}
	...
	Spell aux1Spell = WizardParams::spells[wizard.aux1SpellID];
	...
	{
		assignHotkey(this, 16, wizard.customSpellID);	//hotkey 16 is the auxiliary1 fire hotkey
		hotbarClicked = true;
	}
	...
	if ( canCustomizeHotbar == true )	//play sound, keep menu open by refreshing, and update selected spell 
	{
		if ( hotbarClicked )
		{
			this.ClearButtons();
			this.ClearMenus();
			this.CreateInventoryMenu(getDriver().getScreenCenterPos() + Vec2f(0,288));
...
void onRender( CSprite@ this )
{
	...
	//GUI::DrawIcon( iconsFilename, wiz.primarySpellID, Vec2f(16,16), Vec2f(10,10));
	
	//GUI::DrawIcon( iconsFilename, wiz.secondarySpellID, Vec2f(16,16), Vec2f(10,66)); 
}
