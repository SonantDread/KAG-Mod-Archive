...
const s32 bomb_fuse = 80;
const f32 arrowMediumSpeed = 10.0f;
const f32 arrowFastSpeed = 20.0f;
//maximum is 15 as of 22/11/12 (see ArcherCommon.as)

const f32 ARROW_PUSH_FORCE = 5.0f;
...
f32 ArrowHitBlob( CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData, const u8 arrowType )
{
    if (hitBlob !is null)
    {
		...
		if (!hitShield)
		{
			...
			{
				if (velocity.Length() > arrowFastSpeed) {
					this.getSprite().PlaySound( "arrow_hitfast_ground.ogg" );
				}
				//else {
					//this.getSprite().PlaySound( "arrow_hit_ground.ogg" );
				//}
			}
		}
		else {
