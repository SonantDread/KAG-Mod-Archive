...
void MakeTradeMenu(CBlob@ trader)
{
	...
	s32 cost_bombs = cfg.read_s32("cost_bombs", 25);
	s32 cost_waterbombs = cfg.read_s32("cost_waterbombs", 30);
	s32 cost_keg = cfg.read_s32("cost_keg", 70);
	s32 cost_mine = cfg.read_s32("cost_mine", 40);
	s32 cost_molotov = cfg.read_s32("cost_molotov", 40);
	s32 cost_firemine = cfg.read_s32("cost_firemine", 30);
	s32 cost_slidemine = cfg.read_s32("cost_slidemine", 40);

	s32 cost_arrows = cfg.read_s32("cost_arrows", 15);
	s32 cost_waterarrows = cfg.read_s32("cost_waterarrows", 30);
	...
	addItemForCoin(trader, "Molotov", cost_molotov, true, "$molotov$", "molotov", "Set fire to yer' mates!");
	addItemForCoin(trader, "FireMine", cost_firemine, true, "$firemine$", "firemine", "A mine that explodes with flames.");
	addItemForCoin(trader, "SlideMine", cost_slidemine, true, "$slidemine$", "slidemine", "Slide it across the ground!");
	//archery stuff
	...
	addItemForCoin(trader, "Saw", cost_saw, true, "$saw$", "saw", "Grind up your foes with this piece of machinery.");
	////vehicles
