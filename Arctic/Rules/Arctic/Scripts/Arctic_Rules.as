...
void onPlayerDie( CRules@ this, CPlayer@ victim, CPlayer@ attacker, u8 customData )
{
	...
	if(victim.getTeamNum() < 7)
	{
		...
		if (TeamForts <= 0)
			victim.server_setTeamNum(100+getNumNeutrals());
	}
	...
	for(u8 i = 0; i < getPlayerCount(); i++)
	{
		...
		if(player !is null)
		{
			...
			if(blob is null && player.get_u32("respawn time") <= getGameTime())//if player is dead, then spawn them
			{
				...
				string blobname = "builder";
				if(team >= 100)
					blobname = "neutral";
				
				CBlob@ new_blob = server_CreateBlob(blobname);

				if(team < 7)//if player is already on a team, then spawn him at one of his bases
				{
					CBlob@[] facbases;
					getBlobsByName("facbase", @facbases);
					Vec2f[] spawns;
					for(uint i = 0; i < facbases.length; i++)
					{
						CBlob@ facbase = facbases[i];
						if(facbase !is null && facbase.getTeamNum() == team)
						{
							spawns.push_back(facbase.getPosition());
						}
					}
