...
const u8 default_freezecount = 180;
...
void onRender(CSprite@ this)
{
	...
	s32 maxfreezecount = default_freezecount + offset;
	s32 aircount = blob.get_u8("air_count") + offset + extra;
	s32 freezecount = blob.get_u8("freeze_count") + offset + extra;
	...
	if (freezecount < default_freezecount + offset + extra)
	{
		// draw drown indicator
		Vec2f p = blob.getInterpolatedPosition() + Vec2f(0.0f, -blob.getHeight() * 1.5f);
		p.x -= 8;
		...
		s32 icecount = maxfreezecount / 30; //1 ice = 1 second
		for (s32 ice = 0; ice < icecount; ice++)
		{
			s32 frac = freezecount - (ice * 30);

			u32 frame = (frac > 18) ? 1 : (frac > 12) ? 2 : (frac > 6) ? 3 : 4;
			if(frac < 24)
				GUI::DrawIcon("Frostbar.png", frame, dim,
				              pos + Vec2f(((icecount - 1) / 2.0f - ice + 1) * dim.x, 0) * 2.0f * getCamera().targetDistance,
				              getCamera().targetDistance);
		}
	}
}
