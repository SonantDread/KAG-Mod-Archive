...
CBlob@ server_BuildBlob(CBlob@ this, BuildBlock[]@ blocks, uint index)
{
	...
	if(canBuild(this, blocks, index))
	{
		...
		if(b.buildOnGround)
		{
			...
			if (b.name == "facbase") space = Vec2f(5, 3);
		...
		if(getNet().isServer())
		{
			...
			else
			{
				...
				Vec2f[] floor = {
									bpos + Vec2f(-tilesize*2, tilesize),
									bpos + Vec2f(-tilesize, tilesize),
									bpos + Vec2f(0, tilesize),
									bpos + Vec2f(tilesize, tilesize),
									bpos + Vec2f(tilesize*2, tilesize)
								};
				int count = 0;
				...
				{
					Vec2f temp = floor[f];
					TileType tile = map.getTile(temp).type;
					if (map.isTileSolid(tile) || isTileCustomSolid(tile)) count++;
				}
				if (count != 5)
				{
					send_chat(getRules(), this.getPlayer(), "You need to have 5 blocks floor under you'r base to be created!", SColor(255, 255, 0, 0));
					Sound::Play("/NoAmmo");
					return null;
				}
				...
				if(this.getTeamNum() >= 7)
				{
					...
					{
						CBlob@[] fg;
						getBlobsByTag("faction_base", @fg);
						for(uint i = 0; i < fg.length; i++)
						{
							if(fg[i].getTeamNum() == newteam)
								newteam += 1;
							//else
							//	break;
						}
					...
					if(newteam >= 7)
					{
						send_chat(getRules(), this.getPlayer(), "Woops, it looks like that all 6 teams are already created, you cant create new one!", SColor(255, 255, 0, 0));
						send_chat(getRules(), this.getPlayer(), "sowwy ;w;", SColor(255, 255, 0, 0));
						return null;
					}
					else
					{
