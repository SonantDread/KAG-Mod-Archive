	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null && !dontHitMore) // blob
			{
				...
				{
					bool ground = map.isTileGround(hi.tile) || hi.tile == CMap::tile_purple_grass_dirt || hi.tile == CMap::tile_yellow_grass_dirt || hi.tile == CMap::tile_red_grass_dirt;
					bool dirt_stone = map.isTileStone(hi.tile);
					bool gold = map.isTileGold(hi.tile);
					bool wood = map.isTileWood(hi.tile);
					bool snow = isTileSnow(hi.tile);
					bool ice = isTileIce(hi.tile);
					if (ground || wood || dirt_stone || gold || snow || ice)
					{
						...
						if (dif < 20.0f)
						{
							...
							if ((map.isTileSolid(hi.hitpos - Vec2f(map.tilesize * check_x, 0)) && map.isTileSolid(hi.hitpos - Vec2f(0, map.tilesize * check_y))) || 
								(isTileCustomSolid(map.getTile(hi.hitpos - Vec2f(map.tilesize * check_x, 0)).type) && isTileCustomSolid(map.getTile(hi.hitpos - Vec2f(0, map.tilesize * check_y)).type)))
								continue;
