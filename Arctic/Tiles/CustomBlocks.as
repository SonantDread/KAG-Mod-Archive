...
//blobs
const SColor color_amulet_crow(255, 150, 0, 193);
...
void HandleCustomTile(CMap@ map, int offset, SColor pixel)
{
	...
	else if (pixel == color_snow_brick_back){
	...
	else if (pixel == color_snow_pile){
		CBlob@ pile = server_CreateBlob("SnowPile", -1, pos+Vec2f(4,4));
		pile.getShape().SetStatic(true);
		pile.server_SetHealth((XORRandom(4)+1)/4);
		if(map.isTileGroundBack(map.getTile(pos-Vec2f(0,map.tilesize)).type) || map.isTileGroundBack(map.getTile(pos+Vec2f(-map.tilesize,0)).type) || map.isTileGroundBack(map.getTile(pos+Vec2f(map.tilesize,0)).type))
	...
	else if (pixel == color_grass_purple){
	...
	else if (pixel == color_grass_yellow){
	...
	//blobs
	else if (pixel == color_amulet_crow){
		CBlob@ amulet = server_CreateBlob("crowamulet", -1, pos+Vec2f(4,4));
		//pile.getShape().SetStatic(true);
		//amulet.server_SetHealth((XORRandom(4)+1)/4);
		if(map.isTileGroundBack(map.getTile(pos-Vec2f(0,map.tilesize)).type) || map.isTileGroundBack(map.getTile(pos+Vec2f(-map.tilesize,0)).type) || map.isTileGroundBack(map.getTile(pos+Vec2f(map.tilesize,0)).type))
			map.server_SetTile(pos, CMap::tile_ground_back);
		}
}
...
bool isGrassTile(u16 tile)
{
    return  (tile >= 25 && tile <= 28) || 
			(tile >= CMap::tile_grass_purple && tile <= CMap::tile_ice_spike_v0);
}
