...
void onTick( CMovement@ this )
{
	...
	bool left_or_right = (left || right);
	{
		...
		if (carryBlob !is null)
		{
			...
			else if (carryBlob.hasTag("veryheavy weight"))
			{
				moveVars.walkFactor *= 0.4f;
				moveVars.jumpFactor *= 0.5f;
			}
			else if (carryBlob.hasTag("fastlvl1"))
			{
				moveVars.walkFactor *= 1.15f;
			}
			else if (carryBlob.hasTag("fastlvl2"))
			{
				moveVars.walkFactor *= 1.3f;	
			}
			else if (carryBlob.hasTag("fastlvl3"))
			{
			moveVars.walkFactor *= 1.45f;	
			}
			else if (carryBlob.hasTag("regen"))
			{
			 blob.AddScript("RegenHealth2.as");
			}
			else if (carryBlob.hasTag("regen2"))
			{
			 blob.AddScript("RegenHealth.as");
			}
			else if (carryBlob.hasTag("regen3"))
			{
			 blob.AddScript("RegenHealth3.as");
			}
			else if (carryBlob.hasTag("jumpy"))
			{
				 blob.AddScript("DoubleJump.as");
			}
           else if (carryBlob.hasTag("jumpy2"))
			{
			
				 blob.AddScript("TripleJump.as");
			}
			else if (carryBlob.hasTag("jumpy3"))
			{
